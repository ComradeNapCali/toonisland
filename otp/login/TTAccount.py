...
class TTAccount:
    ...
    def authenticateDelete(self, loginName, password):
        try:
            errorMsg = self.talk(
                "authenticateDelete", data=self.__makeLoginDict(loginName, password)
            )
            if not errorMsg:
                return (1, None)
            if self.response.getInt("errorCode") in (5, 72):
    ...
    def getLastErrorMsg(self, forceCustServNum=0):
        ...
        if errCode < 100:
            ...
            if forceCustServNum:
                msg += (
                    " "
                    + OTPLocalizer.TTAccountCustomerServiceHelp
                    % self.cr.accountServerConstants.getString(
            ...
            msg += (
                " "
                + OTPLocalizer.TTAccountCallCustomerService
                % self.cr.accountServerConstants.getString("customerServicePhoneNumber")
            )
        else:
            self.notify.warning(
