...
if configAccountServer:
    ...
    print('TTAccount: overriding accountServer from panda3d: ', accountServer)
...
class TTAccount:
    ...
    def authenticateParentPassword(self, loginName, password, parentPassword):
        try:
            errorMsg = self.talk('authenticateParentPassword', data=self.__makeLoginDict(
    ...
    def authenticateDelete(self, loginName, password):
        try:
            errorMsg = self.talk('authenticateDelete',
    ...
    def getLastErrorMsg(self, forceCustServNum=0):
        ...
        if errCode < 100:
            ...
            self.notify.warning('unknown error code class: %s: %s' % (
                self.response.getInt('errorCode'), self.response.getString('errorMsg')))
            msg = self.response.getString('errorMsg')
            msg += ' ' + OTPLocalizer.TTAccountCallCustomerService % self.cr.accountServerConstants.getString(
                'customerServicePhoneNumber')
        return msg
    ...
    def talk(self, operation, data={}):
        ...
        if operation in ('play', 'get', 'cancel', 'authenticateParentPassword', 'authenticateDelete', 'authenticateParentPasswordNewStyle', 'authenticateDeleteNewStyle'):
            ...
            self.notify.error(
                "Internal TTAccount error: need to add 'required data' checking for %s operation" % operation)
        op2Php = {'play': 'play',
                  'get': 'get',
                  'cancel': 'cancel',
                  'create': 'create',
        ...
        if operation in ('get',):
            ...
            self.notify.error(
                "Internal TTAccount error: need to set expected response header for '%s' operation" % operation)
        ...
        if operation in ('get', 'forgotPassword', 'authenticateDelete', 'play', 'cancel', 'create', 'purchase', 'setParentPassword', 'authenticateParentPassword', 'authenticateParentPasswordNewStyle', 'authenticateParentUsernameAndPassword', 'authenticateDeleteNewStyle'):
            ...
            self.notify.error(
                'Internal TTAccount error: need to extract useful data for %s operation' % operation)
        return None
