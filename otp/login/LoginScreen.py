...
class LoginScreen(StateData.StateData, GuiScreen.GuiScreen):
    ...
    def enterShowConnectionProblemDialog(self, msg):
        ...
        self.acceptOnce(
            self.connectionProblemDialogDoneEvent, self.__handleConnectionProblemAck
        )
    ...
    def handleLoginResponseMsg3(self, di):
        ...
        self.cr.secretChatAllowed = (
            accountDetailRecord.chatCodeCreation or parentControlledChat
        )
    ...
    def handleLoginResponseMsg2(self, di):
        ...
        if returnCode == 0:
            ...
            self.fsm.request(
                "showLoginFailDialog", [OTPLocalizer.LoginScreenPeriodTimeExpired]
            )
        else:
            self.notify.info("Login failed: %s" % errorString)
            messenger.send(self.doneEvent, [{"mode": "reject"}])
    ...
    def handleLoginResponseMsg(self, di):
        ...
        if returnCode == 0:
            ...
            self.fsm.request(
                "showLoginFailDialog", [OTPLocalizer.LoginScreenBadPassword]
            )
        elif returnCode == 14:
            self.notify.info("Bad word in user name")
            self.fsm.request(
                "showLoginFailDialog", [OTPLocalizer.LoginScreenInvalidUserName]
            )
        elif returnCode == 129:
            self.notify.info("Username not found")
            self.fsm.request(
                "showLoginFailDialog", [OTPLocalizer.LoginScreenUserNameNotFound]
            )
        else:
            self.notify.info("Login failed: %s" % errorString)
            messenger.send(self.doneEvent, [{"mode": "reject"}])
