...
# --- ACCOUNT DATABASES ---
# These classes make up the available account database interfaces for Toon Island: Aftermath.
...
class AccountDB:
    ...
    def __init__(self, gameServicesManager):
        ...
        accountDbFile = simbase.config.GetString(
            'accountdb-local-file', 'dependencies/astron/databases/accounts.db')
...
class DeveloperAccountDB(AccountDB):
    ...
    "TIA_DEVELOPER" access automatically upon login.
    """
...
class LoginOperation(GameOperation):
    ...
    def enterQueryAccountDB(self):
        ...
        self.gameServicesManager.accountDb.lookup(
            self.playToken, self.__handleLookup)
    ...
    def __handleLookup(self, result):
        ...
        if not result.get('success'):
            ...
            self.gameServicesManager.air.writeServerEvent(
                'play-token-rejected', self.target, self.playToken)
            self.demand('Kill', result.get(
                'reason', 'The accounts database rejected your play token.'))
            return
    ...
    def __handleCreate(self, accountId):
        ...
        if self.state != 'CreateAccount':
            ...
            self.notify.warning(
                'Received CreateAccount response outside of the CreateAccount state.')
            return
    ...
    def enterStoreAccountID(self):
        ...
        self.gameServicesManager.accountDb.storeAccountID(
            self.databaseId, self.accountId, self.__handleStored)
    ...
    def __handleStored(self, success=True):
        if not success:
            ...
            self.demand(
                'Kill', 'The account server could not save your account DB ID!')
            return
    ...
    def __handleRetrieve(self, dclass, fields):
        ...
        if dclass != self.gameServicesManager.air.dclassesByName['AccountUD']:
            ...
            self.demand(
                'Kill', 'Your account object (%s) was not found in the database!' % dclass)
            return
    ...
    def enterSetAccount(self):
        ...
        datagram.addServerHeader(
            self.target, self.gameServicesManager.air.ourChannel, CLIENTAGENT_OPEN_CHANNEL)
        datagram.addChannel(
            self.gameServicesManager.GetAccountConnectionChannel(self.accountId))
        self.gameServicesManager.air.send(datagram)

        # Set their sender channel to represent their account affiliation.
        datagram = PyDatagram()
        datagram.addServerHeader(
            self.target, self.gameServicesManager.air.ourChannel, CLIENTAGENT_SET_CLIENT_ID)
        # accountId in high 32 bits, 0 in low (no avatar).
        ...
        self.gameServicesManager.sendUpdateToChannel(
...
class GetAvatarsOperation(AvatarOperation):
    ...
    def enterQueryAvatars(self):
        ...
        for avId in self.avList:
            if avId:
                ...
                self.gameServicesManager.air.dbInterface.queryObject(
                    self.gameServicesManager.air.dbId, avId, response)
    ...
    def enterSendAvatars(self):
        ...
        # state to Off.
        self.gameServicesManager.sendUpdateToAccountId(
            self.target, 'avatarListResponse', [potentialAvatars])
        self.demand('Off')
...
class RemoveAvatarOperation(GetAvatarsOperation):
    ...
    def enterProcessRemove(self):
        ...
        if self.gameServicesManager.air.TIAFriendsManager:
            self.gameServicesManager.air.TIAFriendsManager.clearList(self.avId)
        else:
            friendsManagerDoId = OtpDoGlobals.OTP_DO_ID_TIA_FRIENDS_MANAGER
            friendsManagerDclass = self.gameServicesManager.air.dclassesByName['TIAFriendsManagerUD']
    ...
    def __handleRemove(self, fields):
        ...
        self.gameServicesManager.air.writeServerEvent(
            'avatar-deleted', self.avId, self.target)
...
class LoadAvatarOperation(AvatarOperation):
    ...
    def enterSetAvatar(self):
        ...
        datagram = PyDatagram()
        datagram.addServerHeader(
            channel, self.gameServicesManager.air.ourChannel, CLIENTAGENT_ADD_POST_REMOVE)
        ...
        self.gameServicesManager.sendUpdateToAccountId(
            self.target, 'receiveAccountDays', [accountDays])
        ...
        datagram = PyDatagram()
        datagram.addServerHeader(
            channel, self.gameServicesManager.air.ourChannel, CLIENTAGENT_OPEN_CHANNEL)
        datagram.addChannel(
        ...
        datagram = PyDatagram()
        datagram.addServerHeader(
            channel, self.gameServicesManager.air.ourChannel, CLIENTAGENT_SET_CLIENT_ID)
        ...
        self.gameServicesManager.air.TIAFriendsManager.comingOnline(self.avId, friendsList)
        ...
        # unexpectedly.
        if self.gameServicesManager.air.TIAFriendsManager:
            friendsManagerDclass = self.gameServicesManager.air.TIAFriendsManager.dclass
            cleanupDatagram = friendsManagerDclass.aiFormatUpdate('goingOffline',
                                                                  self.gameServicesManager.air.TIAFriendsManager.doId,
                                                                  self.gameServicesManager.air.TIAFriendsManager.doId,
                                                                  self.gameServicesManager.air.ourChannel, [self.avId])
        else:
            friendsManagerDoId = OtpDoGlobals.OTP_DO_ID_TIA_FRIENDS_MANAGER
        ...
        self.gameServicesManager.air.writeServerEvent(
            'avatar-chosen', avId=self.avId, accId=self.target)
        self.demand('Off')
...
class UnloadAvatarOperation(GameOperation):
    ...
    def enterUnloadAvatar(self):
        ...
        self.gameServicesManager.air.TIAFriendsManager.goingOffline(self.avId)

        # First, remove our POST_REMOVES.
        datagram = PyDatagram()
        datagram.addServerHeader(
            channel, self.gameServicesManager.air.ourChannel, CLIENTAGENT_CLEAR_POST_REMOVES)
        self.gameServicesManager.air.send(datagram)

        # Next, remove the avatar channel.
        datagram = PyDatagram()
        datagram.addServerHeader(
            channel, self.gameServicesManager.air.ourChannel, CLIENTAGENT_CLOSE_CHANNEL)
        datagram.addChannel(
            self.gameServicesManager.GetPuppetConnectionChannel(self.avId))
        self.gameServicesManager.air.send(datagram)
        ...
        datagram = PyDatagram()
        datagram.addServerHeader(
            channel, self.gameServicesManager.air.ourChannel, CLIENTAGENT_REMOVE_SESSION_OBJECT)
...
class GameServicesManagerUD(DistributedObjectGlobalUD):
    ...
    def login(self, playToken):
        ...
        self.notify.debug('Play token %s received from %s' %
                          (playToken, sender))
    ...
    def killConnection(self, connectionId, reason):
        ...
        datagram = PyDatagram()
        datagram.addServerHeader(
            connectionId, self.air.ourChannel, CLIENTAGENT_EJECT)
    ...
    def killConnectionFSM(self, connectionId):
        ...
        if not fsm:
            self.notify.warning(
                'Tried to kill connection %s for duplicate FSMs, but none exist!' % connectionId)
            return

        self.killConnection(
            connectionId, 'An operation is already running: %s' % fsm.name)

    def killAccount(self, accountId, reason):
        # Kills the account's connection given an accountId & a reason.
        self.killConnection(
            self.GetAccountConnectionChannel(accountId), reason=reason)
    ...
    def killAccountFSM(self, accountId):
        ...
        if not fsm:
            self.notify.warning(
                'Tried to kill account %s for duplicate FSMs, but none exist!' % accountId)
            return

        self.killAccount(
            accountId, 'An operation is already running: %s' % fsm.name)
