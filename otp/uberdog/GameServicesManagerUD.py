...
class LoginOperation(GameOperation):
    ...
    def __handleLookup(self, result):
        ...
        if not result.get("success"):
            ...
            self.demand(
                "Kill",
                result.get("reason", "The accounts database rejected your play token."),
            )
            return
    ...
    def enterRetrieveAccount(self):
        ...
        self.gameServicesManager.air.dbInterface.queryObject(
            self.gameServicesManager.air.dbId, self.accountId, self.__handleRetrieve
        )
    ...
    def __handleRetrieve(self, dclass, fields):
        ...
        if dclass != self.gameServicesManager.air.dclassesByName["AccountUD"]:
            ...
            self.demand(
                "Kill",
                "Your account object (%s) was not found in the database!" % dclass,
            )
            return
...
class AvatarOperation(GameOperation):
    ...
    def enterRetrieveAccount(self):
        ...
        self.gameServicesManager.air.dbInterface.queryObject(
            self.gameServicesManager.air.dbId, self.target, self.__handleRetrieve
        )
            ...
            if avId:
                ...
                def response(dclass, fields, avId=avId):
                    ...
                    if (
                        ...
                        self.demand(
                            "Kill",
                            "One of the account's avatars is invalid! dclass = %s, expected = %s"
    ...
    def enterProcessRemove(self):
        ...
        if estateId != 0:
            ...
            self.gameServicesManager.air.dbInterface.updateObject(
                self.gameServicesManager.air.dbId,
                estateId,
                self.gameServicesManager.air.dclassesByName["DistributedEstateAI"],
                {"setSlot%sToonId" % index: [0], "setSlot%sItems" % index: [[]]},
            )
        ...
        if self.gameServicesManager.air.TIAFriendsManager:
            ...
            datagram = friendsManagerDclass.aiFormatUpdate(
                "clearList",
                friendsManagerDoId,
                friendsManagerDoId,
                self.gameServicesManager.air.ourChannel,
...
class LoadAvatarOperation(AvatarOperation):
    ...
    def enterGetTargetAvatar(self):
        ...
        self.gameServicesManager.air.dbInterface.queryObject(
            self.gameServicesManager.air.dbId, self.avId, self.__handleAvatar
        )
    ...
    def enterSetAvatar(self):
        ...
        self.gameServicesManager.air.TIAFriendsManager.comingOnline(
...
class GameServicesManagerUD(DistributedObjectGlobalUD):
    ...
    def killConnectionFSM(self, connectionId):
        ...
        if not fsm:
            self.notify.warning(
                "Tried to kill connection %s for duplicate FSMs, but none exist!"
                % connectionId
            )
            return
    ...
    def killAccountFSM(self, accountId):
        ...
        if not fsm:
            self.notify.warning(
                "Tried to kill account %s for duplicate FSMs, but none exist!"
                % accountId
            )
            return
