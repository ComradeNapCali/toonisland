...
class LevelSpec:
    ...
    SystemEntIds = (LevelConstants.UberZoneEntId,
                    LevelConstants.LevelMgrEntId, LevelConstants.EditMgrEntId)
    ...
    def __init__(self, spec=None, scenario=0):
        ...
        self.entId2specDict.update(
            list2dict(self.getGlobalEntIds(), value=self.privGetGlobalEntityDict()))
        for i in range(self.getNumScenarios()):
            self.entId2specDict.update(list2dict(self.getScenarioEntIds(
                i), value=self.privGetScenarioEntityDict(i)))
    ...
    if __dev__:
        ...
        def insertEntity(self, entId, entType, parentEntId='unspecified'):
            ...
            attribDescs = self.entTypeReg.getTypeDesc(
                entType).getAttribDescDict()
        ...
        def getPrettyString(self):
            ...
            def getPrettyEntityDictStr(name, dict, tabs=0):
                ...
                firstAttribs = ('type', 'name', 'comment', 'parentEntId', 'pos', 'x', 'y',
                                'z', 'hpr', 'h', 'p', 'r', 'scale', 'sx', 'sy', 'sz', 'color', 'model')
                str = t(0) + '%s = {\n' % name
                ...
                for entId in allEntIds:
                    ...
                    for attrib in list(spec.keys()):
                        if attrib not in attribNames:
                            LevelSpec.notify.warning(
                                "entId %s (%s): unknown attrib '%s', omitting" % (entId, spec['type'], attrib))
                            del spec[attrib]

                    for attribName in attribNames:
                        if attribName not in spec:
                            LevelSpec.notify.warning("entId %s (%s): missing attrib '%s'" % (
