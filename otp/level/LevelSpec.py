...
class LevelSpec:
    ...
    def __init__(self, spec=None, scenario=0):
        ...
        for i in range(self.getNumScenarios()):
            self.entId2specDict.update(
                list2dict(
                    self.getScenarioEntIds(i), value=self.privGetScenarioEntityDict(i)
                )
            )
    ...
    if __dev__:
        ...
        def getPrettyString(self):
            ...
            str += getPrettyEntityDictStr(
                "GlobalEntities", self.privGetGlobalEntityDict()
            )
            str += "\n"
        ...
        def __repr__(self):
            return "LevelSpec(%s, scenario=%s)" % (
                repeatableRepr(self.specDict),
