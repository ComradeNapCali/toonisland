...
class DistributedLevel(DistributedObject.DistributedObject, Level.Level):
    ...
    def __init__(self, cr):
        ...
        self.titleText = OnscreenText.OnscreenText('', fg=self.titleColor, shadow=(
            0, 0, 0, 1), font=ToontownGlobals.getSuitFont(), pos=(0, -0.5), scale=0.16, drawOrder=0, mayChange=1)
        self.smallTitleText = OnscreenText.OnscreenText('', fg=self.titleColor, font=ToontownGlobals.getSuitFont(
        ), pos=(0.65, 0.9), scale=0.08, drawOrder=0, mayChange=1, bg=(0.5, 0.5, 0.5, 0.5), align=TextNode.ARight)
    ...
    def initializeLevel(self, levelSpec):
        if __dev__:
            ...
            self.sendUpdate('requestCurrentLevelSpec', [
                            levelSpec.stringHash(), levelSpec.entTypeReg.getHashStr()])
        else:
    ...
    def privGotSpec(self, levelSpec):
        Level.Level.initializeLevel(
            self, self.doId, levelSpec, self.scenarioIndex)
        ...
        if not sameElements(modelZoneNums, specZoneNums):
            self.reportModelSpecSyncError(
                'model zone nums (%s) do not match spec zone nums (%s)' % (modelZoneNums, specZoneNums))
        self.initVisibility()
        self.placeLocalToon()
    ...
    def placeLocalToon(self, moveLocalAvatar=True):
        ...
        if self.entranceId in self.entranceId2entity:
            ...
            if moveLocalAvatar:
                epEnt.placeToon(base.localAvatar, self.avIdList.index(
                    base.localAvatar.doId), len(self.avIdList))
            ...
            if len(zoneEntIds):
                ...
                if moveLocalAvatar:
                    base.localAvatar.setPos(
                        render, initialZoneEnt.getZoneNode().getPos(render))
            else:
    ...
    def fixupLevelModel(self):
        for zoneNum, zoneNode in list(self.zoneNum2node.items()):
            ...
            if len(floorColls) > 0:
                floorCollName = '%s%s' % (
                    DistributedLevel.FloorCollPrefix, zoneNum)
    ...
    def requestReparent(self, entity, parentId, wrt=False):
        ...
        if parent is not None:
            ...
            DistributedLevel.notify.debug(
                'entity %s requesting reparent to %s, not yet created' % (entity, parentId))
            ...
            if parentId not in self.parent2pendingChildren:
                ...
                def doReparent(parentId=parentId, self=self, wrt=wrt):
                    ...
                    for child in self.parent2pendingChildren[parentId]:
                        DistributedLevel.notify.debug(
                            'performing pending reparent of %s to %s' % (child, parent))
                        if wrt:
    ...
    def initVisibility(self):
        ...
        taskMgr.add(self.visChangeTask, self.uniqueName(
            DistributedLevel.VisChangeTaskName), priority=49)
        return
        ...
        if not __debug__:
            if self.lastToonZone not in visibleZoneNums:
                if self.lastToonZone is not None:
                    self.notify.warning(
                        'adding zoneNum %s to visibility list because toon is standing in that zone!' % self.lastToonZone)
    ...
    def spawnTitleText(self):
        ...
        if description and description != '':
            ...
            if self.lastCamZone not in self.zonesEnteredList:
                ...
                titleSeq = Sequence(Func(self.hideSmallTitleText), Func(self.showTitleText), Wait(0.1), Wait(6.0), LerpColorInterval(self.titleText, 0.5, Vec4(
                    self.titleColor[0], self.titleColor[1], self.titleColor[2], self.titleColor[3]), startColor=Vec4(self.titleColor[0], self.titleColor[1], self.titleColor[2], 0.0)))
            smallTitleSeq = Sequence(
                Func(self.hideTitleText), Func(self.showSmallTitle))
            if titleSeq:
    ...
    def showInfoText(self, text='hello world'):
        ...
        if description and description != '':
            ...
            self.titleText.setFg(self.titleColor)
            titleSeq = None
            titleSeq = Sequence(Func(self.hideSmallTitleText), Func(self.showTitleText), Wait(0.1), Wait(3.0), LerpColorInterval(self.titleText, 0.5, Vec4(
                self.titleColor[0], self.titleColor[1], self.titleColor[2], self.titleColor[3]), startColor=Vec4(self.titleColor[0], self.titleColor[1], self.titleColor[2], 0.0)))
            if titleSeq:
        ...
        if not hasattr(self, 'doingOuch'):
            ...
            def doOuch(task, self=self, ouchLevel=ouchLevel, period=period):
                ...
                taskMgr.doMethodLater(
                    period, doOuch, DistributedLevel.OuchTaskName)
