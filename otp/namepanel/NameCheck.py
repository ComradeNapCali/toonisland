...
def checkName(name, otherCheckFuncs=[], font=None):
        ...
        for char in name:
            if not _validCharacter(char):
                if char in string.digits:
                    ...
                    notify.info(
                        "name contains bad char: %s" % TextEncoder().encodeWtext(char)
                    )
        ...
        if font:
            ...
            for c in name:
                if not tn.hasCharacter(str(c)):
                    notify.info(
                        "name contains bad char: %s" % TextEncoder().encodeWtext(c)
                    )
    ...
    def hasLetters(name):
        ...
        for word in words:
            ...
            if len(letters) == 0:
                notify.info(
                    'word "%s" has no letters' % TextEncoder().encodeWtext(word)
                )
    ...
    def hasVowels(name):
        def perWord(word):
            ...
            if len(letters) > 2:
                ...
                if len(vowels) == 0:
                    notify.info(
                        'word "%s" has no vowels' % TextEncoder().encodeWtext(word)
                    )
    ...
    def checkPeriods(name):
        ...
        for word in words:
            ...
            if numPeriods > 2:
                notify.info(
                    'word "%s" has too many periods' % TextEncoder().encodeWtext(word)
                )
                return OTPLocalizer.NCPeriodUsage
            if numPeriods == 2:
    ...
    def checkJapanese(name):
        ...
        if dc < 2:
            ...
            notify.info(
                "name has been occupied more than eight display cells: %0.1f" % dc
            )
            return OTPLocalizer.NCGeneric
