...
class NameTumbler(DirectFrame):
    ...
    def __init__(self, nameList, category):
        DirectFrame.__init__(self, parent=aspect2d, relief='flat', scale=(
            1, 1, 1), state='disabled', frameColor=(1, 1, 1, 0))
    ...
    def loadTumblerGUI(self):
        ...
        if self.category != '':
            self.check = self.makeCheckBox(
                (-0.617, 0, 0.374), self.category, (0, 0.25, 0.5, 1), self.toggleTumbler)
    ...
    def makeLabel(self, te, index, others):
        ...
        if alig == TextNode.ARight:
            ...
            newpos = (0.2, 0, 0)
        df = DirectFrame(state='normal', relief=None, text=te, text_scale=0.1,
    ...
    def makeCheckBox(self, npos, ntex, ntexcolor, comm):
        dcf = DirectCheckButton(parent=aspect2d, relief=None, scale=0.1, boxBorder=0.08, boxImage=self.circle, boxImageScale=(0.4, 0.4, 0.4), boxRelief=None, pos=npos, text=ntex,
                                text_fg=ntexcolor, text_scale=0.8, text_pos=(0.2, 0), indicator_pos=(-0.566667, 0, -0.045), indicator_image_pos=(-0.26, 0, 0.075), command=comm, text_align=TextNode.ALeft)
