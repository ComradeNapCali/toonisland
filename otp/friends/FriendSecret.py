...
class FriendSecretNeedsParentLogin(StateData.StateData):
    ...
    def __handleOKWithParentAccount(self, *args):
        ...
        if okflag:
            ...
            base.localAvatar.chatMgr.problemActivatingChat["text"] = (
                OTPLocalizer.ProblemActivatingChat % message
            )
        else:
            self.dialog[
                "text"
            ] = OTPLocalizer.FriendSecretNeedsPasswordWarningWrongPassword
            self.passwordEntry["focus"] = 1
            self.passwordEntry.enterText("")
    ...
    def __handleOK(self, *args):
        ...
        self.accept(
            OTPGlobals.PlayerFriendRejectUseSecretEvent, self.__handleParentLogin
        )
        self.exit()
    ...
    def __handleParentLogin(self, reason):
        if reason == 0:
            ...
            self.dialog[
                "text"
            ] = OTPLocalizer.FriendSecretNeedsPasswordWarningWrongUsername
            self.usernameEntry["focus"] = 1
            self.usernameEntry.enterText("")
        elif reason == 2:
            self.dialog[
...
class FriendSecret(DirectFrame, StateData.StateData):
    ...
    def __getSecret(self):
        ...
        if self.requestedSecretType == AvatarSecret:
            ...
            self.accept(
                OTPGlobals.PlayerFriendRejectNewSecretEvent, self.__rejectAccountSecret
            )
    ...
    def __enteredSecret(self, result, avId):
        ...
        if result == 1:
            ...
            if handle != None:
                self.nextText["text"] = (
                    OTPLocalizer.FriendSecretEnteredSecretSuccess % handle.getName()
                )
            else:
            ...
            if handle != None:
                self.nextText["text"] = (
                    OTPLocalizer.FriendSecretEnteredSecretFull % handle.getName()
                )
            else:
            ...
            self.nextText["text"] = (
                OTPLocalizer.FriendSecretEnteredSecretWrongProduct % self.prefix
            )
        self.nextText.show()
        self.cancel.hide()
        self.ok1.hide()
    ...
    def __nowFriends(self, avId):
        ...
        if handle != None:
            self.nextText["text"] = (
                OTPLocalizer.FriendSecretNowFriends % handle.getName()
            )
        else:
