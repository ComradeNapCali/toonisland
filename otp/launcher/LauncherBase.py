...
class LauncherBase(DirectObject):
    ...
    def __init__(self):
        ...
        os.environ["CONFIG_CONFIG"] = (
            ":_:configdir_.:configpath_:configname_Configrc.exe:configexe_1:configargs_-stdout "
            + Configrc_args
        )
        ...
        self.setServerVersion(
            launcherConfig.GetString("server-version", "no_version_set")
        )
        ...
        while not connOk:
            ...
            if not connOk:
                ...
                if (
                    statusCode == 407
                    or statusCode == 1407
                    or statusCode == HTTPChannel.SCSocksNoAcceptableLoginMethod
                ):
    ...
    def handleGenericMultifileError(self):
        ...
        if self.curMultifileRetry < self.multifileRetries:
            ...
            self.notify.info(
                "downloadPatchDone: Recovering from error."
                + " Deleting files in: "
                + self.currentMfname
            )
    ...
    def download(self, serverFilePath, localFilename, callback, callbackProgress):
        self.launcherMessage(
            self.Localizer.LauncherDownloadFile
            % {
    ...
    def downloadTask(self, task):
        ...
        if self.httpChannel.isValid() and self.httpChannel.isDownloadComplete():
            ...
            self.notify.info(
                "downloadTask: Download done: %s" % task.serverFileURL.cStr()
            )
            task.callback()
            del task.callback
            return task.done
            ...
            if (
                statusCode == HTTPChannel.SCDownloadOpenError
                or statusCode == HTTPChannel.SCDownloadWriteError
            ):
                ...
                if task.downloadRam:
                    ...
                    self.download(
                        task.serverFilePath, task.localFilename, task.callback, None
                    )
            else:
    ...
    def getPercentPatchComplete(self, bytesWritten):
        return int(
            round(
                (self.patchDownloadSoFar + bytesWritten)
                / float(self.totalPatchDownload)
        ...
        if self.httpChannel.isValid() and self.httpChannel.isDownloadComplete():
            ...
            if (
                ...
                self.downloadMultifile(
                    ...
                    task.totalSize,
                    bytesWritten,
                    task.callbackProgress,
                )
            elif (
                statusCode == 416 or statusCode == HTTPChannel.SCDownloadInvalidRange
            ) and self.httpChannel.getFirstByteRequested() != 0:
    ...
    def decompressFileTask(self, task):
        ...
        if errorCode == EUOk:
            ...
            errorCode == EUErrorWriteOutOfFiles
            or errorCode == EUErrorWriteDiskFull
            or errorCode == EUErrorWriteDiskSectorNotFound
            or errorCode == EUErrorWriteOutOfMemory
            or errorCode == EUErrorWriteSharingViolation
            or errorCode == EUErrorWriteDiskFault
            or errorCode == EUErrorWriteDiskNotFound
        ):
    ...
    def extract(self, mfname, localFilename, destDir, callback):
        self.notify.info(
            "extract: request: " + localFilename.cStr() + " destDir: " + destDir.cStr()
        )
        self.launcherMessage(
            self.Localizer.LauncherExtractingFile
            % {
                "name": self.currentPhaseName,
                "current": self.currentPhaseIndex,
                "total": self.numPhases,
        ...
        if not task.extractor.setMultifile(task.localFilename):
            self.setPandaErrorCode(6)
            self.notify.info(
                "extract: Unable to open multifile %s" % task.localFilename.cStr()
            )
            sys.exit()
        ...
        for i in range(numFiles):
            ...
            if not task.extractor.requestSubfile(Filename(subfile)):
                self.setPandaErrorCode(6)
                self.notify.info(
                    "extract: Unable to find subfile %s in multifile %s"
                    % (subfile, mfname)
                )
                sys.exit()
    ...
    def patch(self, patchFile, patcheeFile, callback):
        self.notify.info(
            "patch: request: " + patchFile.cStr() + " patchee: " + patcheeFile.cStr()
        )
        self.launcherMessage(
            self.Localizer.LauncherPatchingFile
            % {
                "name": self.currentPhaseName,
                "current": self.currentPhaseIndex,
                "total": self.numPhases,
    ...
    def downloadClientDbStarterFile(self):
        ...
        self.download(
            self.clientStarterDbFilePath,
            localFilename,
            self.downloadClientDbStarterFileDone,
            None,
        )
        return
    ...
    def getMultifile(self, mfname):
        ...
        if not self.dldb.clientMultifileExists(mfname):
            self.maybeStartGame()
            self.notify.info(
                "Multifile does not exist in client db,"
                + "creating new record: "
                + mfname
            )
        ...
        if (
            not self.dldb.clientMultifileComplete(mfname)
            or not self.dldb.clientMultifileDecompressed(mfname)
        ) and decompressedFilename.exists():
            clientMd5 = HashVal()
        ...
        if (
            not self.dldb.clientMultifileComplete(mfname)
            or not decompressedFilename.exists()
        ):
            self.maybeStartGame()
            ...
            if currentSize == 0:
                self.notify.info(
                    "Multifile has not been started, "
    ...
    def downloadPatchDone(self):
        ...
        self.decompressFile(
            Filename(self.patchDir, Filename(self.currentPatch + ".pz")),
    ...
    def patchDone(self):
        ...
        destDir = Filename(self.topDir)
        self.extract(
            self.currentMfname, localFilename, destDir, self.updateMultifileDone
        )

    def startReextractingFiles(self):
        self.notify.info(
            "startReextractingFiles: Reextracting "
    ...
    def patchMultifile(self):
        self.launcherMessage(
            self.Localizer.LauncherCheckUpdates
            % {
                "name": self.currentPhaseName,
                "current": self.currentPhaseIndex,
                "total": self.numPhases,
        ...
        if clientVer == 1:
            ...
            self.notify.info(
                "total patch to be downloaded = " + repr((self.totalPatchDownload))
            )
    ...
    def setPercentPhaseComplete(self, phase, percent):
        ...
        if oldPercent != percent:
            ...
            self.overallComplete = (
                int(round(percent * self.phaseOverallMap[phase])) + self.progressSoFar
            )
    ...
    def getBandwidth(self):
        if self.backgrounded:
            bandwidth = (
                self.BANDWIDTH_ARRAY[self.bandwidthIndex] - self.TELEMETRY_BANDWIDTH
            )
        else:
        ...
        if self.bandwidthIndex == 0:
            ...
            if targetBandwidth:
                while (
                    ...
                    and self.BANDWIDTH_ARRAY[self.bandwidthIndex] > targetBandwidth
                ):
                    self.bandwidthIndex -= 1
    ...
    def MakeNTFSFilesGlobalWriteable(self, pathToSet=None):
        ...
        try:
            (
                ...
                sysflags,
                filesystemtype,
            ) = win32api.GetVolumeInformation(DrivePath)
        except:
            return
