...
class LauncherBase(DirectObject):
    ...
    def __init__(self):
        ...
        self.setServerVersion(launcherConfig.GetString(
        ...
        if downloadServerString:
            self.notify.info('Overriding downloadServer to %s.' %
                             downloadServerString)
        else:
        ...
        if self.http.getDirectHostSpec() != '':
            self.notify.info('Direct hosts list is: %s' %
                             self.http.getDirectHostSpec())
        ...
        while not connOk:
            ...
            if not connOk:
                self.notify.warning(
                    'Could not contact download server at %s' % testurl.cStr())
    ...
    def getNextDownloadServer(self):
        ...
        self.notify.info('Using download server %s.' %
                         self.downloadServer.cStr())
    ...
    def handleGenericMultifileError(self):
        ...
        if self.curMultifileRetry < self.multifileRetries:
            self.notify.info('recover attempt: %s / %s' %
        ...
        if self.httpChannel.isValid() and self.httpChannel.isDownloadComplete():
            ...
            if totalBytes:
                ...
                self.launcherMessage(self.Localizer.LauncherDownloadFileBytes % {'name': self.currentPhaseName,
                                                                                 ...
                                                                                 'total': self.numPhases,
                                                                                 'bytes': bytesWritten})
            self.notify.info('downloadTask: Download done: %s' %
            ...
            if statusCode == HTTPChannel.SCDownloadOpenError or statusCode == HTTPChannel.SCDownloadWriteError:
                ...
                self.notify.info(
                    'Connection lost while downloading; got %s bytes.  Reconnecting.' % gotBytes)
                if task.downloadRam:
                    self.downloadRam(task.serverFilePath, task.callback)
                else:
                    self.download(task.serverFilePath,
                                  task.localFilename, task.callback, None)
            else:
                if self.httpChannel.isValid():
                    self.notify.info(
                        'Unexpected situation: no error status, but %s incompletely downloaded.' % task.serverFileURL.cStr())
    ...
    def downloadPatchSimpleProgress(self, task):
        ...
        percentPatchComplete = int(
            round(bytesWritten / float(totalBytes) * self.downloadPercentage))
    ...
    def downloadMultifileWriteToDisk(self, task):
        ...
        if task.totalSize != 0:
            percentComplete = int(
                round(bytesWritten / float(task.totalSize) * self.downloadPercentage))
    ...
    def downloadMultifileTask(self, task):
        ...
        if self.httpChannel.run():
            ...
            if now - task.lastUpdate >= self.UserUpdateDelay:
                ...
                if task.totalSize != 0:
                    percentComplete = int(
                        round(100.0 * bytesWritten / float(task.totalSize)))
                self.launcherMessage(self.Localizer.LauncherDownloadFilePercent % {'name': self.currentPhaseName,
                                                                                   'current': self.currentPhaseIndex,
                                                                                   'total': self.numPhases,
        ...
        if self.httpChannel.isValid() and self.httpChannel.isDownloadComplete():
            ...
            if statusCode == HTTPChannel.SCDownloadOpenError or statusCode == HTTPChannel.SCDownloadWriteError:
                ...
                bytesWritten = startingByte + bytesDownloaded
                self.notify.info(
                    'Connection lost while downloading; got %s bytes.  Reconnecting.' % bytesDownloaded)
                self.downloadMultifile(task.serverFilename, task.localFilename, task.mfname,
                                       task.callback, task.totalSize, bytesWritten, task.callbackProgress)
                ...
                self.downloadMultifile(task.serverFilename, task.localFilename,
                                       task.mfname, task.callback, task.totalSize, 0, task.callbackProgress)
            else:
                if self.httpChannel.isValid():
                    self.notify.info(
                        'Unexpected situation: no error status, but %s incompletely downloaded.' % task.mfname)
                self.handleDownloadFatalError(task)
                self.downloadMultifile(task.serverFilename, task.localFilename,
                                       task.mfname, task.callback, task.totalSize, 0, task.callbackProgress)
            return task.done
    ...
    def decompressFileTask(self, task):
        ...
        if errorCode == EUOk:
            ...
            self.launcherMessage(self.Localizer.LauncherDecompressingPercent % {'name': self.currentPhaseName,
                                                                                ...
                                                                                'percent': 100})
            self.notify.info(
                'decompressTask: Decompress done: ' + task.localFilename.cStr())
            ...
            self.notify.warning(
                'decompressMultifileTask: Unknown success return code: ' + errorToText(errorCode))
            return task.cont
        else:
            self.notify.warning(
                'decompressMultifileTask: Unknown return code: ' + errorToText(errorCode))
            self.handleDecompressFatalError(task, errorCode)
            return task.done
    ...
    def decompressMultifileTask(self, task):
        ...
        if errorCode == EUOk:
            ...
            self.setPercentPhaseComplete(self.currentPhase, totalPercent)
            self.notify.info(
                'decompressMultifileTask: Decompress multifile done: ' + task.localFilename.cStr())
    ...
    def extract(self, mfname, localFilename, destDir, callback):
        self.notify.info('extract: request: ' +
                         localFilename.cStr() + ' destDir: ' + destDir.cStr())
        ...
        if not task.extractor.setMultifile(task.localFilename):
            self.setPandaErrorCode(6)
            self.notify.info('extract: Unable to open multifile %s' %
        ...
        for i in range(numFiles):
            ...
            if not task.extractor.requestSubfile(Filename(subfile)):
                self.setPandaErrorCode(6)
                self.notify.info(
                    'extract: Unable to find subfile %s in multifile %s' % (subfile, mfname))
                sys.exit()

        self.notify.info(
            'Extracting %d subfiles from multifile %s.' % (numFiles, mfname))
    ...
    def extractTask(self, task):
        ...
        if errorCode == EUOk:
            ...
            if now - task.lastUpdate >= self.UserUpdateDelay:
                ...
                totalPercent = self.downloadPercentage + \
                    self.decompressPercentage + percentProgress
                self.setPercentPhaseComplete(self.currentPhase, totalPercent)
            self.foregroundSleep()
            return task.cont
        elif errorCode == EUSuccess:
            self.launcherMessage(self.Localizer.LauncherExtractingPercent % {'name': self.currentPhaseName,
                                                                             ...
                                                                             'percent': 100})
            totalPercent = self.downloadPercentage + \
                self.decompressPercentage + self.extractPercentage
            self.setPercentPhaseComplete(self.currentPhase, totalPercent)
            self.notify.info(
                'extractTask: Extract multifile done: ' + task.localFilename.cStr())
            ...
            self.notify.warning(
                'extractTask: Unknown success return code: ' + errorToText(errorCode))
            return task.cont
        else:
            self.notify.warning(
                'extractTask: Unknown error return code: ' + errorToText(errorCode))
            self.handleExtractFatalError(task, errorCode)
            return task.done
    ...
    def patchTask(self, task):
        ...
        if errorCode == EUOk:
            ...
            self.launcherMessage(self.Localizer.LauncherPatchingPercent % {'name': self.currentPhaseName,
                                                                           ...
                                                                           'total': self.numPhases,
                                                                           'percent': 100})
            self.notify.info('patchTask: Patch done: ' +
            ...
            self.notify.warning(
                'patchTask: Unknown success return code: ' + errorToText(errorCode))
            return task.cont
        else:
            self.notify.warning(
                'patchTask: Unknown error return code: ' + errorToText(errorCode))
            self.handlePatchFatalError(task, errorCode)
            return task.done
    ...
    def readProgressFile(self):
        ...
        if not localFilename.exists():
            self.notify.warning('File does not exist: %s' %
                                localFilename.cStr())
        ...
        self.notify.info('total phases to be downloaded = ' +
                         repr((self.progressSum)))
            ...
            for fileDesc in self.ramfile.readlines():
                ...
                if localHash != serverHash:
                    self.failLauncherFileDb(
                        '%s does not match expected version.' % filename)
    ...
    def downloadClientDbStarterFile(self):
        ...
        localFilename = Filename(
            self.dbDir, Filename(self.compClientDbFilename))
        self.download(self.clientStarterDbFilePath, localFilename,
    ...
    def updateNextMultifile(self):
        if len(self.phaseMultifileNames) > 0:
            ...
            self.notify.info(
                'Done updating multifiles in phase: ' + repr((self.currentPhase)))
    ...
    def getMultifile(self, mfname):
        ...
        if not self.dldb.clientMultifileExists(mfname):
            self.maybeStartGame()
            self.notify.info(
                'Multifile does not exist in client db,' + 'creating new record: ' + mfname)
        ...
        if (not self.dldb.clientMultifileComplete(mfname) or not self.dldb.clientMultifileDecompressed(mfname)) and decompressedFilename.exists():
            ...
            if clientVer != -1:
                self.notify.info(
                    'Decompressed multifile is already on disk and correct: %s (version %s)' % (mfname, clientVer))
                self.dldb.setClientMultifileComplete(mfname)
        ...
        if not self.dldb.clientMultifileComplete(mfname) or not decompressedFilename.exists():
            ...
            if currentSize == 0:
                self.notify.info(
                    'Multifile has not been started, ' + 'downloading new file: ' + mfname)
                curHash = self.dldb.getServerMultifileHash(mfname)
                self.dldb.setClientMultifileHash(mfname, curHash)
                self.phaseNewDownload[self.currentPhase] = 1
                self.downloadMultifile(self.contentDir + mfname, localFilename, mfname,
                                       self.downloadMultifileDone, totalSize, 0, self.downloadMultifileWriteToDisk)
            else:
                ...
                if clientHash.eq(serverHash):
                    self.notify.info(
                        'Multifile is not complete, finishing download for %s, size = %s / %s' % (mfname, currentSize, totalSize))
                    self.downloadMultifile(self.contentDir + mfname, localFilename, mfname,
                                           self.downloadMultifileDone, totalSize, currentSize, self.downloadMultifileWriteToDisk)
    ...
    def extractMultifile(self, mfname):
        if not self.dldb.clientMultifileExtracted(mfname):
            ...
            self.extract(mfname, localFilename, destDir,
    ...
    def extractMultifileDone(self):
        ...
        self.notify.info(
            'extractMultifileDone: Finished updating multifile: ' + self.currentMfname)
        self.patchMultifile()
    ...
    def downloadPatches(self):
        if len(self.patchList) > 0:
            ...
            if self.currentPhase > 3:
                self.download(serverPatchFilePath, localPatchFilename,
                              self.downloadPatchDone, self.downloadPatchSimpleProgress)
            else:
                self.download(serverPatchFilePath, localPatchFilename,
                              self.downloadPatchDone, self.downloadPatchOverallProgress)
        else:
            self.notify.info(
                'applyNextPatch: Done patching multifile: ' + repr((self.currentPhase)))
            self.patchDone()

    def downloadPatchDone(self):
        self.patchDownloadSoFar += self.httpChannel.getBytesDownloaded()
        self.notify.info(
            'downloadPatchDone: Decompressing patch file: ' + self.currentPatch + '.pz')
        self.decompressFile(Filename(self.patchDir, Filename(
            self.currentPatch + '.pz')), self.decompressPatchDone)

    def decompressPatchDone(self):
        self.notify.info('decompressPatchDone: Patching file: ' +
                         self.currentPatchee + ' from ver: ' + repr((self.currentPatchVersion)))
    ...
    def patchDone(self):
        ...
        localFilename = Filename(self.mfDir, Filename(decompressedMfname))
        destDir = Filename(self.topDir)
        self.extract(self.currentMfname, localFilename,
                     destDir, self.updateMultifileDone)

    def startReextractingFiles(self):
        self.notify.info('startReextractingFiles: Reextracting ' + repr(
            (len(self.reextractList))) + ' files for multifile: ' + self.currentMfname)
        self.launcherMessage(self.Localizer.LauncherRecoverFiles)
        self.currentMfile = Multifile()
        decompressedMfname = os.path.splitext(self.currentMfname)[0]
        self.currentMfile.openRead(
            Filename(self.mfDir, Filename(decompressedMfname)))
    ...
    def reextractNextFile(self):
        ...
        while not failure and len(self.reextractList) > 0:
            ...
            if subfileIndex >= 0:
                ...
                self.notify.warning(
                    'reextractNextFile: File not found in multifile: ' + repr(currentReextractFile))
                failure = 1

        if failure:
            sys.exit()
        self.notify.info(
            'reextractNextFile: Done reextracting files for multifile: ' + repr((self.currentPhase)))
    ...
    def patchMultifile(self):
        self.launcherMessage(self.Localizer.LauncherCheckUpdates % {'name': self.currentPhaseName,
                                                                    ...
                                                                    'total': self.numPhases})
        self.notify.info(
            'patchMultifile: Checking for patches on multifile: ' + self.currentMfname)
        self.patchList = []
        clientMd5 = HashVal()
        decompressedMfname = os.path.splitext(self.currentMfname)[0]
        localFilename = Filename(self.mfDir, Filename(decompressedMfname))
        ...
        if clientVer == 1:
            ...
            self.notify.info(
                'patchMultifile: Invalid hash for file: ' + self.currentMfname)
            self.maybeStartGame()
            if self.curMultifileRetry < self.multifileRetries:
                self.notify.info('recover attempt: %s / %s' %
                                 (self.curMultifileRetry, self.multifileRetries))
            ...
            self.notify.info('total patch to be downloaded = ' +
                             repr((self.totalPatchDownload)))
    ...
    def setDisconnectDetails(self, newCode, newMsg):
        self.notify.info('New Disconnect Details: %s - %s ' %
                         (newCode, newMsg))
    ...
    def scanForHacks(self):
        ...
        try:
            ...
            pass
        knownHacksMUI = {
            '!xspeednet': hackName[0], 'aspeeder': hackName[1], 'speed gear': hackName[2]}
        i = 0
        try:
            rh = winreg.OpenKey(
                winreg.HKEY_CURRENT_USER, 'Software\\Microsoft\\Windows\\ShellNoRoam\\MUICache')
            while 1:
        ...
        try:
            ...
            knownHacksExe = {
                '!xspeednet.exe': hackName[0], 'aspeeder.exe': hackName[1], 'speedgear.exe': hackName[2]}
            try:
