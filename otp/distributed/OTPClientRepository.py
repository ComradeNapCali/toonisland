...
from otp.login import LoginTIAAccount
...
class OTPClientRepository(ClientRepositoryBase):
    ...
    def __init__(self, serverVersion, launcher=None, playGame=None):
        ...
        if fakeDISLToken:
            ...
            self.DISLToken = (
                "ACCOUNT_NAME=%s" % fakeDISLPlayerName
                + "&ACCOUNT_NUMBER=%s" % playerAccountId
                + "&ACCOUNT_NAME_APPROVAL=%s"
                % config.GetString("fake-DISL-PlayerNameApproved", "YES")
                + "&SWID=%s"
                % config.GetString(
                    "fake-DISL-SWID", "{1763AC36-D73F-41C2-A54A-B579E58B69C8}"
                )
                + "&FAMILY_NUMBER=%s"
                % config.GetString("fake-DISL-FamilyAccountId", "-1")
                + "&familyAdmin=%s" % config.GetString("fake-DISL-FamilyAdmin", "1")
                + "&PIRATES_ACCESS=%s"
                % config.GetString("fake-DISL-PiratesAccess", "FULL")
                + "&PIRATES_MAX_NUM_AVATARS=%s"
                % config.GetInt("fake-DISL-MaxAvatars", defaultNumAvatars)
                + "&PIRATES_NUM_AVATAR_SLOTS=%s"
                % config.GetInt("fake-DISL-MaxAvatarSlots", defaultNumAvatarSlots)
                + "&expires=%s" % config.GetString("fake-DISL-expire", "1577898000")
                + "&OPEN_CHAT_ENABLED=%s"
                % config.GetString("fake-DISL-OpenChatEnabled", "YES")
                + "&CREATE_FRIENDS_WITH_CHAT=%s"
                % config.GetString("fake-DISL-CreateFriendsWithChat", "YES")
                + "&CHAT_CODE_CREATION_RULE=%s"
                % config.GetString("fake-DISL-ChatCodeCreation", "YES")
            ...
            for i in range(subCount):
                self.DISLToken += (
                    ...
                    % (
                        i,
                        config.GetString(
                            "fake-DISL-Sub-%s-Name" % i, fakeDISLPlayerName
                        ),
                    )
                    ...
                    % (
                        i,
                        config.GetInt(
                            "fake-DISL-Sub-%s-NumAvatars" % i, defaultNumAvatars
                        ),
                    )
                    ...
                    % (
                        i,
                        config.GetInt(
                            "fake-DISL-Sub-%s-NumConcur" % i, defaultNumConcur
                        ),
                    )
                    + "&PIRATES_SUB_%s_OWNERID=%s"
                    % (
                        i,
                        config.GetInt("fake-DISL-Sub-%s-OwnerId" % i, playerAccountId),
                    )
            ...
            self.DISLToken += (
                "&WL_CHAT_ENABLED=%s"
                % config.GetString("fake-DISL-WLChatEnabled", "YES")
                + "&valid=true"
            )
        ...
        self.accountOldAuth = config.GetBool(
            "%s-account-old-auth" % game.name, self.accountOldAuth
        )

        self.loginInterface = LoginTIAAccount.LoginTIAAccount(self)
        self.notify.info("loginInterface: LoginTIAAccount")
        ...
        self.secretChatNeedsParentPassword = base.config.GetBool(
            "secret-chat-needs-parent-password", 0
        ) or (self.launcher and self.launcher.getNeedPwForSecretKey())
        self.parentPasswordSet = base.config.GetBool("parent-password-set", 0) or (
        ...
        self._proactiveLeakChecks = config.GetBool(
            "proactive-leak-checks", 1
        ) or config.GetBool("client-proactive-leak-checks", 1)
        ...
        self.gameServicesManager = None  # To be overridden by subclass.
        self.chatManager = None  # To be overridden by subclass.
    ...
    def startLeakDetector(self):
        ...
        self.messageListenerTypesLeakDetector = (
    ...
    def handleSystemMessage(self, di):
        ...
        whisper = WhisperPopup(
            message, OTPGlobals.getInterfaceFont(), WhisperPopup.WTSystem
        )
    ...
    def __handleLoginDone(self, doneStatus):
        ...
        if mode == "success":
            ...
            self.notify.error(
                "Invalid doneStatus mode from gameServicesManager: " + str(mode)
            )
    ...
    def enterCreateAccount(
        self, createAccountDoneData={"back": "login", "backArgs": []}
    ):
    ...
    def __handleCreateAccountDone(self, doneStatus):
        ...
        if mode == "success":
            ...
            self.notify.error(
                "Invalid doneStatus mode from CreateAccountScreen: " + str(mode)
            )
    ...
    def enterFailedToConnect(self, statusCode, statusString):
        ...
        if self.introduction.getCurrentOrNextState() in ("Disclaimer", "Presents"):
            return
        if not self.introDone:
            if style == OTPDialog.CancelOnly:
                self.introduction.request(
                    "ExitDialog", message, self.loginFSM.request, ["shutdown"]
                )
            else:
                self.introduction.request(
                    "YesNoDialog",
    ...
    def enterFailedToGetServerConstants(self, e):
        ...
        if isinstance(e, HTTPUtil.ConnectionError):
            ...
            self.notify.warning(
                "Got status code %s from connection to %s." % (statusCode, url.cStr())
            )
        else:
            self.notify.warning(
    ...
    def enterMissingGameRootObject(self):
        ...
        if not self.introDone:
            self.introduction.request(
                "YesNoDialog",
                OTPLocalizer.CRMissingGameRootObject,
                self.loginFSM.request,
                ["waitForGameList"],
                self.loginFSM.request,
                ["shutdown"],
            ...
            self.accept(
                "missingGameRootObjectBoxAck", self.__handleMissingGameRootObjectAck
            )
    ...
    def enterNoShards(self):
        ...
        if not self.introDone:
            self.introduction.request(
                "YesNoDialog",
                OTPLocalizer.CRNoDistrictsTryAgain,
                self.loginFSM.request,
                ["noShardsWait"],
                self.loginFSM.request,
                ["shutdown"],
    ...
    def enterNoConnection(self):
        ...
        if (
            self.bootedIndex is not None
            and self.bootedIndex in OTPLocalizer.CRBootedReasons
        ):
            message = OTPLocalizer.CRBootedReasons[self.bootedIndex] % {"name": "N/A"}
        elif self.bootedText:
            message = (
                OTPLocalizer.CRBootedReasonUnknownCode % self.bootedIndex
                + OTPLocalizer.CRBootedAdditionalInfo.format(self.bootedText)
            )
        else:
            message = OTPLocalizer.CRLostConnection
        reconnect = self.bootedIndex not in OTPGlobals.BootedNoReconnect
        additionalInfo = (
            self.bootedIndex in OTPGlobals.BootedMoreInfo and self.bootedText
        )
        if additionalInfo:
            message += OTPLocalizer.CRBootedAdditionalInfo.format(self.bootedText)
    ...
    def __handleLostConnectionAck(self):
        if (
            self.lostConnectionBox.doneStatus == "ok"
            and self.loginInterface.supportsRelogin()
        ):
            self.loginFSM.request("connect", [self.serverList])
        else:
            self.loginFSM.request("shutdown")
    ...
    def enterAfkTimeout(self):
        ...
        self.afkDialog = dialogClass(
            text=msg, command=self.__handleAfkOk, style=OTPDialog.Acknowledge
        )
        self.handler = self.handleMessageType
    ...
    def enterPeriodTimeout(self):
        ...
        self.periodDialog = dialogClass(
            text=msg, command=self.__handlePeriodOk, style=OTPDialog.Acknowledge
        )
        self.handler = self.handleMessageType
    ...
    def _requestAvatarList(self):
        self.gameServicesManager.requestAvatarList()
        if not self.introDone:
            pass
        else:
            self.waitForDatabaseTimeout(requestName="WaitForAvatarList")
        self.acceptOnce(
            OtpAvatarManager.OtpAvatarManager.OnlineEvent, self._requestAvatarList
        )
    ...
    def enterWaitForDeleteAvatarResponse(self, potAv):
        self.gameServicesManager.requestRemoveAvatar(potAv.id)
    ...
    def sendSetAvatarIdMsg(self, avId):
        if avId != self.__currentAvId:
            ...
            self.gameServicesManager.requestPlayAvatar(avId)
            if avId == 0:
    ...
    def detectLeaks(self, okTasks=None, okEvents=None):
        ...
        if leakedTasks or leakedEvents or leakedIvals or leakedGarbage:
            ...
            if (
                errorCode >= OTPLauncherGlobals.NonErrorExitStateStart
                and errorCode <= OTPLauncherGlobals.NonErrorExitStateEnd
            ):
            ...
            if allowExit:
                self.notify.info(
                    "Allowing client to leave, panda error code %s" % errorCode
                )
            else:
                base.userExit()
        else:
            ...
            for hook in problems:
                ...
                for obj in whoAccepts:
                    ...
                    if hasattr(obj, "getCreationStackTraceCompactStr"):
                        msg += (
                            "\n   CREATIONSTACKTRACE:%s"
                            % obj.getCreationStackTraceCompactStr()
                        )
                    else:
                        try:
                            ...
                            if hasattr(guiObj, "getCreationStackTraceCompactStr"):
                                msg += (
                                    "\n   CREATIONSTACKTRACE:%s"
                                    % guiObj.getCreationStackTraceCompactStr()
                                )
                        except:
                            pass
    ...
    def isShardInterestOpen(self):
        self.notify.error(
            "%s must override isShardInterestOpen" % self.__class__.__name__
        )
    ...
    def _removeCurrentShardInterest(self, callback):
        self.notify.error(
            "%s must override _removeCurrentShardInterest" % self.__class__.__name__
        )
    ...
    def isFreeTimeExpired(self):
        ...
        if self.freeTimeExpiresAt < -1:
            self.notify.warning(
                "freeTimeExpiresAt is less than -1 (%s)" % self.freeTimeExpiresAt
            )
    ...
    def allowSecretChat(self):
        return (
            self.secretChatAllowed
            or self.productName == "Terra-DMC"
            and self.isBlue()
            and self.secretChatAllowed
        )
    ...
    def needParentPasswordForSecretChat(self):
        return (
            self.isPaid()
            and self.secretChatNeedsParentPassword
            or self.productName == "Terra-DMC"
            and self.isBlue()
            and self.secretChatNeedsParentPassword
        )
        ...
        if base.logPrivateInfo:
            ...
            if not self.isPaid():
                if self.isFreeTimeExpired():
                    ...
                    self.notify.info(
                        "free time left: %s" % PythonUtil.formatElapsedSeconds(secs)
                    )
    ...
    def getStartingDistrict(self):
        ...
        if district is not None:
            self.notify.debug(
                "chose %s: pop %s" % (district.name, district.avatarCount)
            )
            try:
                Discord = DiscordRPC()
    ...
    def getPlayerAvatars(self):
        return [
            i for i in list(self.doId2do.values()) if isinstance(i, DistributedPlayer)
        ]
    ...
    def waitForDatabaseTimeout(self, extraTimeout=0, requestName="unknown"):
        ...
        globalClock.tick()
        taskMgr.doMethodLater(
            (OTPGlobals.DatabaseDialogTimeout + extraTimeout) * choice(__dev__, 10, 1),
    ...
    def __showWaitingForDatabase(self, requestName):
        ...
        OTPClientRepository.notify.info(
            "timed out waiting for %s at %s" % (requestName, globalClock.getFrameTime())
        )
        dialogClass = OTPGlobals.getDialogClass()
    ...
    def __giveUpWaitingForDatabase(self, requestName):
        OTPClientRepository.notify.info(
            "giving up waiting for %s at %s" % (requestName, globalClock.getFrameTime())
        )
        self.cleanupWaitingForDatabase()
        self.loginFSM.request("noConnection")
        return Task.done
    ...
    def gotInterestDoneMessage(self, di):
        if self.deferredGenerates:
            ...
            di2.getUint32()  # Context; this can be ignored.
    ...
    def handleGenerateWithRequired(self, di, other=False):
        ...
        # Should we add this to the pending generates,
        # or should we process it right away?
        for handle, interest in list(self._interests.items()):
            ...
            self.notify.warning(
                "Received generate for %s from %s:%s, not part of any existing interest!"
                % (doId, parentId, zoneId)
            )
            interest = None

        if not interest or not interest.events:
            # We can generate this object right away.
        ...
        pending.append(
            (doId, parentId, zoneId, classId, Datagram(di.getDatagram()), other)
        )
    ...
    def __playBackGenerates(self, handle):
        ...
        # This interest has pending generates, so let's play them.
        generates = self.__pendingGenerates[handle]
        del self.__pendingGenerates[handle]
        generates.sort(key=lambda x: x[3])  # This will sort by classId.
        ...
        # If we have any messages, play those back too.
    ...
    def __playBackMessages(self, handle):
        ...
        # Play back any pending messages.
    ...
    def addTaggedInterest(
        self, parentId, zoneId, mainTag, desc, otherTags=[], event=None
    ):
