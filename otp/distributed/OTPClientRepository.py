...
class OTPClientRepository(ClientRepositoryBase):
    ...
    def __init__(self, serverVersion, launcher=None, playGame=None):
        ...
        self.parentPasswordSet = base.config.GetBool(
            'parent-password-set', 0) or (self.launcher and self.launcher.getParentPasswordSet())
        ...
        self._proactiveLeakChecks = config.GetBool(
            'proactive-leak-checks', 1) or config.GetBool('client-proactive-leak-checks', 1)
    ...
    def enterConnect(self, serverList):
        ...
        if not self.introDone:
            if self.introduction.getCurrentOrNextState() not in ('Disclaimer', 'Presents'):
    ...
    def __handleCreateAccountDone(self, doneStatus):
        ...
        if mode == 'success':
            ...
            self.loginFSM.request(
                self.createAccountDoneData['back'], self.createAccountDoneData['backArgs'])
        elif mode == 'failure':
            self.loginFSM.request(
                self.createAccountDoneData['back'], self.createAccountDoneData['backArgs'])
        elif mode == 'quit':
    ...
    def enterFailedToConnect(self, statusCode, statusString):
        ...
        self.notify.warning('Failed to connect to %s (%s %s).  Notifying user.' % (
        ...
        if not self.introDone:
            if style == OTPDialog.CancelOnly:
                self.introduction.request('ExitDialog', message, self.loginFSM.request, ['shutdown'])
            else:
                self.introduction.request('YesNoDialog', message, self.loginFSM.request, ['connect', [self.serverList]], self.loginFSM.request, ['shutdown'])
        else:
            dialogClass = OTPGlobals.getGlobalDialogClass()
    ...
    def enterFailedToGetServerConstants(self, e):
        ...
        self.failedToGetConstantsBox = dialogClass(
            message=message, doneEvent='failedToGetConstantsAck', text_wordwrap=18, style=style)
    ...
    def enterWaitForGameList(self):
        self.gameDoDirectory = self.addTaggedInterest(
            self.GameGlobalsId, OTP_ZONE_ID_MANAGEMENT, self.ITAG_PERM, 'game directory', event='GameList_Complete')
    ...
    def enterMissingGameRootObject(self):
        ...
        if self.introduction.getCurrentOrNextState() in ('Disclaimer', 'Presents'):
            return
        if not self.introDone:
            self.introduction.request('YesNoDialog', OTPLocalizer.CRMissingGameRootObject, self.loginFSM.request, ['waitForGameList'], self.loginFSM.request, ['shutdown'])
        else:
            dialogClass = OTPGlobals.getGlobalDialogClass()
    ...
    def enterWaitForShardList(self):
        if not self.isValidInterestHandle(self.shardListHandle):
            self.shardListHandle = self.addTaggedInterest(
                self.GameGlobalsId, OTP_ZONE_ID_DISTRICTS, self.ITAG_PERM, 'LocalShardList', event='ShardList_Complete')
    ...
    def enterNoShards(self):
        ...
        if self.introduction.getCurrentOrNextState() in ('Disclaimer', 'Presents'):
            return
        if not self.introDone:
            self.introduction.request('YesNoDialog', OTPLocalizer.CRNoDistrictsTryAgain, self.loginFSM.request, ['noShardsWait'], self.loginFSM.request, ['shutdown'])
        else:
            dialogClass = OTPGlobals.getGlobalDialogClass()
    ...
    def enterNoConnection(self):
        ...
        if additionalInfo:
            message += OTPLocalizer.CRBootedAdditionalInfo.format(
                self.bootedText)
        ...
        self.lostConnectionBox = dialogClass(
            doneEvent='lostConnectionAck', message=message, text_wordwrap=18, style=style)
    ...
    def enterRejectRemoveAvatar(self, reasonCode):
        ...
        self.rejectRemoveAvatarBox = dialogClass(message='%s\n(%s)' % (
            OTPLocalizer.CRRejectRemoveAvatar, reasonCode), doneEvent='rejectRemoveAvatarAck', style=OTPDialog.Acknowledge)
    ...
    def detectLeakedGarbage(self, callback=None):
        ...
        if gc.garbage:
            self.notify.warning(
                'garbage already contains %d items' % len(gc.garbage))
    ...
    def handleSetShardComplete(self):
        ...
        self.uberZoneInterest = self.addInterest(
            base.localAvatar.defaultShard, OTPGlobals.UberZone, 'uberZone', 'uberZoneInterestComplete')
    ...
    def _removeLocalAvFromStateServer(self):
        ...
        if base.slowCloseShard:
            taskMgr.doMethodLater(base.slowCloseShardDelay * 0.5,
                                  Functor(self.removeShardInterest, callback), 'slowCloseShard')
        else:
    ...
    def _removeShardInterestComplete(self, callback):
        ...
        self.doDataCache.flush()
        if base.slowCloseShard:
            taskMgr.doMethodLater(base.slowCloseShardDelay * 0.5, Functor(
                self._callRemoveShardInterestCallback, callback), 'slowCloseShardCallback')
        else:
    ...
    def handlePlayGame(self, msgType, di):
        if self.notify.getDebug():
            self.notify.debug(
                'handle play game got message type: ' + repr(msgType))
    ...
    def logAccountInfo(self):
        ...
        if base.logPrivateInfo:
            ...
            if self.periodTimerSecondsRemaining != None:
                self.notify.info('period time left: %s' % PythonUtil.formatElapsedSeconds(
    ...
    def waitForDatabaseTimeout(self, extraTimeout=0, requestName='unknown'):
        OTPClientRepository.notify.debug('waiting for database timeout %s at %s' % (
            requestName, globalClock.getFrameTime()))
        self.cleanupWaitingForDatabase()
        globalClock.tick()
        taskMgr.doMethodLater((OTPGlobals.DatabaseDialogTimeout + extraTimeout) * choice(
            __dev__, 10, 1), self.__showWaitingForDatabase, 'waitingForDatabase', extraArgs=[requestName])
    ...
    def __showWaitingForDatabase(self, requestName):
        ...
        taskMgr.doMethodLater(OTPGlobals.DatabaseGiveupTimeout,
                              self.__giveUpWaitingForDatabase, 'waitingForDatabase', extraArgs=[requestName])
        return Task.done
    ...
    def recordPeriodTimer(self, task):
        ...
        taskMgr.doMethodLater(
            freq, self.recordPeriodTimer, 'periodTimerRecorder')
        return Task.done
    ...
    def startPeriodTimer(self):
        if self.periodTimerStarted == None and self.periodTimerSecondsRemaining != None:
            ...
            taskMgr.doMethodLater(self.periodTimerSecondsRemaining,
                                  self.__periodTimerExpired, 'periodTimerCountdown')
            for warning in OTPGlobals.PeriodTimerWarningTime:
                if self.periodTimerSecondsRemaining > warning:
                    taskMgr.doMethodLater(self.periodTimerSecondsRemaining -
                                          warning, self.__periodTimerWarning, 'periodTimerCountdown')
        ...
        if self._isPlayerDclass(dclass):
            if not self._isValidPlayerLocation(parentId, zoneId):
                base.cr.centralLogger.writeClientEvent(
                    'got generate for player avatar %s in invalid location (%s, %s)' % (doId, parentId, zoneId))
                return True
        return False
    ...
    def handleGenerateWithRequired(self, di, other=False):
        ...
        for handle, interest in list(self._interests.items()):
            ...
            self.notify.warning(
                'Received generate for %s from %s:%s, not part of any existing interest!' % (doId, parentId, zoneId))
            interest = None
        ...
        pending.append((doId, parentId, zoneId, classId,
                       Datagram(di.getDatagram()), other))
