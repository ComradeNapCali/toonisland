...
class TimeManager(DistributedObject.DistributedObject):
    ...
    def __init__(self, cr):
        ...
        if self.extraSkew != 0:
            self.notify.info('Simulating clock skew of %0.3f s' %
                             self.extraSkew)
    ...
    def serverTime(self, context, timestamp, timeOfDay):
        ...
        if self.talkResult:
            base.localAvatar.setChatAbsolute('latency %0.0f ms, sync \xc2\xb1%0.0f ms' % (
                elapsed * 1000.0, globalClockDelta.getUncertainty() * 1000.0), CFSpeech | CFTimeout)
        ...
        if toontownTimeManager:
            toontownTimeManager.updateLoginTimes(
                timeOfDay, int(time.time()), globalClock.getRealTime())
    ...
    def setFrameRateInterval(self, frameRateInterval):
        ...
        taskMgr.doMethodLater(
            frameRateInterval, self.frameRateMonitor, 'frameRateMonitor')
    ...
    def frameRateMonitor(self, task):
        ...
        self.d_setFrameRate(max(0, globalClock.getAverageFrameRate()), max(0, globalClock.calcFrameRateDeviation()), len(Avatar.ActiveAvatars), base.locationCode or '', max(0, time.time() - base.locationCodeChanged),
                            max(0, globalClock.getRealTime()), base.gameOptionsCode, vendorId, deviceId, processMemory, pageFileUsage, physicalMemory, pageFaultCount, osInfo, cpuSpeed, numCpuCores, numLogicalCpus, apiName)
        return task.again
    ...
    def getMacOsInfo(self, defaultOsInfo):
        ...
        try:
            ...
            key = re.search('<key>ProductUserVisibleVersion</key>\\s*' +
