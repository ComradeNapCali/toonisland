...
class TimeManager(DistributedObject.DistributedObject):
    ...
    def serverTime(self, context, timestamp, timeOfDay):
        ...
        self.notify.info(
            "Local clock uncertainty +/- %.3f s" % globalClockDelta.getUncertainty()
        )
    ...
    def d_setFrameRate(
        ...
        info = (
            "%0.1f fps|%0.3fd|%s avs|%s|%d|%d|%s|0x%04x|0x%04x|%0.1fMB|%0.1fMB|%0.1fMB|%d|%s|%s|%s cpus|%s"
            % (
                fps,
                deviation,
                numAvs,
        ...
        try:
            ...
            if key is not None:
                try:
                    ...
                    minor = int(parts[1])
                    bugfix = int(parts[2])
                    result = (sys.platform, bugfix, major, minor)
