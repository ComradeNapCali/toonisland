...
class Barrier(DirectObject.DirectObject):
    ...
    def clear(self, avId):
        if avId not in self.pendingAvatars:
            self.notify.warning(
                '%s: tried to clear %s, who was not listed.' % (self.uniqueName, avId))
            return
        self.notify.debug('%s: clearing avatar %s' % (self.uniqueName, avId))
        self.pendingAvatars.remove(avId)
        if len(self.pendingAvatars) == 0:
            self.notify.debug('%s: barrier cleared by %s' %
    ...
    def __timerExpired(self, task):
        self.notify.warning('%s: timeout expired; responses not received from %s' % (
