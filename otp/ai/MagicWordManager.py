...
class MagicWordManager(DistributedObject.DistributedObject):
    ...
    def doMagicWord(self, word, avId, zoneId):
        ...
        if wordIs("~oobe"):
            ...
            self.setMagicWordResponse(
                choice(
                    started, "leak detector started", "leak detector already started"
                )
            )
            ...
            DelayedCall(
                Functor(self.notify.error, "~clientcrash: simulating a client crash")
            )
            ...
            DelayedCall(
                Functor(base.cr.deleteObjectLocation, ScratchPad(doId=doId), 1, 1)
            )
    ...
    def d_setMagicWord(self, magicWord, avId, zoneId):
        self.sendUpdate(
            "setMagicWord", [magicWord, avId, zoneId, base.cr.userSignature]
        )
    ...
    def identifyDistributedObjects(self, name):
        ...
        for obj in list(self.cr.doId2do.values()):
            ...
            if (
                string.lower(name) == lowerName
                or string.lower(className) == lowerName
                or string.lower(className) == "distributed" + lowerName
            ):
    ...
    def getCSBitmask(self, str):
        ...
        for w in words:
            if w == "wall":
                ...
                bitmask |= (
                    OTPGlobals.FurnitureSideBitmask
    ...
    def showfont(self, fontname):
        ...
        for pi in range(numPages):
            ...
            (
                np.setPos(
                    float(x) / numXPages * 2 - 1 + pageMargin,
                    0,
    ...
    def identifyAvatar(self, name):
        ...
        for av in Avatar.Avatar.ActiveAvatars:
            if (
                isinstance(av, self.GameAvatarClass)
                and string.strip(string.lower(av.getName())) == lowerName
            ):
                return av.doId
