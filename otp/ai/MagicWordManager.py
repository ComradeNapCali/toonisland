...
class MagicWordManager(DistributedObject.DistributedObject):
    ...
    def doMagicWord(self, word, avId, zoneId):
        ...
        if wordIs('~oobe'):
            ...
            ContainerReport.ContainerReport(
                '~containers', log=True, limit=limit, threaded=True)
        elif wordIs('~garbage'):
            args = word.split()
            full = 'full' in args
            ...
            GarbageReport.GarbageLogger('~garbage', fullReport=full, threaded=True,
                                        safeMode=safeMode, delOnly=delOnly, doneCallback=self.garbageReportDone)
            ...
            self.setMagicWordResponse('frame profiling %s%s' % (
                choice(setting, 'ON', 'OFF'), choice(wasOn == setting, ' already', '')))
            ...
            self.setMagicWordResponse('task profiling %s%s' % (
                choice(setting, 'ON', 'OFF'), choice(wasOn == setting, ' already', '')))
            ...
            self.setMagicWordResponse(
                choice(started, 'leak detector started', 'leak detector already started'))
            ...
            DelayedCall(Functor(self.notify.error,
            ...
            DelayedCall(Functor(base.cr.deleteObjectLocation,
                        ScratchPad(doId=doId), 1, 1))
    ...
    def toggleRun(self):
        if config.GetBool('want-running', 1):
            inputState.set('debugRunning', inputState.isSet(
                'debugRunning') != True)
    ...
    def b_setMagicWord(self, magicWord, avId=None, zoneId=None):
        if self.cr.wantMagicWords:
            ...
            if magicWord.count("~exception"):
                self.notify.error(
                    "~exception: simulating a client exception...")
                s = ""
                while 1:
                    s += "INVALIDNAME"
                    eval(s)
    ...
    def showShadowCollisions(self):
        try:
            ...
            self.setMagicWordResponse(
                'CollisionVisualizer is not compiled in.')
    ...
    def doFps(self, word, avId, zoneId):
        ...
        if base.frameRateMeter:
            globalClock.setAverageFrameRateInterval(
                ConfigVariableDouble('average-frame-rate-interval').getValue())
        if response != None:
            self.setMagicWordResponse(response)
        return
    ...
    def garbageReportDone(self, garbageReport):
        self.setMagicWordResponse('%s garbage cycles' %
