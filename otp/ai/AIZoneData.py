...
class AIZoneData:
    ...
    def __init__(self, air, parentId, zoneId):
        ...
        self._zoneId = zoneId
        self._data = self._air.getZoneDataStore().getDataForZone(
...
class AIZoneDataObj:
    ...
    def __str__(self):
        ...
        output += 'Num traversers: %s  Num total colliders: %s' % (
            totalTraversers, totalColliders)
        return output
    ...
    def getRender(self):
        if not hasattr(self, '_render'):
            self._render = NodePath('render-%s-%s' %
                                    (self._parentId, self._zoneId))
    ...
    def startCollTrav(self, respectPrevTransform=1, cTravName=None):
        ...
        if cTravName not in self._collTravsStarted:
            ...
            taskMgr.add(self._doCollisions, self._getCTravTaskName(
                name=cTravName), priority=OTPGlobals.AICollisionPriority, extraArgs=[self._zoneId])
...
class AIZoneDataStore:
    ...
    def printStats(self):
        self.notify.debug('%s zones have zone data allocated' %
                          len(self._zone2data))
