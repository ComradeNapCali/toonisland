...
\1limeText\Thanks for everything!\2
    ...
    def startCredits(self):
        self.creditsMusic = base.loader.loadMusic(
            "phase_3.5/audio/bgm/hall_of_fame.ogg"
        )
        base.playMusic(self.creditsMusic, looping=0, volume=0.9)

        self.showCredits = Sequence(
            LerpColorScaleInterval(
                self.screenCover, 1, Vec4(1, 1, 1, 1), startColorScale=Vec4(1, 1, 1, 0)
            ),
            LerpColorScaleInterval(
                self.text, 1, Vec4(1, 1, 1, 1), startColorScale=Vec4(1, 1, 1, 0)
            ),
            Wait(1),
            self.text.posInterval(35, Point3(0, 0, 6)),
            Wait(1),
            LerpColorScaleInterval(
                self.screenCover, 1, Vec4(1, 1, 1, 0), startColorScale=Vec4(1, 1, 1, 1)
            ),
