...
class TTDialog(OTPDialog):
    def __init__(self, parent=None, style=NoButtons, **kw):
        ...
        Sequence(
            LerpScaleInterval(
                self, 0.2, Vec3(1.1, 1.1, 1.1), Vec3(0, 0, 0), blendType="easeInOut"
            ),
            LerpScaleInterval(
                self, 0.09, Vec3(1, 1, 1), Vec3(1.1, 1.1, 0.1), blendType="easeInOut"
            ),
        ).start()
...
class TTGlobalDialog(GlobalDialog):
    def __init__(
        ...
        **kw
        ...
        self.path = "phase_3/models/gui/dialog_box_buttons_gui"
        GlobalDialog.__init__(
            self, message, doneEvent, style, okButtonText, cancelButtonText, **kw
        )
        ...
        Sequence(
            ...
            LerpScaleInterval(
                self, 0.09, Vec3(1, 1, 1), Vec3(1.1, 1.1, 1.1), blendType="easeInOut"
            ),
        ).start()
