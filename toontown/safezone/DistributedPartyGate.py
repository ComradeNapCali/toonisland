...
class DistributedPartyGate(DistributedObject.DistributedObject):
    ...
    def loadClockSounds(self):
        ...
        for i in range(1, 13):
            ...
            self.clockSounds.append(
                base.loader.loadSfx("phase_4/audio/sfx/clock%s.ogg" % si)
            )
    ...
    def generate(self):
        ...
        self.partyGateCollNodePath = partyGate.find(
            "**/partyGate_stepsLocator"
        ).attachNewNode(self.partyGateSphere)
        self.__enableCollisions()
        self.toontownTimeGui = ServerTimeGui(partyGate, hourCallback=self.hourChange)
        self.toontownTimeGui.setPos(
            partyGate.find("**/clockText_locator").getPos() + Point3(0.0, 0.0, -0.2)
        )
    ...
    def announceGenerate(self):
        ...
        self.toontownTimeGui.hourLabel[
            "text_fg"
        ] = PartyGlobals.PlayGroundToPartyClockColors[playground]
    ...
    def __handleAskDone(self):
        ...
        if doneStatus is None:
            ...
            return
        self.sendUpdate(
            "partyChoiceRequest", [base.localAvatar.doId, doneStatus[0], doneStatus[1]]
        )
        return
    ...
    def hourChange(self, currentHour):
        ...
        seq2.append(
            self.clockFlat.scaleInterval(
                timeForEachDeformation / 2.0, Vec3(1.0, 1.0, 1.0), blendType="easeInOut"
            )
        )
