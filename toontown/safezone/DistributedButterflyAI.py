...
class DistributedButterflyAI(DistributedObjectAI.DistributedObjectAI):
    def __init__(self, air, playground, area, ownerId):
        ...
        self.fsm = ClassicFSM.ClassicFSM(
            ...
            [
                ...
                State.State("Landed", self.enterLanded, self.exitLanded, ["Flying"]),
            ],
            "off",
            "off",
        )
        ...
        (
            ...
            self.destIndex,
            self.time,
        ) = ButterflyGlobals.getFirstRoute(self.playground, self.area, self.ownerId)
        return None
    ...
    def delete(self):
        try:
            ...
            ButterflyGlobals.recycleIndex(
                self.curIndex, self.playground, self.area, self.ownerId
            )
            ButterflyGlobals.recycleIndex(
                self.destIndex, self.playground, self.area, self.ownerId
            )
    ...
    def enterFlying(self):
        ...
        self.d_setState(
            ButterflyGlobals.FLYING, self.curIndex, self.destIndex, self.time
        )
        taskMgr.doMethodLater(
            self.time, self.__handleArrival, self.uniqueName("butter-flying")
        )
        return None
    ...
    def enterLanded(self):
        ...
        self.d_setState(
            ButterflyGlobals.LANDED, self.curIndex, self.destIndex, self.time
        )
