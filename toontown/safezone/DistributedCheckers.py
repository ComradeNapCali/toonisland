...
class DistributedCheckers(DistributedNode.DistributedNode):
    def __init__(self, cr):
        ...
        for x in range(0, 32):
            ...
            tempList.append(
                self.locatorList[x].attachNewNode(CollisionNode("picker%d" % x))
            )
    ...
    def setTimer(self, timerEnd):
        if (
            self.fsm.getCurrentState() != None
            and self.fsm.getCurrentState().getName() == "waitingToBegin"
            and not self.table.fsm.getCurrentState().getName() == "observing"
        ):
            self.clockNode.stop()
    ...
    def handleClicked(self, index):
        ...
        if self.moveList == []:
            ...
            self.blinker.append(
                LerpColorInterval(self.locatorList[index], 0.7, self.tintConstant, col)
            )
            self.blinker.append(
                LerpColorInterval(self.locatorList[index], 0.7, col, self.tintConstant)
            )
            self.blinker.loop()
            self.sound.start()
    ...
    def existsLegalJumpsFrom(self, index, peice):
        if peice == "king":
            for x in range(4):
                if (
                    self.board.squareList[index].getAdjacent()[x] != None
                    and self.board.squareList[index].getJumps()[x] != None
                ):
                    ...
                    if adj.getState() == 0:
                        ...
                        adj.getState() == self.playerNum
                        or adj.getState() == self.playerNum + 2
                    ):
                        pass
                    elif jump.getState() == 0:
                        if (
                            index not in self.moveList
                            and jump.getNum() not in self.moveList
                        ):
                            return True
            return False
        elif peice == "normal":
    ...
    def checkLegalMove(self, firstSquare, secondSquare, peice):
        if (
            firstSquare.getNum() not in self.mySquares
            and firstSquare.getNum() not in self.myKings
        ):
            return False
        if self.playerNum == 1:
            moveForward = [1, 2]
        ...
        if peice == "king":
            for x in range(4):
                if firstSquare.getAdjacent()[x] != None:
                    if (
                        self.board.squareList[firstSquare.getAdjacent()[x]].getState()
                        == 0
                        and secondSquare.getNum() in firstSquare.getAdjacent()
                    ):
                        return True
            ...
            for x in moveForward:
                if (
                    firstSquare.getAdjacent()[x] != None
                    and secondSquare.getNum() in firstSquare.getAdjacent()
                ):
                    if (
                        self.board.squareList[firstSquare.getAdjacent()[x]].getState()
                        == 0
                        and firstSquare.getAdjacent().index(secondSquare.getNum()) == x
                    ):
                        return True
    ...
    def checkLegalJump(self, firstSquare, secondSquare, peice):
        ...
        if peice == "king":
            if secondSquare.getNum() in firstSquare.getJumps():
                ...
                if (
                    self.board.squareList[firstSquare.getAdjacent()[index]].getState()
                    in opposingPeices
                ):
                    return True
                else:
            ...
            if secondSquare.getNum() in firstSquare.getJumps():
                ...
                if index in moveForward:
                    if (
                        self.board.squareList[
                            firstSquare.getAdjacent()[index]
                        ].getState()
                        in opposingPeices
                    ):
                        return True
    ...
    def setGameState(self, tableState, moveList):
        if moveList != []:
            if (
                self.board.squareList[moveList[0]].getState() == 1
                or self.board.squareList[moveList[0]].getState() == 3
            ):
                playerColor = "white"
            else:
                playerColor = "black"
