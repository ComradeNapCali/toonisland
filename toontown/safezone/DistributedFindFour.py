...
class DistributedFindFour(DistributedNode.DistributedNode):
    ...
    def moveCameraForGame(self):
        ...
        if self.seatPos <= 2:
            ...
            int = LerpPosHprInterval(
                camera, 2, position, Vec3(0, -38, 0), camera.getPos(), camera.getHpr()
            )
        else:
    ...
    def mouseClick(self):
        ...
        if (
            self.isMyTurn == True
            and self.inGame == True
            and not self.moveSequence.isPlaying()
        ):
        ...
        if self.myHandler.getNumEntries() > 0:
            ...
            if pickedObj:
                ...
                if self.board[0][colVal] == 0:
                    if self.moveCol != None:
                        for x in (
                            self.startingPositions[self.moveCol]
                            .getChild(1)
                            .getChildren()
                        ):
                            x.hide()
                    ...
                    self.moveCol = colVal
                    if self.playerNum == 1:
                        self.startingPositions[self.moveCol].getChild(1).getChild(
                            2
                        ).show()
                    elif self.playerNum == 2:
                        self.startingPositions[self.moveCol].getChild(1).getChild(
    ...
    def announceWinnerPosition(self, x, y, winDirection, playerNum):
        ...
        if blinkList != []:
            ...
            downBlinkerParallel = Parallel(
                LerpColorInterval(
                    self.locatorList[val0],
                    0.3,
                    Vec4(0.5, 0.5, 0.5, 0.5),
                    Vec4(1, 1, 1, 1),
                ),
                LerpColorInterval(
                    self.locatorList[val1],
                    0.3,
                    Vec4(0.5, 0.5, 0.5, 0.5),
                    Vec4(1, 1, 1, 1),
                ),
                LerpColorInterval(
                    self.locatorList[val2],
                    0.3,
                    Vec4(0.5, 0.5, 0.5, 0.5),
                    Vec4(1, 1, 1, 1),
                ),
                LerpColorInterval(
                    self.locatorList[val3],
                    0.3,
                    Vec4(0.5, 0.5, 0.5, 0.5),
                    Vec4(1, 1, 1, 1),
                ),
    ...
    def tie(self):
        ...
        whisper = WhisperPopup(
            "This Find Four game has resulted in a Tie!",
    ...
    def animatePeice(self, tableState, moveCol, movePos, turn):
        ...
        self.moveSequence.append(
            LerpPosInterval(
                self.startingPositions[moveCol],
                1.5,
                self.locatorList[arrayLoc].getPos(self),
                startPos,
            )
        )
