...
class DistributedGolfKartAI(DistributedObjectAI.DistributedObjectAI):
    ...
    def clearEmptyNow(self, seatIndex):
        self.sendUpdate(
            "emptySlot" + str(seatIndex), [0, globalClockDelta.getRealNetworkTime()]
        )
    ...
    def clearFullNow(self, seatIndex):
        ...
        if not avId:
            self.notify.warning(
                "Clearing an empty seat index: " + str(seatIndex) + " ... Strange..."
            )
        else:
    ...
    def enterEntering(self):
        ...
        self.seats = [None, None, None, None]
        taskMgr.doMethodLater(
            TROLLEY_ENTER_TIME, self.waitEmptyTask, self.uniqueName("entering-timer")
        )
        return
    ...
    def enterLeaving(self):
        ...
        self.accepting = 0
        taskMgr.doMethodLater(
            TROLLEY_EXIT_TIME, self.trolleyLeftTask, self.uniqueName("leaving-timer")
        )
    ...
    def trolleyLeft(self):
        ...
        if numPlayers > 0:
            ...
            golfZone = GolfManagerAI.GolfManagerAI().readyGolfCourse(
                avIdList, self.golfCourse
            )
