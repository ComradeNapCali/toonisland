...
class DistributedChineseCheckersAI(DistributedNodeAI):
    ...
    def requestWin(self):
        ...
        if avId not in self.playersGamePos:
            self.air.writeServerEvent(
                ...
                % (self.parent.seats, self.playersGamePos),
            )
            return
    ...
    def checkLegalMoves(self, moveList):
        ...
        for x in range(len(moveList) - 1):
            y = self.checkLegalMove(
                self.board.getSquare(moveList[x]), self.board.getSquare(moveList[x + 1])
            )
            if y == False:
                return False
    ...
    def testWin(self):
        ...
        self.board.squareList[
            self.startingPositions[0][len(self.startingPositions[0]) - 1]
        ].setState(0)
