...
class PicnicBasket(StateData.StateData):
    def __init__(self, safeZone, parentFSM, doneEvent, tableNumber, seatNumber):
        ...
        self.fsm = ClassicFSM.ClassicFSM(
            ...
            [
                ...
                State.State(
                    "trolleyHFA", self.enterTrolleyHFA, self.exitTrolleyHFA, ["final"]
                ),
                State.State(
                    "trolleyTFA", self.enterTrolleyTFA, self.exitTrolleyTFA, ["final"]
                ),
                State.State(
                    "requestBoard",
                ...
                State.State(
                    "boarding", self.enterBoarding, self.exitBoarding, ["boarded"]
                ),
                State.State(
                    "boarded",
                    self.enterBoarded,
    ...
    def enter(self):
        ...
        if base.localAvatar.hp > 0:
            messenger.send(
                "enterPicnicTableOK_%d_%d" % (self.tableNumber, self.seatNumber)
            )
    ...
    def enterBoarding(self, nodePath, side):
        ...
        self.cameraBoardTrack = LerpPosHprInterval(
            camera, 1.5, Point3(14.4072 * side, 0, 3.8667), Point3(heading, -15, 0)
        )
