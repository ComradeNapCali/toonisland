...
class DistributedPicnicBasket(DistributedObject.DistributedObject):
    ...
    def emptySlot(self, index, avId, timestamp):
        ...
        if avId == 0:
            ...
            self.notify.debug(
                "### empty slot - unexpetected: fullSeat = %s" % self.fullSeat
            )
    ...
    def __enableCollisions(self):
        for i in range(self.numSeats):
            self.accept(
                "enterpicnicTable_sphere_%d_%d" % (self.getDoId(), i),
                self.handleEnterPicnicTableSphere,
                [i],
            )
            self.accept(
                "enterPicnicTableOK_%d_%d" % (self.getDoId(), i),
                self.handleEnterPicnicTable,
                [i],
            )
    ...
    def generateBasketAppearTrack(self):
        ...
        basketTrack = Sequence(
            ...
            Sequence(
                LerpScaleInterval(
                    self.basket, scale=Point3(1.1, 1.1, 0.1), duration=0.2
                ),
                LerpScaleInterval(
                    self.basket, scale=Point3(1.6, 1.6, 0.2), duration=0.1
                ),
                LerpScaleInterval(
                    self.basket, scale=Point3(1.0, 1.0, 0.4), duration=0.1
                ),
                LerpScaleInterval(
                    self.basket, scale=Point3(1.5, 1.5, 2.5), duration=0.2
                ),
                LerpScaleInterval(
                    self.basket, scale=Point3(2.5, 2.5, 1.5), duration=0.1
                ),
                LerpScaleInterval(
                    self.basket, scale=Point3(2.0, 2.0, 2.0), duration=0.1
                ),
    ...
    def generateBasketDisappearTrack(self):
        ...
        pos.addZ(-1)
        basketTrack = Sequence(
            LerpScaleInterval(self.basket, scale=Point3(2.0, 2.0, 1.8), duration=0.1),
            LerpScaleInterval(self.basket, scale=Point3(1.0, 1.0, 2.5), duration=0.1),
            LerpScaleInterval(self.basket, scale=Point3(2.0, 2.0, 0.5), duration=0.2),
            LerpScaleInterval(self.basket, scale=Point3(0.5, 0.5, 1.0), duration=0.1),
            LerpScaleInterval(self.basket, scale=Point3(1.1, 1.1, 0.1), duration=0.1),
            LerpScaleInterval(self.basket, scale=Point3(0.1, 0.1, 0.1), duration=0.2),
            SoundInterval(
                globalBattleSoundCache.getSound("GUI_balloon_popup.ogg"),
                node=self.basket,
            ),
    ...
    def generateFoodAppearTrack(self, seat):
        if self.fullSeat[seat] == self.seatState.Full:
            ...
            self.food[seat].setPos(
                ...
                self.seats[seat].getPos(self.tablecloth)[1] / 2,
                0,
            )
            foodTrack = Sequence(
                ...
                Sequence(
                    LerpScaleInterval(
                        self.food[seat], scale=Point3(1.1, 1.1, 0.1), duration=0.2
                    ),
                    LerpScaleInterval(
                        self.food[seat], scale=Point3(1.6, 1.6, 0.2), duration=0.1
                    ),
                    LerpScaleInterval(
                        self.food[seat], scale=Point3(1.0, 1.0, 0.4), duration=0.1
                    ),
                    LerpScaleInterval(
                        self.food[seat], scale=Point3(1.5, 1.5, 2.5), duration=0.2
                    ),
                    LerpScaleInterval(
                        self.food[seat], scale=Point3(2.5, 2.5, 1.5), duration=0.1
                    ),
                    LerpScaleInterval(
                        self.food[seat], scale=Point3(2.0, 2.0, 2.0), duration=0.1
                    ),
    ...
    def generateFoodDisappearTrack(self, seat):
        ...
        foodTrack = Sequence(
            LerpScaleInterval(
                self.food[seat], scale=Point3(2.0, 2.0, 1.8), duration=0.1
            ),
            LerpScaleInterval(
                self.food[seat], scale=Point3(1.0, 1.0, 2.5), duration=0.1
            ),
            LerpScaleInterval(
                self.food[seat], scale=Point3(2.0, 2.0, 0.5), duration=0.2
            ),
            LerpScaleInterval(
                self.food[seat], scale=Point3(0.5, 0.5, 1.0), duration=0.1
            ),
            LerpScaleInterval(
                self.food[seat], scale=Point3(1.1, 1.1, 0.1), duration=0.1
            ),
            LerpScaleInterval(
                self.food[seat], scale=Point3(0.1, 0.1, 0.1), duration=0.2
            ),
            SoundInterval(
                globalBattleSoundCache.getSound("GUI_balloon_popup.ogg"),
                node=self.food[seat],
