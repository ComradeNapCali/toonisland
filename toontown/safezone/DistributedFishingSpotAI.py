...
class DistributedFishingSpotAI(DistributedObjectAI):
        ...
        if self.avId is not None:
            if self.avId == avId:
                self.air.writeServerEvent(
                    "suspicious", avId, "Toon requested to enter a fishing spot twice!"
                )
    ...
    def requestExit(self):
        ...
        if self.avId != avId:
            self.air.writeServerEvent(
                "suspicious",
                avId,
                "Toon requested to exit a fishing spot they're not on!",
            )
            return
    ...
    def doCast(self, p, h):
        ...
        if money < cost:
            self.air.writeServerEvent(
                "suspicious", avId, "Toon tried to cast without enough jellybeans!"
            )
            return
    ...
    def considerReward(self, target):
        if not self.cast:
            self.air.writeServerEvent(
                "suspicious", self.avId, "Toon tried to fish without casting!"
            )
            return
        ...
        self.lastFish = catch
        self.d_setMovie(
            FishGlobals.PullInMovie, catch[0], catch[1], catch[2], catch[3], 0, 0
        )
        self.cast = False
