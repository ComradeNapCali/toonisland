...
class Playground(Place.Place):
    ...
    def __init__(self, loader, parentFSM, doneEvent):
        ...
        self.fsm = ClassicFSM.ClassicFSM(
            ...
            [
                ...
                State.State(
                    "sit", self.enterSit, self.exitSit, ["walk", "DFA", "trialerFA"]
                ),
                State.State(
                    "drive",
                    self.enterDrive,
                ...
                State.State("trolley", self.enterTrolley, self.exitTrolley, ["walk"]),
                State.State("doorIn", self.enterDoorIn, self.exitDoorIn, ["walk"]),
                State.State("doorOut", self.enterDoorOut, self.exitDoorOut, ["walk"]),
                State.State("TFA", self.enterTFA, self.exitTFA, ["TFAReject", "DFA"]),
                State.State(
                    "TFAReject", self.enterTFAReject, self.exitTFAReject, ["walk"]
                ),
                State.State(
                    "trialerFA",
                    self.enterTrialerFA,
                ...
                State.State(
                    "DFA", self.enterDFA, self.exitDFA, ["DFAReject", "NPCFA", "HFA"]
                ),
                State.State(
                    "DFAReject", self.enterDFAReject, self.exitDFAReject, ["walk"]
                ),
                State.State(
                    "NPCFA", self.enterNPCFA, self.exitNPCFA, ["NPCFAReject", "HFA"]
                ),
                State.State(
                    "NPCFAReject", self.enterNPCFAReject, self.exitNPCFAReject, ["walk"]
                ),
                State.State(
                    "HFA",
                    self.enterHFA,
                ...
                State.State(
                    "HFAReject", self.enterHFAReject, self.exitHFAReject, ["walk"]
                ),
                State.State(
                    "deathAck", self.enterDeathAck, self.exitDeathAck, ["teleportIn"]
                ),
                State.State(
                    "teleportIn",
    ...
    def enterHFACallback(self, requestStatus, doneStatus):
        ...
        if doneStatus["mode"] == "complete":
            ...
            self.notify.error(
                "Unknown done status for HealthForceAcknowledge: " + repr(doneStatus)
            )
    ...
    def enterNPCFACallback(self, requestStatus, doneStatus):
        ...
        if doneStatus["mode"] == "complete":
            ...
            self.notify.error(
                "Unknown done status for NPCForceAcknowledge: " + repr(doneStatus)
            )
    ...
    def __teleportOutDone(self, requestStatus):
        teleportDebug(
            requestStatus, "Playground.__teleportOutDone(%s)" % (requestStatus,)
        )
        ...
        if (
            hoodId == self.loader.hood.hoodId
            and zoneId == self.loader.hood.hoodId
            and shardId == None
        ):
