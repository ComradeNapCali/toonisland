...
class TTWhiteList(WhiteList):
    ...
    def getWhitelistUrl(self):
        result = base.config.GetString(
            "fallback-whitelist-url", "http://cdn.toontown.disney.go.com/toontown/en/"
        )
        ...
        if override:
            ...
            try:
                ...
                if launcherUrl:
                    result = launcherUrl
                    self.notify.info(
                        "got GAME_WHITELIST_URL from launcher using %s" % result
                    )
                else:
                    self.notify.info(
                        "blank GAME_WHITELIST_URL from launcher, using %s" % result
                    )
            except:
                self.notify.warning(
    ...
    def findWhitelistDir(self):
        ...
        if not found:
            ...
            self.setErrorMessage(
                TTLocalizer.NewsPageErrorDownloadingFile % self.WhitelistFileName
            )
            return None
