...
class ToontownChatManager(ChatManager.ChatManager):
    ...
    def __init__(self, cr, localAvatar):
        ...
        self.whisperCancelButton = DirectButton(
            ...
            scale=1.179,
            relief=None,
            text=("", OTPLocalizer.ChatManagerCancel, OTPLocalizer.ChatManagerCancel),
            text_scale=0.05,
            text_fg=(0, 0, 0, 1),
            text_pos=(0, -0.09),
            textMayChange=0,
    ...
    def sendSCToontaskWhisperMessage(
        self, taskId, toNpcId, toonProgress, msgIndex, whisperAvatarId, toPlayer
    ):
        if toPlayer:
    ...
    def enterUnpaidChatWarning(self):
        ...
        if base.cr.productName in ["DisneyOnline-UK", "JP", "DE", "BR", "FR"]:
            ...
            if (
                "QuickLauncher" not in str(base.cr.launcher.__class__)
    ...
    def __handleNoSecretChatWarningOK(self, *args):
        ...
        if okflag:
            ...
            self.problemActivatingChat["text"] = (
                OTPLocalizer.ProblemActivatingChat % message
            )
        else:
            self.noSecretChatWarning[
                "text"
            ] = OTPLocalizer.NoSecretChatWarningWrongPassword
            self.passwordEntry["focus"] = 1
            self.passwordEntry.enterText("")
