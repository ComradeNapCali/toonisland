...
if not os.path.isfile("user/spellbook.json"):
    with open("user/spellbook.json", "w") as data:
        data.write(spellbookJsonDefaultValues)
        data.close()

with open("user/spellbook.json") as data:
...
class TIAMagicWordManagerAI(DistributedObjectAI.DistributedObjectAI):
    notify = DirectNotifyGlobal.directNotify.newCategory("TIAMagicWordManagerAI")

    def requestExecuteMagicWord(
        self, affectRange, affectType, affectExtra, lastClickedAvId, magicWord
    ):
        avId = self.air.getAvatarIdFromSender()
        toon = self.air.doId2do.get(avId)
        if not toon:
            self.notify.warning(
                "requestExecuteMagicWord: Magic Word use requested but invoker avatar is non-existant!"
            )
            return
        ...
        if (
            affectRange in [AFFECT_OTHER, AFFECT_BOTH]
        ) and affectType == AFFECT_SINGULAR:
            if lastClickedAvId:
        ...
        if affectType in [AFFECT_ZONE, AFFECT_SERVER, AFFECT_RANK]:
            ...
            for doId in list(self.air.doId2do.keys())[:]:
                ...
                if (
                    isinstance(do, DistributedPlayerAI)
                    and do.isPlayerControlled()
                    and do != toon
                ):
                    if affectType == AFFECT_ZONE and do.zoneId == toon.zoneId:
                        ...
                        affectType == AFFECT_RANK and do.getAccessLevel() == affectExtra
                    ):
                        toonIds.append(doId)
        ...
        if toon.getAccessLevel() < OTPGlobals.AccessLevelName2Int.get(
            magicWordInfo["access"]
        ):
        ...
        if len(argList) < minArgs:
            self.generateResponse(
                avId=avId,
                responseType="NotEnoughArgs",
                extraMessageData="{} argument{}".format(
                    minArgs, "s" if minArgs != 1 else ""
                ),
            )
            return
            ...
            self.generateResponse(
                avId=avId,
                responseType="TooManyArgs",
                extraMessageData="{} argument{}".format(
                    maxArgs, "s" if maxArgs != 1 else ""
                ),
            )
            return
        ...
        if magicWordInfo["execLocation"] == EXEC_LOC_CLIENT:
            self.sendClientCommand(
                avId,
                magicWord,
                magicWordInfo["classname"],
                targetIds=targetIds,
                parsedArgList=parsedArgList,
                affectRange=affectRange,
                affectType=affectType,
                affectExtra=affectExtra,
            ...
            if returnValue:
                self.generateResponse(
                    avId=avId, responseType="Success", returnValue=returnValue
                )
            else:
                self.generateResponse(
                    avId=avId,
                    responseType="SuccessNoResp",
                    magicWord=magicWord,
                    parsedArgList=parsedArgList,
                    affectRange=affectRange,
                    affectType=affectType,
