...
class TIAMagicWordManager(DistributedObject.DistributedObject):
    notify = DirectNotifyGlobal.directNotify.newCategory("TIAMagicWordManager")
    neverDisable = 1
    ...
    def generateResponse(
        ...
        affectType=None,
        affectExtra=None,
        lastClickedAvId=None,
        extraMessageData=None,
    ):
        response = self.generateMagicWordResponse(
            responseType,
            magicWord,
            args,
            returnValue,
        ...
        base.localAvatar.setSystemMessage(
            0, self.wizardName + ": " + response, WhisperPopup.WTMagicWord
        )
        self.notify.info(response)

    def generateMagicWordResponse(
        self,
        responseType,
        magicWord,
        args,
        ...
        if responseType == "SuccessNoResp" and magicWord:
            ...
            return response
        response += MAGIC_WORD_RESPONSES.get(
            responseType, "...I don't know how to respond!"
        )
            ...
            for magicWord in list(magicWordIndex.keys()):
                if word in magicWord:
                    self.generateResponse(
                        responseType="CloseWord", extraMessageData=magicWord
                    )
                    return
