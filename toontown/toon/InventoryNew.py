...
class InventoryNew(InventoryBase.InventoryBase, DirectFrame):
    ...
    def show(self):
        if self.tutorialFlag:
            self.tutArrows.arrowsOn(
                -0.43, -0.12, 180, -0.43, -0.24, 180, onTime=1.0, offTime=0.2
            )
    ...
    def load(self):
        ...
        self.detailFrame = DirectFrame(
            parent=self.invFrame, relief=None, pos=(1.05, 0, -0.08)
        )
        ...
        for track in range(0, len(Tracks)):
            ...
            self.trackBars.append(
                DirectWaitBar(
                    ...
                    scale=0.25,
                    frameColor=(
                        TrackColors[track][0] * 0.6,
                        TrackColors[track][1] * 0.6,
                        TrackColors[track][2] * 0.6,
                        1,
                    ),
                    barColor=(
                        TrackColors[track][0] * 0.9,
            ...
            for item in range(0, len(Levels[track])):
                button = DirectButton(
                    ...
                    pos=(
                        self.ButtonXOffset + item * self.ButtonXSpacing + adjustLeft,
                        -0.1,
                        0,
                    ),
    ...
    def __handleSelection(self, track, level):
        if (
            self.activateMode == "purchaseDelete"
            or self.activateMode == "bookDelete"
            or self.activateMode == "storePurchaseDelete"
        ):
    ...
    def showDetail(self, track, level, event=None):
        ...
        self.detailNameLabel.configure(
            text=AvPropStrings[track][level], image_image=self.invModels[track][level]
        )
        ...
        if (
            (organicBonus or propBonus)
            and track == LURE_TRACK
            and level == 0
            or (organicBonus or propBonus)
            and track == LURE_TRACK
            and level == 1
        ):
            accString = (
                TTLocalizer.BattleGlobalLureAccLow
                + TTLocalizer.BattleGlobalLureTrackBonus
            )
        ...
        if (
            ...
            accString = (
                TTLocalizer.BattleGlobalLureAccLow2
                + TTLocalizer.BattleGlobalLureTrackBonus
            )
        if (
            (organicBonus or propBonus)
            ...
            accString = (
                TTLocalizer.BattleGlobalLureAccMedium
                + TTLocalizer.BattleGlobalLureTrackBonus
            )
    ...
    def setActivateModeBroke(self):
        if self.activateMode == "storePurchase":
            ...
            self.notify.error(
                "Unexpected mode in setActivateModeBroke(): %s" % self.activateMode
            )
        self.enableUberGags()
            ...
            if self.toon.hasTrackAccess(track):
                ...
                for level in range(len(Levels[track])):
                    ...
                    if self.itemIsUsable(track, level):
                        ...
                        if (
                            self.numItem(track, level) >= self.getMax(track, level)
                            or totalProps == maxProps
                            or unpaid
                            and gagIsPaidOnly(track, level)
    ...
    def showTrack(self, trackIndex):
        ...
        if (
            curExp >= UnpaidMaxSkills[trackIndex]
            and self.toon.getGameAccess() != OTPGlobals.AccessFull
        ):
    ...
    def loadBattleFrame(self):
        ...
        self.fireButton = DirectButton(
            ...
            text_scale=TTLocalizer.INfireButton,
            text_pos=(0, -0.02),
            text_fg=Vec4(1, 1, 1, 1),
            textMayChange=0,
            image=(self.upButton, self.downButton, self.rolloverButton),
