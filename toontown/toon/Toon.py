...
def loadDialog():
    ...
    for file in suitCostumeFiles:
        SuitCostumeArray.append(base.loader.loadSfx(loadPath + file + ".ogg"))
...
class Toon(Avatar.Avatar, ToonHead):
    ...
    def __init__(self):
        ...
        self.animFSM = ClassicFSM(
            ...
            [
                ...
                State(
                    "GolfRotateLeft", self.enterGolfRotateLeft, self.exitGolfRotateLeft
                ),
                State(
                    "GolfRotateRight",
                    self.enterGolfRotateRight,
                ...
                State(
                    "CogThiefRunning",
                    self.enterCogThiefRunning,
                    self.exitCogThiefRunning,
                ),
                State(
                    "ScientistJealous",
                    self.enterScientistJealous,
                    self.exitScientistJealous,
                ),
                State(
                    "ScientistEmcee", self.enterScientistEmcee, self.exitScientistEmcee
                ),
                State("ScientistWork", self.enterScientistWork, self.exitScientistWork),
                State(
                    "ScientistLessWork",
                    self.enterScientistLessWork,
                    self.exitScientistLessWork,
                ),
                State(
                    "ScientistPlay", self.enterScientistPlay, self.enterScientistPlay
                ),
            ],
            "off",
            "off",
    ...
    def setupToonNodes(self):
        ...
        for lodName in self.getLODNames():
            ...
            if base.config.GetBool("want-new-anims", 1):
                if (
                    not self.getPart("torso", lodName)
                    .find("**/def_joint_right_hold")
                    .isEmpty()
                ):
                    hand = self.getPart("torso", lodName).find(
                        "**/def_joint_right_hold"
                    )
            else:
                hand = self.getPart("torso", lodName).find("**/joint_Rhold")
    ...
    def getBookActors(self):
        ...
        bookActor = Actor.Actor(
            "phase_3.5/models/props/book-mod",
    ...
    def resetHeight(self):
        if hasattr(self, "style") and self.style:
            ...
            if (
                self.cheesyEffect == ToontownGlobals.CEBigToon
                or self.cheesyEffect == ToontownGlobals.CEBigWhite
            ):
    ...
    def generateHat(self, fromRTM=False):
        ...
        if hat[0] != 0:
            ...
            if hatGeom:
                ...
                hatGeom.setScale(
                    transOffset[2][0], transOffset[2][1], transOffset[2][2]
                )
    ...
    def generateGlasses(self, fromRTM=False):
        ...
        if glasses[0] >= len(ToonDNA.GlassesModels):
            self.sendLogSuspiciousEvent(
                "tried to put a wrong glasses idx %d" % glasses[0]
            )
            return
        ...
        if glasses[0] != 0:
            ...
            if glassesGeom:
                ...
                if AccessoryGlobals.ExtendedGlassesTransTable.get(glasses[0]):
                    transOffset = AccessoryGlobals.ExtendedGlassesTransTable[
                        glasses[0]
                    ].get(self.style.head[:2])
                if transOffset is None:
                    ...
                    if transOffset is None:
                        return
                glassesGeom.setPos(
                    transOffset[0][0], transOffset[0][1], transOffset[0][2]
                )
                glassesGeom.setHpr(
                    transOffset[1][0], transOffset[1][1], transOffset[1][2]
                )
    ...
    def generateBackpack(self, fromRTM=False):
        ...
        if backpack[0] >= len(ToonDNA.BackpackModels):
            self.sendLogSuspiciousEvent(
                "tried to put a wrong backpack idx %d" % backpack[0]
            )
            return
        ...
        if backpack[0] != 0:
            ...
            if geom:
                ...
                if AccessoryGlobals.ExtendedBackpackTransTable.get(backpack[0]):
                    transOffset = AccessoryGlobals.ExtendedBackpackTransTable[
                        backpack[0]
                    ].get(self.style.torso[:1])
                if transOffset is None:
                    transOffset = AccessoryGlobals.BackpackTransTable.get(
                        self.style.torso[:1]
                    )
                    if transOffset is None:
                        return
    ...
    def getDialogueArray(self):
        ...
        if animalType == "dog":
            ...
            dialogueArray = None
        if self.isDisguised:
            dialogueArray = SuitCostumeArray
        ...
        if nodePathList:
            nodePathList.sort(
                key=functools.cmp_to_key(
                    (lambda x, y: cmp(x[0].getDistance(self), y[0].getDistance(self)))
                )
            )
    ...
    def setSpeed(self, forwardSpeed, rotateSpeed):
        ...
        if self.standWalkRunReverse != None:
            ...
            if (
                showWake
                and self.getZ(render) < wakeWaterHeight
                and abs(forwardSpeed) > ToontownGlobals.WalkCutOff
            ):
                currT = globalClock.getFrameTime()
                deltaT = currT - self.lastWakeTime
                if (
                    action == OTPGlobals.RUN_INDEX
                    and deltaT > ToontownGlobals.WakeRunDelta
                    or deltaT > ToontownGlobals.WakeWalkDelta
                ):
    ...
    def startBobSwimTask(self):
        ...
        self.bobSwim = Sequence(
            geomNode.posInterval(
                1, Point3(0, -3, 3), startPos=Point3(0, -3, 4), blendType="easeInOut"
            ),
            geomNode.posInterval(
                1, Point3(0, -3, 4), startPos=Point3(0, -3, 3), blendType="easeInOut"
            ),
        )
        self.bobSwim.loop()
    ...
    def enterOpenBook(self, animMultiplier=1, ts=0, callback=None, extraArgs=[]):
        ...
        if hasattr(self, "uniqueName"):
            ...
            trackName = "openBook"
        self.track = Sequence(
            Func(self.showBooks), bookTracks, Wait(0.1), name=trackName
        )
        if callback:
    ...
    def enterCloseBook(self, animMultiplier=1, ts=0, callback=None, extraArgs=[]):
        ...
        if hasattr(self, "uniqueName"):
            ...
            trackName = "closeBook"
        self.track = Sequence(
            Func(self.showBooks), bookTracks, Func(self.hideBooks), name=trackName
        )
        if callback:
            self.track.setDoneEvent(self.track.getName())
            self.acceptOnce(self.track.getName(), callback, extraArgs)
    ...
    def enterTeleportOut(self, animMultiplier=1, ts=0, callback=None, extraArgs=[]):
        ...
        self.track.setDoneEvent(self.track.getName())
        self.acceptOnce(
            self.track.getName(), self.finishTeleportOut, [callback, extraArgs]
        )
    ...
    def enterDied(self, animMultiplier=1, ts=0, callback=None, extraArgs=[]):
        ...
        if callback:
            self.track = Sequence(
                self.track, Func(callback, *extraArgs), autoFinish=autoFinishTrack
            )
        self.track.start(ts)
        self.setActiveShadow(0)
        return
    ...
    def enterSleep(self, animMultiplier=1, ts=0, callback=None, extraArgs=[]):
        ...
        if self == base.localAvatar:
            ...
            taskMgr.doMethodLater(
                self.afkTimeout, self.__handleAfkTimeout, self.uniqueName("afkTimeout")
            )
        self.setActiveShadow(0)
    ...
    def exitSleep(self):
        ...
        if config.GetBool("stuck-sleep-fix", 1):
            doClear = SLEEP_STRING in (
                self.nametag.getChat(),
        ...
        self.track = Sequence(
            ...
            Parallel(
                Sequence(
                    Wait(0.4),
                    LerpScaleInterval(
                        node, lerpTime, VBase3(1.4, 1.4, 1.4), blendType="easeInOut"
                    ),
                    LerpScaleInterval(
                        node,
                        lerpTime / 2.0,
                    ...
                    LerpScaleInterval(
                        node, lerpTime / 3.0, origScale, blendType="easeInOut"
                    ),
                ),
    ...
    def stunToon(self, ts=0, callback=None, knockdown=0):
        if not self.isStunned:
            ...
            if knockdown:
                self.stunTrack = Parallel(
                    ActorInterval(self, animName="slip-backward"), track
                )
            else:
    ...
    def reapplyCheesyEffect(self, lerpTime=0):
        ...
        self.effectTrack = Sequence(
            self.__undoCheesyEffect(effect, 0), self.__doCheesyEffect(effect, lerpTime)
        )
        self.effectTrack.start()
        return
    ...
    def __doHeadScale(self, scale, lerpTime):
        ...
        for hi in range(self.headParts.getNumPaths()):
            ...
            track.append(
                LerpScaleInterval(head, lerpTime, scale, blendType="easeInOut")
            )
    ...
    def __doLegsScale(self, scale, lerpTime):
        ...
        for li in range(self.legsParts.getNumPaths()):
            ...
            track.append(
                LerpScaleInterval(legs, lerpTime, scale, blendType="easeInOut")
            )
            track.append(
                LerpScaleInterval(torso, lerpTime, invScale, blendType="easeInOut")
            )
    ...
    def __doToonScale(self, scale, lerpTime):
        ...
        track = Sequence(
            Parallel(
                LerpHprInterval(
                    node, lerpTime, Vec3(0.0, 0.0, 0.0), blendType="easeInOut"
                ),
    ...
    def doToonColorScale(self, scale, lerpTime, keepDefault=0):
        ...
        track.append(
            LerpColorScaleInterval(node, lerpTime, scale, blendType="easeInOut")
        )
        if scale[3] == 1:
        ...
        if toPumpkin:
            ...
            def showHiddenParts():
                ...
                for head in self.headParts:
                    for p in head.getChildren():
                        if (
                            not self.pumpkins.hasPath(p)
                            and p.getTag("pumpkin") == "enabled"
                        ):
                            p.show()
            ...
            def showHiddenParts():
                ...
                for hi in range(self.headParts.getNumPaths()):
                    ...
                    for pi in range(parts.getNumPaths()):
                        p = parts[pi]
                        if (
                            not self.snowMen.hasPath(p)
                            and p.getTag("snowman") == "enabled"
                        ):
                            p.show()
    ...
    def __doPartsColorScale(self, scale, lerpTime):
        ...
        for piece in pieces:
            if piece.getName()[:7] == "muzzle-" and piece.getName()[-8:] != "-neutral":
                continue
            p1.append(
                LerpColorScaleInterval(piece, lerpTime, scale, blendType="easeInOut")
            )
        ...
        if scale[3] == 1:
            ...
            for piece in pieces:
                if (
                    piece.getName()[:7] == "muzzle-"
                    and piece.getName()[-8:] != "-neutral"
                ):
                    continue
    ...
    def __doCheesyEffect(self, effect, lerpTime):
        if effect == ToontownGlobals.CEBigHead:
            ...
            return self.__doBigAndWhite(
                VBase4(1, 1, 1, 1), ToontownGlobals.BigToonScale, lerpTime
            )
            ...
            if localAvatar.seeGhosts:
                alpha = 0.2
            return Sequence(
                self.__doToonGhostColorScale(
                    VBase4(1, 1, 1, alpha), lerpTime, keepDefault=1
                ),
    ...
    def putOnSuit(self, suitType, setDisplayName=True, rental=False):
        ...
        if self.isLocal():
            ...
            indices = list(
                range(
                    OTPLocalizer.SCMenuCommonCogIndices[0],
                    OTPLocalizer.SCMenuCommonCogIndices[1] + 1,
                )
            )
    ...
    def getPresentPieInterval(self, x, y, z, h, p, r):
        ...
        track = Sequence(
            ...
            Parallel(
                pie.scaleInterval(1, self.pieScale, startScale=MovieUtil.PNT3_NEARZERO),
                ActorInterval(self, "throw", startFrame=0, endFrame=31),
                animPie,
            ),
    ...
    def getTossPieInterval(self, x, y, z, h, p, r, power, beginFlyIval=Sequence()):
        ...
        toss = Track(
            (
                ...
                Sequence(
                    ...
                    Func(pie.reparentTo, self.rightHand),
                    Func(pie.setPosHpr, 0, 0, 0, 0, 0, 0),
                    Parallel(ActorInterval(self, "throw", startFrame=48), animPie),
        ...
        fly = Track(
            (14.0 / 24.0, SoundInterval(sound, node=self)),
            (
                16.0 / 24.0,
                Sequence(
                    Func(flyPie.reparentTo, render),
                    Func(flyPie.setScale, self.pieScale),
                    Func(
                        flyPie.setPosHpr, self, 0.52, 0.97, 2.24, 89.42, -10.56, 87.94
                    ),
                    beginFlyIval,
    ...
    def getFeedPetIval(self):
        return Sequence(
            ActorInterval(self, "feedPet"), Func(self.animFSM.request, "neutral")
        )

    def getScratchPetIval(self):
        return Sequence(
            ActorInterval(self, "pet-start"),
    ...
    def enterScientistJealous(
        self, animMultiplier=1, ts=0, callback=None, extraArgs=[]
    ):
