...
class DistributedNPCClerk(DistributedNPCToonBase):
        ...
        if mode == NPCToons.PURCHASE_MOVIE_TIMEOUT:
            ...
            if self.isLocalToon:
                taskMgr.doMethodLater(
                    1.0, self.popupPurchaseGUI, self.uniqueName("popupPurchaseGUI")
                )
            ...
            self.setChatAbsolute(
                TTLocalizer.STOREOWNER_NEEDJELLYBEANS, CFSpeech | CFTimeout
            )
            self.resetClerk()
        return
    ...
    def __handlePurchaseDone(self):
        ...
        self.d_setInventory(
            base.localAvatar.inventory.makeNetString(), base.localAvatar.getMoney(), 1
        )
        self.purchase.exit()
        self.purchase.unload()
        self.purchase = None
