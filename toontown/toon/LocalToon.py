...
WantNewsPage = base.config.GetBool(
    "want-news-page", ToontownGlobals.DefaultWantNewsPageSetting
)
...
class LocalToon(DistributedToon.DistributedToon, LocalAvatar.LocalAvatar):
    ...
    def __init__(self, cr):
        try:
            ...
            if not hasattr(base.cr, "lastLoggedIn"):
                base.cr.lastLoggedIn = (
                    self.cr.toontownTimeManager.convertStrToToontownTime("")
                )
            self.setLastTimeReadNews(base.cr.lastLoggedIn)
            self.acceptingNewFriends = base.settings.getBool(
                "game", "accepting-new-friends", True
            ) and base.config.GetBool("accepting-new-friends-default", True)
            self.acceptingNonFriendWhispers = base.settings.getBool(
                "game", "accepting-non-friend-whispers", True
            ) and base.config.GetBool("accepting-non-friend-whispers-default", True)
        ...
        if base.cr.isWebPlayToken() or __dev__:
            if base.cr.isPaid():
                if base.cr.productName in [
                    ...
                    paidNoParentPassword = (
                        launcher and not launcher.getParentPasswordSet()
                    )
            else:
    ...
    def displayTalkWhisper(self, fromId, avatarName, rawString, mods):
        ...
        whisper = WhisperPopup(
            chatString, OTPGlobals.getInterfaceFont(), WhisperPopup.WTNormal
        )
    ...
    def canChat(self):
        if not self.cr.allowAnyTypedChat():
            return 0
        if self.commonChatFlags & (
            ToontownGlobals.CommonChat | ToontownGlobals.SuperChat
        ):
            return 1
    ...
    def handleTunnelOut(self, startTime, startX, startY, x, y, z, h):
        ...
        tracks.append(
            Sequence(
                Wait(toonTrack.getDuration() - (irisDur + 0.1)),
    ...
    def localPresentPie(self, time):
        ...
        ival = self.getPresentPieInterval(
    ...
    def __pieHit(self, entry):
        ...
        timestamp32 = globalClockDelta.getFrameNetworkTime(bits=32)
        self.sendUpdate(
            "pieSplat", [pos[0], pos[1], pos[2], sequence, pieCode, timestamp32]
        )
    ...
    def updatePieButton(self):
        ...
        if self.__pieButton == None:
            ...
            if self.pieType >= len(inv.invModels[ToontownBattleGlobals.THROW_TRACK]):
                ...
                gui = None
                pieGui = (
                    inv.invModels[ToontownBattleGlobals.THROW_TRACK][self.pieType],
                )
                pieScale = 0.85
    ...
    def loadFurnitureGui(self):
        ...
        DirectLabel(
            parent=self.__furnitureGui, relief=None, image=guiModels.find("**/rooftile")
        )
        ...
        DirectButton(
            ...
            relief=None,
            image=[bMoveStartUp, bMoveStartDown, bMoveStartRollover, bMoveStartUp],
            text=[
                "",
    ...
    def clarabelleNewsPageCollision(self, show=True):
        ...
        if (
            self.__catalogNotifyDialog == None
            or self.__catalogNotifyDialog.frame == None
        ):
            return
    ...
    def loadClarabelleGui(self):
        ...
        self.__clarabelleButton.reparentTo(
            base.a2dTopRight, DGG.BACKGROUND_SORT_INDEX - 1
        )
    ...
    def showClarabelleGui(self, mailboxItems):
        ...
        if (
            not self.mailboxNotify
            and not self.awardNotify
            and self.catalogNotify == ToontownGlobals.OldItems
            and (
                self.simpleMailNotify != ToontownGlobals.NoItems
                or self.inviteMailNotify != ToontownGlobals.NoItems
            )
        ):
            self.__clarabelleButton["text"] = [
                "",
    ...
    def refreshOnscreenButtons(self):
        ...
        if self.friendsListButtonActive and self.friendsListButtonObscured <= 0:
            ...
            if self.clarabelleButtonObscured <= 0 and self.isTeleportAllowed():
                if (
                    self.catalogNotify == ToontownGlobals.NewItems
                    or self.mailboxNotify == ToontownGlobals.NewItems
                    or self.simpleMailNotify == ToontownGlobals.NewItems
                    or self.inviteMailNotify == ToontownGlobals.NewItems
                    or self.awardNotify == ToontownGlobals.NewItems
                ):
                    showClarabelle = not launcher or launcher.getPhaseComplete(5.5)
                    for quest in self.quests:
                        if (
                            quest[0] in Quests.PreClarabelleQuestIds
                            and self.mailboxNotify != ToontownGlobals.NewItems
                            and self.awardNotify != ToontownGlobals.NewItems
                        ):
                            showClarabelle = 0
                    ...
                    if showClarabelle:
                        newItemsInMailbox = (
                            self.mailboxNotify == ToontownGlobals.NewItems
                            or self.awardNotify == ToontownGlobals.NewItems
                        )
        ...
        if self.moveFurnitureButtonObscured <= 0:
            if self.furnitureManager != None and self.furnitureDirector == self.doId:
                ...
                taskMgr.remove("lerpFurnitureButton")
                self.__furnitureGui.posHprScaleInterval(
                    pos=Point3(0.143333, 0, -0.67),
        ...
        if self.mailboxNotify == ToontownGlobals.NoItems:
            if self.catalogNotify == ToontownGlobals.NewItems:
                if self.catalogScheduleCurrentWeek == 1:
                    message = (
                        TTLocalizer.CatalogNotifyFirstCatalog,
    ...
    def levelWater(self, change=1):
        ...
        self.waterTrack = Sequence(
            ...
            SoundInterval(
                globalBattleSoundCache.getSound("GUI_balloon_popup.ogg"), node=self
            ),
            Wait(1.0),
    ...
    def __expressState(self, task=None):
        ...
        if place:
            ...
            if state.getName() != self.lastPlaceState:
                print(
                    "Place State Change From %s to %s"
    ...
    def setShovelSkill(self, skillLevel):
        ...
        if hasattr(self, "shovel"):
            ...
            almostMaxedSkill = (
                GardenGlobals.ShovelAttributes[GardenGlobals.MAX_SHOVELS - 1][
                    "skillPts"
                ]
                - 2
            )
            if skillLevel >= GardenGlobals.ShovelAttributes[self.shovel]["skillPts"]:
                ...
                oldShovelSkill == almostMaxedSkill
                and newShovelPower == GardenGlobals.getNumberOfShovelBoxes()
            ):
                self.promoteShovelSkill(self.shovel, self.shovelSkill)
        return
    ...
    def promoteShovelSkill(self, shovelLevel, shovelSkill):
        ...
        if shovelBeans == GardenGlobals.getNumberOfShovelBoxes():
            if (
                shovelSkill
                == GardenGlobals.ShovelAttributes[shovelLevel]["skillPts"] - 1
            ):
                doPartyBall = True
    ...
    def promoteWateringCan(self, wateringCanlevel=0):
        message = (
            TTLocalizer.GardenWateringCanLevelUp
            + " \n"
    ...
    def setSleepAutoReply(self, fromId):
        ...
        if isinstance(av, DistributedToon.DistributedToon):
            base.localAvatar.setSystemMessage(
                0,
                TTLocalizer.sleep_auto_reply % av.getName(),
    ...
    def isReadingNews(self):
        ...
        if (
            ...
            and base.cr.playGame
            and base.cr.playGame.getPlace()
            and hasattr(base.cr.playGame.getPlace(), "fsm")
            and base.cr.playGame.getPlace().fsm
        ):
            fsm = base.cr.playGame.getPlace().fsm
            curState = fsm.getCurrentState().getName()
    ...
    def doTeleportResponse(
        self, fromAvatar, toAvatar, avId, available, shardId, hoodId, zoneId, sendToId
    ):
        localAvatar.d_teleportResponse(
            avId, available, shardId, hoodId, zoneId, sendToId
        )

    def d_teleportResponse(
        self, avId, available, shardId, hoodId, zoneId, sendToId=None
    ):
