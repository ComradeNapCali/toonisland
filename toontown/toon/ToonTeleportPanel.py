...
class ToonTeleportPanel(DirectFrame):
    ...
    def enterUnknownHood(self, hoodId):
        self[
            "text"
        ] = TTLocalizer.TeleportPanelUnknownHood % base.cr.hoodMgr.getFullnameFromId(
            hoodId
        )
        self.bOk.show()
    ...
    def enterUnavailableHood(self, hoodId):
        self["text"] = (
            TTLocalizer.TeleportPanelUnavailableHood
    ...
    def __teleportResponse(self, avId, available, shardId, hoodId, zoneId):
        teleportNotify.debug(
            "__teleportResponse%s" % ((avId, available, shardId, hoodId, zoneId),)
        )
        if avId != self.avId:
            return
        if available == 0:
