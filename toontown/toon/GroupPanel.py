...
class GroupPanel(DirectObject.DirectObject):
    ...
    def __defineConstants(self):
        ...
        self.textFgcolor = NametagGlobals.getNameFg(
            NametagGroup.CCSpeedChat, PGButton.SInactive
        )
    ...
    def __checkGroupStatus(self):
        if not self.boardingParty:
            return
        self.notify.debug(
            "__checkGroupStatus %s"
            % self.boardingParty.getGroupMemberList(localAvatar.doId)
        )
