...
class AvatarPanelBase(AvatarPanel.AvatarPanel):
    ...
    def getIgnoreButtonInfo(self):
        if base.cr.avatarFriendsManager.checkIgnored(self.avId):
            return (
                TTLocalizer.AvatarPanelStopIgnoring,
                self.handleStopIgnoring,
                STOP_IGNORE_SCALE,
            )
        else:
    ...
    def confirmReport(self):
        if base.cr.isFriend(self.avId) or base.cr.playerFriendsManager.isPlayerFriend(
            self.avId
        ):
    ...
    def handleReportCategoryConfirm(self, value):
        ...
        if value > 0:
            if self.category == CentralLogger.ReportHacking:
                base.cr.centralLogger.reportPlayer(
                    self.category, self.playerId, self.avId
                )
    ...
    def reportComplete(self, removed, isPlayer):
        ...
        if removed:
            if isPlayer:
                string += (
                    " " + TTLocalizer.ReportPanelRemovedPlayerFriend % self.playerId
                )
            else:
    ...
    def requestStopped(self):
        if (
            not base.cr.playGame.getPlace().fsm.getCurrentState().getName()
            == "stickerBook"
        ):
            if base.cr.playGame.getPlace().fsm.hasStateNamed("stopped"):
                ...
                self.notify.warning(
                    "skipping request to stopped in %s" % base.cr.playGame.getPlace()
                )
        else:
            self.cleanup()
