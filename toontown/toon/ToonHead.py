...
class ToonHead(Actor.Actor):
    ...
    EyesSurprised = loader.loadTexture(
        "phase_3/maps/eyesSurprised.jpg", "phase_3/maps/eyesSurprised_a.rgb"
    )
    ...
    LeftAD = Point3(
        LeftA[0] - LeftA[2] * (LeftD[0] - LeftA[0]) / (LeftD[2] - LeftA[2]), 0.0, 0.0
    )
    LeftBC = Point3(
        LeftB[0] - LeftB[2] * (LeftC[0] - LeftB[0]) / (LeftC[2] - LeftB[2]), 0.0, 0.0
    )
    RightAD = Point3(
        RightA[0] - RightA[2] * (RightD[0] - RightA[0]) / (RightD[2] - RightA[2]),
        0.0,
        0.0,
    )
    RightBC = Point3(
        RightB[0] - RightB[2] * (RightC[0] - RightB[0]) / (RightC[2] - RightB[2]),
        0.0,
        0.0,
    )
    ...
    def setupHead(self, dna, forGui=0):
        ...
        self.getGeomNode().setScale(
            headScale[0] * bodyScale * 1.3,
    ...
    def __setPupilDirection(self, x, y):
        ...
        left0 = Point3(
            0.0,
            0.0,
            left1[2] - left1[0] * (left2[2] - left1[2]) / (left2[0] - left1[0]),
        )
        right0 = Point3(
            0.0,
            0.0,
            right1[2] - right1[0] * (right2[2] - right1[2]) / (right2[0] - right1[0]),
        )
        if x < 0.0:
            x2 = -x
    ...
    def __lookHeadAt(self, node, point, frac=1.0, lod=None):
        ...
        if decomposeMatrix(rot, scale, hpr, CSDefault):
            ...
            if frac != 1:
                ...
                reachedTarget = (
                    abs(hpr[0] - currentHpr[0]) < 1.0
                    and abs(hpr[1] - currentHpr[1]) < 1.0
                )
    ...
    def startBlink(self):
        ...
        taskMgr.doMethodLater(
            self.randGen.random() * 4.0 + 1, self.__blinkCloseEyes, self.__blinkName
        )
    ...
    def lerpLookAt(self, point, time=1.0, blink=0):
        ...
        self.lookAtTrack = Parallel(
            Sequence(
                LerpPosInterval(
                    self.__lpupil, lookToTgtTime, endLpupil, blendType=blendType
                ),
                Wait(0.5),
                LerpPosInterval(
                    self.__lpupil, returnToEyeCenterTime, origin, blendType=blendType
                ),
            ),
            Sequence(
                LerpPosInterval(
                    self.__rpupil, lookToTgtTime, endRpupil, blendType=blendType
                ),
                Wait(0.5),
                LerpPosInterval(
                    self.__rpupil, returnToEyeCenterTime, origin, blendType=blendType
                ),
            ),
                    ...
                    if lodName == "1000" or lodName == "500":
                        ...
                        if base.config.GetBool("want-new-anims", 1):
                            if not self.find(
                                ...
                                muzzles.reparentTo(
                                    self.find("**/" + lodName + "/**/joint_toHead")
                                )
