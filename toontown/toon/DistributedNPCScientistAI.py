...
class DistributedNPCScientistAI(DistributedNPCToonBaseAI.DistributedNPCToonBaseAI):
    def __init__(self, air, npcId, questCallback=None, hq=0):
        DistributedNPCToonBaseAI.DistributedNPCToonBaseAI.__init__(
            self, air, npcId, questCallback
        )
        self.scientistFSM = ClassicFSM.ClassicFSM(
            "Scientist",
            [
                State.State(
                    "Neutral",
                ...
                State.State(
                    "Phase0", self.enterPhase0, self.exitPhase0, ["Phase1", "Neutral"]
                ),
                State.State(
                    "Phase1", self.enterPhase1, self.exitPhase1, ["Phase2", "Neutral"]
                ),
                State.State(
                    "Phase2", self.enterPhase2, self.exitPhase2, ["Phase2_5", "Neutral"]
                ),
                State.State(
                    "Phase2_5",
                    self.enterPhase2_5,
                ...
                State.State(
                    "Phase3", self.enterPhase3, self.exitPhase3, ["Phase4", "Neutral"]
                ),
                State.State(
                    "Phase4", self.enterPhase4, self.exitPhase4, ["Phase5", "Neutral"]
                ),
    ...
    def selectPhase(self, newPhase):
        try:
            if newPhase <= 4:
                ...
                if (
                    not self.scientistFSM.getCurrentState()
                    == self.scientistFSM.getStateNamed("Neutral")
                ):
            ...
            if (
                not self.scientistFSM.getCurrentState()
                == self.scientistFSM.getStateNamed(gotoPhase)
            ):
    ...
    def getPhaseToRun(self):
        ...
        ) or simbase.air.holidayManager.isHolidayRunning(
    ...
    def enterPhase0(self):
        if self.npcId == 2020:
            self.air.dialogueManager.requestDialogue(
                self, TTLocalizer.AprilToonsPhasePreTopTopic, endPause=30
            )
    ...
    def exitPhase0(self):
        if self.npcId == 2020:
            self.air.dialogueManager.leaveDialogue(
                self, TTLocalizer.AprilToonsPhasePreTopTopic
            )
        self.ignore("SillyMeterPhase")
    ...
    def enterPhase2_5(self):
        if self.npcId == 2020:
            if simbase.air.holidayManager.isHolidayRunning(
                ...
                self.air.dialogueManager.requestDialogue(
                    self, TTLocalizer.AprilToonsExtPhaseTopTopic, endPause=30
                )
            else:
                self.air.dialogueManager.requestDialogue(
                    self, TTLocalizer.AprilToonsPhaseTopTopic, endPause=30
                )
        elif self.npcId == 2018 or self.npcId == 2019:
    ...
    def exitPhase2_5(self):
        if self.npcId == 2020:
            if simbase.air.holidayManager.isHolidayRunning(
                ...
                self.air.dialogueManager.leaveDialogue(
                    self, TTLocalizer.AprilToonsExtPhaseTopTopic
                )
            else:
                self.air.dialogueManager.leaveDialogue(
                    self, TTLocalizer.AprilToonsPhaseTopTopic
                )
        self.ignore("SillyMeterPhase")
    ...
    def enterPhase3(self):
        if self.npcId == 2020:
            if simbase.air.holidayManager.isHolidayRunning(
                ...
                self.air.dialogueManager.requestDialogue(
                    self, TTLocalizer.AprilToonsPhasePostTopTopic, endPause=30
                )
        elif self.npcId == 2018 or self.npcId == 2019:
    ...
    def exitPhase3(self):
        if self.npcId == 2020:
            if simbase.air.holidayManager.isHolidayRunning(
                ...
                self.air.dialogueManager.leaveDialogue(
                    self, TTLocalizer.AprilToonsPhasePostTopTopic
                )
        self.ignore("SillyMeterPhase")
