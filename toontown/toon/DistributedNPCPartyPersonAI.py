    ...
    def avatarEnter(self):
        ...
        if not self.air.partyManager.canBuyParties():
            ...
            taskMgr.doMethodLater(
                30.0, self.sendTimeoutMovie, self.uniqueName("clearMovie")
            )
        else:
    ...
    def answer(self, wantsToPlan):
        ...
        if avId not in self.busy:
            self.air.writeServerEvent(
                "suspicious",
                avId,
                "DistributedNPCPartyPersonAI.answer busy with %s" % self.busy,
            )
            self.notify.warning(
        ...
        if wantsToPlan:
            ...
            if av:
                if av.getGameAccess() != ToontownGlobals.AccessFull:
                    ...
                    self.d_setMovie(
                        avId, NPCToons.PARTY_MOVIE_COMPLETE, [hoodId, zoneId]
                    )
        else:
            av = simbase.air.doId2do.get(avId)
            if av:
