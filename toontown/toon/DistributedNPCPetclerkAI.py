...
class DistributedNPCPetclerkAI(DistributedNPCToonBaseAI):
    ...
    def fishSold(self):
        ...
        if avId not in self.busy:
            ...
            self.notify.warning(
                "somebody called fishSold that I was not busy with! avId: %s" % avId
            )
            return
    ...
    def petAdopted(self, petNum, nameIndex):
        ...
        if avId not in self.busy:
            self.air.writeServerEvent(
                "suspicious",
                avId,
                "DistributedNPCPetshopAI.petAdopted busy with %s" % self.busy,
            )
            self.notify.warning(
                "somebody called petAdopted that I was not busy with! avId: %s" % avId
            )
            return
        av = simbase.air.doId2do.get(avId)
        if av:
            ...
            if petNum not in range(0, len(self.petSeeds)):
                self.air.writeServerEvent(
                    "suspicious",
                    avId,
                    "DistributedNPCPetshopAI.petAdopted and no such pet!",
                )
                self.notify.warning(
                    "somebody called petAdopted on a non-existent pet! avId: %s" % avId
                )
                return
            ...
            if cost > av.getTotalMoney():
                self.air.writeServerEvent(
                    "suspicious",
                    avId,
                    "DistributedNPCPetshopAI.petAdopted and toon doesn't have enough money!",
                )
                self.notify.warning(
                    "somebody called petAdopted and didn't have enough money to adopt! avId: %s"
                    % avId
                )
                return
            if av.petId != 0:
            ...
            if nameIndex not in range(0, TTLocalizer.PetNameIndexMAX):
                self.air.writeServerEvent(
                    "avoid_crash",
                    avId,
                    "DistributedNPCPetclerkAI.petAdopted and didn't have valid nameIndex!",
                )
                self.notify.warning(
                    "somebody called petAdopted and didn't have valid nameIndex to adopt! avId: %s"
                    % avId
                )
                return
            simbase.air.petMgr.createNewPetFromSeed(
                avId,
                self.petSeeds[petNum],
                nameIndex=nameIndex,
                gender=gender,
                safeZoneId=zoneId,
            )
    ...
    def petReturned(self):
        ...
        if avId not in self.busy:
            self.air.writeServerEvent(
                "suspicious",
                avId,
                "DistributedNPCPetshopAI.petReturned busy with %s" % self.busy,
            )
            self.notify.warning(
                "somebody called petReturned that I was not busy with! avId: %s" % avId
            )
            return
        av = simbase.air.doId2do.get(avId)
        if av:
    ...
    def transactionDone(self):
        ...
        if avId not in self.busy:
            self.air.writeServerEvent(
                "suspicious",
                avId,
                "DistributedNPCPetshopAI.transactionDone busy with %s" % self.busy,
            )
            self.notify.warning(
