...
def doVictory(toon, volume=1):
    ...
    sfxTrack = SoundInterval(
        sfx, loop=1, duration=sfxDuration, node=toon, volume=volume
    )
...
def doAngryEyes(toon, volume=1):
    track = Sequence(
        Func(toon.angryEyes), Func(toon.blinkEyes), Wait(10.0), Func(toon.normalEyes)
    )
...
def doHappy(toon, volume=1):
    track = Sequence(
        Func(toon.play, "jump"), Func(toon.normalEyes), Func(toon.blinkEyes)
    )
...
def doResistanceSalute(toon, volume=1):
    ...
    animTrack = Sequence(
        Func(
            ...
            OTPLocalizer.CustomSCStrings[4020],
            CFSpeech | CFTimeout,
        ),
...
def doUpset(toon, volume=1):
    ...
    exitTrack = Sequence(
        Func(toon.hideSadMuzzle), Func(toon.normalEyes), Func(stopAnim, anim)
    )
...
def doDelighted(toon, volume=1):
    ...
    anim = Sequence(
        ActorInterval(toon, "left"), Wait(1), ActorInterval(toon, "left", playRate=-1)
    )
    track = Sequence(
        Func(toon.blinkEyes),
    ...
    exitTrack = Sequence(
        Func(toon.hideSmileMuzzle), Func(toon.blinkEyes), Func(stopAnim, anim)
    )
...
def doFurious(toon, volume=1):
    ...
    exitTrack = Sequence(
        Func(toon.normalEyes), Func(toon.blinkEyes), Func(toon.hideAngryMuzzle)
    )
    return (track, duration, exitTrack)
...
def doLaugh(toon, volume=1):
    ...
    exitTrack = Sequence(
        Func(toon.hideLaughMuzzle), Func(toon.blinkEyes), Func(stopAnim)
    )
...
def getSingingNote(toon, note, volume=1):
    ...
    sfx = base.loader.loadSfx(
        filePath + filePrefix + speciesName + "_" + note + fileSuffix
    )
    ...
    track = Sequence(
        Func(toon.showSurpriseMuzzle), Parallel(Func(playAnim), Func(playSfx, volume))
    )
...
class TTEmote(Emote.Emote):
    ...
    def __init__(self):
        ...
        if len(self.emoteFunc) != len(OTPLocalizer.EmoteList):
            self.notify.error(
                "Emote.EmoteFunc and OTPLocalizer.EmoteList are different lengths."
            )
        self.track = None
    ...
    def doEmote(self, toon, emoteIndex, ts=0, volume=1):
        ...
        if track != None:
            ...
            track = Sequence(
                track, Func(self.releaseAll, toon, "doEmote"), autoFinish=1
            )
            if toon.isLocal():
