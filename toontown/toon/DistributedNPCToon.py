...
class DistributedNPCToon(DistributedNPCToonBase):
    ...
    def setupCamera(self, mode):
        ...
        if (
            mode == NPCToons.QUEST_MOVIE_QUEST_CHOICE
            or mode == NPCToons.QUEST_MOVIE_TRACK_CHOICE
        ):
            quat = Quat()
            quat.setHpr((155, -2, 0))
            self.cameraLerp = camera.posQuatInterval(
                1,
    ...
    def setMovie(self, mode, npcId, avId, quests, timestamp):
        ...
        if mode == NPCToons.QUEST_MOVIE_COMPLETE:
            ...
            return
        fullString = Quests.fillInQuestNames(
            fullString, avName=av._name, fromNpcId=npcId, toNpcId=toNpcId
        )
        self.acceptOnce(
            self.uniqueName("doneChatPage"),
            self.finishMovie,
