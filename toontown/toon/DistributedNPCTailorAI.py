...
class DistributedNPCTailorAI(DistributedNPCToonBaseAI):
    ...
    def sendShoppingMovie(self, avId, flag):
        ...
        self.sendUpdate(
            "setMovie",
            [flag, self.npcId, avId, ClockDelta.globalClockDelta.getRealNetworkTime()],
        )
        taskMgr.doMethodLater(
            NPCToons.TAILOR_COUNTDOWN_TIME,
    ...
    def setDNA(self, blob, finished, which):
        ...
        if avId in self.air.doId2do:
            ...
            if finished == 2 and which > 0:
                if (
                    self.air.questManager.removeClothingTicket(av, self) == 1
                    or self.freeClothes
                ):
                    av.b_setDNAString(blob)
                    if which & ClosetGlobals.SHIRT:
                        if (
                            av.addToClothesTopsList(
        ...
        if avId in self.busy:
            ...
            self.notify.warning(
                "setDNA from unknown avId: %s busy: %s" % (avId, self.busy)
            )
    ...
    def __handleUnexpectedExit(self, avId):
        ...
        if self.customerId == avId:
            ...
            self.notify.warning(
                "invalid customer avId: %s, customerId: %s " % (avId, self.customerId)
            )
        if avId in self.busy:
            self.sendClearMovie(None)
        else:
