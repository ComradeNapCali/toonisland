...
class DistributedNPCKartClerkAI(DistributedNPCToonBaseAI):
    ...
    def buyKart(self, whichKart):
        ...
        if avId not in self.busy:
            self.air.writeServerEvent(
                "suspicious",
                avId,
                "DistributedNPCKartClerkAI.buyKart busy with %s" % self.busy,
            )
            self.notify.warning(
                "somebody called buyKart that I was not busy with! avId: %s" % avId
            )
            return
        ...
        if av:
            ...
            if cost == "key error":
                self.air.writeServerEvent(
                    "suspicious",
                    avId,
                    "Player trying to buy non-existant kart %s" % whichKart,
                )
                self.notify.warning(
    ...
    def buyAccessory(self, whichAcc):
        ...
        if avId not in self.busy:
            ...
            self.notify.warning(
                "somebody called buyAccessory that I was not busy with! avId: %s" % avId
            )
            return
        ...
        if av:
            ...
            self.air.writeServerEvent(
                "kartingAccessoryPurchased", avId, "%s" % whichAcc
            )
