...
class ToonAvatarDetailPanel(DirectFrame):
    ...
    def __init__(self, avId, avName, playerId=None, parent=base.a2dTopRight, **kw):
        ...
        self.fsm = ClassicFSM.ClassicFSM(
            ...
            [
                ...
                State.State(
                    "begin", self.enterBegin, self.exitBegin, ["query", "data", "off"]
                ),
                State.State(
                    "query", self.enterQuery, self.exitQuery, ["data", "invalid", "off"]
                ),
                State.State("data", self.enterData, self.exitData, ["off"]),
                State.State("invalid", self.enterInvalid, self.exitInvalid, ["off"]),
            ],
            "off",
            "off",
        )
    ...
    def enterQuery(self):
        ...
        base.cr.getAvatarDetails(
            self.avatar, self.__handleAvatarDetails, "DistributedToon"
        )
        return
    ...
    def __showData(self):
        av = self.avatar
        toonID = "TIA-" + (str(self.avId)[1:-4]) + "-" + (str(self.avId)[5:])
        online = 1
        ...
        if online:
            ...
            text = TTLocalizer.AvatarDetailPanelOffline % {"toonid": toonID}
