...
class DistributedFurnitureManagerAI(DistributedObjectAI):
    ...
    def moveItemToAtticMessage(self, itemId, context):
        ...
        self.items.remove(item)
        self.sendUpdateToAvatarId(
            avId, "moveItemToAtticResponse", [ToontownGlobals.FM_MovedItem, context]
        )
    ...
    def deleteItemFromAtticMessage(self, blob, index, context):
        ...
        try:
            ...
            self.air.writeServerEvent(
                "suspicious", avId, "av tried to delete nonexistent attic item"
            )
            self.sendUpdateToAvatarId(
                avId,
                "deleteItemFromAtticResponse",
                [ToontownGlobals.FM_InvalidIndex, context],
            )
            return
    ...
    def moveWallpaperFromAtticMessage(self, index, room, context):
        ...
        try:
            ...
            self.sendUpdateToAvatarId(
                avId,
                "moveWallpaperFromAtticResponse",
                [ToontownGlobals.FM_InvalidIndex, context],
            )
            return
    ...
    def deleteWallpaperFromAtticMessage(self, blob, index, context):
        ...
        try:
            ...
            self.sendUpdateToAvatarId(
                avId,
                "deleteWallpaperFromAtticResponse",
                [ToontownGlobals.FM_InvalidIndex, context],
            )
            return
    ...
    def moveWindowFromAtticMessage(self, index, slot, context):
        ...
        if slot > 5:
            self.air.writeServerEvent(
                "suspicious", avId, "av tried to put window in invalid slot!"
            )
            return
    ...
    def deleteWindowFromAtticMessage(self, blob, index, context):
        ...
        try:
            ...
            self.air.writeServerEvent(
                "suspicious", avId, "av tried to delete nonexistent window"
            )
            self.sendUpdateToAvatarId(
                avId,
                "deleteWindowFromAtticResponse",
                [ToontownGlobals.FM_InvalidIndex, context],
            )
            return
    ...
    def recoverDeletedItemMessage(self, blob, index, context):
        ...
        numItems = (
            len(self.atticItems)
            + len(self.atticWallpaper)
            + len(self.atticWindows)
            + len(self.items)
        )
        if numItems + 1 > ToontownGlobals.MaxHouseItems:
            self.sendUpdateToAvatarId(
                avId,
                "recoverDeletedItemResponse",
        ...
        try:
            ...
            self.air.writeServerEvent(
                "suspicious", avId, "av tried to recover nonexistent item"
            )
            self.sendUpdateToAvatarId(
                avId,
                "recoverDeletedItemResponse",
        ...
        for itemType in list(type2attic.keys()):
            if isinstance(item, itemType):
                ...
                self.removeDeletedItemBlob(base64.b64encode(item.getBlob()))
                self.sendUpdateToAvatarId(
                    avId,
                    "recoverDeletedItemResponse",
    ...
    def loadFurniture(self):
        for item in CatalogItemList(
            self.house.getInteriorItems(),
            store=CatalogItem.Customization | CatalogItem.Location,
        ):
            self.generateItem(item)

        self.wallpaper = CatalogItemList(
    ...
    def _verifyOwner(self, avId, message, context):
        if self.house.avatarId != avId:
            ...
            self.sendUpdateToAvatarId(
                avId, message + "Response", [ToontownGlobals.FM_NotOwner, context]
            )
            return False
        elif self.director != avId:
            self.sendUpdateToAvatarId(
                avId, message + "Response", [ToontownGlobals.FM_NotDirector, context]
            )
            return False
    ...
    def _deleteItem(self, item):
        ...
        self.b_setDeletedItems(self.getDeletedItems())
        self.addDeletedItemBlob(base64.b64encode(item.getBlob()))

    def getNumItems(self):
        numItems = (
            len(self.house.interiorItems)
            + len(self.house.atticItems)
            + len(self.house.atticWallpaper)
            + len(self.house.atticWindows)
            + len(self.house.interiorWallpaper)
            + len(self.house.interiorWindows)
        )
        return numItems
    ...
    def removeDeletedItemBlob(self, deletedItemBlob, dayId=None):
        ...
        dayId = str(dayId)
        if (
            dayId in list(self.day2deletedItems.keys())
    ...
    def __deletedItemsTask(self, task):
        ...
        for deletedItemDay, deletedItemBlobs in list(self.day2deletedItems.items()):
            ...
            if (
                deletedItemDayId + int(ToontownGlobals.DeletedItemLifetime / 60 / 24)
                <= dayId
            ):
                for deletedItemBlob in deletedItemBlobs[:]:
                    try:
                        deletedItem = CatalogItem.getItem(
                            base64.b64decode(deletedItemBlob)
                        )
                    except:
