...
class DistributedPhone(DistributedFurnitureItem.DistributedFurnitureItem):
    ...
    def announceGenerate(self):
        ...
        taskMgr.doMethodLater(
            6, self.ringIfHasPhoneQuest, self.uniqueName("ringDoLater")
        )
    ...
    def setupCamera(self, mode):
        ...
        if mode == PhoneGlobals.PHONE_MOVIE_PICKUP:
            ...
            self.cameraLerp = LerpPosQuatInterval(
                camera,
                1,
                (4, -4, base.localAvatar.getHeight() - 0.5),
                quat,
                blendType="easeOut",
    ...
    def __showPhoneGui(self):
        if self.toonScale:
            self.sendUpdate(
                "setNewScale", [self.toonScale[0], self.toonScale[1], self.toonScale[2]]
            )
    ...
    def requestGiftPurchase(self, item, targetDoID, callback, optional=-1):
        ...
        context = self.getCallbackContext(callback, [item])
        self.sendUpdate(
            "requestGiftPurchaseMessage", [context, targetDoID, blob, optional]
        )
    ...
    def takePhoneInterval(self, toon):
        ...
        walkToPhone = Sequence(
            ...
            toon.posHprInterval(
                walkTime, destToonPos, destToonHpr, blendType="easeInOut"
            ),
    ...
    def replacePhoneInterval(self, toon):
        ...
        interval = Sequence(
            Parallel(
                ActorInterval(self.model, phoneBackAnim),
            ...
            self.scaleInterval(
                scaleTime,
                localAvatar.getGeomNode().getScale()[2],
                blendType="easeInOut",
            ),
            Func(toon.loop, "neutral"),
        )
    ...
    def ringIfHasPhoneQuest(self, task):
        if Quests.avatarHasPhoneQuest(
            base.localAvatar
        ) and not Quests.avatarHasCompletedPhoneQuest(base.localAvatar):
            self.ring()
        return Task.done
    ...
    def ring(self):
        ...
        w = 0.05
        shakeOnce = Sequence(
            Func(phone.setR, r), Wait(w), Func(phone.setR, -r), Wait(w)
        )
        shakeSeq = Sequence()
        for i in range(16):
        ...
        ringIval = Parallel(
            Func(base.playSfx, self.ringSfx), shakeSeq, Func(phone.setR, 0)
        )
