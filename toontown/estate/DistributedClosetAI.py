...
class DistributedClosetAI(DistributedFurnitureItemAI):
    ...
    def __handleOwnerQuery(self, dclass, fields):
        ...
        taskMgr.doMethodLater(
            ClosetGlobals.TIMEOUT_TIME,
            self.__handleClosetTimeout,
            "closet-timeout-%d" % self.customerId,
            extraArgs=[self.customerId],
        )
    ...
    def removeItem(self, dnaString, itemType):
        ...
        if not av:
            self.air.writeServerEvent(
                "suspicious", avId, "av not in same shard as closet!"
            )
            return

        if av.getLocation() != self.getLocation():
            self.air.writeServerEvent(
                "suspicious", avId, "av not in same zone as closet!"
            )
            return
    ...
    def setDNA(self, dnaString, finished, whichItems):
        ...
        if avId != self.customerId:
            if self.customerId:
                ...
                self.notify.warning(
                    "customerId: %s, but got setDNA for: %s" % (self.customerId, avId)
                )
        ...
        if not testDNA.isValidNetString(dnaString):
            self.air.writeServerEvent(
                "suspicious",
                avId,
                "DistributedClosetAI.setDNA: invalid dna: %s" % dnaString,
            )
            return
        ...
        if finished == 0:
            if not self.__validChange(testDNA):
                ...
                self.air.writeServerEvent(
                    "suspicious",
                    avId,
                    "DistributedClosetAI.setDNA: av tried to switch body dna",
                )
                return
            ...
            if avId != self.ownerId:
                self.air.writeServerEvent(
                    "suspicious", avId, "av tried to steal clothing!"
                )
                return
            ...
            if whichItems & ClosetGlobals.SHIRT:
                if av.replaceItemInClothesTopsList(
                    ...
                    self.air.writeServerEvent(
                        "suspicious", avId, "av tried to put on shirt they don't own"
                    )
                    return
            ...
            if whichItems & ClosetGlobals.SHORTS:
                if av.replaceItemInClothesBottomsList(
                    ...
                    self.air.writeServerEvent(
                        "suspicious", avId, "av tried to put on shorts they don't own"
                    )
                    return
            ...
            for item in self.removedShirts[:]:
                if not av.removeItemInClothesTopsList(*item):
                    self.air.writeServerEvent(
                        "suspicious", avId, "av tried to delete shirt they don't own"
                    )
                    return
            ...
            for item in self.removedBottoms[:]:
                if not av.removeItemInClothesBottomsList(*item):
                    self.air.writeServerEvent(
                        "suspicious", avId, "av tried to delete bottom they don't own"
                    )
                    return
    ...
    def __handleUnexpectedExit(self, avId):
        if avId != self.customerId:
            self.notify.warning(
                "received unexpected exit for av %s that is not using the closet!"
                % avId
            )
            return
