...
class DistributedCannon(DistributedObject.DistributedObject):
    ...
    def __init__(self, cr):
        ...
        self.codeFSM = ClassicFSM.ClassicFSM(
            "CannonCode",
            [
                State.State("init", self.enterInit, self.exitInit, ["u1", "init"]),
    ...
    def setMovie(self, mode, avId):
        ...
        if mode == CannonGlobals.CANNON_MOVIE_CLEAR:
            ...
            if self.avId in self.cr.doId2do:
                ...
                self.notify.warning(
                    "Unknown avatar %d in cannon %d" % (self.avId, self.doId)
                )
    ...
    def __firePressed(self):
        ...
        self.__unmakeGui()
        self.sendUpdate(
            "setCannonLit", [self.cannonPosition[0], self.cannonPosition[1]]
        )
    ...
    def __broadcastLocalCannonPosition(self):
        self.sendUpdate(
            "setCannonPosition", [self.cannonPosition[0], self.cannonPosition[1]]
        )
    ...
    def __handleCannonHit(self, collisionEntry):
        ...
        if (
            ...
            or hitNode.find("mailboxSphere") == 0
        ...
        if hitPylonAboveWater or hitNode in [
            "matCollisions",
            "collision1",
            "floor",
            "sand_collision",
            ...
            hitNode == "roofOutside"
            or hitNode == "collision_roof"
            or hitNode == "roofclision"
        ):
    ...
    def __hitRoof(self, avatar, collisionEntry, extraArgs=[]):
        ...
        self.notify.debug(
            "avatar pos = %s, landingPos = %s" % (avatar.getPos(), self.landingPos)
        )
        ...
        h = self.barrel.getH(render)
        t = Sequence(
            LerpPosInterval(avatar, 0.5, Point3(0, 0, -0.5), blendType="easeInOut"),
            ...
            Parallel(
                ...
                LerpHprInterval(avatar, 0.3, Vec3(h, 0, 0), blendType="easeOut"),
            ),
    ...
    def __hitCannonBumper(self, avatar, collisionEntry, extraArgs=[]):
        ...
        score, multiplier = ToontownGlobals.PinballScoring[
            ToontownGlobals.PinballCannonBumper
        ]
    ...
    def __hitCloudPlatform(self, avatar, collisionEntry, extraArgs=[]):
        if True:
            self.__hitBumper(avatar, collisionEntry, self.sndHitHouse, kr=0.4, angVel=5)
            score, multiplier = ToontownGlobals.PinballScoring[
                ToontownGlobals.PinballCloudBumperLow
            ]
            intoNodePath = collisionEntry.getIntoNodePath()
            name = intoNodePath.getParent().getName()
            splitParts = name.split("-")
        ...
        if estate:
            ...
            for id in doIds:
                ...
                if t:
                    ...
                    rad = 10.5
                    height = 10.0
                    t_impact = trajectory.checkCollisionWithCylinderSides(
                        pos, rad, height
                    )
                    if t_impact > 0:
    ...
    def __smokeTask(self, task):
        ...
        track = Sequence(
            Parallel(
                LerpScaleInterval(self.smoke, 0.5, 3),
                LerpColorScaleInterval(self.smoke, 0.5, Vec4(2, 2, 2, 0)),
            ),
    ...
    def incrementPinballInfo(self, score, multiplier):
        if base.localAvatar.doId == self.avId:
            ...
            self.notify.debug(
                "score =%d multiplier=%d  curscore=%d curMult=%d"
                % (score, multiplier, self.curPinballScore, self.curPinballMultiplier)
            )
            self.d_setPinballInfo()

    def d_setPinballInfo(self):
        self.notify.debug(
            "d_setPinballInfo %d %d" % (self.curPinballScore, self.curPinballMultiplier)
        )
        target = base.cr.doId2do[self.targetId]
        target.b_setCurPinballScore(
            self.avId, self.curPinballScore, self.curPinballMultiplier
        )
    ...
    def createBlock(self, collisionName=None):
        ...
        for quad in quads:
            colQuad = CollisionPolygon(
                Point3(*vertices[quad[0]]),
    ...
    def loadCannonBumper(self):
        ...
        self.notify.debug(
            "------------self.cannonBumper.setPos %.2f %.2f %.2f"
            % (
    ...
    def setCannonBumperPos(self, x, y, z):
        ...
        taskMgr.doMethodLater(
            0.25, self.turnOnBumperCollision, self.uniqueName("BumperON")
        )

    def turnOnBumperCollision(self, whatever=0):
        if self.bumperCol:
            self.bumperCol.setCollideMask(
