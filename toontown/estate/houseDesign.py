...
class ObjectManager(NodePath, DirectObject):
    ...
    def enterFurnitureMode(self, furnitureManager, fDirector):
        ...
        self.guiInterval = self.furnitureGui.posHprScaleInterval(
            1.0,
            Point3(0.173333, 1, -1.03),
            Vec3(0),
            Vec3(0.06),
            startPos=Point3(0.143333, 1, -0.67),
            startHpr=Vec3(0),
    ...
    def moveObjectInit(self):
        self.dragPointNP.setPosHpr(
            self.selectedObject, self.selectedObject.dragPoint, Vec3(0)
        )
    ...
    def createSelectedObjectPanel(self, guiModels):
        ...
        self.rotateLeftButton = DirectButton(
            parent=self.buttonFrame,
            relief=None,
            image=(guiCCWArrowUp, guiCCWArrowDown, guiCCWArrowRollover, guiCCWArrowUp),
            image_pos=(0, 0, 0.1),
            image_scale=0.15,
        ...
        self.rotateRightButton = DirectButton(
            parent=self.buttonFrame,
            relief=None,
            image=(guiCWArrowUp, guiCWArrowDown, guiCWArrowRollover, guiCWArrowUp),
            image_pos=(0, 0, 0.1),
            image_scale=0.15,
            image3_color=Vec4(0.5, 0.5, 0.5, 0.75),
    ...
    def createMainControls(self, guiModels):
        ...
        self.atticRoof = DirectLabel(
            parent=self.furnitureGui, relief=None, image=guiModels.find("**/rooftile")
        )
    ...
    def regenerateAtticPicker(self):
        ...
        if self.atticPicker:
            ...
            self.atticPicker = None
        itemList = (
            self.atticItemPanels + self.atticWallpaperPanels + self.atticWindowPanels
        )
        if self.deleteMode:
    ...
    def sendItemToAttic(self):
        ...
        if self.selectedObject:
            callback = PythonUtil.Functor(
                self.__sendItemToAtticCallback, self.selectedObject.getKey()
            )
    ...
    def deleteItemFromAttic(self, item, itemIndex):
        ...
        self.furnitureManager.deleteItemFromAttic(
            item, itemIndex, self.__deleteItemFromAtticCallback
        )
    ...
    def __deleteItemFromAtticCallback(self, retcode, item, itemIndex):
        ...
        if retcode < 0:
            self.notify.info(
                "Unable to delete furniture item %s, reason %s." % (itemIndex, retcode)
            )
            return
        self.atticItemPanels[itemIndex].destroy()
        del self.atticItemPanels[itemIndex]
    ...
    def deleteWallpaperFromAttic(self, item, itemIndex):
        ...
        self.furnitureManager.deleteWallpaperFromAttic(
            item, itemIndex, self.__deleteWallpaperFromAtticCallback
        )
    ...
    def __deleteWallpaperFromAtticCallback(self, retcode, item, itemIndex):
        ...
        if retcode < 0:
            self.notify.info(
                "Unable to delete wallpaper %s, reason %s." % (itemIndex, retcode)
            )
            return
        self.atticWallpaperPanels[itemIndex].destroy()
    ...
    def deleteWindowFromAttic(self, item, itemIndex):
        ...
        self.furnitureManager.deleteWindowFromAttic(
            item, itemIndex, self.__deleteWindowFromAtticCallback
        )
    ...
    def __deleteWindowFromAtticCallback(self, retcode, item, itemIndex):
        ...
        if retcode < 0:
            self.notify.info(
                "Unable to delete window %s, reason %s." % (itemIndex, retcode)
            )
            return
        self.atticWindowPanels[itemIndex].destroy()
        del self.atticWindowPanels[itemIndex]
    ...
    def requestDelete(self, item, itemIndex, deleteFunction):
        self.__cleanupVerifyDelete()
        if (
            self.furnitureManager.ownerId != base.localAvatar.doId
            or not item.isDeletable()
        ):
    ...
    def warnNonDeletableItem(self, item):
        ...
        if self.furnitureManager.ownerId != base.localAvatar.doId:
            message = (
                TTLocalizer.HDNonDeletableNotOwner % self.furnitureManager.ownerName
            )
    ...
    def __recoverDeletedItemCallback(self, retcode, item, itemIndex):
        ...
        if retcode < 0:
            ...
            self.notify.info(
                "Unable to recover deleted item %s, reason %s." % (itemIndex, retcode)
            )
            return
        self.__enableItemButtons(1)
        ...
        if (
            itemType == CatalogItemTypes.WALLPAPER_ITEM
            or itemType == CatalogItemTypes.FLOORING_ITEM
            or itemType == CatalogItemTypes.MOULDING_ITEM
            or itemType == CatalogItemTypes.WAINSCOTING_ITEM
        ):
