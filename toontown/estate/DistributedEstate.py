...
class DistributedEstate(DistributedObject.DistributedObject):
    ...
    def loadWitch(self):
        ...
        replacement = Sequence(
            LerpColorScaleInterval(self.airplane, 0.1, Vec4(1, 1, 1, 0)),
            Func(__replaceAirplane__),
            LerpColorScaleInterval(self.airplane, 0.1, Vec4(1, 1, 1, 1)),
        )
        replacement.start()
    ...
    def __pauseAirplaneTask(self):
        ...
        taskMgr.doMethodLater(
            pause, self.airplaneFlyTask, self.taskName("estate-airplane")
        )
    ...
    def __dayTimeTask(self, task):
        ...
        track = Sequence(
            ...
            Parallel(
                LerpColorScaleInterval(
                    base.cr.playGame.hood.loader.geom,
                    HouseGlobals.HALF_NIGHT_PERIOD,
                    Vec4(0.2, 0.2, 0.5, 1),
                ),
                LerpColorScaleInterval(
                    base.cr.playGame.hood.sky,
    ...
    def __sunTask(self, task):
        ...
        track = Sequence(
            Parallel(
                LerpHprInterval(
                    sunMoonNode, HouseGlobals.HALF_DAY_PERIOD, Vec3(0, 0, 0)
                ),
                LerpColorScaleInterval(
                    sun, HouseGlobals.HALF_DAY_PERIOD, Vec4(1, 1, 0.5, 1)
                ),
            ),
            ...
            Wait(HouseGlobals.HALF_NIGHT_PERIOD - 0.5),
            LerpHprInterval(sunMoonNode, HouseGlobals.HALF_NIGHT_PERIOD, Vec3(0, 0, 0)),
            ...
            Wait(HouseGlobals.HALF_DAY_PERIOD - 0.5),
        )
        if self.sunTrack:
        ...
        if hasattr(task, "ts"):
            ts = task.ts
            if (
                ts > HouseGlobals.HALF_DAY_PERIOD
        ...
        taskMgr.doMethodLater(
            HouseGlobals.DAY_NIGHT_PERIOD - ts, self.__sunTask, self.taskName("sunTask")
        )
        return Task.done
