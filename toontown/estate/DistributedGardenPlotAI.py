...
class DistributedGardenPlotAI(DistributedLawnDecorAI):
    ...
    def __initialSanityCheck(self, wantedType=None, forceOwner=False):
        ...
        if not av:
            self.air.writeServerEvent(
                "suspicious",
                avId,
                "called DistributedGardenPlotAI method outside shard!",
            )
            return
        ...
        if wantedType is not None and self.plotType != wantedType:
            self.air.writeServerEvent(
                "suspicious",
                avId,
                "called incorrect DistributedGardenPlotAI method!",
    ...
    def plantFlower(self, species, variety, usingFlowerAll=False):
        av = self.__initialSanityCheck(
            GardenGlobals.FLOWER_TYPE if not usingFlowerAll else None, usingFlowerAll
        )
        if not av:
            return
        ...
        if not usingFlowerAll:
            cost = len(
                GardenGlobals.Recipes[plantAttributes["varieties"][variety][0]]["beans"]
            )
            av.takeMoney(cost)
            self.d_setMovie(GardenGlobals.MOVIE_PLANT)

        def handlePlantFlower(task):
            flower = self.mgr.plantFlower(
                self.getFlowerIndex(),
                species,
                variety,
                plot=self,
            ...
            if not usingFlowerAll:
                flower.d_setMovie(
                    GardenGlobals.MOVIE_FINISHPLANTING, self.__plantingAvId
                )
            ...
            self.air.writeServerEvent(
                ...
                variety=variety,
                plot=self.plot,
                name=plantAttributes.get("name", "unknown flower"),
            )
            if task:
                return task.done
        ...
        if usingFlowerAll:
            ...
            taskMgr.doMethodLater(
                7, handlePlantFlower, self.uniqueName("handle-plant-flower")
            )
    ...
    def plantGagTree(self, track, index):
        ...
        def handlePlantTree(task):
            ...
            tree = self.mgr.plantTree(
                self.getTreeIndex(),
                GardenGlobals.getTreeTypeIndex(track, index),
                plot=self,
                ownerIndex=self.ownerIndex,
                plotId=self.plot,
                pos=(self.getPos(), self.getH()),
            )
    ...
    def plantStatuary(self, species):
        ...
        def handlePlaceStatuary(task):
            ...
            statuary = self.mgr.placeStatuary(
                self.mgr.S_pack(0, 0, species, 0),
                plot=self,
                ownerIndex=self.ownerIndex,
                plotId=self.plot,
                pos=(self.getPos(), self.getH()),
            ...
            self.air.writeServerEvent(
                "plant-statuary", self.__plantingAvId, species=species, plot=self.plot
            )
            return task.done

        taskMgr.doMethodLater(
            7, handlePlaceStatuary, self.uniqueName("handle-place-statuary")
        )
        self.__plantingAvId = av.doId
    ...
    def plantToonStatuary(self, species, dnaCode):
        ...
        def handlePlaceStatuary(task):
            ...
            statuary = self.mgr.placeStatuary(
                self.mgr.S_pack(dnaCode, 0, species, 0),
                plot=self,
                ownerIndex=self.ownerIndex,
                plotId=self.plot,
                pos=(self.getPos(), self.getH()),
