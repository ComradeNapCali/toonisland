...
class DistributedCloset(DistributedFurnitureItem.DistributedFurnitureItem):
    ...
    def __init__(self, cr):
        ...
        self.fsm = ClassicFSM.ClassicFSM(
            ...
            [
                State.State(
                    "off", self.enterOff, self.exitOff, ["ready", "open", "closed"]
                ),
                State.State(
                    "ready", self.enterReady, self.exitReady, ["open", "closed"]
                ),
                State.State(
                    "closed", self.enterClosed, self.exitClosed, ["open", "off"]
                ),
                State.State("open", self.enterOpen, self.exitOpen, ["closed", "off"]),
            ],
            "off",
            "off",
        )
    ...
    def enterOpen(self):
        if self.ownerId:
            ...
            if self.av:
                ...
                self.av.stopSmooth()
                self.avMoveTrack = Sequence(
                    Parallel(
                        Func(self.av.play, "walk"),
                        LerpPosHprInterval(
                            nodePath=self.av,
                            other=self,
                            duration=1.0,
    ...
    def handleEnterSphere(self, collEntry):
        ...
        if (
            ...
            and globalClock.getFrameTime() <= self.lastTime + 0.5
        ):
    ...
    def _handlePurchaseDone(self, timeout=0):
        if timeout == 1:
            ...
            which = 0
            if hasattr(self.closetGUI, "topChoice") and hasattr(
                self.closetGUI, "bottomChoice"
            ):
        ...
        if avId and avId != base.localAvatar.doId:
            ...
            if av:
                if self.av == base.cr.doId2do[avId]:
                    ...
                    if (
                        len(oldTorso) == 2
                        and len(self.av.style.torso) == 2
                        and self.av.style.torso[1] != oldTorso[1]
                    ):
    ...
    def printInfo(self):
        ...
        print(
            "current top = %s,%s,%s,%s and  bot = %s,%s,"
            % (
                self.av.style.topTex,
    ...
    def _handleNotOwnerMessageOK(self):
        self.popupInfo.reparentTo(hidden)
        taskMgr.doMethodLater(
            0.1, self.popupChangeClothesGUI, self.uniqueName("popupChangeClothesGUI")
        )
