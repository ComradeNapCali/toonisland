...
class DistributedTarget(DistributedObject.DistributedObject):
        ...
        if self.enabled != enabled:
            ...
            if enabled:
                self.fadeTrack = Sequence(
                    ...
                    Func(self.geom.colorScaleInterval, 1.0, Vec4(1.0, 1.0, 1.0, 1.0)),
                    Wait(1),
                    Func(
                        base.localAvatar.setSystemMessage,
                        0,
                ...
                self.fadeTrack = Sequence(
                    Func(self.geom.colorScaleInterval, 1.0, Vec4(1.0, 1.0, 1.0, 0.0)),
    ...
    def handleHitTarget(self, avId=None, vel=None):
        ...
        if vel:
            ...
            self.targetBounceTrack = Sequence(
                LerpPosInterval(
                    self.geom, duration=0.1, pos=newPos, blendType="easeOut"
                ),
                LerpPosInterval(
                    self.geom, duration=0.25, pos=springPos, blendType="easeOut"
                ),
    ...
    def showScore(self):
        ...
        if pinballEntry:
            ...
            scoreText += TTLocalizer.PinballScoreHolder % (
