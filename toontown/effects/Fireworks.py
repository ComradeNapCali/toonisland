...
def shootFireworkRing(x, y, z, color1, color2, amp):
    ...
    force0 = LinearSourceForce(
        Point3(x, y, z), LinearDistanceForce.FTONEOVERR, 0.1, 1.1 * amp, 1
    )
    force0.setActive(1)
    f0.addForce(force0)
...
def shootFireworkRocket(x, y, z, color1, color2, amp):
    ...
    t = Sequence(
        Func(f.start, render, render),
        Func(sfx.play),
        LerpPosInterval(f, 2.0, Vec3(x, y, z + 20 * amp), blendType="easeInOut"),
        Func(p0.setBirthRate, 3),
        Wait(0.5),
        Func(f.cleanup),
...
def shootPop(x, y, z, color1, color2, amp):
    ...
    sfx = loader.loadSfx(sfxName)
    t = Sequence(
        Func(sfx.play), Wait(3), name=getNextSequenceName("shootFireworkRocket")
    )
    t.start()
...
def shootFireworkCircleGeneric(x, y, z, color1, color2, amp, poolSize):
    ...
    sfxName = random.choice(
        (
            "phase_4/audio/sfx/firework_explosion_01.ogg",
