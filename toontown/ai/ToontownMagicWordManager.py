...
class ToontownMagicWordManager(MagicWordManager.MagicWordManager):
    ...
    def doLoginMagicWords(self):
        ...
        if autoRestock != -1:
            self.d_setMagicWord(
                "~autoRestock %s" % autoRestock, base.localAvatar.doId, 0
            )
        ...
        if autoRestockSOS != -1:
            self.d_setMagicWord(
                "~autoRestockSOS %s" % autoRestockSOS, base.localAvatar.doId, 0
            )
        ...
        if autoRestockSummons != -1:
            self.d_setMagicWord(
                "~autoRestockSummons %s" % autoRestockSummons, base.localAvatar.doId, 0
            )
        paidStatus = base.config.GetString("force-paid-status", "none")
        if paidStatus != "none":
            self.d_setMagicWord(
                "~setPaid %s" % choice(paidStatus == "paid", 1, 0), localAvatar.doId, 0
            )
    ...
    def doMagicWord(self, word, avId, zoneId):
        ...
        if MagicWordManager.MagicWordManager.doMagicWord(self, word, avId, zoneId) == 1:
            ...
            if base.wantKarts:
                ...
                response = (
                    "Kart %s has been purchased with body and accessory color %s."
    ...
    def getCSBitmask(self, str):
        ...
        for w in words:
            if w == "wall":
                ...
                bitmask |= (
                    ToontownGlobals.FurnitureSideBitmask
    ...
    def doGolf(self, word):
        ...
        if len(args) == 1:
            ...
            if golfHole:
                if (
                    hasattr(golfHole, "golfBarrier")
                    and not golfHole.golfBarrier.isEmpty()
                ):
