...
class HalloweenHolidayDecorator(HolidayDecorator.HolidayDecorator):
    ...
    def __checkStreetValidity(self):
        if (
            ...
            and isinstance(base.cr.playGame.getPlace(), Street.Street)
            and hasattr(base.cr.playGame.getPlace(), "loader")
            and base.cr.playGame.getPlace().loader
            and hasattr(base.cr.playGame.getPlace().loader, "geom")
            and base.cr.playGame.getPlace().loader.geom
        ):
            return True
        else:
    ...
    def __checkHoodValidity(self):
        if (
            ...
            and base.cr.playGame.getPlace()
            and (
                isinstance(base.cr.playGame.getPlace(), Playground.Playground)
                or isinstance(base.cr.playGame.getPlace(), Estate.Estate)
            )
            and hasattr(base.cr.playGame.getPlace(), "loader")
            and base.cr.playGame.getPlace().loader
            and hasattr(base.cr.playGame.getPlace().loader, "hood")
            and base.cr.playGame.getPlace().loader.hood
            and hasattr(base.cr.playGame.getPlace().loader.hood, "loader")
            and base.cr.playGame.getPlace().loader.hood.loader
            and hasattr(base.cr.playGame.getPlace().loader.hood.loader, "geom")
    ...
    def __startSpookySky(self):
        if (
            (self.__checkHoodValidity() or self.__checkStreetValidity())
            and hasattr(base.cr.playGame.hood, "sky")
    ...
    def decorate(self):
        ...
        def __lightDecorationOn__():
            ...
            if hasattr(place, "halloweenLights"):
                ...
                place.loader.hood.halloweenLights = (
                    place.loader.hood.loader.geom.findAllMatches("**/*light*")
                )
                place.loader.hood.halloweenLights += (
                    place.loader.hood.loader.geom.findAllMatches("**/*lamp*")
                )
                place.loader.hood.halloweenLights += (
                    place.loader.hood.loader.geom.findAllMatches("**/prop_snow_tree*")
                )
            ...
            preShow = Sequence(
                Parallel(
                    LerpColorScaleInterval(
                        base.cr.playGame.hood.sky, 1.5, Vec4(1, 1, 1, 0.25)
                    ),
                    LerpColorScaleInterval(
    ...
    def undecorate(self):
        if (
            ...
            postShow = Sequence(
                Parallel(
                    LerpColorScaleInterval(
                        base.cr.playGame.hood.sky, 1.5, Vec4(1, 1, 1, 1)
                    ),
                    LerpColorScaleInterval(
                        base.cr.playGame.hood.loader.geom, 2.5, Vec4(1, 1, 1, 1)
                    ),
                ),
