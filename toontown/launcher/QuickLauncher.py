...
class QuickLauncher(LauncherBase):
    ...
    def resumeMultifileDownload(self):
        ...
        localFilename = Filename(
            self.topDir,
            Filename("_%s.%s.%s" % (self.currentMfname, curVer, self.CompressionExt)),
        )
    ...
    def resumeInstall(self):
        for self.currentPhaseIndex in range(len(self.LauncherPhases)):
            ...
            if sys.platform == "darwin" and (
                self.currentMfname == "phase_1.mf" or self.currentMfname == "phase_2.mf"
            ):
            ...
            if self.curPhaseFile.exists():
                ...
                self.notify.info(
                    "clientSize: %s expectedSize: %s" % (fileSize, expectedSize)
                )
        ...
        if not self.requiredInstallFiles:
            ...
            return
        raise Exception(
            "Some phases not listed in LauncherPhases: %s" % self.requiredInstallFiles
        )
    ...
    def parseWebAcctParams(self):
        ...
        if "secretsNeedsParentPassword" in dict and 1:
            ...
            self.notify.info(
                "secretNeedsParentPassword = %d" % self.secretNeedsParentPasswordKey
            )
        else:
