...
def makeFanfareWithMessage(
    delay, toon, showToonName, message, messagePos, messageScale, wordwrap=100
):
...
def makeFanfareWithImage(
    delay, toon, showToonName, image, imagePos, imageScale, wordwrap=100
):
    panel = makePanel(toon, showToonName)
...
def doFanfare(delay, toon, panel):
    ...
    openBall = Parallel(
        LerpHprInterval(
            partyBallLeft, duration=0.2, startHpr=Vec3(90, 0, 0), hpr=Vec3(90, 30, 0)
        ),
        LerpHprInterval(
            partyBallRight, duration=0.2, startHpr=Vec3(-90, 0, 0), hpr=Vec3(-90, 30, 0)
        ),
    )
    ...
    trumpetsAppear = LerpColorInterval(
        trumpetNode, 0.3, startColor=Vec4(1, 1, 0, 0), color=Vec4(1, 1, 0, 1)
    )
    trumpetsVanish = LerpColorInterval(
        trumpetNode, 0.3, startColor=Vec4(1, 1, 0, 1), color=Vec4(1, 1, 0, 0)
    )
    ...
    killParticles = Parallel(
        Func(blue_p0.setLitterSize, 0),
        Func(red_p0.setLitterSize, 0),
        Func(yellow_p0.setLitterSize, 0),
    )
    p = Parallel(
        ParticleInterval(
            confettiBlue, confettiNode, worldRelative=0, duration=3, cleanup=True
        ),
        ParticleInterval(
            confettiRed, confettiNode, worldRelative=0, duration=3, cleanup=True
        ),
        ParticleInterval(
            confettiYellow, confettiNode, worldRelative=0, duration=3, cleanup=True
        ),
    )
    pOff = Parallel(
        Func(confettiBlue.remove), Func(confettiRed.remove), Func(confettiYellow.remove)
    )
    partInterval = Parallel(
        p,
        Sequence(Wait(1.7), killParticles, Wait(1.3), pOff, Func(p.finish)),
        Sequence(Wait(3), Parallel(ballVanish)),
    )
    seq1 = Parallel(
        Sequence(
            Wait(delay + 4.1),
            SoundInterval(drumroll, node=toon),
            Wait(0.25),
        ...
        Sequence(
            ...
            Wait(0.2),
            Wait(0.1),
            Parallel(openBall, partInterval),
    ...
    seq = Parallel(
        ...
        Sequence(
            Wait(delay),
            Parallel(trumpetTurn, Sequence(Wait(0.5), play)),
            Wait(0.5),
            trumpetsVanish,
        ),
    )
