    ...
    for trapList in suitTrapLists:
        ...
        for i in range(len(trapList)):
            ...
            if level == 0:
                ...
                notify.warning(
                    "__doTraps() - Incorrect trap level:                 %d" % level
                )
...
def __createThrownTrapMultiTrack(
    ...
    if trapName == "banana":
        ...
        soundTrack = getSoundTrack(
            "TL_dynamite.ogg", delay=0.8, duration=0.7, node=suit
        )
...
def __trapBanana(trap, trapProps, explode):
    ...
    notify.debug(
        "toon: %s lays banana peel in front of suit: %d" % (toon.getName(), suit.doId)
    )
    bananas = trapProps
...
def __trapRake(trap, trapProps, explode):
    ...
    suit = trap["target"][0]["suit"]
    notify.debug(
        "toon: %s lays rake in front of suit: %d" % (toon.getName(), suit.doId)
    )
    rakes = trapProps
...
def __trapMarbles(trap, trapProps, explode):
    ...
    suit = trap["target"][0]["suit"]
    notify.debug(
        "toon: %s lays marbles in front of suit: %d" % (toon.getName(), suit.doId)
    )
...
def __trapQuicksand(trap, trapProps, explode):
    ...
    suit = trap["target"][0]["suit"]
    notify.debug(
        "toon: %s lays quicksand in front of suit: %d" % (toon.getName(), suit.doId)
    )
...
def __trapTrapdoor(trap, trapProps, explode):
    ...
    for target in targets:
        suit = target["suit"]
        notify.debug(
            "toon: %s lays trapdoor in front of suit: %d" % (toon.getName(), suit.doId)
        )
...
def __createPlacedGroupTrapTrack(
    trap, prop, propName, centerSuit, propPos=None, propHpr=None, explode=0
):
    toon = trap["toon"]
    if "npc" in trap:
        toon = trap["npc"]
