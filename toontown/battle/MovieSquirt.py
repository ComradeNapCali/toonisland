...
hitSoundFiles = (
    "AA_squirt_flowersquirt.ogg",
    "AA_squirt_glasswater.ogg",
    "AA_squirt_neonwatergun.ogg",
    "AA_squirt_seltzer.ogg",
    "firehose_spray.ogg",
    "AA_throw_stormcloud.ogg",
    "AA_squirt_Geyser.ogg",
)
missSoundFiles = (
    "AA_squirt_flowersquirt_miss.ogg",
    "AA_squirt_glasswater_miss.ogg",
    "AA_squirt_neonwatergun_miss.ogg",
    "AA_squirt_seltzer_miss.ogg",
    "firehose_spray.ogg",
    "AA_throw_stormcloud_miss.ogg",
    "AA_squirt_Geyser.ogg",
)
sprayScales = [0.2, 0.3, 0.1, 0.6, 0.8, 1.0, 2.0]
...
def __doSquirt(squirt, delay, fShowStun, uberClone=0):
    ...
    if type(squirt["target"]) == type([]):
        for target in squirt["target"]:
            notify.debug(
                "toon: %s squirts prop: %d at suit: %d for hp: %d"
                % (
                    squirt["toon"].getName(),
                    squirt["level"],
                    target["suit"].doId,
...
def __getSuitTrack(
    ...
    if hp > 0:
        ...
        if kbbonus > 0 and not geyser:
            ...
            moveTrack = Sequence(
                Wait(0.2), LerpPosInterval(suit, 0.6, pos=suitPos, other=battle)
            )
        ...
        if kbbonus > 0:
            ...
            bonusTrack.append(
                Func(
                    suit.showHpText, -kbbonus, 2, openEnded=0, attackTrack=SQUIRT_TRACK
                )
            )
        if hpbonus > 0:
            bonusTrack.append(Wait(0.75))
            bonusTrack.append(
                Func(
                    suit.showHpText, -hpbonus, 1, openEnded=0, attackTrack=SQUIRT_TRACK
                )
            )
        if died != 0:
...
def __doFlower(squirt, delay, fShowStun):
    ...
    tracks.append(
        __getSoundTrack(level, hitSuit, tTotalFlowerToonAnimationTime - 0.4, toon)
    )
    ...
    if hp > 0:
        tracks.append(
            __getSplashTrack(targetPoint, scale, tSprayStarts + dSprayScale, battle)
        )
    if hp > 0 or delay <= 0:
        tracks.append(
            __getSuitTrack(
...
def __doWaterGun(squirt, delay, fShowStun):
    ...
    pistolTrack = Sequence(
        ...
        LerpScaleInterval(
            pistol, dPistolScale, pistol.getScale(), startScale=MovieUtil.PNT3_NEARZERO
        ),
...
def __doSeltzerBottle(squirt, delay, fShowStun):
    ...
    bottleTrack = Sequence(
        ...
        LerpScaleInterval(
            bottle, dBottleScale, bottle.getScale(), startScale=MovieUtil.PNT3_NEARZERO
        ),
...
def __doFireHose(squirt, delay, fShowStun):
    ...
    propTrack = Sequence(
        ...
        Wait(dHoseHold - 0.2),
        LerpScaleInterval(
            hydrantScale, 0.2, Point3(1, 1, 0.01), startScale=Point3(1, 1, 1)
        ),
...
def __doStormCloud(squirt, delay, fShowStun):
    ...
    def getCloudTrack(
        ...
        track = Sequence(
            ...
            LerpScaleInterval(
                cloud, 1.5, scaleUpPoint, startScale=MovieUtil.PNT3_NEARZERO
            ),
            Wait(rainDelay),
        )
        if useEffect == 1:
    ...
    for target in squirt["target"]:
        ...
        def getGeyserTrack(
            ...
            track = Sequence(
                ...
                LerpScaleInterval(
                    geyserWater, 1.0, scaleUpPoint, startScale=MovieUtil.PNT3_NEARZERO
                ),
                Wait(geyserHold * 0.5),
                LerpScaleInterval(
                    geyserWater, 0.5, MovieUtil.PNT3_NEARZERO, startScale=scaleUpPoint
                ),
            )
