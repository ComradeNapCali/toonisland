...
class RewardPanel(DirectFrame):
    ...
    def __init__(self, name):
        ...
        self.avNameLabel = DirectLabel(
            parent=self, relief=None, pos=(0, 0, 0.3), text=name, text_scale=0.08
        )
        ...
        self.cogPartLabel = DirectLabel(
            parent=self.cogPartFrame, text="", text_scale=0.06
        )
        self.missedItemLabel = DirectLabel(
            parent=self.missedItemFrame, text="", text_scale=0.06
        )
        ...
        for i in range(len(SuitDNA.suitDepts)):
            deptName = TextEncoder.upper(
                SuitDNA.suitDeptFullnames[SuitDNA.suitDepts[i]]
            )
            self.meritLabels.append(
                DirectLabel(
        ...
        self.skipButton = DirectButton(
            ...
            relief=None,
            image=(
                self._battleGui.find("**/tt_t_gui_gen_skipSectionUp"),
    ...
    def initQuestFrame(self, toon, avQuests):
        ...
        for i in range(len(avQuests)):
            ...
            if quest:
                ...
                completed = (
                    quest.getCompletionStatus(toon, questDesc) == Quests.COMPLETE
                )
                questLabel = self.questLabelList[i]
                questLabel.show()
    ...
    def initGagFrame(self, toon, expList, meritList, noSkip=False):
        ...
        for i in range(len(expList)):
            ...
            if toon.hasTrackAccess(i):
                trackBar.show()
                if (
                    curExp >= ToontownBattleGlobals.UnpaidMaxSkills[i]
                    and toon.getGameAccess() != OTPGlobals.AccessFull
                ):
    ...
    def incrementExp(self, track, newValue, toon):
        ...
        if (
            newValue >= ToontownBattleGlobals.UnpaidMaxSkills[track]
            and toon.getGameAccess() != OTPGlobals.AccessFull
        ):
            newValue = oldValue
    ...
    def resetBarColor(self, track):
        self.trackBars[track]["barColor"] = (
            ToontownBattleGlobals.TrackColors[track][0] * 0.8,
    ...
    def incrementMerits(self, toon, dept, newValue, totalMerits):
        ...
        if totalMerits:
            ...
            if newValue == totalMerits:
                meritBar["text"] = TTLocalizer.RewardPanelMeritAlert
                meritBar["barColor"] = (
                    DisguisePage.DeptColors[dept][0],
    ...
    def resetMeritBarColor(self, dept):
        self.meritBars[dept]["barColor"] = (
            DisguisePage.DeptColors[dept][0] * 0.8,
    ...
    def endTrack(self, toon, toonList, track):
        ...
        gagLast = base.localAvatar.inventory.buttonLookup(
            track, ToontownBattleGlobals.UBER_GAG_LEVEL_INDEX
        )
    ...
    def getTrackIntervalList(
        self, toon, track, origSkill, earnedSkill, hasUber, guestWaste=0
    ):
        if hasUber < 0:
            print(
                (toon.doId, "Reward Panel received an invalid hasUber from an uberList")
            )
        tickDelay = 1.0 / 60
        intervalList = []
        if (
            origSkill + earnedSkill >= ToontownBattleGlobals.UnpaidMaxSkills[track]
            and toon.getGameAccess() != OTPGlobals.AccessFull
        ):
            lostExp = (
                origSkill + earnedSkill - ToontownBattleGlobals.UnpaidMaxSkills[track]
        ...
        finalGagFlag = 0
        while (
            origSkill + earnedSkill >= nextExpValue
            and origSkill < nextExpValue
        ...
        uberSkill = (
            ToontownBattleGlobals.UberSkill
            + ToontownBattleGlobals.Levels[track][
        ...
        if currentSkill >= uberSkill and not hasUber > 0:
            ...
            intervalList.append(
                Func(self.showTrackIncLabel, track, -displayedSkillDiff)
            )
            for i in range(numTicks):
                t = (i + 1) / float(numTicks)
    ...
    def getMeritIntervalList(self, toon, dept, origMerits, earnedMerits):
        ...
        if totalMerits and origMerits != totalMerits:
            ...
            intervalList.append(
                Func(self.showMeritIncLabel, dept, min(neededMerits, earnedMerits))
            )
        barTime = 0.5
        numTicks = int(math.ceil(barTime / tickDelay))
        for i in range(numTicks):
    ...
    def promotion(self, toon, dept):
        ...
        self.promotionFrame["text"] = (
            TTLocalizer.RewardPanelPromotion % SuitDNA.suitDeptFullnames[name]
        )
            ...
            if quest and i < len(self.questLabelList):
                ...
                if quest.getType() == Quests.RecoverItemQuest:
                    ...
                    for cogDict in cogList:
                        if cogDict["isVP"]:
                            num = quest.doesVPCount(
                                avId, cogDict, zoneId, toonShortList
                            )
                        elif cogDict["isCFO"]:
                            num = quest.doesCFOCount(
                                avId, cogDict, zoneId, toonShortList
                            )
                        else:
                            num = quest.doesCogCount(
                                avId, cogDict, zoneId, toonShortList
                            )
                        if num:
                ...
                if earned > 0 or base.localAvatar.tutorialAck == 0 and num == 1:
                    ...
                    for i in range(numTicks):
                        ...
                        if (
                            ...
                            intervalList.append(
                                Func(questLabel.setProp, "text_fg", (0, 0.3, 0, 1))
                            )
    ...
    def getExpTrack(
        ...
        track = Sequence(
            Func(self.initGagFrame, toon, origExp, origMeritList, noSkip=noSkip),
            Wait(1.0),
        )
        ...
        for trackIndex in range(len(earnedExp)):
            if (
                earnedExp[trackIndex] > 0
                or origExp[trackIndex] >= ToontownBattleGlobals.MaxSkill
            ):
                ...
                maxExp = (
                    ToontownBattleGlobals.MaxSkill - ToontownBattleGlobals.UberSkill
                )
                if (
                    origExp[trackIndex] < maxExp
                    and earnedExp[trackIndex] + origExp[trackIndex] >= maxExp
                ):
        ...
        for dept in range(len(SuitDNA.suitDepts)):
            if meritList[dept]:
                track += self.getMeritIntervalList(
                    toon, dept, origMeritList[dept], meritList[dept]
                )
        ...
        questList = self.getQuestIntervalList(
            toon, deathList, toonList, origQuestsList, itemList, helpfulToonsList
        )
        if questList:
            avQuests = []
            for i in range(0, len(origQuestsList), 5):
