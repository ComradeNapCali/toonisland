...
def createTrainTrackAppearTrack(dyingSuit, toon, battle, npcs):
    ...
    if closestSuit and closestSuit.battleTrapProp.isHidden():
        ...
        retval.append(
            LerpColorScaleInterval(closestSuit.battleTrapProp, 3.0, Vec4(1, 1, 1, 1))
        )
    else:
...
def createSuitReviveTrack(suit, toon, battle, npcs=[]):
    ...
    deathSoundTrack = Sequence(
        Wait(0.8),
        SoundInterval(
            spinningSound, duration=1.2, startTime=1.5, volume=0.2, node=suit
        ),
        SoundInterval(
            spinningSound, duration=3.0, startTime=0.6, volume=0.8, node=suit
        ),
    ...
    gearPoint = Point3(
        suitPos.getX(), suitPos.getY(), suitPos.getZ() + suit.height - 0.2
    )
    ...
    gears1Track = Sequence(
        Wait(2.1),
        ParticleInterval(
            smallGears, battle, worldRelative=0, duration=4.3, cleanup=True
        ),
        name="gears1Track",
    )
    gears2MTrack = Track(
        ...
        (
            0.7,
            ParticleInterval(
                singleGear, battle, worldRelative=0, duration=5.7, cleanup=True
            ),
        ),
        (
            5.2,
            ParticleInterval(
                smallGearExplosion, battle, worldRelative=0, duration=1.2, cleanup=True
            ),
        ),
        (
            5.4,
            ParticleInterval(
                bigGearExplosion, battle, worldRelative=0, duration=1.0, cleanup=True
            ),
        ),
        name="gears2MTrack",
    )
...
def createSuitDeathTrack(suit, toon, battle, npcs=[]):
    ...
    deathSoundTrack = Sequence(
        Wait(0.8),
        SoundInterval(
            spinningSound, duration=1.2, startTime=1.5, volume=0.2, node=deathSuit
        ),
        SoundInterval(
            spinningSound, duration=3.0, startTime=0.6, volume=0.8, node=deathSuit
        ),
...
def createSuitDodgeMultitrack(tDodge, suit, leftSuits, rightSuits):
    suitTracks = Parallel()
    suitDodgeList, sidestepAnim = avatarDodge(
        leftSuits, rightSuits, "sidestep-left", "sidestep-right"
    )
    for s in suitDodgeList:
        suitTracks.append(
            Sequence(ActorInterval(s, sidestepAnim), Func(s.loop, "neutral"))
        )

    suitTracks.append(
        Sequence(ActorInterval(suit, sidestepAnim), Func(suit.loop, "neutral"))
    )
...
def getSprayTrack(
    ...
    track.append(
        Func(showSpray, sprayScale, sprayRot, sprayProp, origin, target, parent)
    )

    def calcTargetScale(
        target=target, origin=origin, horizScale=horizScale, vertScale=vertScale
    ):
        if callable(target):
            target = target()
        if callable(origin):
    ...
    track.append(
        LerpScaleInterval(
            sprayScale, dScaleUp, calcTargetScale, startScale=PNT3_NEARZERO
        )
    )
...
def getToonTeleportInInterval(toon):
    ...
    holeAnimTrack.append(
        ActorInterval(
            hole, "hole", startTime=T_TELEPORT_ANIM, endTime=T_HOLE_LEAVES_HAND
        )
    )
    holeAnimTrack.append(
        ActorInterval(
            hole, "hole", startTime=T_HOLE_LEAVES_HAND, endTime=T_TELEPORT_ANIM
        )
    )
    holeAnimTrack.append(Func(hole.reparentTo, hidden))
    delay = T_TELEPORT_ANIM - T_HOLE_LEAVES_HAND
    jumpTrack = Sequence(
        Wait(delay), Func(toon.reparentTo, render), ActorInterval(toon, "jump")
    )
