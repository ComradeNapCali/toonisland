    ...
    if sidestep == 0:
        if kbbonus == 1 or hp > 0:
            ...
            if suitName in MovieUtil.largeSuits:
                moveTrack = lerpSuit(
                    suit, 0.0, reachAnimDuration / 2.5, retardPos, battle, trapProp
                )
...
def __createMagnetMultiTrack(lure, magnet, pos, hpr, scale, isSmallMagnet=1, npcs=[]):
    ...
    tracks = Parallel()
    tracks.append(
        Sequence(ActorInterval(toon, "hold-magnet"), Func(toon.loop, "neutral"))
    )
    ...
    magnetTrack = Sequence(
        Wait(0.7),
        Func(MovieUtil.showProps, magnets, hands, pos, hpr, scale),
        Wait(6.3),
        ...
        if sidestep == 0:
            ...
            if kbbonus == 1 or hp > 0:
                ...
                damageTrack = Sequence(
                    Wait(0.5), Func(suit.showHpText, AvLureRounds[3], openEnded=0)
                )
                suitTrack.append(
                    ActorInterval(suit, "landing", startTime=2.37, endTime=1.82)
                )
                ...
                suitTrack.append(
                    ActorInterval(suit, "landing", startTime=0.7, duration=1.3)
                )
                suitTrack.append(Func(suit.loop, "neutral"))
                suitTrack.append(Func(battle.lureSuit, suit))
                if hp > 0:
                ...
                tracks.append(
                    lerpSuit(
                        suit,
                        suitDelay + 0.55 + shakeTotalDuration,
                        suitMoveDuration,
                        reachPos,
                        battle,
                        trapProp,
    ...
    for target in targets:
        ...
        if sidestep == 0:
            ...
            if kbbonus == 1 or hp > 0:
                ...
                tracks.append(suitTrack)
                tracks.append(
                    lerpSuit(suit, suitDelay + 1.7, 0.7, reachPos, battle, trapProp)
                )
        else:
            tracks.append(
...
def __lureOneDollar(lure):
    ...
    return __createFishingPoleMultiTrack(lure, dollar, dollarProp, dollarType=1)
...
def __lureSmallMagnet(lure, npcs=[]):
    ...
    return __createMagnetMultiTrack(
        lure, magnet, pos, hpr, scale, isSmallMagnet=1, npcs=npcs
    )
...
def __lureFiveDollar(lure):
    dollarProp = "5dollar"
    dollar = globalPropPool.getProp(dollarProp)
    return __createFishingPoleMultiTrack(lure, dollar, dollarProp, dollarType=5)
...
def __lureLargeMagnet(lure, npcs=[]):
    ...
    return __createMagnetMultiTrack(
        lure, magnet, pos, hpr, scale, isSmallMagnet=0, npcs=npcs
    )
...
def __lureTenDollar(lure):
    dollarProp = "10dollar"
    dollar = globalPropPool.getProp(dollarProp)
    return __createFishingPoleMultiTrack(lure, dollar, dollarProp, dollarType=10)
...
def __createSuitDamageTrack(battle, suit, hp, lure, trapProp):
    ...
    if trapName == "banana":
        ...
        trapTrack = Sequence(
            Wait(2.4), LerpScaleInterval(trapProp, 0.8, Point3(0.01, 0.01, 0.01))
        )
        moveTrack = Sequence(
            Wait(0.9),
        ...
        soundTrack = Sequence(
            Wait(0.7),
            SoundInterval(
                globalBattleSoundCache.getSound("TL_quicksand.ogg"), node=suit
            ),
            Wait(0.1),
            SoundInterval(
                globalBattleSoundCache.getSound("Toon_bodyfall_synergy.ogg"), node=suit
        ...
        soundTrack = Sequence(
            Wait(0.8),
            SoundInterval(
                globalBattleSoundCache.getSound("TL_trap_door.ogg"), node=suit
            ),
            Wait(0.8),
            SoundInterval(
                globalBattleSoundCache.getSound("Toon_bodyfall_synergy.ogg"), node=suit
        ...
        explosionTrack = Sequence(
            Wait(2.3),
            createTNTExplosionTrack(battle, trapProp=trapProp, relativeTo=parent),
        )
        suitTrack = Sequence(
        ...
        explosionSound = base.loader.loadSfx(
        ...
        result.append(
            Parallel(tntTrack, suitTrack, damageTrack, explosionTrack, soundTrack)
        )
...
def getSplicedLerpAnimsTrack(
    object, animName, origDuration, newDuration, startTime=0, fps=30
):
    track = Sequence()
    addition = 0
    ...
    for i in range(0, int(numIvals)):
        ...
        track.append(
            ActorInterval(
                object, animName, startTime=startTime + addition, duration=animInterval
            )
        )
...
def createTNTExplosionTrack(
    parent, explosionPoint=None, trapProp=None, relativeTo=render
):
    explosionTrack = Sequence()
...
def createIncomingTrainInterval(battle, suit, hp, lure, trapProp):
    ...
    materializeIval.append(
        LerpColorScaleInterval(train, TRAIN_MATERIALIZE_TIME, Point4(1, 1, 1, 1))
    )
    for tunnel in tunnels:
        materializeIval.append(
            LerpColorScaleInterval(tunnel, TRAIN_MATERIALIZE_TIME, Point4(1, 1, 1, 1))
        )
    ...
    for tunnel in tunnels:
        ...
        tunnelScaleIval.append(
            LerpScaleInterval(
                tunnel, TRAIN_MATERIALIZE_TIME - 1.0, Point3(1.0, 2.0, 2.5)
            )
        )
    ...
    trainIval.append(
        LerpPosInterval(train, TRAIN_DURATION, Point3(endingX, 0, 0), other=battle)
    )
    trainIval.append(
        LerpColorScaleInterval(train, TRAIN_MATERIALIZE_TIME, Point4(1, 1, 1, 0))
    )
...
def __createSlideshowMultiTrack(lure, npcs=[]):
    ...
    propTrack.append(
        LerpScaleInterval(
            nodePath=slideShowProp, scale=Point3(1.0, 1.0, 0.1), duration=shrinkDuration
        )
    )
    ...
    tracks.append(
        getSoundTrack("AA_drop_trigger_box.ogg", delay=slideshowDelay, node=toon)
    )
    return tracks
