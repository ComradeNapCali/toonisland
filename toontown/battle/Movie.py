...
class Movie(DirectObject.DirectObject):
    ...
    def playTutorialReward_1(self):
        ...
        self.track += self.rewardPanel.getTrackIntervalList(
            base.localAvatar, THROW_TRACK, 0, 1, 0
        )
    ...
    def playTutorialReward_2(self, value):
        ...
        self.track += self.rewardPanel.getTrackIntervalList(
            base.localAvatar, SQUIRT_TRACK, 0, 1, 0
        )
    ...
    def playTutorialReward_3(self, value):
        ...
        if base.config.GetString("language", "english") == "japanese":
            ...
            self.tomDialogue04 = base.loader.loadSfx(
                "phase_3.5/audio/dial/CC_tom_movie_tutorial_reward02.ogg"
            )
            self.tomDialogue05 = base.loader.loadSfx(
                "phase_3.5/audio/dial/CC_tom_movie_tutorial_reward03.ogg"
            )
        ...
        if self.questList:
            self.track1 = Sequence(
                Wait(1.0),
                Func(
                    self.rewardPanel.initQuestFrame,
                    base.localAvatar,
                ...
                Func(base.localAvatar.setPosHpr, 40.31, 22.0, -0.47, 150.0, 360.0, 0.0),
                Wait(0.5),
                ...
                Func(
                    self.tutorialTom.setLocalPageChat,
                    TTLocalizer.MovieTutorialReward3,
                    0,
                    None,
                    [self.tomDialogue03],
                ),
            ...
            self.track2 = Sequence(
                ...
                Func(
                    self.tutorialTom.setLocalPageChat,
                    TTLocalizer.MovieTutorialReward4,
                    1,
                    None,
                    [self.tomDialogue04],
                ),
            ...
            self.track3 = Parallel(
                Sequence(
                    Func(self.tutorialTom.setPlayRate, -1.8, "right-hand-start"),
                    Func(self.tutorialTom.play, "right-hand-start"),
                ...
                Sequence(
                    Wait(0.5),
                    Func(
                        self.tutorialTom.setChatAbsolute,
        ...
        for ta in toonAttacks:
            ...
            if track != NO_ATTACK:
                ...
                if track == SOS:
                    ...
                    track == NPCSOS
                    or track == NPC_COGS_MISS
                    or track == NPC_TOONS_HIT
                    or track == NPC_RESTOCK_GAGS
                    or track == PETSOS
                ):
                    adict["special"] = 1
                    toonHandles = []
                    ...
                    for s in suits:
                        if s != -1:
                            ...
                            if ta[TOON_TRACK_COL] == NPCSOS:
                                if (
                                    track == LURE
                                    and self.battle.isSuitLured(target) == 1
                                ):
                                    continue
                                elif track == TRAP and (
            ...
            for a in self.suitAttackDicts:
                ...
                if a["group"] == ATK_TGT_GROUP:
                    for target in targetField:
                        if (
                            target["died"]
                            and target["toon"].doId == base.localAvatar.doId
                        ):
                            isLocalToonSad = True
                    ...
                    if (
                        targetField["died"]
                        and targetField["toon"].doId == base.localAvatar.doId
                    ):
                        isLocalToonSad = True
                if isLocalToonSad:
                    break
