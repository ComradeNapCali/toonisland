...
def doSuitAttack(attack):
    ...
    if groupStatus == ATK_TGT_SINGLE:
        ...
        toonHprTrack = Sequence(
            Func(toon.headsUp, battle, MovieUtil.PNT3_ZERO), Func(toon.loop, "neutral")
        )
    else:
    ...
    if battle.isSuitLured(suit):
        ...
        waitTrack = Sequence(
            Wait(resetTrack.getDuration()), Func(battle.unlureSuit, suit)
        )
...
def getSuitTrack(attack, delay=1e-06, splicedAnims=None):
    ...
    track = Sequence(
        Wait(delay), Func(suit.setChatAbsolute, taunt, CFSpeech | CFTimeout)
    )
...
def getToonDodgeTrack(
    target, dodgeDelay, dodgeAnimNames, splicedDodgeAnims, showMissedExtraTime
):
    toon = target["toon"]
    toonTrack = Sequence()
...
def getAllyToonsDodgeParallel(target):
    ...
    for t in toonDodgeList:
        toonTracks.append(
            Sequence(ActorInterval(t, sidestepAnim), Func(t.loop, "neutral"))
        )

    toonTracks.append(
        Sequence(ActorInterval(toon, sidestepAnim), Func(toon.loop, "neutral"))
    )
    ...
    def getLambdas(list, prop, toon):
        for i in range(len(list)):
            if list[i] == "face":
                ...
                list[i] = lambda prop=prop, toon=toon: __throwBounceMissPoint(
                    prop, toon
                )
...
def getToonTakeDamageTrack(
    ...
    if damageAnimNames:
        ...
        indicatorTrack = Sequence(
            Wait(delay + showDamageExtraTime), Func(__doDamage, toon, dmg, died)
        )
    else:
    ...
    for nextAnim in anims:
        ...
        if len(nextAnim) <= 0:
            ...
            if duration < 0:
                ...
                track.append(
                    ActorInterval(
                        actor, nextAnim[0], startTime=startTime, endTime=endTime
                    )
                )
            else:
                track.append(
                    ActorInterval(
                        actor, nextAnim[0], startTime=nextAnim[2], duration=duration
                    )
                )
        elif len(nextAnim) == 5:
            track.append(Wait(delay))
...
def getSplicedLerpAnims(
    animName, origDuration, newDuration, startTime=0, fps=30, reverse=0
):
    anims = []
    addition = 0
...
def doPoundKey(attack):
    ...
    propTrack = Sequence(
        Wait(0.3),
        Func(
            __showProp, phone, suit.getLeftHand(), phonePosPoints[0], phonePosPoints[1]
        ),
        Func(
            __showProp,
            receiver,
        ...
        LerpPosHprInterval(
            receiver, 0.0001, Point3(-0.45, 0.48, -0.62), VBase3(-87.47, -18.21, 7.82)
        ),
        Wait(3.14),
...
def doShred(attack):
    ...
    shredderPropTrack = getPropTrack(
        shredder,
        suit.getLeftHand(),
        shredderPosPoints,
        1,
    ...
    toonTrack = getToonTrack(
        attack,
        suitTrack.getDuration() - 1.1,
        ["conked"],
        suitTrack.getDuration() - 3.1,
        ["sidestep"],
    )
    soundTrack = getSoundTrack("SA_shred.ogg", delay=3.4, node=suit)
    return Parallel(
        suitTrack, paperPropTrack, shredderPropTrack, partTrack, toonTrack, soundTrack
    )
...
def doFillWithLead(attack):
    ...
    BattleParticles.setEffectTexture(
        headSmotherEffect, "roll-o-dex", color=Vec4(0, 0, 0, 1)
    )
    BattleParticles.setEffectTexture(
        torsoSmotherEffect, "roll-o-dex", color=Vec4(0, 0, 0, 1)
    )
    BattleParticles.setEffectTexture(
        legsSmotherEffect, "roll-o-dex", color=Vec4(0, 0, 0, 1)
    )
    suitTrack = getSuitTrack(attack)
    ...
    pencilPropTrack = getPropTrack(
        pencil, suit.getRightHand(), pencilPosPoints, 0.7, 3.2, scaleUpTime=0.2
    )
    sharpenerPosPoints = [Point3(0.0, 0.0, -0.03), MovieUtil.PNT3_ZERO]
    sharpenerPropTrack = getPropTrack(
        sharpener,
        suit.getLeftHand(),
        sharpenerPosPoints,
        1.3,
    ...
    toonTrack = getToonTrack(
        ...
        dodgeDelay=suitTrack.getDuration() - 3.1,
        dodgeAnimNames=["sidestep"],
        showDamageExtraTime=4.5,
    ...
    torsoEffectHeight = (
        ToontownGlobals.torsoHeightDict[toon.style.torso] * bodyScale / 2 + legsHeight
    )
    ...
    partIvalDelay = 0.7
    partDuration = 1.0
    headTrack = getPartTrack(
        headSmotherEffect, partDelay, partDuration, [headSmotherEffect, toon, 0]
    )
    torsoTrack = getPartTrack(
        torsoSmotherEffect,
        partDelay + partIvalDelay,
        partDuration,
        [torsoSmotherEffect, toon, 0],
    )
    legsTrack = getPartTrack(
        legsSmotherEffect,
        partDelay + partIvalDelay * 2,
        partDuration,
...
def doFountainPen(attack):
    ...
    missPoint = lambda prop=pen, toon=toon: __toonMissPoint(
...
def doRubOut(attack):
    ...
    pencilPropTrack = getPropTrack(
        pencil, suit.getRightHand(), pencilPosPoints, 0.5, 2.57
    )
    ...
    def hideParts(parts):
        ...
        for partNum in range(0, parts.getNumPaths()):
            ...
            track.append(
                LerpFunctionInterval(
                    nextPart.setAlphaScale, fromData=1, toData=0, duration=0.2
                )
            )
...
def doFingerWag(attack):
    ...
    toonTrack = getToonTrack(
        attack, damageDelay, ["slip-backward"], dodgeDelay, ["sidestep"]
    )
...
def doWriteOff(attack):
    ...
    padPropTrack = getPropTrack(
        pad, suit.getLeftHand(), padPosPoints, 0.5, 2.57, Point3(1.89, 1.89, 1.89)
    )
    ...
    soundTrack = Sequence(
        ...
        SoundInterval(
            globalBattleSoundCache.getSound("SA_writeoff_ding_only.ogg"), node=suit
        ),
    )
    return Parallel(suitTrack, toonTrack, padPropTrack, pencilPropTrack, soundTrack)
...
def doRubberStamp(attack):
    ...
    propTrack = Sequence(
        Func(
            __showProp, stamp, suit.getRightHand(), stampPosPoints[0], stampPosPoints[1]
        ),
    ...
    soundTrack = getSoundTrack(
        "SA_rubber_stamp.ogg", delay=1.3, duration=1.1, node=suit
    )
...
def doRazzleDazzle(attack):
    ...
    if hitSuit:
        ...
        hitPoint = (
            lambda particleEffect=particleEffect, toon=toon, suit=suit: __toonMissPoint(
                particleEffect, toon, parent=suit.getRightHand()
            )
        )
...
def doSynergy(attack):
    ...
    synergySoundTrack = Sequence(
        Wait(0.9),
        SoundInterval(globalBattleSoundCache.getSound("SA_synergy.ogg"), node=suit),
    )
    if hitAtleastOneToon > 0:
        fallingSoundTrack = Sequence(
            Wait(damageDelay + 0.5),
            SoundInterval(
                globalBattleSoundCache.getSound("Toon_bodyfall_synergy.ogg"), node=suit
            ),
        )
        return Parallel(
            suitTrack,
...
def doTeeOff(attack):
    ...
    clubPropTrack = getPropTrack(
        club, suit.getLeftHand(), clubPosPoints, 0.5, 5.2, Point3(1.1, 1.1, 1.1)
    )
    suitName = attack["suitName"]
    if suitName == "ym":
...
def doBuzzWord(attack):
    ...
    for i in range(0, 5):
        ...
        if random.random() > 0.5:
            BattleParticles.setEffectTexture(
                effect, texturesList[i], color=Vec4(1, 0.94, 0.02, 1)
            )
        else:
            BattleParticles.setEffectTexture(
                effect, texturesList[i], color=Vec4(0, 0, 0, 1)
            )
    ...
    for effect in particleEffects:
        particleTracks.append(
            getPartTrack(effect, partDelay, partDuration, [effect, suit, 0])
        )
...
def doCanned(attack):
    ...
    throwTrack.append(
        getThrowTrack(can, hitPoint, duration=throwDuration, parent=battle)
    )
    if dmg > 0:
        ...
        soundTrack = Sequence(
            Wait(2.6),
            SoundInterval(
                globalBattleSoundCache.getSound("SA_canned_tossup_only.ogg"), node=suit
            ),
            SoundInterval(
                globalBattleSoundCache.getSound("SA_canned_impact_only.ogg"), node=suit
            ),
        )
    else:
    ...
    damageAnims = [
        ["struggle", propDelay + suitDelay + throwDuration, 0.01, 0.7],
...
def doPinkSlip(attack):
    ...
    paperAppearTrack = Sequence(
        getPropAppearTrack(
            paper, suit.getRightHand(), posPoints, 0.8, Point3(8, 8, 8), scaleUpTime=0.5
        )
    )
...
def doSacked(attack):
    ...
    sackAppearTrack.append(
        getThrowTrack(sack, hitPoint, duration=throwDuration, parent=battle)
    )
    if dmg > 0:
        ...
        hprTrack = Sequence(
            Wait(propDelay + suitDelay), LerpHprInterval(sack, throwDuration, sackHpr)
        )
        sackTrack = Sequence(
...
def doHalfWindsor(attack):
    ...
    tiePropTrack = getPropAppearTrack(
        tie, suit.getRightHand(), posPoints, 0.5, Point3(7, 7, 7), scaleUpTime=0.5
    )
    tiePropTrack.append(Wait(throwDelay))
...
def doHeadShrink(attack):
    ...
    cloudTrack.append(
        LerpFunctionInterval(
            shrinkCloud.setAlphaScale, fromData=1, toData=0, duration=0.7
        )
    )
...
def doRolodex(attack):
    ...
    partTrack2 = getPartTrack(
        particleEffect2, part2Delay, part2Duration, [particleEffect2, suit, 0]
    )
    partTrack3 = getPartTrack(
        particleEffect3, part3Delay, part3Duration, [particleEffect3, suit, 0]
    )
    suitTrack = getSuitTrack(attack)
    propTrack = getPropTrack(
        rollodex,
        suit.getLeftHand(),
        propPosPoints,
        1e-06,
...
def doEvilEye(attack):
    ...
    suitSplicedAnims.extend(
        getSplicedLerpAnims("glower", suitHoldDuration, 1.1, startTime=suitHoldStart)
    )
...
def doPlayHardball(attack):
    ...
    if dmg > 0:
        propTrack.append(
            LerpPosInterval(ball, 0.5, __toonFacePoint(toon, parent=battle))
        )
...
def doDoubleTalk(attack):
    ...
    partTrack2 = getPartTrack(
        particleEffect2, partDelay, 1.8, [particleEffect2, suit, 0]
    )
...
def doFreezeAssets(attack):
    ...
    cloudPropTrack.append(Wait(partDelay))
    cloudPropTrack.append(
        ParticleInterval(snowEffect, cloud, worldRelative=0, duration=2.1, cleanup=True)
    )
    cloudPropTrack.append(Wait(0.4))
    cloudPropTrack.append(LerpScaleInterval(cloud, 0.5, MovieUtil.PNT3_NEARZERO))
    cloudPropTrack.append(Func(MovieUtil.removeProp, cloud))
...
def doHotAir(attack):
    ...
    baseFlameTrack = getPartTrack(
        baseFlameEffect, flameDelay, flameDuration, [baseFlameEffect, toon, 0]
    )
    flameTrack = getPartTrack(
        flameEffect, flameDelay, flameDuration, [flameEffect, toon, 0]
    )
    flecksTrack = getPartTrack(
...
def doFilibuster(attack):
    ...
    dodgeDelay = 1.7
    suitTrack = getSuitTrack(attack)
    sprayTrack = getPartTrack(
        sprayEffect, partDelay, partDuration, [sprayEffect, suit, 0]
    )
    sprayTrack2 = getPartTrack(
        sprayEffect2, partDelay + 0.8, partDuration, [sprayEffect2, suit, 0]
    )
    sprayTrack3 = getPartTrack(
        sprayEffect3, partDelay + 1.6, partDuration, [sprayEffect3, suit, 0]
    )
    sprayTrack4 = getPartTrack(
        sprayEffect4, partDelay + 2.4, partDuration, [sprayEffect4, suit, 0]
    )
    damageAnims = []
    for i in range(0, 4):
...
def doSchmooze(attack):
    ...
    for i in range(0, 4):
        ...
        BattleParticles.setEffectTexture(
            lowerEffect, textureNames[i], color=Vec4(0, 0, 1, 1)
        )
    ...
    for i in range(0, 4):
        upperPartTracks.append(
            getPartTrack(
                upperEffects[i], partDelay + i * 0.65, 0.8, [upperEffects[i], suit, 0]
            )
        )
        lowerPartTracks.append(
            getPartTrack(
                lowerEffects[i],
                partDelay + i * 0.65 + 0.7,
                1.0,
                [lowerEffects[i], suit, 0],
            )
        )
...
def doHangUp(attack):
    ...
    if suitName == "tf":
        ...
        receiverPosPoints = [
            Point3(-0.13, -0.07, -0.06),
            VBase3(-1.854, 2.434, -177.579),
        ]
    ...
    propTrack = Sequence(
        ...
        LerpPosHprInterval(
            receiver, 0.0001, Point3(-0.53, 0.21, -0.54), VBase3(-99.49, -35.27, 1.84)
        ),
        Wait(dialDuration),
        Func(receiver.wrtReparentTo, phone),
...
def doRedTape(attack):
    ...
    propTrack.append(
        getPropThrowTrack(
            attack, tape, [hitPoint], [__toonGroundPoint(attack, toon, 0.7)]
        )
    )
    hips = toon.getHipsParts()
    animal = toon.style.getAnimal()
...
def doParadigmShift(attack):
    ...
    for t in targets:
        ...
        if dmg > 0:
            ...
            liftTracks.append(
                getPartTrack(liftEffect, 1.1, 4.1, [liftEffect, battle, 0])
            )
    ...
    if hitAtleastOneToon == 1:
        ...
        return Parallel(
            suitTrack, sprayTrack, soundTrack, liftTracks, toonTracks, toonRiseTracks
        )
    else:
...
def doBounceCheck(attack):
    ...
    bounce1Point = lambda suit=suit, toon=toon, battle=battle: getThrowEndPoint(
        suit, toon, battle, "one"
    )
    bounce2Point = lambda suit=suit, toon=toon, battle=battle: getThrowEndPoint(
        suit, toon, battle, "two"
    )
    hit3Point = lambda suit=suit, toon=toon, battle=battle: getThrowEndPoint(
        suit, toon, battle, "threeHit"
    )
    miss3Point = lambda suit=suit, toon=toon, battle=battle: getThrowEndPoint(
        suit, toon, battle, "threeMiss"
    )
    bounce4Point = lambda suit=suit, toon=toon, battle=battle: getThrowEndPoint(
        suit, toon, battle, "four"
    )
    suitType = getSuitBodyType(attack["suitName"])
    if suitType == "a":
        throwDelay = 2.5
    ...
    if hitSuit:
        checkPropTrack.append(
            getThrowTrack(check, hit3Point, duration=0.7, parent=toon)
        )
    else:
        checkPropTrack.append(
            getThrowTrack(check, miss3Point, duration=0.7, parent=toon)
        )
        checkPropTrack.append(
            getThrowTrack(check, bounce4Point, duration=0.7, parent=toon)
        )
    ...
    soundTracks = Sequence(
        getSoundTrack(
            "SA_pink_slip.ogg", delay=throwDelay + 0.5, duration=0.6, node=suit
        ),
...
def doWatercooler(attack):
    ...
    hitPoint = lambda toon=toon: __toonFacePoint(toon)
    missPoint = lambda prop=watercooler, toon=toon: __toonMissPoint(
        prop, toon, 0, parent=render
    )
    hitSprayTrack = MovieUtil.getSprayTrack(
        battle,
    ...
    toonTrack = getToonTrack(
        attack, suitTrack.getDuration() - 1.5, ["cringe"], 2.4, ["sidestep"]
    )
    soundTrack = Sequence(
        Wait(1.1),
        SoundInterval(
...
def doFired(attack):
    ...
    baseFlameSmallTrack = getPartTrack(
...
def doAudit(attack):
    ...
    toonTrack = getToonTrack(
        attack, 3.2, ["conked"], 0.9, ["duck"], showMissedExtraTime=2.2
    )
...
def doCalculate(attack):
    ...
    particleEffect2 = BattleParticles.createParticleEffect("Calculate")
    BattleParticles.setEffectTexture(
        particleEffect2, "audit-plus", color=Vec4(0, 0, 0, 1)
    )
    particleEffect3 = BattleParticles.createParticleEffect("Calculate")
    BattleParticles.setEffectTexture(
        particleEffect3, "audit-mult", color=Vec4(0, 0, 0, 1)
    )
    particleEffect4 = BattleParticles.createParticleEffect("Calculate")
    BattleParticles.setEffectTexture(
        particleEffect4, "audit-three", color=Vec4(0, 0, 0, 1)
    )
    particleEffect5 = BattleParticles.createParticleEffect("Calculate")
    BattleParticles.setEffectTexture(
        particleEffect5, "audit-div", color=Vec4(0, 0, 0, 1)
    )
    suitTrack = getSuitTrack(attack)
    partTrack = getPartTrack(particleEffect, 2.1, 1.9, [particleEffect, suit, 0])
    partTrack2 = getPartTrack(particleEffect2, 2.2, 2.0, [particleEffect2, suit, 0])
...
def doTabulate(attack):
    ...
    particleEffect = BattleParticles.createParticleEffect("Calculate")
    BattleParticles.setEffectTexture(
        particleEffect, "audit-plus", color=Vec4(0, 0, 0, 1)
    )
    particleEffect2 = BattleParticles.createParticleEffect("Calculate")
    BattleParticles.setEffectTexture(
        particleEffect2, "audit-minus", color=Vec4(0, 0, 0, 1)
    )
    particleEffect3 = BattleParticles.createParticleEffect("Calculate")
    BattleParticles.setEffectTexture(
        particleEffect3, "audit-mult", color=Vec4(0, 0, 0, 1)
    ...
    particleEffect4 = BattleParticles.createParticleEffect("Calculate")
    BattleParticles.setEffectTexture(
        particleEffect4, "audit-div", color=Vec4(0, 0, 0, 1)
    )
    particleEffect5 = BattleParticles.createParticleEffect("Calculate")
    BattleParticles.setEffectTexture(
        particleEffect5, "audit-one", color=Vec4(0, 0, 0, 1)
    )
    suitTrack = getSuitTrack(attack)
    partTrack = getPartTrack(particleEffect, 2.1, 1.9, [particleEffect, suit, 0])
    partTrack2 = getPartTrack(particleEffect2, 2.2, 2.0, [particleEffect2, suit, 0])
...
def doCrunch(attack):
    ...
    for i in range(0, numOfNumbers - 1):
        ...
        nextSprayTrack = getPartTrack(
            nextSpray, nextStartTime, nextDuration, [nextSpray, suit, 0]
        )
...
def doLiquidate(attack):
    ...
    cloudPropTrack.append(
        Parallel(
            Sequence(
                ParticleInterval(
                    rainEffect, cloud, worldRelative=0, duration=2.1, cleanup=True
                )
            ),
            Sequence(
                Wait(0.1),
                ParticleInterval(
                    rainEffect2, cloud, worldRelative=0, duration=2.0, cleanup=True
                ),
            ),
            Sequence(
                Wait(0.1),
                ParticleInterval(
                    rainEffect3, cloud, worldRelative=0, duration=2.0, cleanup=True
                ),
            ),
            Sequence(
    ...
    if dmg > 0:
        ...
        puddleTrack = Sequence(
            ...
            LerpScaleInterval(
                puddle, 1.7, Point3(1.7, 1.7, 1.7), startScale=MovieUtil.PNT3_NEARZERO
            ),
            Wait(3.2),
            LerpFunctionInterval(
...
def doMarketCrash(attack):
    ...
    paperTrack.append(
        getThrowTrack(paper, hitPoint, duration=throwDuration, parent=battle)
    )
...
def doChomp(attack):
    ...
    if dmg > 0:
        ...
        teethAppearTrack.append(
            LerpPosInterval(teeth, 0.1, pos=Point3(x, y - 0.7, toonHeight * 0.4))
        )
        teethAppearTrack.append(Wait(0.4))
        scaleTrack = Sequence(
            Wait(throwDelay),
...
def doEvictionNotice(attack):
    ...
    missPoint.setX(missPoint.getX() - 1.1)
    propTrack.append(
        getPropThrowTrack(attack, paper, [hitPoint], [missPoint], parent=battle)
    )
...
def doWithdrawal(attack):
    ...
    partTrack = getPartTrack(
        particleEffect, 1e-05, suitTrack.getDuration() + 1.2, [particleEffect, suit, 0]
    )
    toonTrack = getToonTrack(
        attack,
        1.2,
    ...
    if dmg > 0:
        ...
        colorTrack.append(
            Parallel(
                changeColor(headParts), changeColor(torsoParts), changeColor(legsParts)
            )
        )
...
def doJargon(attack):
    ...
    suitTrack = getSuitTrack(attack)
    partTrack = getPartTrack(
        particleEffect, partDelay + partInterval * 0, 2, [particleEffect, suit, 0]
    )
    partTrack2 = getPartTrack(
        particleEffect2, partDelay + partInterval * 1, 2, [particleEffect2, suit, 0]
    )
    partTrack3 = getPartTrack(
        particleEffect3, partDelay + partInterval * 2, 2, [particleEffect3, suit, 0]
    )
    partTrack4 = getPartTrack(
        particleEffect4, partDelay + partInterval * 3, 1.0, [particleEffect4, suit, 0]
    )
    damageAnims = []
    ...
    toonTrack = getToonTrack(
        ...
        splicedDodgeAnims=dodgeAnims,
        showMissedExtraTime=1.6,
        showDamageExtraTime=0.7,
    )
    soundTrack = getSoundTrack("SA_jargon.ogg", delay=2.1, node=suit)
    return Parallel(
        suitTrack, toonTrack, soundTrack, partTrack, partTrack2, partTrack3, partTrack4
    )
...
def doLegalese(attack):
    ...
    dodgeDelay = 1.1
    suitTrack = getSuitTrack(attack)
    sprayTrack1 = getPartTrack(
        sprayEffect1, partDelay, partDuration, [sprayEffect1, suit, 0]
    )
    sprayTrack2 = getPartTrack(
        sprayEffect2, partDelay + 0.8, partDuration, [sprayEffect2, suit, 0]
    )
...
def doPeckingOrder(attack):
    ...
    for i in range(0, numBirds):
        ...
        scaleTrack = Sequence(
            Wait(throwDelay), LerpScaleInterval(next, 0.15, Point3(9, 9, 9))
        )
        birdTracks.append(
            Sequence(Parallel(birdTrack, scaleTrack), Func(MovieUtil.removeProp, next))
        )
