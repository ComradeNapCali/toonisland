...
soundFiles = (
    "AA_sound_bikehorn.ogg",
    "AA_sound_whistle.ogg",
    "AA_sound_bugle.ogg",
    "AA_sound_aoogah.ogg",
    "AA_sound_elephant.ogg",
    "SZ_DD_foghorn.ogg",
    "AA_sound_Opera_Singer.ogg",
)
appearSoundFiles = (
    "MG_tag_1.ogg",
...
def __getSuitTrack(
    sound, lastSoundThatHit, delay, hitCount, targets, totalDamage, hpbonus, toon, npcs
):
    tracks = Parallel()
    attacks = 0
    uberDelay = 0.0
    ...
    for target in targets:
        ...
        if totalDamage > 0 and sound == lastSoundThatHit:
            ...
            if isUber:
                ...
                suitTrack.append(
                    Func(
                        ...
                        breakEffect,
                        suit,
                        Point3(0, 0.0, suit.getHeight() - 1.0),
                    )
                )
                suitTrack.append(
                    Parallel(
                        ...
                        __getPartTrack(
                            breakEffect, 0.0, 1.0, [breakEffect, suit, 0], softStop=-0.5
                        ),
                    )
                )
            else:
            ...
            tracks.append(
                Sequence(Wait(2.9), Func(MovieUtil.indicateMissed, suit, 1.0))
            )
...
def __doBikehorn(sound, delay, toon, targets, level):
    ...
    grow = getScaleIntervals(
        instruments, duration=0.2, startScale=instrMin, endScale=instrMax
    )
    instrumentAppear = Parallel(
        grow, Sequence(Wait(0.15), SoundInterval(instrumentAppearSfx, node=toon))
    )
    ...
    stretchMega = getScaleBlendIntervals(
        megaphones,
        duration=0.2,
    ...
    backMega = getScaleBlendIntervals(
        megaphones,
        duration=0.2,
        startScale=0.9,
    ...
    attackTrack = Parallel(
        Sequence(stretchInstr, backInstr), Sequence(stretchMega, backMega)
    )
    hasLuredSuits = __hasLuredSuits(sound)
    delayTime = delay
    if hasLuredSuits:
    ...
    instrumentshrink = getScaleIntervals(
        instruments, duration=0.1, startScale=instrMax, endScale=instrMin
    )
    if soundEffect:
...
def __doWhistle(sound, delay, toon, targets, level):
    ...
    instrumentAppear = Parallel(
        grow, Sequence(Wait(0.05), SoundInterval(instrumentAppearSfx, node=toon))
    )
    stretchInstr = getScaleBlendIntervals(
        instruments,
        duration=0.2,
...
def __doBugle(sound, delay, toon, targets, level):
    ...
    def longshake(models, num):
        inShake = getScaleBlendIntervals(
            models,
            duration=0.2,
            startScale=instrMax,
            endScale=instrStretch,
        ...
        outShake = getScaleBlendIntervals(
            models,
            duration=0.2,
            startScale=instrStretch,
            endScale=instrMax,
...
def __doElephant(sound, delay, toon, targets, level):
    ...
    instrumentAppearSfx = globalBattleSoundCache.getSound(appearSoundFiles[level])
    grow1 = getScaleIntervals(
        instruments, duration=0.3, startScale=instrMin, endScale=instrMax1
    )
    grow2 = getScaleIntervals(
        instruments, duration=0.3, startScale=instrMax1, endScale=instrMax2
    )
    instrumentAppear = Parallel(
        Sequence(grow1, grow2),
        Sequence(Wait(0.05), SoundInterval(instrumentAppearSfx, node=toon)),
    )
    stretchInstr1 = getScaleBlendIntervals(
        instruments,
        duration=0.1,
    ...
    stretchInstr2 = getScaleBlendIntervals(
        instruments,
        duration=0.1,
    ...
    soundEffect = globalBattleSoundCache.getSound(soundFiles[level])
    instrumentshrink = getScaleIntervals(
        instruments, duration=0.1, startScale=instrMax2, endScale=instrMin
    )
    if soundEffect:
        delayTime = delay + tSound
        if hasLuredSuits:
...
def __doFoghorn(sound, delay, toon, targets, level):
    ...
    instrumentAppearSfx = globalBattleSoundCache.getSound(appearSoundFiles[level])
    grow1 = getScaleIntervals(
        instruments, duration=1, startScale=instrMin, endScale=instrMax1
    )
    grow2 = getScaleIntervals(
        instruments, duration=0.1, startScale=instrMax1, endScale=instrMax2
    )
    instrumentAppear = Parallel(
        Sequence(grow1, grow2),
        Sequence(Wait(0.05), SoundInterval(instrumentAppearSfx, node=toon)),
    ...
    attackTrack = Parallel(
        Sequence(Wait(0.2), spinInstr), Sequence(stretchInstr, Wait(0.5), backInstr)
    )
    hasLuredSuits = __hasLuredSuits(sound)
    delayTime = delay
    if hasLuredSuits:
...
def getScaleIntervals(props, duration, startScale, endScale):
    ...
    for prop in props:
        tracks.append(
            LerpScaleInterval(prop, duration, endScale, startScale=startScale)
        )
...
def getScaleBlendIntervals(props, duration, startScale, endScale, blendType):
    ...
    for prop in props:
        tracks.append(
            LerpScaleInterval(
                prop, duration, endScale, startScale=startScale, blendType=blendType
            )
        )
...
def __getPartTrack(
    particleEffect, startDelay, durationDelay, partExtraArgs, softStop=0
):
