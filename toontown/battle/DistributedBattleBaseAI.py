...
class DistributedBattleBaseAI(DistributedObjectAI.DistributedObjectAI, BattleBase):
    ...
    def __init__(
        ...
        self.fsm = ClassicFSM.ClassicFSM(
            ...
            [
                ...
                State.State(
                    "Off", self.enterOff, self.exitOff, ["FaceOff", "WaitForJoin"]
                ),
            ],
            "Off",
            "Off",
        )
        self.joinableFsm = ClassicFSM.ClassicFSM(
            "Joinable",
            [
                State.State(
                    "Joinable", self.enterJoinable, self.exitJoinable, ["Unjoinable"]
                ),
                State.State(
                    "Unjoinable",
        ...
        self.runableFsm = ClassicFSM.ClassicFSM(
            "Runable",
            [
                State.State(
                    "Runable", self.enterRunable, self.exitRunable, ["Unrunable"]
                ),
                State.State(
                    "Unrunable", self.enterUnrunable, self.exitUnrunable, ["Runable"]
                ),
            ],
            "Unrunable",
            "Unrunable",
    ...
    def abortBattle(self):
        ...
        for toonId in toonsCopy:
            ...
            if (
                ...
                or self.fsm.getCurrentState().getName() == "MakeMovie"
            ):
    ...
    def getMovie(self):
        ...
        for t in self.activeToons:
            if t in self.toonAttacks:
                ...
                if (
                    (track == NO_ATTACK or attackAffectsGroup(track, ta[TOON_LVL_COL]))
                    and track != NPCSOS
                    and track != PETSOS
                ):
                    target = -1
            ...
            if suit != None:
                if not suit.isEmpty():
                    if not self.joiningSuits.count(suit) == 1:
                        self.notify.warning(
                            "__makeAvPending(%d) in zone: %d" % (avId, self.zoneId)
                        )
    ...
    def __joinToon(self, avId, pos):
        ...
        taskMgr.doMethodLater(
            toPendingTime, self.__serverJoinDone, taskName, extraArgs=(avId, taskName)
        )
        self.taskNames.append(taskName)
                ...
                for suit in self.activeSuits:
                    if hasattr(suit, "dna"):
                        self.suitsEncountered.append(
                            {"type": suit.dna.name, "activeToons": self.activeToons[:]}
                        )
                    else:
                        self.notify.warning(
    ...
    def adjustDone(self):
        ...
        if self.ignoreAdjustingResponses == 1:
            ...
            self.notify.warning(
                "adjustDone() - in state %s" % self.fsm.getCurrentState().getName()
            )
            return
    ...
    def timeout(self):
        ...
        if self.ignoreResponses == 1:
            ...
            self.notify.warning(
                "timeout() - in state: %s" % self.fsm.getCurrentState().getName()
            )
            return
        elif self.toons.count(toonId) == 0:
    ...
    def movieDone(self):
        ...
        if self.ignoreResponses == 1:
            ...
            self.notify.warning(
                "movieDone() - in state %s" % self.fsm.getCurrentState().getName()
            )
            return
        elif self.toons.count(toonId) == 0:
    ...
    def joinDone(self, avId):
        ...
        if avId not in self.joinResponses:
            self.notify.debug(
                "joinDone() - no entry for: %d - ignoring: %d" % (avId, toonId)
            )
            return
        jr = self.joinResponses[avId]
        if toonId in jr:
    ...
    def requestAttack(self, track, level, av):
        ...
        if self.ignoreResponses == 1:
            ...
            self.notify.warning(
                "requestAttack() - in state: %s" % self.fsm.getCurrentState().getName()
            )
            return
        ...
        if track == SOS:
            ...
            if track == HEAL:
                if (
                    self.runningToons.count(av) == 1
                    or attackAffectsGroup(track, level)
                    and len(self.activeToons) < 2
                ):
                    self.toonAttacks[toonId] = getToonAttack(toonId, track=UN_ATTACK)
                    validResponse = 0
    ...
    def requestPetProxy(self, av):
        ...
        if petId == av:
            if toonId not in self.pets:
                ...
                def handleGetPetProxy(
                    success, pet, petId=petId, zoneId=zoneId, toonId=toonId
                ):
                    if success:
                        ...
                        petProxy.setRestlessnessThreshold(
                            pet.getRestlessnessThreshold()
                        )
                        ...
                        def handlePetDeleted(task):
                            ...
                            petProxy.generateWithRequiredAndId(
                                petId, self.air.districtId, self.zoneId
                            )
                        ...
                        self.acceptOnce(
                            self.air.getAvatarExitEvent(petId),
                            lambda: taskMgr.doMethodLater(
                                0,
                                handlePetDeleted,
    ...
    def __requestMovie(self, timeout=0):
        if self.adjustFsm.getCurrentState().getName() == "Adjusting":
            ...
            if movieDelay:
                taskMgr.doMethodLater(
                    0.8, self.__makeMovie, self.uniqueName("make-movie")
                )
    ...
    def __makeMovie(self, task=None):
        ...
        if self._DOAI_requestedDelete:
            self.notify.warning(
                "battle %s requested delete, then __makeMovie was called!" % self.doId
            )
    ...
    def enterPlayMovie(self):
        ...
        self.resetResponses()
        movieTime = (
            TOON_ATTACK_TIME * (len(self.activeToons) + self.numNPCAttacks)
                        ...
                        for i in range(len(self.activeToons)):
                            ...
                            if toon != None:
                                if i < len(hps):
                                    ...
                                    self.notify.warning(
                                        "Invalid targetIndex %s in hps %s." % (i, hps)
                                    )
                        ...
                        for suit in self.activeSuits:
                            ...
                            if targetIndex < 0 or targetIndex >= len(hps):
                                self.notify.warning(
                                    ...
                                    % (track, level, targetIndex, len(hps), hps)
                                )
                            else:
                                hp = hps[targetIndex]
                ...
                if adict["group"] == ATK_TGT_GROUP:
                    for activeToon in self.activeToons:
                        ...
                        if toon != None:
                            ...
                            toonDied = (
                                self.suitAttacks[i][TOON_DIED_COL] & 1 << targetIndex
                            )
                            if targetIndex >= len(hps):
                                self.notify.warning(
                    ...
                    if targetIndex >= len(hps):
                        ...
                        if hp > 0:
                            self.notify.debug(
                                "DAMAGE: toon: %d hit for dmg: %d" % (toonId, hp)
                            )
                            if toonDied != 0:
                                toonHpDict[toon.doId][2] = 1
                            toonHpDict[toon.doId][1] += hp
    ...
    def enterResume(self):
        ...
        self.air.writeServerEvent(
            "battleCogsDefeated", self.doId, "%s|%s" % (msgText, self.getTaskZoneId())
        )
    ...
    def enterAdjusting(self):
        ...
        self.adjustingTimer.startCallback(
            self.__estimateAdjustTime() + SERVER_BUFFER_TIME, self.__serverAdjustingDone
        )
        return None
