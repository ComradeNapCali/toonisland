...
shoulderHeights = {"a": 13.28 / 4.0, "b": 13.74 / 4.0, "c": 10.02 / 4.0}
...
def doDrops(drops):
    ...
    camTrack = MovieCamera.chooseDropShot(
        drops, suitDropsDict, camDuration, enterDuration, exitDuration
    )
...
def __getSoundTrack(level, hitSuit, node=None):
    ...
    if soundEffect:
        ...
        if fallingSound:
            soundTrack.append(
                SoundInterval(fallingSound, duration=fallingDuration, node=node)
            )
        if not level == UBER_GAG_LEVEL_INDEX:
...
def __doGroupDrops(groupDrops):
    ...
    for drop in groupDrops:
        ...
        track = __dropGroupObject(
            drop, delay, closestTarget, alreadyDodged, alreadyTeased
        )
        if track:
            toonTracks.append(track)
            delay = delay + TOON_DROP_SUIT_DELAY
...
def __dropGroupObject(drop, delay, closestTarget, alreadyDodged, alreadyTeased):
    ...
    for i in range(len(drop["target"])):
        ...
        suitTrack = __createSuitTrack(
            drop, delay, level, alreadyDodged, alreadyTeased, target, npcs
        )
        if suitTrack:
...
def __dropObjectForSingle(
    drop, delay, objName, level, alreadyDodged, alreadyTeased, npcs, target, npcDrops
):
    singleDropParallel = __dropObject(
        drop,
        delay,
        objName,
        level,
...
def __dropObject(
    ...
    if hp > 0 or level == 1 or level == 2:
        if hasattr(object, "getAnimControls"):
            ...
            shrinkProp = LerpScaleInterval(
                object, dShrink, Point3(0.01, 0.01, 0.01), startScale=object.getScale()
            )
            ...
            animProp = LerpPosInterval(
                object, landFrames[level] / 24.0, endingPos, startPos=startPos
            )
            shrinkProp = LerpScaleInterval(
                object, dShrink, Point3(0.01, 0.01, 0.01), startScale=startingScale
            )
            bounceProp = Effects.createZBounce(object, 2, endingPos, 0.5, 1.5)
            objAnimShrink = Sequence(
                Func(object.setScale, startingScale),
        ...
        shrinkProp = LerpScaleInterval(
            object, dShrinkOnMiss, Point3(0.01, 0.01, 0.01), startScale=startingScale
        )
        bounceProp = Effects.createZBounce(object, 2, endingPos, 0.5, 1.5)
        objAnimShrink = Sequence(
            Func(object.setScale, startingScale),
