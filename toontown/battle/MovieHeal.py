...
from dependencies.libotp.nametag import *
...
def __healTickle(heal, hasInteractivePropHealBonus):
    ...
    featherTrack = Parallel(
        ...
        Sequence(
            ...
            MovieUtil.getScaleIntervals(
                feathers, dFeatherScaleUp, MovieUtil.PNT3_NEARZERO, feathers[0].getScale
            ),
        ),
        Sequence(
            Wait(toon.getDuration("tickle") - dFeatherScaleDown),
            MovieUtil.getScaleIntervals(
                feathers, dFeatherScaleDown, None, MovieUtil.PNT3_NEARZERO
            ),
        ),
    )
    tHeal = 3.0
    mtrack = Parallel(
        ...
        Sequence(
            ...
            ActorInterval(
                target, "cringe", startTime=20.0 / target.getFrameRate("cringe")
            ),
        ),
    )
    track.append(mtrack)
...
def __healJoke(heal, hasInteractivePropHealBonus):
    ...
    tDoSoundAnimation = tSpeakPunchline - float(fSpeakPunchline) / toon.getFrameRate(
        "sound"
    )
    ...
    tracks.append(
        Sequence(
            ...
            MovieUtil.getScaleIntervals(
                megaphones, dMegaphoneScale, MovieUtil.PNT3_NEARZERO, MovieUtil.PNT3_ONE
            ),
            Wait(toon.getDuration("sound") - 2.0 * dMegaphoneScale),
            MovieUtil.getScaleIntervals(
                megaphones, dMegaphoneScale, MovieUtil.PNT3_ONE, MovieUtil.PNT3_NEARZERO
            ),
    ...
    tracks.append(
        Sequence(
            Wait(tSpeakSetup), Func(toon.setChatAbsolute, joke[0], CFSpeech | CFTimeout)
        )
    )
    tracks.append(
        Sequence(
...
def __healSmooch(heal, hasInteractivePropHealBonus):
    ...
    lipsTrack = Sequence(
        ...
        LerpScaleInterval(
            lips, dScale, Point3(3, 3, 3), startScale=MovieUtil.PNT3_NEARZERO
        ),
        Wait(tThrow - tLips - dScale),
        LerpPosInterval(
            lips, dThrow, Point3(target.getPos() + Point3(0, 0, target.getHeight()))
        ),
...
def __healDance(heal, hasInteractivePropHealBonus):
    ...
    for target in targets:
        ...
        hp = target["hp"]
        reactIval = Func(
            __healToon, targetToon, hp, ineffective, hasInteractivePropHealBonus
        )
        if first:
    ...
    propTrack = Sequence(
        ...
        MovieUtil.getScaleIntervals(
            hats + canes, dScale, MovieUtil.PNT3_NEARZERO, MovieUtil.PNT3_ONE
        ),
        Wait(toon.getDuration("happy-dance") - 2.0 * dScale),
        MovieUtil.getScaleIntervals(
            hats + canes, dScale, MovieUtil.PNT3_ONE, MovieUtil.PNT3_NEARZERO
        ),
...
def __healDive(heal, hasInteractivePropHealBonus):
    ...
    propTrack = Sequence(
        ...
        LerpScaleInterval(
            ladder, ladderGrowTime, scaleUpPoint, startScale=MovieUtil.PNT3_NEARZERO
        ),
        ...
        Wait(1.8),
        LerpScaleInterval(
            ladder, ladderGrowTime, MovieUtil.PNT3_NEARZERO, startScale=scaleUpPoint
        ),
            ...
            Parallel(
                ...
                Sequence(
                    ...
                    LerpColorScaleInterval(
                        toonNode, 0.25, VBase4(1, 1.0, 1, 0.0), blendType="easeInOut"
                    ),
