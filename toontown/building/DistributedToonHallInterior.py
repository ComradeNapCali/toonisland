...
class DistributedToonHallInterior(DistributedToonInterior):
    def __init__(self, cr):
        ...
        self.sillyFSM = ClassicFSM.ClassicFSM(
            ...
            [
                ...
                State.State(
                    "Phase15", self.enterPhase15, self.exitPhase15, ["Flat", "Off"]
                ),
    ...
    def selectPhase(self, newPhase):
        try:
            ...
            if self.sillyFSM.hasStateNamed(
                gotoPhase
            ) and not self.sillyFSM.getCurrentState() == self.sillyFSM.getStateNamed(
                gotoPhase
            ):
    ...
    def calculateFrameOffset(self, phaseDuration, numFrames):
        ...
        if valid and base.cr.SillyMeterMgr.getIsRunning():
            startTime = time.mktime(
                base.cr.SillyMeterMgr.getCurPhaseStartDate().timetuple()
            )
            serverTime = time.mktime(
                base.cr.toontownTimeManager.getCurServerDateTime().timetuple()
            )
    ...
    def enterSetup(self):
        ...
        self.sillyMeter.makeSubpart(
            "meter", ["def_pivot"], ["uvj_progressBar*", "def_springA"]
        )
