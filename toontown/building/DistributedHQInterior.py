...
class DistributedHQInterior(DistributedObject.DistributedObject):
    ...
    def setLeaderBoard(self, avIds, names, scores):
        self.notify.debug(
            "setLeaderBoard: avIds: %s, names: %s, scores: %s" % (avIds, names, scores)
        )
    ...
    def setupDoors(self):
        ...
        self.colors = ToonInteriorColors.colors[ToonIslandCentral]
        ...
        for npIndex in range(numDoorOrigins):
            ...
            DNADoor.setupDoor(
                doorNP, newNodePath, doorOrigin, self.dnaStore, triggerId, doorColor
            )
