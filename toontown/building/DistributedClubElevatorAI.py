...
class DistributedClubElevatorAI(DistributedElevatorFSMAI.DistributedElevatorFSMAI):
    ...
    def generateWithRequired(self, zoneId):
        self.zoneId = zoneId
        DistributedElevatorFSMAI.DistributedElevatorFSMAI.generateWithRequired(
            self, self.zoneId
        )
    ...
    def acceptBoarder(self, avId, seatIndex):
        DistributedElevatorFSMAI.DistributedElevatorFSMAI.acceptBoarder(
            self, avId, seatIndex
        )
        self.acceptOnce(
        ...
        if self.state == "WaitEmpty" and self.countFullSeats() < self.countAvsInZone():
            ...
            self.state in ("WaitCountdown", "WaitEmpty")
            and self.countFullSeats() >= self.countAvsInZone()
        ):
            taskMgr.doMethodLater(
                ElevatorConstants.TOON_BOARD_ELEVATOR_TIME,
                self.goAllAboard,
                self.quickBoardTask,
            )
    ...
    def acceptExiter(self, avId):
        ...
        if seatIndex == None:
            ...
            self.sendUpdate(
                "emptySlot" + str(seatIndex),
                [avId, bailFlag, globalClockDelta.getRealNetworkTime()],
            )
            if self.countFullSeats() == 0:
                self.request("WaitEmpty")
            taskMgr.doMethodLater(
                ElevatorConstants.TOON_EXIT_ELEVATOR_TIME,
                self.clearEmptyNow,
    ...
    def enterOpening(self):
        ...
        taskMgr.doMethodLater(
            ElevatorConstants.ElevatorData[ElevatorConstants.ELEVATOR_NORMAL][
                "openTime"
            ],
            self.waitEmptyTask,
    ...
    def enterWaitCountdown(self):
        ...
        taskMgr.doMethodLater(
            self.countdownTime, self.timeToGoTask, self.uniqueName("countdown-timer")
        )
    ...
    def enterAllAboard(self):
        ...
        taskMgr.doMethodLater(
            waitTime, self.closeTask, self.uniqueName("waitForAllAboard")
        )
    ...
    def enterClosing(self):
        ...
        taskMgr.doMethodLater(
            ElevatorConstants.ElevatorData[ElevatorConstants.ELEVATOR_STAGE][
                "closeTime"
            ],
