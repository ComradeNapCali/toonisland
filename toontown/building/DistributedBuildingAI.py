...
from toontown.cogdominium.DistributedCogdoElevatorExtAI import (
    DistributedCogdoElevatorExtAI,
)
...
class DistributedBuildingAI(DistributedObjectAI.DistributedObjectAI):
    ...
    def cogdoTakeOver(self, suitTrack, difficulty, buildingHeight):
        if not self.isToonBlock():
            return
        self.updateSavedBy(None)
        numFloors = self.FieldOfficeNumFloors + (1)
        self.track = suitTrack
        self.difficulty = difficulty
        self.numFloors = numFloors
        self.becameSuitTime = time.time()
    ...
    def isSuitBuilding(self):
        ...
        return (
            state == "suit"
            or state == "becomingSuit"
            or state == "clearOutToonInterior"
        )
    ...
    def isCogdo(self):
        ...
        return (
            state == "cogdo"
            or state == "becomingCogdo"
            or state == "becomingCogdoFromCogdo"
            or state == "clearOutToonInteriorForCogdo"
        )
    ...
    def recordVictorResponse(self, avId):
        ...
        if index == None:
            self.air.writeServerEvent(
                "suspicious",
                avId,
                "DistributedBuildingAI.setVictorReady from toon not in %s."
                % self.victorList,
            )
            return
    ...
    def setVictorReady(self):
        ...
        if self.victorResponses == None:
            self.air.writeServerEvent(
                "suspicious",
                avId,
                "DistributedBuildingAI.setVictorReady in state %s."
                % self.fsm.getCurrentState().getName(),
            )
            return
    ...
    def victorsTimedOutTask(self, task):
        ...
        if hasattr(self, "interior"):
            self.notify.info(
                "victorsTimedOutTask: ejecting players by deleting interior."
            )
            self.interior.requestDelete()
            del self.interior
            task.delayTime = 15.0
            return task.again
        self.notify.info(
            "victorsTimedOutTask: suspicious players remaining, advancing state."
        )
        for i in range(len(self.victorList)):
            if self.victorList[i] and self.victorResponses[i] == 0:
                self.air.writeServerEvent(
                    "suspicious",
                    self.victorList[i],
                    "DistributedBuildingAI toon client refused to leave building.",
                )
        ...
        for t in victorList:
            ...
            if t:
                ...
                self.air.writeServerEvent(
                    ...
                    t,
                    "%s|%s|%s|%s"
                    % (self.track, self.numFloors, self.zoneId, victorList),
                )
            if toon != None:
    ...
    def enterWaitForVictorsFromCogdo(self, victorList, savedBy):
            ...
            for t in victorList:
                ...
                if toon != None:
                    self.air.questManager.toonKilledCogdo(
                        toon, self.difficulty, self.numFloors, self.zoneId, activeToons
                    )
        ...
        taskMgr.doMethodLater(
            30,
            self.victorsTimedOutTask,
            self.taskName(str(self.block) + "_waitForVictors-timer"),
        )
    ...
    def enterBecomingToon(self):
        ...
        taskMgr.doMethodLater(
            SuitBuildingGlobals.VICTORY_SEQUENCE_TIME, self.becomingToonTask, name
        )
    ...
    def enterToon(self):
        ...
        if (
            simbase.config.GetBool("want-new-toonhall", 1)
            and ZoneUtil.getCanonicalZoneId(interiorZoneId) == ToonHall
        ):
        ...
        self.air.writeServerEvent(
            "building-toon", self.doId, "%s|%s" % (self.zoneId, self.block)
        )

    def createExteriorDoor(self):
        result = DistributedDoorAI.DistributedDoorAI(
            self.air, self.block, DoorTypes.EXT_STANDARD
        )
        return result
    ...
    def enterBecomingSuit(self):
        self.sendUpdate(
            "setSuitData", [ord(self.track), self.difficulty, self.numFloors]
        )
        self.d_setState("becomingSuit")
        name = self.taskName(str(self.block) + "_becomingSuit-timer")
        taskMgr.doMethodLater(
            SuitBuildingGlobals.TO_SUIT_BLDG_TIME, self.becomingSuitTask, name
        )
    ...
    def enterBecomingCogdo(self):
        ...
        taskMgr.doMethodLater(
            SuitBuildingGlobals.TO_SUIT_BLDG_TIME, self.becomingCogdoTask, name
        )
    ...
    def enterBecomingCogdoFromCogdo(self):
        ...
        taskMgr.doMethodLater(
            SuitBuildingGlobals.VICTORY_RUN_TIME, self.becomingCogdoTask, name
        )
    ...
    def enterCogdo(self):
        ...
        self.elevator = DistributedCogdoElevatorExtAI(
            self.air, self, fSkipOpening=self.fSkipElevatorOpening
        )
        self.fSkipElevatorOpening = False
        self.elevator.generateWithRequired(exteriorZoneId)
        self.air.writeServerEvent(
    ...
    def _createSuitInterior(self):
        return DistributedSuitInteriorAI.DistributedSuitInteriorAI(
            self.air, self.elevator
        )
