...
class DistributedElevator(DistributedObject.DistributedObject):
    ...
    def fillSlot(self, index, avId, wantBoardingShow=0):
        ...
        if avId == 0:
            ...
            if avId == base.localAvatar.getDoId():
                ...
                cameraTrack.append(
                    Func(elevator.fsm.request, "boarding", [self.getElevatorModel()])
                )
            ...
            if self.canHideBoardingQuitBtn(avId):
                track = Sequence(
                    Func(localAvatar.boardingParty.groupPanel.disableQuitButton), track
                )
            if avId == base.localAvatar.getDoId():
    ...
    def emptySlot(self, index, avId, bailFlag, timestamp, timeSent=0):
        ...
        if avId == 0:
            ...
            if avId in self.cr.doId2do:
                ...
                if self.canHideBoardingQuitBtn(avId):
                    track.append(
                        Func(localAvatar.boardingParty.groupPanel.enableQuitButton)
                    )
    ...
    def handleEnterSphere(self, collEntry):
        ...
        if self.allowedToEnter(self.zoneId):
            if (
                self.elevatorTripId
                and localAvatar.lastElevatorLeft == self.elevatorTripId
            ):
    ...
    def canHideBoardingQuitBtn(self, avId):
        if (
            avId == localAvatar.doId
            and hasattr(localAvatar, "boardingParty")
            and localAvatar.boardingParty
            and localAvatar.boardingParty.groupPanel
        ):
            return True
        else:
            return False
