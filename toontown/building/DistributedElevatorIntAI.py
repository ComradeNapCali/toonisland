...
class DistributedElevatorIntAI(DistributedElevatorAI.DistributedElevatorAI):
    ...
    def requestBuildingExit(self):
        ...
        if self.accepting:
            if avId in self.avIds:
                ...
                self.notify.warning(
                    "avId: %s not known, but tried to exit the building" % avId
                )
    ...
    def enterWaitCountdown(self):
        ...
        taskMgr.doMethodLater(
            self.countdownTime + BattleBase.SERVER_BUFFER_TIME,
            self.timeToGoTask,
    ...
    def enterClosing(self):
        ...
        taskMgr.doMethodLater(
            ElevatorData[ELEVATOR_NORMAL]["closeTime"] + BattleBase.SERVER_BUFFER_TIME,
