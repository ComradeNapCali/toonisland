    ...
    def getBuilding(self):
        if "building" not in self.__dict__:
            if self.doorType == DoorTypes.EXT_ANIM_STANDARD:
                ...
                self.notify.error(
                    "DistributedAnimDoor.getBuiding with doorType=%s" % self.doorType
                )
        ...
        if self.doorType == DoorTypes.EXT_ANIM_STANDARD:
            if hasattr(self, "tempDoorNodePath"):
                ...
                self.notify.debug(
                    ...
                    % (str(doorNP.getPos()), str(doorNP.getHpr()))
                )
            ...
            self.notify.error(
                "DistributedAnimDoor.getDoorNodePath with doorType=%s" % self.doorType
            )
        return otherNP
