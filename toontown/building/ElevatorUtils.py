...
def getOpenInterval(
    distObj, leftDoor, rightDoor, openSfx, finalOpenSfx, type=ELEVATOR_NORMAL
):
    ...
    if finalOpenSfx:
        sound = Sequence(
            SoundInterval(
                openSfx, duration=openDuration, volume=sfxVolume, node=leftDoor
            ),
...
def getCloseInterval(
    distObj, leftDoor, rightDoor, closeSfx, finalCloseSfx, type=ELEVATOR_NORMAL
):
    ...
    if finalCloseSfx:
        sound = Sequence(
            SoundInterval(
                closeSfx, duration=closeDuration, volume=sfxVolume, node=leftDoor
            ),
...
def getRideElevatorInterval(type=ELEVATOR_NORMAL):
    ...
    if type in (ELEVATOR_VP, ELEVATOR_CFO, ELEVATOR_CJ, ELEVATOR_BB):
        ...
        ival = Sequence(
            ...
            LerpPosInterval(
                camera, 0.5, Point3(0, yValue, zMid), startPos=Point3(0, yValue, zMin)
            ),
            Wait(1.0),
            LerpPosInterval(
                camera,
            ...
            LerpPosInterval(
                camera, 1.0, Point3(0, yValue, zMid), startPos=Point3(0, yValue, zMax)
            ),
        )
    else:
