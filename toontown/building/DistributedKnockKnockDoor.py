...
class DistributedKnockKnockDoor(DistributedAnimatedProp.DistributedAnimatedProp):
    ...
    def acceptAvatar(self):
        self.acceptOnce(
            "enterKnockKnockDoorSphere_" + str(self.propId), self.enterTrigger
        )
    ...
    def knockKnockTrack(self, avatar, duration):
        ...
        if place:
            ...
            if branch == ToontownGlobals.BeachballBoulevard:
                if self.propId == 44:
                    joke = KnockKnockContestJokes[ToontownGlobals.BeachballBoulevard]
            elif branch == ToontownGlobals.AlohaAvenue:
                if self.propId in list(
                    KnockKnockContestJokes[ToontownGlobals.AlohaAvenue].keys()
                ):
                    joke = KnockKnockContestJokes[ToontownGlobals.AlohaAvenue][
                        self.propId
                    ]
            elif branch == ToontownGlobals.PineapplePlace:
                if self.propId == 1:
                    joke = KnockKnockContestJokes[ToontownGlobals.PineapplePlace]
