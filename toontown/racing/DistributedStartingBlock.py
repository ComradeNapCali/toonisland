...
class DistributedStartingBlock(DistributedObject.DistributedObject, FSM):
    ...
    def announceGenerate(self):
        ...
        self.accept(
            self.uniqueName("enterStartingBlockSphere"), self.__handleEnterSphere
        )
        if __debug__:
            if self.testLOD:
    ...
    def __handleEnterSphere(self, collEntry):
        ...
        if base.localAvatar.hp > 0:
            ...
            def handleEnterRequest(self=self):
                self.ignore("stoppedAsleep")
                if (
                    hasattr(self.dialog, "doneStatus")
                    and self.dialog.doneStatus == "ok"
                ):
            ...
            if self.kartPad.isPractice():
                ...
                numTickets = RaceGlobals.getEntryFee(
                    self.kartPad.trackId, self.kartPad.trackType
                )
                msg = TTLocalizer.StartingBlock_EnterNonPractice % (
                    raceName,
                    numTickets,
                )
    ...
    def setOccupied(self, avId):
        ...
        if self.avId == 0:
            ...
            if av != None:
                ...
                self.kartNode.setPosHpr(
                    self.nodePath.getPos(render), self.nodePath.getHpr(render)
                )
                self.kart = Kart()
                ...
                self.notify.warning(
                    "Unknown avatar %d in kart block %d " % (self.avId, self.doId)
                )
                self.avId = 0
        ...
        toonTrack = Sequence(
            ...
            Parallel(
                LerpPosInterval(
                    ...
                    1.0,
                    Point3(
                        self.movieNode.getX(self.avParent),
    ...
    def generateKartAppearTrack(self):
        ...
        kartTrack = Parallel(
            Sequence(ActorInterval(self.av, "feedPet"), Func(self.av.loop, "neutral")),
            Sequence(
                ...
                Parallel(
                    LerpHprInterval(
                        self.kart, hpr=self.kartNode.getHpr(render), duration=1.2
                    ),
                    ProjectileInterval(
                        self.kart,
                ...
                Sequence(
                    LerpScaleInterval(
                        self.kart, scale=Point3(1.1, 1.1, 0.1), duration=0.2
                    ),
                    LerpScaleInterval(
                        self.kart, scale=Point3(0.9, 0.9, 0.1), duration=0.1
                    ),
                    LerpScaleInterval(
                        self.kart, scale=Point3(1.0, 1.0, 0.1), duration=0.1
                    ),
                    LerpScaleInterval(
                        self.kart, scale=Point3(1.0, 1.0, 1.1), duration=0.2
                    ),
                    LerpScaleInterval(
                        self.kart, scale=Point3(1.0, 1.0, 0.9), duration=0.1
                    ),
                    LerpScaleInterval(
                        self.kart, scale=Point3(1.0, 1.0, 1.0), duration=0.1
                    ),
    ...
    def generateToonJumpTrack(self):
        ...
        jumpTrack = Sequence(
            ...
            Func(self.av.setPosHpr, 0, 0.45, -0.25, 0, 0, 0),
    ...
    def generateKartDisappearTrack(self):
        def getHoleTrack(hole, holeParent):
            holeTrack = Sequence(
                ...
                Func(hole.setPos, holeParent, Point3(0, 0.0, -0.6)),
        ...
        def getKartShrinkTrack(kart):
            ...
            pos.addZ(-1.0)
            kartTrack = Sequence(
                LerpScaleInterval(kart, scale=Point3(1.0, 1.0, 0.9), duration=0.1),
                LerpScaleInterval(kart, scale=Point3(1.0, 1.0, 1.1), duration=0.1),
                LerpScaleInterval(kart, scale=Point3(1.0, 1.0, 0.1), duration=0.2),
                LerpScaleInterval(kart, scale=Point3(0.9, 0.9, 0.1), duration=0.1),
                LerpScaleInterval(kart, scale=Point3(1.1, 1.1, 0.1), duration=0.1),
                LerpScaleInterval(kart, scale=Point3(0.1, 0.1, 0.1), duration=0.2),
                Wait(0.2),
    ...
    def enterEnterMovie(self):
        ...
        if self.av is not None and self.localToonKarting:
            ...
            self.movieTrack = Sequence(
                Parallel(cameraTrack, toonTrack),
                Parallel(
            ...
            self.finishMovie()
            self.movieTrack = Sequence(
                toonTrack, kartTrack, jumpTrack, name=name, autoFinish=1
            )
    ...
    def exitEnterMovie(self):
        self.notify.debug(
            "%d exitEnterMovie: Exiting the Enter Movie State." % self.doId
        )
    ...
    def enterExitMovie(self):
        self.notify.debug(
            "%d enterExitMovie: Entering the Exit Movie State." % self.doId
        )
        self.hideGui()
        ...
        self.finishMovie()
        self.movieTrack = Sequence(
            Func(self.kart.kartLoopSfx.stop),
            jumpTrack,
            kartTrack,
    ...
    def doExitToRaceTrack(self):
        ...
        scaleLerpIval = LerpScaleInterval(
            self.kartNode, scale=oldBlockScale * 0.2, duration=2.0
        )
        ...
        self.movieTrack.append(
            Sequence(
                Parallel(blockLerpIval, scaleLerpIval),
    ...
    def bulkLoad(self):
        base.loader.beginBulkLoad(
            "atRace",
            TTLocalizer.StartingBlock_Loading,
            60,
            1,
