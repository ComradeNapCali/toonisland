...
class RaceManagerAI(DirectObject.DirectObject):
    ...
    def exitedRace(self, race, playerInfo):
        ...
        if totalTime <= RaceGlobals.getQualifyingTime(race.trackId):
            ...
            self.checkPersonalBest(
                race.trackId, totalTime, race.raceType, race.toonCount, playerInfo.avId
            )
            if race.raceType == RaceGlobals.Practice:
                winnings = RaceGlobals.PracticeWinnings
                self.notify.debug(
                    "GrandTouring: Checking from branch: practice %s" % playerInfo.avId
                )
                ...
                self.air.writeServerEvent(
                    "kartingPlaced", playerInfo.avId, "%s|%s" % (place, race.toonCount)
                )
                if race.raceType != RaceGlobals.Circuit:
            ...
            if av:
                ...
                self.notify.debug(
                    "circuitTotalBonusTickets: %s" % race.circuitTotalBonusTickets
                )
                av.b_setTickets(newTickets)
        else:
            race.circuitTimeList[playerInfo.avId][currentTimeIndex][1] = -1
            self.notify.debug(
                "GrandTouring: Checking from branch: Not Qualified %s" % playerInfo.avId
            )
            trophies = self.checkForNonRaceTrophies(playerInfo.avId)
            if trophies:
                self.updateTrophiesFromList(playerInfo.avId, trophies)
        ...
        for avId in finalStandings:
            ...
            if race.isLastRace():
                ...
                if av and avId in race.playersFinished:
                    self.air.writeServerEvent(
                        "kartingCircuitFinished", avId, "%s|%s" % (place, places)
                    )
                    ...
                    race.d_setCircuitPlace(
                        avId, place, entryFee, winnings, finalBonus, trophies
                    )
    ...
    def updateTrophiesFromList(self, avId, trophies):
        self.air.writeServerEvent(
            "Writing Trophies: Updating trophies from list", avId, "%s" % trophies
        )
        av = self.air.doId2do.get(avId)
        if not av:
            return
    ...
    def checkForCircuitTrophies(self, race, avId, inHistory=None):
        ...
        newTrophies = []
        newTrophies.extend(
            self.checkHistoryForTrophy(
                trophies, history, winIndex, winReqList, winIndices
            )
        )
        newTrophies.extend(
            self.checkHistoryForTrophy(
                trophies, history, sweepIndex, sweepReqList, sweepIndices
            )
        )
        newTrophies.extend(
            self.checkHistoryForTrophy(
                trophies, history, qualIndex, qualReqList, qualIndices
            )
        )
    ...
    def checkForRaceTrophies(self, race, avId, inHistory=None):
        ...
        for genre in range(numTrackGenres):
            singleGenreNewTrophies = self.checkHistoryForTrophy(
                trophies,
                history,
        ...
        singleGenreNewTrophies = self.checkHistoryForTrophyByValue(
            trophies,
            history,
            totalWins,
    ...
    def checkForNonRaceTrophies(self, avId, inHistory=None):
        ...
        if not trophies[RaceGlobals.GrandTouring]:
            ...
            self.notify.debug(
                "GrandTouring: checking for grand touring %s"
                % trophies[RaceGlobals.GrandTouring]
            )
            ...
            self.notify.debug(
                "counter %s tracks %s" % (counter, len(RaceGlobals.TrackDict))
            )
            self.notify.debug(
                "GrandTouring: bests comparison counter: %s tracks: %s"
            ...
            if counter >= len(RaceGlobals.TrackDict):
                ...
                addTrophyCount += 1
                self.air.writeServerEvent(
                    "kartingTrophy", avId, "%s" % RaceGlobals.GrandTouring
                )
                self.notify.debug(
                    "trophy: "
                    + TTLocalizer.KartTrophyDescriptions[RaceGlobals.GrandTouring]
                )
                self.notify.debug(
                    "GrandTouring: awarding grand touring new trophies %s" % newTrophies
                )
        else:
            self.notify.debug("already has grandtouring")
            self.notify.debug("trophies %s" % trophies)
            self.notify.debug(
                "GrandTouring: already has grand touring %s"
                % trophies[RaceGlobals.GrandTouring]
            )
        for i in range(1, RaceGlobals.NumTrophies // RaceGlobals.TrophiesPerCup + 1):
            cupNum = (trophies[: RaceGlobals.NumTrophies].count(1) + addTrophyCount) / (
                i * RaceGlobals.TrophiesPerCup
            )
            ...
            if cupNum and not trophies[trophyIndex]:
                ...
                self.notify.debug(
                    "trophy: " + TTLocalizer.KartTrophyDescriptions[trophyIndex]
                )

        self.notify.debug("NONRACE TROPHIES %s" % newTrophies)
        return newTrophies

    def checkHistoryForTrophyByValue(
        self, trophies, history, historyIndexValue, trophyReqList, trophyIndices
    ):
        newTrophies = []
        self.notify.debug("Checking History for Trophy")
        self.notify.debug(
            "Index %s Num %s ReqList %s Indices %s"
            % (0, historyIndexValue, trophyReqList, trophyIndices)
        )
    ...
    def checkHistoryForTrophy(
        self, trophies, history, historyIndex, trophyReqList, trophyIndices
    ):
        newTrophies = []
        self.notify.debug("Checking History for Trophy")
        self.notify.debug(
    ...
    def checkForTrophies(self, place, trackId, raceType, numRacers, avId):
        ...
        if place == 1:
            ...
            trophiesWon += self.checkForTrophy(
                place,
                trackId,
                raceType,
                numRacers,
    ...
    def checkForTrophy(
        ...
        for i in range(0, len(trophyReqList)):
            if not trophies[trophyIndices[i]]:
                if history[historyIndex] == trophyReqList[i]:
                    ...
                    self.air.writeServerEvent(
                        "kartingTrophy", avId, "%s" % trophyIndices[i]
                    )
                    self.notify.debug(
                        "trophy: "
                        + TTLocalizer.KartTrophyDescriptions[trophyIndices[i]]
                    )
                    break
        ...
        if not raceType == RaceGlobals.Circuit:
            if not trophies[totalTrophyIndex]:
                ...
                self.notify.debug(
                    "checking for total trophy: %s >= %s" % (total, totalReq)
                )
                if total >= totalReq:
                    trophies[totalTrophyIndex] = 1
                    newTrophies.append(totalTrophyIndex)
                    self.air.writeServerEvent(
                        "kartingTrophy", avId, "%s" % totalTrophyIndex
                    )
                    self.notify.debug(
                        "trophy: "
                        + TTLocalizer.KartTrophyDescriptions[totalTrophyIndex]
                    )
        for i in range(1, RaceGlobals.NumTrophies / RaceGlobals.TrophiesPerCup + 1):
            cupNum = trophies[: RaceGlobals.NumTrophies].count(1) / (
                i * RaceGlobals.TrophiesPerCup
            )
            self.notify.debug("cupNum: %s" % cupNum)
            trophyIndex = RaceGlobals.TrophyCups[i - 1]
    ...
    def checkTimeRecord(self, trackId, time, raceType, numRacers, avId):
        ...
        for period in RaceGlobals.PeriodIds:
            for record in range(0, RaceGlobals.NumRecordsPerPeriod):
                recordTime = self.trackRecords[str(trackId)][str(period)][record][0]
                if time < recordTime:
                    newRecord = 1
                    self.notify.debug(
                        "new %s record!" % TTLocalizer.RecordPeriodStrings[period]
                    )
                    av = simbase.air.doId2do.get(avId)
                    if av:
                        name = av.name
                        self.trackRecords[str(trackId)][str(period)].insert(
                            record, (time, raceType, numRacers, name)
                        )
                        self.trackRecords[str(trackId)][
                            str(period)
                        ] = self.trackRecords[str(trackId)][str(period)][
                            : RaceGlobals.NumRecordsPerPeriod
                        ]
                        self.updateLeaderboards(trackId, period)
                        bonus = RaceGlobals.PeriodDict[period]
                        self.air.writeServerEvent(
                            "kartingRecord", avId, "%s|%s|%s" % (period, trackId, time)
                        )
                    else:
    ...
    def updateRecordFile(self):
        try:
            ...
            json.dump(self.trackRecords, file, indent=4)
            file.close()
    ...
    def getFilename(self):
        return "%s%s_trackRecords.json" % (self.serverDataFolder, self.shard)
    ...
    def loadRecords(self):
        ...
        for trackId in RaceGlobals.TrackIds:
            if str(trackId) not in records:
                records[str(trackId)] = {}
                for i in RaceGlobals.PeriodIds:
                    records[str(trackId)][str(i)] = []
                    for j in range(0, RaceGlobals.NumRecordsPerPeriod):
                        records[str(trackId)][str(i)].append(
    ...
    def getRecords(self, trackId, period):
        return self.trackRecords[str(trackId)][str(period)]
