...
class RaceResultsPanel(DirectFrame):
    ...
    def __init__(self, numRacers, race, raceEndPanel, *args, **kwargs):
        ...
        for x in range(self.numRacers):
            ...
            self.entryList.append(
                (pLabel, fFrame, nLabel, tLabel, wLabel, cpLabel, ncpLabel)
            )
    ...
    def displayRacer(
        ...
        self.notify.debug(
            "displayRacer: place=%d name=%s tickets=%d bonus=%d"
            % (place, name, tickets, bonus)
        )
        ...
        def flipText(
            flip,
            label=self.entryList[place - 1][3],
            timeStr=timeStr,
            ...
            if flip:
                ...
                self.entryList[place - 1][3].configure(
        ...
        if trophies:
            DirectFrame(
                parent=headFrame,
                relief=None,
                image=loader.loadModel("phase_6/models/karting/trophy"),
                image_pos=(0.25, -1.01, -0.25),
                image_scale=0.25,
            )
        ...
        if not circuitPoints == []:
            ...
            def totalPointTicker(
                t,
                label=self.entryList[place - 1][5],
            ...
            seq = Sequence(
                Wait(1),
                Parallel(
                    LerpFunc(totalPointTicker, duration=1),
                    LerpFunc(newPointTicker, duration=1),
                ),
                Func(endTicker),
            )
    ...
    def updateWinnings(self, place, newTotalTickets):
        self.notify.debug(
            "updateWinnings: self.tickets=%s place=%d newTotalTickets=%d"
            % (self.tickets, place, newTotalTickets)
        )
    ...
    def circuitFinished(self, placeFixup):
        ...
        for oldPlace, newPlace in placeFixup:
            if oldPlace != newPlace:
                ...
                shiftRacersSeq.append(
                    Parallel(
                        Func(fixPlaceValue),
                        LerpPosInterval(self.rowFrame[oldPlace], 1, newPos),
                    )
                )
        ...
        if not len(self.race.circuitLoop) == 0:
            self.notify.debug(
                "Not the last race in a circuit, pressing next race in 30 secs"
            )
...
class RaceWinningsPanel(DirectFrame):
    def __init__(self, race, *args, **kwargs):
        ...
        tLabel = DirectLabel(
            ...
            relief=None,
            pos=(0.0, 0.0, 0.25),
            text=TTLocalizer.KartRace_Tickets,
            text_fg=(0.0, 0.0, 0.0, 1.0),
            text_scale=0.1,
            text_font=DGG.getDefaultFont(),
        )
        ...
        ticketPic = DirectFrame(
            parent=tFrame,
            relief=None,
            image=loader.loadModel("phase_6/models/karting/tickets"),
            image_pos=(0.5, 0, -0.02),
            image_scale=0.4,
        )
        self.ticketFrame = tFrame
        self.ticketComponents = (dLabel, wLabel, bLabel, fLabel)
        tFrame = DirectFrame(parent=frame, relief=None, pos=(0, 0, 0))
        tLabel = DirectLabel(
            parent=tFrame,
            relief=None,
            pos=(0.0, 0.0, 0.25),
            text=TTLocalizer.KartRace_Bonus,
            text_fg=(0.0, 0.0, 0.0, 1.0),
            text_scale=0.1,
            text_font=DGG.getDefaultFont(),
        )
        ...
        tLabel = DirectLabel(
            ...
            relief=None,
            pos=(0.0, 0.0, 0.25),
            text=TTLocalizer.KartRace_Trophies,
            text_fg=(0.0, 0.0, 0.0, 1.0),
            text_scale=0.1,
            text_font=DGG.getDefaultFont(),
        )
    ...
    def generateDisplaySequences(
        self, track, ticDeposit, ticWon, ticBonus, trophies, endOfCircuitRace=False
    ):
        ...
        if (
            ...
            not in base.cr.newsManager.getHolidayIdList()
            or self.race.raceType != RaceGlobals.Practice
        ):
            self.doubleTicketsLabel.stash()
        if ticBonus:
            ticketSeq.append(
                Sequence(
                    Func(self.ticketFrame.hide),
                    Func(self.bonusFrame.show),
                    Func(self.trophyFrame.hide),
                    Func(
                        self.bonusComponents[0].configure,
                        text=wrapStr(
                            TTLocalizer.KartRace_RecordString
                            % (
                                TTLocalizer.KartRecordStrings[bonusType],
                                TTLocalizer.KartRace_TrackNames[track],
                                str(ticBonus),
                            )
                        ),
                    ),
        ...
        ticketSeq.append(
            Sequence(
                Func(self.bonusFrame.hide),
                Func(self.trophyFrame.hide),
                Func(self.ticketFrame.show),
                Func(self.ticketComponents[3].configure, text_color=Vec4(0, 0, 0, 1)),
                Func(self.ticketComponents[0].configure, text_color=Vec4(1, 0, 0, 1)),
                Parallel(
                    LerpFunc(
                        ticketTicker,
                        duration=(ticDeposit and [1] or [0])[0],
                ...
                Func(self.ticketComponents[1].configure, text_color=Vec4(1, 0, 0, 1)),
                Parallel(
                    LerpFunc(
                        ticketTicker,
                        duration=(ticWon and [1] or [0])[0],
                ...
                Func(self.ticketComponents[2].configure, text_color=Vec4(1, 0, 0, 1)),
                Parallel(
                    LerpFunc(
                        ticketTicker,
                        duration=(origTicBonus and [1] or [0])[0],
        ...
        def showCorrectTrophy(trophyId):
            ...
            self.trophyImage = RacingTrophy(
                level=trophyId, parent=self.trophyComponents[1], pos=(0.5, 0, -0.25)
            )
            if (
                trophyId == RaceGlobals.GrandTouring
                or trophyId == RaceGlobals.TotalQuals
                or trophyId == RaceGlobals.TotalWins
            ):
        ...
        if trophies:
            ...
            for x in trophies:
                winningsSeq.append(
                    Sequence(
                        Func(
                            self.trophyComponents[0].configure,
                            text=wrapStr(TTLocalizer.KartTrophyDescriptions[x]),
                        ),
                        Func(showCorrectTrophy, x),
                        Wait(5),
                    )
                )
...
class RaceEndPanel(DirectFrame):
    def __init__(self, numRacers, race, *args, **kwargs):
        ...
        self.results = RaceResultsPanel(
            numRacers, race, self, parent=self, pos=(0, 0, 0.525)
        )
    ...
    def displayRacer(
        ...
        self.results.displayRacer(
            place,
            headFrame,
            name,
            time,
            qualify,
            entryFee + winnings + bonus,
            bonus,
            trophies,
            circuitPoints,
            circuitTime,
    ...
    def startWinningsPanel(
        self, entryFee, winnings, track, bonus=None, trophies=(), endOfCircuitRace=False
    ):
        if not self.enabled:
            return
        taskMgr.remove("showExitButton")
        ...
        tSeq, wSeq = self.winnings.generateDisplaySequences(
            track, entryFee, winnings, bonus, trophies, endOfCircuitRace
        )
        ...
        if self.race.raceType != RaceGlobals.Circuit:
            if self.seq.getDuration() < 5.0:
                ...
                taskMgr.doMethodLater(
                    self.seq.getDuration(), showButton, "showExitButton", extraArgs=[]
                )
        self.seq.start()
        return
