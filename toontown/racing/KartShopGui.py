...
class KartShopGuiMgr(DirectObject.DirectObject):
    ...
    class ReturnKartDlg(DirectFrame):
        ...
        def __init__(self, doneEvent):
            ...
            self.kartView.setBounds(-0.38, 0.38, -0.04, 0.49)
            self.kartView.setBgColor(1.0, 1.0, 0.8, 1.0)
            self.kartView.show()
            model.removeNode()
            self.initialize = True
    ...
    class BoughtKartDlg(DirectFrame):
        ...
        def __init__(self, doneEvent, kartID):
            ...
            self.kartView.setBounds(-0.38, 0.38, -0.0425, 0.49)
            self.kartView.setBgColor(1.0, 1.0, 0.8, 1.0)
            self.kartView.show()
            model.removeNode()
            self.initialize = True
    ...
    class ConfirmBuyKartDlg(DirectFrame):
        notify = DirectNotifyGlobal.directNotify.newCategory(
            "KartShopGuiMgr.ConfirmBuyKartDlg"
        )
    ...
    class BuyAccessoryDlg(DirectFrame):
        notify = DirectNotifyGlobal.directNotify.newCategory(
            "KartShopGuiMgr.buyAccessoryDlg"
        )
        ...
        def __handleAccessoryChange(self, nDir):
            if len(self.unownedAccDict[self.curAccType]) < 1:
                ...
                self.curAccIndex[self.curAccType] = (
                    self.curAccIndex[self.curAccType] + nDir
                ) % len(self.unownedAccDict[self.curAccType])
        ...
        def showAccessory(self):
            ...
            if len(self.unownedAccDict[self.curAccType]) < 1:
                ...
                if self.curAccIndex[self.curAccType] + 1 < len(
                ...
                for d in range(len(curDNA)):
                    if (
                        d == KartDNA.bodyType
                        or d == KartDNA.accColor
                        or d == KartDNA.bodyColor
                    ):
                        continue
                    else:
                        curDNA[d] = -1

                curAcc = self.unownedAccDict[self.curAccType][
                    self.curAccIndex[self.curAccType]
                ]
            ...
            if (
                len(base.localAvatar.getKartAccessoriesOwned())
    ...
    class BoughtAccessoryDlg(DirectFrame):
        notify = DirectNotifyGlobal.directNotify.newCategory(
            "KartShopGuiMgr.BoughtAccessoryDlg"
        )
        ...
        def __init__(self, doneEvent, accID):
            ...
            if accType in [
                ...
                tex = loader.loadTexture(
                    "phase_6/maps/%s.jpg" % texNodePath,
    ...
    class ConfirmBuyAccessoryDlg(DirectFrame):
        notify = DirectNotifyGlobal.directNotify.newCategory(
            "KartShopGuiMgr.ConfirmBuyAccessoryDlg"
        )
        ...
        def __init__(self, doneEvent, accID):
            ...
            cm = CardMaker("accViewer")
            cm.setFrame(
                xCenter - radius, xCenter + radius, yCenter - radius, yCenter + radius
            )
            self.kartView["geom"] = NodePath(cm.generate())
            self.kartView.component("geom0").setColorScale(1.0, 1.0, 0.8, 1.0)
            self.kartView.component("geom0").setTransparency(True)
        ...
        if __name__ == "__main__":
            ...
            class Main(DirectObject.DirectObject):
                def __init__(self):
                    ...
                    self.accept(
                        KartShopGlobals.EVENTDICT["buyKart"], self.__handleBuyKart
                    )

                def __popupKartShopGui(self):
                    if not hasattr(self, "kartShopGui") or self.kartShopGui == None:
                        self.acceptOnce(
                            KartShopGlobals.EVENTDICT["guiDone"], self.__handleGuiDone
                        )
