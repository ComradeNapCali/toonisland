...
class PetLookerAI:
    ...
    def destroy(self):
        ...
        if len(self.others):
            PetLookerAI.notify.warning(
                "%s: self.others not empty: %s" % (self.doId, list(self.others.keys()))
            )
            self.others = {}
    ...
    def enterPetLook(self):
        ...
        if len(self.others):
            PetLookerAI.notify.warning(
                "%s: len(self.others) != 0: %s" % (self.doId, list(self.others.keys()))
            )
            self.others = {}
        self.__active = 1
    ...
    def _createPetLookSphere(self):
        ...
        if isPet:
            ...
            fromCollideMask = (
                ToontownGlobals.PetLookatPetBitmask
    ...
    def _handleZoneChange(self, newZoneId, oldZoneId):
        ...
        if newZoneData.hasCollTrav():
            newZoneData.getCollTrav().addCollider(
    ...
    def _handleLookingAtOtherStart(self, other):
        ...
        PetLookerAI.notify.debug(
            "_handleLookingAtOtherStart: %s looking at %s" % (self.doId, other)
        )
        if other in self.others:
            PetLookerAI.notify.warning(
                "%s: other (%s) is already in self.others!" % (self.doId, other)
            )
    ...
    def _handleLookingAtOtherStop(self, other):
        ...
        if isinstance(other, CollisionEntry):
            ...
            if other == 0:
                ...
                return
        PetLookerAI.notify.debug(
            "_handleLookingAtOtherStop: %s no longer looking at %s" % (self.doId, other)
        )
