...
from dependencies.libtoontown import *
...
class DistributedPetAI(
    ...
    def _initDBVals(
        ...
        traitSeed=0,
        dna=None,
        safeZone=ToontownGlobals.ToonIslandCentral,
    ):
    ...
    def announceZoneChange(self, newZoneId, oldZoneId):
        DistributedPetAI.notify.debug(
            "%s.announceZoneChange: %s->%s" % (self.doId, oldZoneId, newZoneId)
        )
    ...
    def announceGenerate(self):
        ...
        taskMgr.doMethodLater(
            simbase.petMovePeriod * random.random(), self.move, self.getMoveTaskName()
        )
    ...
    def _doDeleteCleanup(self):
        ...
        if hasattr(self, "zoneId"):
            ...
            simbase.air.writeServerEvent(
                "Pet RequestDelete duplicate", myDoId, "from task %s" % myTaskName
            )
            simbase.air.writeServerEvent(
                "Pet RequestDelete duplicate StackTrace", myDoId, "%s" % myStackTrace
            )
            simbase.air.writeServerEvent(
    ...
    def feed(self, avatar):
        if avatar.takeMoney(PetConstants.FEED_AMOUNT):
            ...
            self.brain.observe(
                PetObserve.PetActionObserve(PetObserve.Actions.FEED, avatar.doId)
            )
    ...
    def scratch(self, avatar):
        self.startLockPetMove(avatar.doId)
        self.brain.observe(
            PetObserve.PetActionObserve(PetObserve.Actions.SCRATCH, avatar.doId)
        )
    ...
    def handleShoo(self, avatar):
        self.brain.observe(
            PetObserve.PetPhraseObserve(PetObserve.Phrases.GO_AWAY, avatar.doId)
        )
    ...
    def d_setMovie(self, avId, flag):
        self.sendUpdate(
            "setMovie", [flag, avId, ClockDelta.globalClockDelta.getRealNetworkTime()]
        )
    ...
    def handleAvPetInteraction(self, mode, avId):
        if mode not in (
            ...
            self.air.writeServerEvent(
                "suspicious", avId, "DistributedPetAI: unknown mode: %s" % mode
            )
            return
    ...
    def __petMovieStart(self, avId):
        ...
        taskMgr.doMethodLater(
            time, self.__petMovieComplete, self.uniqueName("PetMovieComplete")
        )
