...
class PetBrain(DirectObject.DirectObject, CPetBrain):
    ...
    def destroy(self):
        ...
        if self.doId2goals:
            self.notify.warning(
                "destroy(): self.doId2goals is not empty: %s"
                % list(self.doId2goals.keys())
            )
        ...
        if not self.inMovie:
            ...
            if tSinceLastLonelinessUpdate >= PetConstants.LonelinessUpdatePeriod:
                ...
                if numLookers:
                    ...
                    if numLookers > 5:
                        self.pet.lerpMood(
                            "excitement", min(1.0, dt * 0.001 * numLookers)
                        )
        if __dev__:
            self.pscResc.start()
        taskMgr.doMethodLater(
            simbase.petThinkPeriod, self._think, self.getThinkTaskName()
        )
        if __dev__:
            self.pscResc.stop()
        return Task.done
    ...
    def _handleLookingAtOtherStart(self, avId):
        ...
        self.observe(
            PetObserve.PetActionObserve(PetObserve.Actions.ATTENDING_START, avId)
        )
    ...
    def _handleLookingAtOtherStop(self, avId):
        ...
        self.observe(
            PetObserve.PetActionObserve(PetObserve.Actions.ATTENDING_STOP, avId)
        )
    ...
    def _handleLookedAtByOtherStart(self, avId):
        ...
        self.observe(
            PetObserve.PetActionObserve(PetObserve.Actions.ATTENDED_START, avId)
        )
    ...
    def _handleLookedAtByOtherStop(self, avId):
        ...
        self.observe(
            PetObserve.PetActionObserve(PetObserve.Actions.ATTENDED_STOP, avId)
        )
    ...
    def _handleActionObserve(self, observe):
        ...
        if action == OA.ATTENDED_START:
            ...
            self.pet.lerpMoods(
                {"boredom": -0.1, "excitement": 0.05, "loneliness": -0.05}
            )
            ...
            if avId != self.pet.doId:
                ...
                PetBrain.notify.debug(
                    "%s.CHANGE_ZONE: %s, %s->%s"
                    % (self.pet.doId, avId, oldZoneId, newZoneId)
                )
                ...
                if self.pet.inEstate:
                    if avId in (self.pet.ownerId, self.pet.estateOwnerId):
                        if (
                            oldZoneId in self.pet.estateZones
                            and newZoneId not in self.pet.estateZones
                        ):
    ...
    def _handleOwnerLeave(self):
        ...
        taskMgr.doMethodLater(
            PetConstants.TELEPORT_OUT_DURATION,
