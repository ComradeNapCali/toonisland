...
class PetMood:
    ...
    def __init__(self, pet=None):
        ...
        self.tBoredom = calcDrift(
            PetMood.TBoredom, pet.traits.traits["boredomThreshold"]
        )
        ...
        self.tLoneliness = calcDrift(
            PetMood.TLoneliness, pet.traits.traits["lonelinessThreshold"]
        )
        self.tSadness = calcDrift(
            PetMood.TSadness, pet.traits.traits["sadnessThreshold"], True
        )
        self.tFatigue = calcDrift(
            PetMood.TFatigue, pet.traits.traits["fatigueThreshold"], True
        )
        ...
        self.tExcitement = calcDrift(
            PetMood.TExcitement, pet.traits.traits["excitementThreshold"]
        )
        self.tSurprise = calcDrift(
            PetMood.TSurprise, pet.traits.traits["surpriseThreshold"], True
        )
        self.tAffection = calcDrift(
            PetMood.TAffection, pet.traits.traits["affectionThreshold"]
        )
        self.tAngerDec = calcDrift(
            PetMood.TAngerDec, pet.traits.traits["angerThreshold"], True
        )
        self.tAngerInc = calcDrift(
            PetMood.TAngerInc, pet.traits.traits["angerThreshold"]
        )
    ...
    def driftMood(self, dt=None, curMood=None):
        ...
        if abuse < tipPoint:
            ...
            tAnger = lerp(
                PetMood.LONGTIME, self.tAngerInc, (abuse - tipPoint) / (1.0 - tipPoint)
            )
