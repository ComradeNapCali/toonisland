...
class PetTraits:
    class StdIncDistrib(TraitDistribution):
        ...
        Sz2MinMax = {
            ToontownGlobals.ToonIslandCentral: (0.2, 0.65),
            ToontownGlobals.RainbowRise: (0.3, 0.7),
            ToontownGlobals.WitheringWoods: (0.4, 0.75),
            ToontownGlobals.OliveOasis: (0.5, 0.8),
            ToontownGlobals.CirrusCircus: (0.6, 0.85),
            ToontownGlobals.MintyMines: (0.7, 0.9),
        }

    class StdDecDistrib(TraitDistribution):
        TraitType = TraitDistribution.TraitTypes.DECREASING
        Sz2MinMax = {
            ToontownGlobals.ToonIslandCentral: (0.35, 0.8),
            ToontownGlobals.RainbowRise: (0.3, 0.7),
            ToontownGlobals.WitheringWoods: (0.25, 0.6),
            ToontownGlobals.OliveOasis: (0.2, 0.5),
            ToontownGlobals.CirrusCircus: (0.15, 0.4),
            ToontownGlobals.MintyMines: (0.1, 0.3),
        }

    class ForgetfulnessDistrib(TraitDistribution):
        TraitType = TraitDistribution.TraitTypes.DECREASING
        Sz2MinMax = {
            ToontownGlobals.ToonIslandCentral: (0.0, 1.0),
            ToontownGlobals.RainbowRise: (0.0, 0.9),
            ToontownGlobals.WitheringWoods: (0.0, 0.8),
            ToontownGlobals.OliveOasis: (0.0, 0.7),
            ToontownGlobals.CirrusCircus: (0.0, 0.6),
            ToontownGlobals.MintyMines: (0.0, 0.5),
        }
    ...
    def __init__(self, traitSeed, safeZoneId, traitValueList=[]):
        ...
        for trait in list(self.traits.values()):
            ...
            while (
                i < len(extremeTraits)
                and extremeTraits[i].howExtreme > trait.howExtreme
            ):
                i += 1
    ...
    def getExtremeTraitDescriptions(self):
        ...
        for name, quality in self.extremeTraits:
            descs.append(
                TTLocalizer.PetTrait2descriptions[name][Quality2index[quality]]
            )
