...
class PetshopGUI(DirectObject):
    ...
    class NamePicker(DirectFrame):
        ...
        def rebuildNameList(self):
            ...
            self.nameList = self.makeScrollList(
                self.gui,
                (0.277, 0, -0.075),
                (1, 0.8, 0.8, 1),
                self.curNames,
                self.makeLabel,
                [TextNode.ACenter, "name"],
                5,
            )
        ...
        def makeLabel(self, te, index, others):
            ...
            df = DirectButton(
                ...
                text_align=alig,
                textMayChange=0,
                command=lambda: self.nameClickedOn(listName, index),
            )
            return df
        ...
        def makeScrollList(
            ...
            ds = DirectScrolledList(
                ...
                pos=ipos,
                scale=0.44,
                incButton_image=arrowList,
                incButton_image_pos=(1.015, 0, 3.32),
                incButton_relief=None,
                incButton_scale=incScale,
                incButton_image3_color=Vec4(0.4, 0.4, 0.4, 1),
                decButton_image=arrowList,
                decButton_image_pos=(1.015, 0, 1.11),
                decButton_relief=None,
                decButton_scale=decScale,
                decButton_image3_color=Vec4(0.4, 0.4, 0.4, 1),
                numItemsVisible=nVisibleItems,
                forceHeight=fHeight,
            )
            return ds
    ...
    class MainMenuDlg(DirectFrame):
        ...
        def __init__(self, doneEvent):
            ...
            DirectFrame.__init__(
                ...
                state="normal",
                geom=model,
                geom_scale=(modelScale, modelScale, modelScale),
                pos=modelPos,
                frameSize=(-1, 1, -1, 1),
            )
            ...
            self.cancelButton = DirectButton(
                ...
                text_pos=TTLocalizer.PGUIcancelButtonPos,
                text_scale=0.8,
                pressEffect=False,
                command=lambda: messenger.send(doneEvent, [0]),
            )
            self.sellFishButton = DirectButton(
                parent=self,
                relief=None,
                image=sellFishImageList,
                image3_color=disabledImageColor,
                geom=fishLogoImageList,
                scale=(modelScale, modelScale, modelScale),
                text=TTLocalizer.PetshopSell,
            ...
            self.adoptPetButton = DirectButton(
                parent=self,
                relief=None,
                image=adoptImageList,
                geom=pawLogoAdoptImageList,
                scale=(modelScale, modelScale, modelScale),
                text=TTLocalizer.PetshopAdoptAPet,
                text_scale=textScale,
                text_pos=(0, 12.5),
                text0_fg=text0Color,
                text1_fg=text1Color,
                ...
                text3_fg=text3Color,
                pressEffect=False,
                command=lambda: messenger.send(doneEvent, [2]),
            )
            self.returnPetButton = DirectButton(
                parent=self,
                relief=None,
                image=returnImageList,
                geom=pawLogoReturnImageList,
                image3_color=disabledImageColor,
                scale=(modelScale, modelScale, modelScale),
                text=TTLocalizer.PetshopReturnPet,
                text_scale=textScale,
                text_pos=(-0.6, 9.2),
                text0_fg=text2Color,
                text1_fg=text2Color,
                ...
                text3_fg=text3Color,
                pressEffect=False,
                command=lambda: messenger.send(doneEvent, [3]),
            )
    ...
    class AdoptPetDlg(DirectFrame):
        ...
        def __init__(self, doneEvent, petSeed, petNameIndex):
            ...
            DirectFrame.__init__(
                ...
                frameSize=(-1, 1, -1, 1),
                pos=modelPos,
                text=TTLocalizer.PetshopAdoptConfirm % (name, cost),
                text_wordwrap=12,
                text_scale=0.05,
                text_pos=(0, 0.55),
                text_fg=text0Color,
        ...
        def __init__(self, doneEvent):
            def showDialog(avatar):
                ...
                self.okButton = DirectButton(
                    ...
                    geom=checkIcon,
                    scale=modelScale,
                    text=("", TTLocalizer.PetshopReturn),
                    text_pos=(5.8, 4.4),
                    text_scale=0.7,
                    pressEffect=False,
                    command=lambda: messenger.send(doneEvent, [1]),
        ...
        def makePetList(self):
            ...
            for i in range(self.numPets):
                ...
                if traitList:
                    ...
                    descList.append(
                        TTLocalizer.PetshopDescTrait % TTLocalizer.PetshopDescStandard
                    )
                traitList.extend(["", "", "", ""])
    ...
    def doDialog(self, nDialog):
        ...
        if nDialog == Dialog_MainMenu:
            ...
            self.dialog = self.AdoptPetDlg(
                self.adoptPetDoneEvent,
    ...
    def __handleAdoptPetDlg(self, exitVal):
        if exitVal == 0:
            ...
            self.destroyDialog()
            messenger.send(
                self.eventDict["petAdopted"], [self.adoptPetNum, self.adoptPetNameIndex]
            )
            messenger.send(self.eventDict["guiDone"])
