...
class TownBattleToonPanel(DirectFrame):
    ...
    def __init__(self, id):
        ...
        self.whichText = DirectLabel(
            parent=self, text="", pos=(0.1, 0, -0.08), text_scale=0.05
        )
        self.hide()
        gui.removeNode()
        return
    ...
    def setLaffMeter(self, avatar):
        ...
        if self.avatar == avatar:
            messenger.send(
                self.avatar.uniqueName("hpChange"), [avatar.hp, avatar.maxHp, 1]
            )
            return None
        else:
            if self.avatar:
    ...
    def setValues(
        self, index, track, level=None, numTargets=None, targetIndex=None, localNum=None
    ):
        self.notify.debug(
        ...
        if track == BattleBase.NO_ATTACK or track == BattleBase.UN_ATTACK:
            ...
            track == BattleBase.SOS
            or track == BattleBase.NPCSOS
            or track == BattleBase.PETSOS
        ):
            self.sosText.show()
