...
class CatalogAtticItem(CatalogItem.CatalogItem):
    ...
    def getHouseInfo(self, avatar):
        ...
        if not house:
            self.notify.warning(
                "House %s (for avatar %s) not instantiated." % (houseId, avatar.doId)
            )
            return (None, ToontownGlobals.P_InvalidIndex)
        numAtticItems = (
            len(house.atticItems) + len(house.atticWallpaper) + len(house.atticWindows)
        )
        numHouseItems = numAtticItems + len(house.interiorItems)
        if (
            numHouseItems >= ToontownGlobals.MaxHouseItems
            and not self.replacesExisting()
        ):
