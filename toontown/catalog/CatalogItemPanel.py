...
class CatalogItemPanel(DirectFrame):
    ...
    def load(self):
        ...
        self.auxText = DirectLabel(
            parent=self, relief=None, scale=0.05, pos=(-0.2, 0, 0.16)
        )
    ...
    def getTeaserPanel(self):
        ...
        if (
            typeName == TTLocalizer.EmoteTypeName
            or typeName == TTLocalizer.ChatTypeName
        ):
            page = "emotions"
        elif (
            typeName == TTLocalizer.GardenTypeName
            or typeName == TTLocalizer.GardenStarterTypeName
        ):
            page = "gardening"
        else:
            page = "clothing"
    ...
    def updateBuyButton(self):
        ...
        if isNameTag and not localAvatar.getGameAccess() == OTPGlobals.AccessFull:
            ...
            self.buyButton["state"] = DGG.DISABLED
        elif hasattr(self["item"], "isSkillTooLow") and self["item"].isSkillTooLow(
            base.localAvatar
        ):
            ...
            "item"
        ].getEmblemPrices() and not base.localAvatar.isEnoughMoneyAndEmblemsToBuy(
            self["item"].getPrice(self["type"]), self["item"].getEmblemPrices()
        ):
            self.buyButton["state"] = DGG.DISABLED
        elif (
            self["item"].getPrice(self["type"])
        ...
        if self["item"].replacesExisting() and self["item"].hasExisting():
            ...
            if emblemPrices:
                ...
                if price and silver and gold:
                    ...
                    self.notify.warning(
                        "is this a completely free item %s?" % self["item"].getName()
                    )
                    message = TTLocalizer.CatalogVerifyPurchase % {
                        "item": self["item"].getName(),
                        "price": self["item"].getPrice(self["type"]),
    ...
    def __handleGiftRequest(self):
        ...
        self.verify = TTDialog.TTGlobalDialog(
            doneEvent="verifyGiftDone", message=message, style=TTDialog.TwoChoice
        )
        self.verify.show()
    ...
    def updateGiftButton(self, giftUpdate=0):
        ...
        if numFriends > 0:
            ...
            if self["item"].isGift() <= 0:
                ...
                if (
                    self["item"].forBoysOnly()
                    and avatar.getStyle().getGender() == "f"
                    ...
                    len(avatar.mailboxContents) + len(avatar.onGiftOrder)
