...
NUM_CATALOG_COLS = 2
CatalogPanelCenters = [
    [Point3(-0.95, 0, 0.91), Point3(-0.275, 0, 0.91)],
    [Point3(-0.95, 0, 0.275), Point3(-0.275, 0, 0.275)],
    [Point3(-0.95, 0, -0.4), Point3(-0.275, 0, -0.4)],
]
CatalogPanelColors = {
    CatalogItemTypes.FURNITURE_ITEM: Vec4(0.733, 0.78, 0.933, 1.0),
...
class CatalogScreen(DirectFrame):
    ...
    def showNextPage(self):
        ...
        if (
            self.viewing == "New"
            and self.pageIndex > self.maxPageIndex
            and self.numBackPages > 0
        ):
            self.showBackorderItems()
        if (
            self.viewing == "New"
            and self.pageIndex > self.maxPageIndex
            and self.numLoyaltyPages > 0
        ):
    ...
    def showBackPage(self):
        ...
        if self.viewing == "Backorder" and self.pageIndex < 0 and self.numNewPages > 0:
            ...
            self.viewing == "Emblem" and self.pageIndex < 0 and self.numLoyaltyPages > 0
        ):
    ...
    def packPages(self, panelList, pageList, prefix):
        ...
        for item in panelList:
            ...
            itemList = self.panelDict.get(page.getKey(), [])
            itemList.append(item)
            self.panelDict[page.getKey()] = itemList
            j += 1
    ...
    def load(self, guiItems, guiButton, guiBack):
        ...
        self.newCatalogButton = DirectButton(
            ...
            text=TTLocalizer.CatalogNew,
            text_font=ToontownGlobals.getSignFont(),
            text_pos=(-0.4 - lift, 0.13),
            text3_pos=(-0.4 - lift, 0.1),
            text_scale=0.08,
        ...
        self.backCatalogButton = DirectButton(
            ...
            text=TTLocalizer.CatalogBackorder,
            text_font=ToontownGlobals.getSignFont(),
            text_pos=(0.25 - lift, 0.132),
            text3_pos=(0.25 - lift, 0.112),
            text_scale=TTLocalizer.CSbackCatalogButton,
            text_fg=(0.392, 0.549, 0.627, 1.0),
            text2_fg=(0.392, 0.349, 0.427, 1.0),
        )
        self.backCatalogButton.hide()
        ...
        for item in itemList:
            if (
                item in allClosetItems
                and item.furnitureType in CatalogFurnitureItem.MaxClosetIds
            ):
        ...
        for item in itemList:
            ...
            if (
                isMaxClosetOfferred
                and item in allClosetItems
                and item.furnitureType not in CatalogFurnitureItem.MaxClosetIds
            ):
                continue
        ...
        numPages = self.packPages(
            self.loyaltyPanelList, self.loyaltyPageList, "loyalty"
        )
        ...
        self.hangup.setBin("gui-popup", 0)
    ...
    def loadClarabelle(self):
        ...
        self.clarabelle.setPosHprScale(
            -0.56, 6.43, -3.81, 121.61, 0.0, 0.0, 1.0, 1.0, 1.0
        )
        self.clarabelleFrame.setPosHprScale(
    ...
    def reload(self):
        ...
        for item in itemList:
            if item.loyaltyRequirement() != 0:
                ...
                self.panelList.append(
                    CatalogItemPanel.CatalogItemPanel(
                        parent=hidden, item=item, type=CatalogItem.CatalogTypeWeekly
                    )
                )
        ...
        for item in itemList:
            if item.loyaltyRequirement() != 0:
                ...
                self.backPanelList.append(
                    CatalogItemPanel.CatalogItemPanel(
                        parent=hidden, item=item, type=CatalogItem.CatalogTypeBackorder
                    )
                )
        ...
        self.setNumLoyaltyPages(numPages)
        seriesNumber = (
            base.localAvatar.catalogScheduleCurrentWeek - 1
        ) // ToontownGlobals.CatalogNumWeeksPerSeries + 1
    ...
    def __handleGiftPurchaseRequest(self, item):
        item.requestGiftPurchase(
            self["phone"], self.frienddoId, self.__handleGiftPurchaseResponse
        )
        taskMgr.remove("clarabelleAskAnythingElse")
    ...
    def __handlePurchaseResponse(self, retCode, item):
        ...
        self.setClarabelleChat(
            item.getRequestPurchaseErrorText(retCode),
    ...
    def setClarabelleChat(self, str, timeout=6):
        ...
        if timeout:
            taskMgr.doMethodLater(
                timeout, self.clearClarabelleChat, "clearClarabelleChat"
            )
    ...
    def __loadFriend(self):
        ...
        if self.frienddoId and self.allowGetDetails:
            ...
            base.cr.getAvatarDetails(
                self.giftAvatar, self.__handleAvatarDetails, "DistributedToon"
            )
            self.gotAvatar = 0
