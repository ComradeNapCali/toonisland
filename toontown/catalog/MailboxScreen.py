...
class MailboxScreen(DirectObject.DirectObject):
    ...
    def load(self):
        self.accept(
            "setMailboxContents-%s" % base.localAvatar.doId, self.__refreshItems
        )
        self.accept(
            "setAwardMailboxContents-%s" % base.localAvatar.doId, self.__refreshItems
        )
    ...
    def __handleAccept(self):
        ...
        if not base.cr.isPaid() and not (isinstance(item, InviteInfoBase) or isAward):
            ...
            item = self.items[self.itemIndex]
            item.acceptItem(
                self.mailbox, self.acceptingIndex, self.__acceptItemCallback
            )
        return
    ...
    def __handleDiscard(self, buttonValue=None):
        if self.acceptingIndex is not None:
            ...
            item = self.items[self.itemIndex]
            item.discardItem(
                self.mailbox, self.acceptingIndex, self.__discardItemCallback
            )
        return
    ...
    def __showCurrentItem(self):
        ...
        if self.itemIndex == self.acceptingIndex:
            self.gettingText[
                "text"
            ] = TTLocalizer.MailboxGettingItem % self.getItemName(item)
            ...
            if item.isAward():
                ...
                if item.giftCode == ToontownGlobals.GIFT_RAT:
                    ...
                    self.giftTagPanel["text"] = (
                        TTLocalizer.MailboxGiftTag % nameOfSender
                    )
            else:
                self.giftTagPanel["text"] = ""
        ...
        if self.itemPanel and item.getTypeName() != TTLocalizer.ChatTypeName:
            ...
            isinstance(item, CatalogItem.CatalogItem)
            and item.getTypeName() == TTLocalizer.ChatTypeName
        ):
            self.itemPanel.reparentTo(self.itemBoard, -1)
