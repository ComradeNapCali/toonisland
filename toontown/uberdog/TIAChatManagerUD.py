...
class TIAChatManagerUD(DistributedObjectGlobalUD):
    notify = DirectNotifyGlobal.directNotify.newCategory("TIAChatManagerUD")
    ...
    def chatMessage(self, message):
        ...
        if not avId:
            self.air.writeServerEvent(
                "suspicious",
                accId=accId,
                issue="Account sent chat without an avatar!",
                message=message,
            )
            return
    ...
    def whisperMessage(self, message, receiverAvId):
        ...
        def handleAvatar(dclass, fields):
            ...
            self.air.send(datagram)
            self.air.writeServerEvent(
                "whisper-message-said",
                avId=avId,
                receiverAvId=receiverAvId,
                message=message,
                filteredMessage=filteredMessage,
            )
    ...
    def filterWhiteList(self, message):
        ...
        for modStart, modStop in modifications:
            filteredMessage = (
                filteredMessage[:modStart]
                + "*" * (modStop - modStart + 1)
                + filteredMessage[modStop + 1 :]
            )
