...
class SetNamePatternOperation(AvatarOperation):
    ...
    def enterSetName(self):
        ...
        for p, f in self.pattern:
            part = self.gameServicesManager.nameGenerator.nameDictionary.get(
                p, ("", "")
            )[1]
            if f:
        ...
        self.gameServicesManager.air.dbInterface.updateObject(
            ...
            {"WishNameState": ("LOCKED",), "WishName": ("",), "setName": (name,)},
        )
        ...
        self.gameServicesManager.sendUpdateToAccountId(
            self.target, "namePatternResponse", [self.avId, 1]
        )
        self.demand("Off")
    ...
    def enterJudgeName(self):
        ...
        if self.avId and status:
            ...
            self.gameServicesManager.air.dbInterface.updateObject(
                ...
                self.avId,
                self.gameServicesManager.air.dclassesByName["DistributedToonUD"],
                {"WishNameState": ("PENDING",), "WishName": (self.name,)},
            )
...
class CreateAvatarOperation(GameOperation):
    ...
    def __handleStoreAvatar(self, fields):
        ...
        self.gameServicesManager.air.writeServerEvent(
            "avatar-created", self.avId, self.target, self.dna.hex(), self.index
        )
        self.gameServicesManager.sendUpdateToAccountId(
            self.target, "createAvatarResponse", [self.avId]
        )
        self.demand("Off")
...
class AcknowledgeNameOperation(AvatarOperation):
    ...
    def __handleAvatar(self, dclass, fields):
        ...
        if wishNameState == "APPROVED":
            ...
            self.demand(
                "Kill",
                "Tried to acknowledge name on an avatar in invalid state (%s) !"
                % wishNameState,
            )
            return
        ...
        self.gameServicesManager.sendUpdateToAccountId(
            self.target, "acknowledgeAvatarNameResponse", []
        )
        self.demand("Off")
...
class TTGameServicesManagerUD(GameServicesManagerUD):
    ...
    def setNamePattern(self, avId, p1, f1, p2, f2, p3, f3, p4, f4):
        ...
        self.runOperation(
            SetNamePatternOperation, avId, [(p1, f1), (p2, f2), (p3, f3), (p4, f4)]
        )
