...
class CogdoFlyingLevelQuadrant:
    ...
    def _initPlatforms(self, parent):
        platformModels = self._model.findAllMatches(
            "**/%s" % Globals.Level.PlatformName
        )
    ...
    def generateGatherables(
            ...
            for gatherPath in gatherPaths:
                ...
                while t < mopath.getMaxT():
                    ...
                    while angle < 360.0:
                        ...
                        mopath.goTo(pickup, t)
                        pickup.setX(
                            parent, pickup.getX() + r * math.cos(math.radians(angle))
                        )
                        pickup.setZ(
                            parent, pickup.getZ() + r * math.sin(math.radians(angle))
                        )
                        self.gatherables.append(pickup)
                        angle += angleSpread
        ...
        def generatePowerUps():
            for powerupType, locName in Globals.Level.PowerupType2Loc.items():
                if (
                    powerupType == Globals.Level.GatherableTypes.LaffPowerup
                    and Globals.Level.IgnoreLaffPowerups
                ):
                    continue
        ...
        def initWhirlwinds():
            ...
            for obstacleLoc in obstacles:
                motionPath = self._model.find(
                    "**/%s%s" % (obstacleLoc.getName(), Globals.Level.WhirlwindPathName)
                )
