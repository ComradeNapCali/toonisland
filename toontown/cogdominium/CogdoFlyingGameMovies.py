    ...
    def load(self):
        ...
        self._ival = Sequence(
            ...
            Parallel(
                flyThroughIval,
                Sequence(
                    Func(self.displayLine, "cog", self._getRandomLine(dialogue[0])),
                    Wait(waitDur),
                    Func(self.displayLine, "toon", self._getRandomLine(dialogue[1])),
                    Wait(waitDur),
                    Func(self.displayLine, "cog", self._getRandomLine(dialogue[2])),
                    Wait(waitDur),
                ),
            ),
            Func(end),
...
class CogdoFlyingGameFinish(CogdoGameMovie):
    ...
    def load(self):
        ...
        showExitIval = Sequence(
            ...
            Parallel(
                camera.posInterval(
                    exitDur, Point3(0, -55, 40), other=self._exit, blendType="easeInOut"
                ),
