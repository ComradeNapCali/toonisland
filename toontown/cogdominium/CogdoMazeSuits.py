...
class CogdoMazeSuit(MazeSuit, FSM, CogdoMazeSplattable):
    ...
    def __init__(
        ...
        CogdoMazeSplattable.__init__(
            self, self.suit, "%s-%i" % (Globals.SuitCollisionName, self.serialNum), 1.5
        )
        if "scale" in data:
    ...
    def _initSfx(self):
        ...
        self._deathSoundIval = Sequence(
            ...
            audioMgr.createSfxIval("cogDeath", volume=0.32, source=self.deathSuit),
        )
    ...
    def initCollisions(self):
        ...
        self.accept(
            self.uniqueName("again" + self.COLL_SPHERE_NAME), self.handleEnterSphere
        )
    ...
    def _getSuitAnimationIval(
        self, animName, startFrame=0, duration=1, partName=None, nextState=None
    ):
    ...
    def doDeathTrack(self):
        ...
        gears1Track = Sequence(
            ParticleInterval(
                smallGears, self.deathSuit, worldRelative=0, duration=4.3, cleanup=True
            ),
            name="gears1Track",
        )
        gears2MTrack = Track(
        ...
        removeParticles = Sequence(
            Func(removeParticle, smallGears),
        ...
        self.deathTrack = Sequence(
            Parallel(suitTrack, gears2MTrack, gears1Track, self._deathSoundIval),
            removeParticles,
        )
...
class CogdoMazeBossSuit(CogdoMazeSuit):
    ...
    def _initSfx(self):
        ...
        self._stompSfxIval = audioMgr.createSfxIval(
            "cogStomp", source=self.suit, cutoff=Globals.BossStompSfxCutoff, volume=0.3
        )
    ...
    def spin(self):
        ...
        spinIval = LerpHprInterval(
            part, time, (self.suit.getH() + degrees, 0, 0), blendType="easeOut"
        )
        spinIval.start()
    ...
    def __startBlinkTask(self):
        self.__stopBlinkTask()
        taskMgr.doMethodLater(
            Globals.BlinkFrequency, self.__blink, CogdoMazeBossSuit.BlinkTaskName
        )
    ...
    def __blink(self, task):
        blink = Sequence(
            LerpColorScaleInterval(
                self.suit, Globals.BlinkSpeed, VBase4(1.0, 1.0, 1.0, 1.0)
            ),
            LerpColorScaleInterval(self.suit, Globals.BlinkSpeed, Globals.BlinkColor),
        )
        blink.start()
        return Task.again
