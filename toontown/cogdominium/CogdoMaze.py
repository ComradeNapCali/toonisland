...
class CogdoMazeFactory:
    ...
    def _generateBarrierData(self):
        ...
        def getAvailableDirections(ax, ay, ignore=None):
            ...
            if (
                ax - 1 >= 0
                and data[ay][ax - 1][BARRIER_DATA_RIGHT] == 1
                and (ax, ay) != ignore
            ):
                dirs.append(dirLeft)
            if (
                ax + 1 < self.width
                and data[ay][ax][BARRIER_DATA_RIGHT] == 1
                and (ax, ay) != ignore
            ):
                dirs.append(dirRight)
            if (
                ay - 1 >= 0
                and data[ay - 1][ax][BARRIER_DATA_TOP] == 1
                and (ax, ay) != ignore
            ):
                dirs.append(dirDown)
            if (
                ay + 1 < self.height
                and data[ay][ax][BARRIER_DATA_TOP] == 1
                and (ax, ay) != ignore
            ):
                dirs.append(dirUp)
            return dirs
    ...
    def _generateMazeData(self):
        ...
        self._data["width"] = (
            self.width + 1
        ) * self.frameWallThickness + self.width * self.quadrantSize
        self._data["height"] = (
            self.height + 1
        ) * self.frameWallThickness + self.height * self.quadrantSize
