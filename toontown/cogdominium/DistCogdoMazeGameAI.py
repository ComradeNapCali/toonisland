...
class DistCogdoMazeGameAI(DistCogdoGameAI, DistCogdoMazeGameBase):
            ...
            if elapsed > 10:
                ...
                if self.requestCount >= 3 and secondsPerGrab <= 0.4:
                    simbase.air.writeServerEvent(
                        "suspicious",
                        avId,
                        "suitHit %s suits in %s seconds" % (self.requestCount, elapsed),
                    )
    ...
    def d_broadcastPickup(self, senderId, pickupNum, networkTime):
        self.sendUpdate("pickUp", [senderId, pickupNum, networkTime])
    ...
    def requestAction(self, action, data):
        ...
        if action == Globals.GameActions.EnterDoor:
            if not self.doorIsOpen:
                self.logSuspiciousEvent(
                    senderId,
                    "CogdoMazeGameAI.requestAction(EnterDoor): door isn't open yet",
                )
            ...
            self.logSuspiciousEvent(
                senderId, "CogdoMazeGameAI.requestAction: invalid action %s" % action
            )

    def d_broadcastDoAction(self, action, data=0, networkTime=0):
        self.sendUpdate("doAction", [action, data, networkTime])
    ...
    def d_broadcastToonHitBySuit(self, toonId, suitType, suitNum, networkTime):
        self.sendUpdate("toonHitBySuit", [toonId, suitType, suitNum, networkTime])
    ...
    def requestGag(self, waterCoolerIndex):
        ...
        threshold = (
            Globals.WaterCoolerTriggerRadius + Globals.PlayerCollisionRadius
        ) * 1.05
    ...
    def d_broadcastHasGag(self, senderId, networkTime):
        self.sendUpdate("hasGag", [senderId, networkTime])
    ...
    def requestPickUp(self, pickupNum):
        ...
        if pickupNum not in self.pickups:
            self.logSuspiciousEvent(
                senderId,
                "CogdoMazeGameAI.requestPickUp: invalid pickupNum %s" % pickupNum,
            )
            return False
        ...
        if not toon:
            simbase.air.writeServerEvent(
                "suspicious", senderId, "CogdoMazeGame.requestPickUp: toon not present?"
            )
            return False
    ...
    def enterGame(self):
        ...
        self._countdownTimerTask = taskMgr.doMethodLater(
            ...
            self.taskName(DistCogdoMazeGameAI.CountdownTimerTaskName),
            [],
        )
        self._startGameTimer()
        self._timeoutTimerTask = taskMgr.doMethodLater(
            endTime,
            self.handleEndGameTimerExpired,
            self.taskName(DistCogdoMazeGameAI.TimeoutTimerTaskName),
            [],
        )
        ...
        for toonId in self.getToonIds():
            ...
            if toon:
                ...
                self._speedMonitor.setSpeedLimit(
                    token,
                    config.GetFloat(
                        "cogdo-maze-speed-limit", Globals.ToonRunSpeed * 1.1
                    ),
                    Functor(self._toonOverSpeedLimit, toonId),
                )

    def _toonOverSpeedLimit(self, toonId, speed):
        self._bootPlayerForHacking(
            toonId,
            "speeding in cogdo maze game (%.2f feet/sec)" % speed,
            config.GetBool("want-ban-cogdo-maze-speeding", 0),
        )

    def _toonHackingRequestGag(self, toonId):
        simbase.air.writeServerEvent(
            "suspicious", toonId, "CogdoMazeGame: toon caught hacking requestGag"
        )
        self._bootPlayerForHacking(
            toonId,
            "hacking cogdo maze game requestGag",
            config.GetBool("want-ban-cogdo-maze-requestgag-hacking", 0),
        )
    ...
    def _bootPlayerForHacking(self, toonId, reason, wantBan):
        ...
        if not toon:
            simbase.air.writeServerEvent(
                "suspicious",
                toonId,
                "CogdoMazeGame._bootPlayerForHacking(%s): toon not present" % (reason,),
            )
            return
    ...
    def _startGameTimer(self):
        self.d_broadcastDoAction(
            Globals.GameActions.Countdown, networkTime=self.getCurrentNetworkTime()
        )
    ...
    def openDoor(self):
        ...
        self.d_broadcastDoAction(
            Globals.GameActions.OpenDoor, networkTime=self.getCurrentNetworkTime()
        )
        self._gameTimerExpiredTask = taskMgr.doMethodLater(
            Globals.SecondsUntilGameEnds,
            self.handleEndGameTimerExpired,
            self.taskName(DistCogdoMazeGameAI.TimerExpiredTaskName),
            [],
        )

    def handleCountdownTimer(self):
        self.d_broadcastDoAction(
            Globals.GameActions.TimeAlert, networkTime=self.getCurrentNetworkTime()
        )
