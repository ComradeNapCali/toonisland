...
class KeyCodes(DirectObject):
    ...
    def __init__(
        self, keyMap=ARROW_KEYCODE_MAP, patterns=None, timeout=KEYCODE_TIMEOUT_SECONDS
    ):
        self._keyMap = keyMap
    ...
    def __handleKeyDown(self, key):
        ...
        if self._keyDown is None and self._keysPressed == 1:
            ...
            messenger.send(
                KeyCodes.KEY_DOWN_EVENT, [self._keyMap[key], self._keyCodeCount]
            )
    ...
    def __updateElapsedTime(self):
        if (
            self._keyCodeTime != 0.0
            and globalClock.getFrameTime() - self._keyCodeTime >= self._timeout
        ):
    ...
    def __checkForPattern(self):
        if self._keyCode in self._patterns:
            ...
            self._keyCodeCount == self._patternLimit
            or len(self.getPossibleMatchesList()) == 0
        ):
