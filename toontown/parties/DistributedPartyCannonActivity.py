...
class DistributedPartyCannonActivity(DistributedPartyActivity):
    ...
    def __fireCannonTask(self, task):
        ...
        if self.isLocalToonId(toonId):
            ...
            if config.GetBool("isclient-check", False):
                if not isClient():
                    print(
                        "EXECWARNING DistributedPartyCannonActivity: %s" % flightResults
                    )
        ...
        if self.isLocalToonId(toonId):
            ...
            hpr = toon.getHpr()
            toon.d_setPosHpr(
                startPos[0], startPos[1], startPos[2], hpr[0], hpr[1], hpr[2]
            )
            ...
            netLaunchTime = globalClockDelta.localToNetworkTime(
                launchTime + frameTime, bits=31
            )
            self.sendUpdate(
                "setToonTrajectoryAi",
                [
    ...
    def setToonTrajectory(self, avId, launchTime, x, y, z, h, p, r, vx, vy, vz):
        ...
        self._avId2trajectoryInfo[avId] = ScratchPad(
            startPos=startPos,
            startHpr=startHpr,
            startVel=startVel,
            startT=startT,
    ...
    def _remoteToonFlyTask(self, task=None):
        ...
        for avId, trajInfo in self._avId2trajectoryInfo.items():
            ...
            groundTime = (
                trajectory.calcTimeOfImpactOnPlane(0.0) / self.TimeFactor + startTime
            )
            now = frameTime
            if now < startTime:
                now = startTime
    ...
    def __updateFlightVelocity(self, trajectory):
        ...
        if self._dirtyNewVel:
            self.sendUpdate(
                "updateToonTrajectoryStartVelAi",
                [self._dirtyNewVel[0], self._dirtyNewVel[1], self._dirtyNewVel[2]],
            )
    ...
    def __isFlightKeyPressed(self):
        return (
            self.gui.leftPressed
            or self.gui.rightPressed
            or self.gui.upPressed
            or self.gui.downPressed
        )
    ...
    def __startCollisionHandler(self):
        self.flyColSphere = CollisionSphere(
            0, 0, self.localFlyingToon.getHeight() / 2.0, 1.0
        )
    ...
    def __handleFlyingToonCollision(self, collisionEntry):
        ...
        self.notify.debug(
            "hitNodePath.getParent = %s" % collisionEntry.getIntoNodePath().getParent()
        )
        ...
        if (
            hitNode.find("cSphere") == 0
            or hitNode.find("treasureSphere") == 0
            ...
            or hitNode == "sign_collision"
            or hitNode == "FlowerSellBox"
        ...
        if (
            vel.dot(hitNormal) > 0
            and not hitNode == "collision_roof"
            and not hitNode == "collision_fence"
        ):
            self.notify.debug(
        ...
        bumperNodes = (
            ["sky_collision"]
            + PartyCannonCollisions["bounce"]
            + PartyCannonCollisions["fence"]
        )
        cloudBumpers = PartyCannonCollisions["clouds"]
        bumperNodes += cloudBumpers
        if (
            hitNode in bumperNodes
            or hitNode.find("cogPie") == 0
            or PartyCannonCollisions["trampoline_bounce"] in hitNode
        ):
            if (
                hitNode == "sky_collision"
                or hitNode in PartyCannonCollisions["fence"]
                or hitNode.find("cogPie") == 0
            ):
                self.__hitFence(self.localFlyingToon, collisionEntry)
            elif (
                PartyCannonCollisions["trampoline_bounce"] in hitNode
                or hitNode in PartyCannonCollisions["bounce"]
            ):
                ...
                self.hitCloud = 1
                self.__hitBumper(
                    self.localFlyingToon, collisionEntry, hitSound, kr=0.09, angVel=5
                )
                self.hitBumper = 0
        ...
        if hitNode in PartyCannonCollisions["ground"]:
            ...
            track.append(
                Func(self.__setToonUpright, self.localFlyingToon, self.landingPos)
            )
            ...
            track.append(
                Func(self.__hitWater, self.localFlyingToon, pos, collisionEntry)
            )
            ...
            track.append(
                Func(self.__hitCloudPlatform, self.localFlyingToon, collisionEntry)
            )
        else:
            self.notify.warning(
    ...
    def __hitGround(self, avatar, pos, extraArgs=[]):
        ...
        self.notify.debug(
            "pos = %s, hpr = %s" % (avatar.getPos(render), avatar.getHpr(render))
        )
    ...
    def d_requestCloudHit(self, cloudNumber, color):
        self.sendUpdate(
            "requestCloudHit", [cloudNumber, color.getX(), color.getY(), color.getZ()]
        )
    ...
    def __hitCloudPlatform(self, avatar, collisionEntry, extraArgs=[]):
        if not self.hitBumper and not self.hitCloud:
            ...
            if self._lastCloudHit is None:
                ...
                self.d_requestCloudHit(
                    cloudNumber, base.localAvatar.style.getHeadColor()
                )
    ...
    def __checkHoodValidity(self):
        if (
            hasattr(base.cr.playGame, "hood")
            and base.cr.playGame.hood
            and hasattr(base.cr.playGame.hood, "loader")
            and base.cr.playGame.hood.loader
            and hasattr(base.cr.playGame.hood.loader, "geom")
            and base.cr.playGame.hood.loader.geom
        ):
            return True
        else:
            return False
    ...
    def handleToonExited(self, toonId):
        self.notify.debug(
            "DistributedPartyCannonActivity handleToonExited( toonId=%s ) " % toonId
        )
