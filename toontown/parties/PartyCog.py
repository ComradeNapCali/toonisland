...
class PartyCog(FSM):
    ...
    def exitActive(self):
        ...
        self.clearHitInterval()
        self.resetRollIval = self.root.hprInterval(
            0.5, Point3(self.root.getH(), 0.0, 0.0), blendType="easeInOut"
        )
    ...
    def enterDown(self):
        ...
        self.hitInterval = Sequence(
            LerpFunc(
                self.setAlongSpline, duration=1.0, fromData=self.currentT, toData=0.0
            ),
            LerpScaleInterval(
                self.hole,
                duration=0.175,
