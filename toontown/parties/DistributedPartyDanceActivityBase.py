...
class DistributedPartyDanceActivityBase(DistributedPartyActivity):
    ...
    def __init__(
        self, cr, actId, dancePatternToAnims, model="phase_13/models/parties/danceFloor"
    ):
    ...
    def load(self):
        ...
        self.discoBallSequence = Parallel(
            ...
            Sequence(
                discoBall.posInterval(3, Point3(0, 0, 1), blendType="easeInOut"),
        ...
        if anim:
            ...
            if pattern not in self.localPatternsMatched:
                ...
                self.localToonDanceSequence = Sequence(
                    ...
                    Parallel(
                        camera.posInterval(0.5, pos, blendType="easeOut"),
    ...
    def _requestToonState(self, toonId, state, anim):
        if toonId in self.dancingToonFSMs:
            ...
            try:
                ...
                self.notify.warning(
                    "could not go from state=%s to state %s" % (curState, state)
                )
    ...
    def __setViewMode(self, mode):
        ...
        if mode == DanceViews.Normal:
            ...
            self.cameraParallel = Parallel(
                ...
                camera.hprInterval(
                    0.5, Point3(0, -27, 0), other=toon.getParent(), blendType="easeIn"
                ),
            )
