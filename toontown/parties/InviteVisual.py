...
class InviteVisual(DirectFrame):
    ...
    def __init__(self, parent):
        ...
        self.inviteThemesIdToInfo = {
            PartyGlobals.InviteTheme.Birthday: (
                ...
                TTLocalizer.PartyPlannerBirthdayTheme,
                (0.0, 0.0, 0.0, 1.0),
            ),
            PartyGlobals.InviteTheme.GenericMale: (
                self.gui.find("**/genericMalePage"),
                TTLocalizer.PartyPlannerGenericMaleTheme,
                (0.7, 0.7, 0.0, 1.0),
            ),
    ...
    def updateInvitation(self, hostsName, partyInfo):
        ...
        self.whosePartyLabel["text"] = (
            TTLocalizer.PartyPlannerInvitationWhoseSentence % hostsName
        )
        ...
        if self.noFriends:
            self.activityTextLabel[
                "text"
            ] = TTLocalizer.PartyPlannerInvitationThemeWhatSentenceNoFriends % (
                publicPrivateText,
                activities,
            )
        else:
            self.activityTextLabel[
                "text"
            ] = TTLocalizer.PartyPlannerInvitationThemeWhatSentence % (
                publicPrivateText,
                activities,
            )
        if self.noFriends:
            self.whenTextLabel[
                "text"
            ] = TTLocalizer.PartyPlannerInvitationWhenSentenceNoFriends % (
                PartyUtils.formatDate(
                    self.partyInfo.startTime.year,
                    self.partyInfo.startTime.month,
                    self.partyInfo.startTime.day,
                ),
                PartyUtils.formatTime(
                    self.partyInfo.startTime.hour, self.partyInfo.startTime.minute
                ),
            )
        else:
            self.whenTextLabel[
                "text"
            ] = TTLocalizer.PartyPlannerInvitationWhenSentence % (
                PartyUtils.formatDate(
                    self.partyInfo.startTime.year,
                    self.partyInfo.startTime.month,
                    self.partyInfo.startTime.day,
    ...
    def getActivitiesFormattedCorrectly(self):
        ...
        if len(activityList) == 1:
            return (
                "\n"
                + TTLocalizer.PartyPlannerInvitationThemeWhatActivitiesBeginning
                + activityList[0]
            )
        ...
        activitiesString = (
            activitiesString[: activitiesString.rfind(",")]
            + conjunction
            + activitiesString[activitiesString.rfind(",") + 1 :]
        )
        activitiesString = (
            TTLocalizer.PartyPlannerInvitationThemeWhatActivitiesBeginning
            + activitiesString
