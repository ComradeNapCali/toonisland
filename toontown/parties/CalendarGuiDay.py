...
class CalendarGuiDay(DirectFrame):
    ...
    def createGuiObjects(self):
        ...
        self.listXorigin = 0
        self.listFrameSizeX = (
            self.scrollBottomRightLocator.getX() - self.scrollLocator.getX()
        )
        self.scrollHeight = (
            self.scrollLocator.getZ() - self.scrollBottomRightLocator.getZ()
        )
    ...
    def adjustForMonth(self):
        ...
        if self.onlyFutureDaysClickable:
            if (
                self.myDate.year < curServerDate.year
                or self.myDate.year == curServerDate.year
                and self.myDate.month < curServerDate.month
                or self.myDate.year == curServerDate.year
                and self.myDate.month == curServerDate.month
                and self.myDate.day < curServerDate.day
            ):
        ...
        try:
            for item in self.scrollList["items"]:
                if (
                    ...
                    and hasattr(item.description, "destroy")
                ):
    ...
    def addWeeklyHolidays(self):
        if (
            not self.filter == ToontownGlobals.CalendarFilterShowAll
            and not self.filter == ToontownGlobals.CalendarFilterShowOnlyHolidays
        ):
            return
        ...
        if base.config.GetBool("calendar-test-items", 0):
            if (
                self.myDate.date() + datetime.timedelta(days=-1)
            ...
            if (
                self.myDate.date() + datetime.timedelta(days=-2)
                == base.cr.toontownTimeManager.getCurServerDateTime().date()
            ):
    ...
    def collectTimedEvents(self):
        ...
        if base.cr.newsManager and (
            ...
            for holiday in yearlyHolidays:
                ...
                if (
                    holidayStart[0] == self.myDate.month
                    and holidayStart[1] == self.myDate.day
                ):
                    myTime = datetime.time(holidayStart[2], holidayStart[3])
                elif (
                    holidayEnd[0] == self.myDate.month
                    and holidayEnd[1] == self.myDate.day
                ):
            ...
            for holiday in oncelyHolidays:
                ...
                holidayType = holiday[0]
                if (
                    holidayStart[0] == self.myDate.year
                    and holidayStart[1] == self.myDate.month
                    and holidayStart[2] == self.myDate.day
                ):
                    myTime = datetime.time(holidayStart[3], holidayStart[4])
                elif (
                    holidayEnd[0] == self.myDate.year
                    and holidayEnd[1] == self.myDate.month
                    and holidayEnd[2] == self.myDate.day
                ):
        ...
        for timedEvent in self.timedEvents:
            if isinstance(timedEvent[1], PartyInfo):
                ...
                isinstance(timedEvent[1], tuple)
                and timedEvent[1][0] == NewsManager.OncelyMultipleStartHolidayType
            ):
    ...
    def addYearlyHolidayToScrollList(self, holiday):
        ...
        if holidayStart[0] == holidayEnd[0] and holidayStart[1] == holidayEnd[1]:
            ...
            self.myDate.month == holidayStart[0] and self.myDate.day == holidayStart[1]
        ):
            holidayText = myStrftime(startTime)
            holidayText += " " + holidayName
            ...
            holidayDesc += (
                ". "
                + TTLocalizer.CalendarStartedOn
                + startDate.strftime(TTLocalizer.HolidayFormat)
                + myStrftime(startTime)
            )
        else:
    ...
    def addOncelyHolidayToScrollList(self, holiday):
        ...
        if holidayStart[1] == holidayEnd[1] and holidayStart[2] == holidayEnd[2]:
            ...
            self.myDate.year == holidayStart[0]
            and self.myDate.month == holidayStart[1]
            and self.myDate.day == holidayStart[2]
        ):
            holidayText = myStrftime(startTime)
            holidayText += " " + holidayName
            holidayDesc = holidayName + ". " + holidayDesc
            ...
            self.myDate.year == holidayEnd[0]
            and self.myDate.month == holidayEnd[1]
            and self.myDate.day == holidayEnd[2]
        ):
            holidayText = myStrftime(endTime)
            holidayText += " " + TTLocalizer.CalendarEndDash + holidayName
            holidayDesc = TTLocalizer.CalendarEndOf + holidayName
