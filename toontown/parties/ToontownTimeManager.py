...
class ToontownTimeManager:
    ...
    def __init__(
        ...
        if self.serverTimeZoneString == "US/Pacific":
            ...
            self.notify.error(
                "Time Zone %s is not supported." % self.serverTimeZoneString
            )

        self.updateLoginTimes(
            serverTimeUponLogin, clientTimeUponLogin, globalClockRealTimeUponLogin
        )
        self.debugSecondsAdded = 0

    def updateLoginTimes(
        self, serverTimeUponLogin, clientTimeUponLogin, globalClockRealTimeUponLogin
    ):
    ...
    def convertStrToToontownTime(self, dateStr):
        ...
        try:
            curDateTime = datetime.fromtimestamp(
                time.mktime(time.strptime(dateStr, self.formatStr)), self.serverTimeZone
            )
        except:
