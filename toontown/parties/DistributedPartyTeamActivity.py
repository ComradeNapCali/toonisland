...
class DistributedPartyTeamActivity(DistributedPartyActivity):
    ...
    def handleToonShifted(self, toonId):
        self.notify.error(
            "handleToonShifted( toonId=%d ) must be overriden by child class." % toonId
        )
    ...
    def handleToonDisabled(self, toonId):
        self.notify.error(
            "handleToonDisabled( toonId=%d ) must be overriden by child class." % toonId
        )
    ...
    def d_toonJoinRequest(self, team):
        if self.isLocalToonInActivity():
            return
        if (
            self.activityFSM.state in ["WaitForEnough", "WaitToStart"]
    ...
    def switchTeamRequestDenied(self, reason):
        ...
        if reason == PartyGlobals.DenialReasons.Full:
            ...
            self.showMessage(
                TTLocalizer.PartyTeamActivitySwitchDenied, endState="activity"
            )
        if (
            self.isLocalToonPlaying
            and (self.isState("WaitToStart") or self.isState("WaitForEnough"))
            and self._canSwitchTeams
        ):
            self.teamActivityGui.enableSwitchButton()
