...
class DistributedPartyActivity(DistributedObject.DistributedObject):
    def __init__(
        self, cr, activityId, activityType, wantLever=False, wantRewardGui=False
    ):
        ...
        childName = "%s" % self
        childName = childName[
            childName.rfind(".DistributedParty")
            + len(".DistributedParty") : childName.rfind("Activity instance")
        ]
    ...
    def handleToonJoined(self, toonId):
        self.notify.error(
            "BASE: handleToonJoined should be overridden %s" % self.activityName
        )

    def handleToonExited(self, toonId):
        self.notify.error(
            "BASE: handleToonExited should be overridden %s" % self.activityName
        )

    def handleToonDisabled(self, toonId):
        self.notify.error(
            "BASE: handleToonDisabled should be overridden %s" % self.activityName
        )
    ...
    def _leverPulled(self, collEntry):
        ...
        if (
            self.activityType == PartyGlobals.ActivityTypes.HostInitiated
            and base.localAvatar.doId != self.party.partyInfo.hostId
        ):
            return False
        return True
    ...
    def getToonPullingLeverInterval(self, toon):
        ...
        reachAndPull = Sequence(
            ActorInterval(
                toon, "walk", loop=True, duration=walkTime - reach.getDuration()
            ),
            reach,
            pull,
        )
    ...
    def __handleMessageDone(self):
        ...
        if hasattr(base.cr.playGame.getPlace(), "fsm"):
            if self.messageGui and hasattr(self.messageGui, "endState"):
                self.notify.info(
                    "__handleMessageDone (endState=%s)" % self.messageGui.endState
                )
    ...
    def showJellybeanReward(self, earnedAmount, jarAmount, message):
        if (
            not self.isLocalToonInActivity()
    ...
    def isLocalToonInActivity(self):
        ...
        if (
            place
            and place.__class__.__name__ == "Party"
            and hasattr(place, "fsm")
            and place.fsm
        ):
    ...
    def finishRules(self):
        ...
        if hasattr(self, "rulesPanel"):
            ...
            del self.rulesPanel
            base.setCellsAvailable(
                base.bottomCells + [base.leftCells[0], base.rightCells[1]], True
            )
