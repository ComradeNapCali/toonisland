...
from toontown.parties.DistributedPartyCatchActivityBase import (
    DistributedPartyCatchActivityBase,
)
from toontown.parties.DistributedPartyCannonActivity import (
...
class DistributedPartyCatchActivity(
    DistributedPartyActivity, DistributedPartyCatchActivityBase
):
    ...
    def load(self, loadModels=1, arenaModel="partyCatchTree"):
        ...
        base.cr.parentMgr.registerParent(
            self._avatarNodePathParentToken, self.avatarNodePath
        )
        self.toonSDs = {}
    ...
    def _toonEnteredTree(self, collEntry):
        ...
        self.notify.debug(
            "_toonEnteredTree : currentState = %s" % self.activityFSM.state
        )
    ...
    def setGenerations(self, generations):
        ...
        for id, timestamp32, numPlayers in generations:
            gen2t[id] = (
                globalClockDelta.networkToLocalTime(timestamp32, bits=32)
    ...
    def scheduleDrops(self, genId=None):
        ...
        if gen.hasBeenScheduled:
            return
        fruitIndex = int(
            (gen.startTime + 0.5 * self.DropPeriod) / PartyGlobals.CatchActivityDuration
        )
        ...
        dropPlacer = PartyRegionDropPlacer(
            self, gen.numPlayers, genId, gen.droppedObjNames, startTime=gen.startTime
        )
