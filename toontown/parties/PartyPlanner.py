...
class PartyPlanner(DirectFrame, FSM):
    ...
    def exitDate(self):
        ...
        if self.selectedCalendarGuiDay is not None:
            self.partyTime = self.cleanPartyTime
            self.alterPartyTime(
                year=self.selectedCalendarGuiDay.myDate.year,
    ...
    def enterTime(self, *args):
        ...
        self.nextButton.show()
        self.timePage.show()
        self.timePageRecapToontownTimeLabel2["text"] = "%s" % PartyUtils.formatDateTime(
            self.partyTime
        )
    ...
    def enterInvitation(self, *args):
        ...
        if hasattr(base.cr, "newsManager") and base.cr.newsManager:
            if (
                ToontownGlobals.VICTORY_PARTY_HOLIDAY
                in base.cr.newsManager.getHolidayIdList()
            ):
                defaultInviteTheme = PartyGlobals.InviteTheme.VictoryParty
            elif (
                ToontownGlobals.KARTING_TICKETS_HOLIDAY
                in base.cr.newsManager.getHolidayIdList()
                ...
                ToontownGlobals.VALENTINES_DAY in base.cr.newsManager.getHolidayIdList()
            ):
        ...
        self.partyInfo = PartyInfo(
            ...
            0,
            self.partyTime.year,
            self.partyTime.month,
            self.partyTime.day,
            self.partyTime.hour,
            self.partyTime.minute,
            endTime.year,
            endTime.month,
            endTime.day,
            endTime.hour,
        ...
        if self.noFriends or len(self.getInvitees()) == 0:
            ...
            self.invitationTitleLabel[
                "text"
            ] = TTLocalizer.PartyPlannerConfirmTitleNoFriends
    ...
    def setInviteTheme(self, themeNumber):
        ...
        self.selectedInviteThemeLabel["text"] = "%s %s (%d/%d)" % (
            self.inviteVisual.inviteThemesIdToInfo[self.currentInvitationTheme][1],
    ...
    def _createNavButtons(self):
        ...
        self.nextButton = DirectButton(
            ...
            relief=None,
            geom=(
                self.gui.find("**/bottomNext_button/nextButton_up"),
        ...
        self.prevButton = DirectButton(
            ...
            geom=(
                ...
                self.gui.find("**/bottomPrevious_button/previousButton_rollover"),
            ),
    ...
    def __createNametag(self, parent):
        if self.nametagGroup == None:
            ...
            nametagPos = self.gui.find(
                "**/step_01_partymanPeteNametag_locator"
            ).getPos()
            self.nametagNP.setPosHprScale(
                nametagPos[0], 0, nametagPos[2], 0, 0, 0, 0.1, 1, 0.1
            )
            ...
            self.chatNP.setPosHprScale(
                chatPos[0], 0, chatPos[2], 0, 0, 0, 0.08, 1, 0.08
            )
        return
    ...
    def _createDatePage(self):
        ...
        self.makePartyNowButton = DirectButton(
            ...
            relief=None,
            geom=(
                self.gui.find("**/send_up"),
                self.gui.find("**/send_down"),
    ...
    def alterPartyTime(self, year=None, month=None, day=None, hour=None, minute=None):
        self.partyTime = datetime(
            year=self.positiveTime("year", year),
    ...
    def positiveTime(self, type, amount):
        ...
        if type == "hour" or type == "minute":
            if amount < 0:
                return (
                    self.timeTypeToMaxValue[type]
                    + 1
                    + self.timeTypeToChangeAmount[type][1]
                )
    ...
    def _createTimePage(self):
        ...
        self.clockImage = DirectFrame(
            parent=page, relief=None, geom=self.gui.find("**/toontownTime_background")
        )
        ...
        self.timePageRecapToontownTimeLabel2 = DirectLabel(
            ...
            pos=self.gui.find("**/step_03_toontownDateAndTime_loactor").getPos(),
            textMayChange=True,
            scale=0.06,
        )
        ...
        (
            self.timeInputHourLabel,
            self.timeInputHourUpButton,
            self.timeInputHourDownButton,
        ) = self.getTimeWidgets(page, "hour")
        (
            self.timeInputMinuteLabel,
            self.timeInputMinuteUpButton,
            self.timeInputMinuteDownButton,
        ) = self.getTimeWidgets(page, "minute")
        (
            self.timeInputAmPmLabel,
            self.timeInputAmPmUpButton,
            self.timeInputAmPmDownButton,
        ) = self.getTimeWidgets(page, "ampm")
    ...
    def getTimeWidgets(self, page, type):
        ...
        label = DirectLabel(
            ...
            text="%s" % data,
            textMayChange=True,
            pos=self.gui.find("**/step_03_%s_locator" % type).getPos(),
            scale=0.12,
        )
        ...
        def changeValue(self, amount):
            if type == "ampm":
                ...
                if type == "hour":
                    ...
                    if (
                        self.timeInputAmPmLabel["text"]
                        == TTLocalizer.PartyTimeFormatMeridiemPM
                    ):
                    ...
                    PartyPlanner.notify.error(
                        "Invalid type for changeValue in PartyPlanner: %s" % type
                    )
            ...
            self.timePageRecapToontownTimeLabel2[
                "text"
            ] = "%s" % PartyUtils.formatDateTime(self.partyTime)
            self.timePageRecapLocalTimeLabel["text"] = "%s%s" % (
                TTLocalizer.PartyPlannerTimeLocalTime,
                PartyUtils.formatDateTime(self.partyTime, inLocalTime=True),
            )
        ...
        upButton = DirectButton(
            ...
            relief=None,
            geom=(
                self.gui.find("**/%sButtonUp_up" % type),
        ...
        downButton = DirectButton(
            ...
            relief=None,
            geom=(
                self.gui.find("**/%sButtonDown_up" % type),
    ...
    def _createGuestPage(self):
        ...
        if len(base.localAvatar.friendsList) == 0:
            ...
            for friendPair in base.localAvatar.friendsList:
                self.friendList.addFriend(
                    determineFriendName(friendPair), friendPair[0]
                )
        ...
        self.privateButton = DirectButton(
            ...
            relief=None,
            geom=(
                self.gui.find("**/privateButton_up"),
        ...
        self.checkAllButton = DirectButton(
            ...
            relief=None,
            geom=(
                self.gui.find("**/checkAllButton_up"),
        ...
        self.uncheckAllButton = DirectButton(
            ...
            relief=None,
            geom=(
                self.gui.find("**/uncheckAllButton_up"),
    ...
    def _createPartyEditorPage(self):
        ...
        self.LayoutTitleLabel = DirectLabel(
            ...
            pos=self.gui.find("**/title_locator").getPos() + Point3(0.0, 0.0, 0.075),
            scale=self.titleScale,
        )
        self.costLabel = DirectLabel(
            ...
            pos=(-0.74, 0.0, 0.17),
            relief=None,
            text=TTLocalizer.PartyPlannerTotalCost % 0,
            text_align=TextNode.ACenter,
        ...
        self.elementTitleLabel = DirectLabel(
            ...
            relief=None,
            text=" ",
            pos=self.gui.find("**/step_05_activitiesName_text_locator").getPos()
            + Point3(0.0, 0.0, 0.04),
        ...
        self.elementDescription.setPos(
            self.gui.find("**/step_05_activityDescription_text_locator").getPos()
            + Point3(0.0, 0.0, 0.04)
        )
    ...
    def _createInvitationPage(self):
        ...
        self.invitationBackground = DirectFrame(
            parent=page, relief=None, geom=self.gui.find("**/invitationBackground")
        )
        ...
        self.nextThemeButton = DirectButton(
            ...
            relief=None,
            geom=(
                self.gui.find("**/topNext_button/nextButton_up"),
        ...
        self.prevThemeButton = DirectButton(
            ...
            relief=None,
            geom=(
                self.gui.find("**/topPrevious_button/previousButton_up"),
                self.gui.find("**/topPrevious_button/previousButton_down"),
                self.gui.find("**/topPrevious_button/previousButton_rollover"),
            ),
    ...
    def __handleHolidays(self):
        ...
        if hasattr(base.cr, "newsManager") and base.cr.newsManager:
            ...
            if (
                ToontownGlobals.WINTER_DECORATIONS not in holidayIds
                and ToontownGlobals.WACKY_WINTER_DECORATIONS not in holidayIds
            ):
    ...
    def _createFarewellPage(self):
        ...
        self.closePlannerButton = DirectButton(
            ...
            relief=None,
            geom=(
                self.gui.find("**/close_up"),
                self.gui.find("**/close_down"),
    ...
    def __handleComplete(self):
        ...
        base.cr.partyManager.sendAddParty(
            hostId,
            self.partyTime.strftime("%Y-%m-%d %H:%M:%S"),
            endTime.strftime("%Y-%m-%d %H:%M:%S"),
            self.isPrivate,
            self.currentInvitationTheme,
            self.partyActivities,
            decorations,
            invitees,
        )
    ...
    def processAddPartyResponse(self, hostId, errorCode):
        ...
        if errorCode == PartyGlobals.AddPartyErrorCode.AllOk:
            ...
            self.confirmTitleLabel[
                "text"
            ] = TTLocalizer.PartyPlannerConfirmationAllOkTitle
            if self.noFriends or len(self.getInvitees()) == 0:
                confirmRecapText = (
                    TTLocalizer.PartyPlannerConfirmationAllOkTextNoFriends
                )
            else:
            ...
            self.confirmTitleLabel[
                "text"
            ] = TTLocalizer.PartyPlannerConfirmationErrorTitle
