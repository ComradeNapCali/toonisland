...
class AvatarChoice(DirectButton):
    ...
    def __init__(self, av=None, position=0, paid=0, okToLockout=1):
        ...
        DirectButton.__init__(
            self, relief=None, text="", text_font=ToontownGlobals.getSignFont()
        )
        ...
        if self.mode is AvatarChoice.MODE_LOCKED:
            ...
            self.logoModelImage = loader.loadModel(
                "phase_3/models/gui/members_only_gui"
            ).find("**/MembersOnly")
            logo = DirectFrame(
                state=DGG.DISABLED,
                parent=self,
        ...
        if self.deleteWithPasswordFrame == None:
            ...
            if self.deleteWithPassword:
                ...
                self.passwordEntry = DirectEntry(
                    ...
                    cursorKeys=1,
                    obscured=1,
                    command=self.__handleDeleteWithPasswordOK,
                )
                DirectButton(
                    parent=self.deleteWithPasswordFrame,
                    image=okButtonImage,
            ...
            DirectButton(
                ...
                textMayChange=1,
                pos=(0.2, 0.0, -0.35),
                command=self.__handleDeleteWithPasswordCancel,
            )
            buttons.removeNode()
    ...
    def __handleDeleteWithPasswordOK(self, *args):
        ...
        if okFlag:
            ...
            self.deleteWithPasswordFrame[
                "text"
            ] = TTLocalizer.AvatarChoiceDeleteWrongPassword
            self.passwordEntry["focus"] = 1
            self.passwordEntry.enterText("")
        return
    ...
    def __handleDeleteWithConfirmOK(self, *args):
        ...
        if password == passwordMatch:
            ...
            self.deleteWithPasswordFrame[
                "text"
            ] = TTLocalizer.AvatarChoiceDeleteWrongConfirm % {
                "name": self.name,
                "confirm": TTLocalizer.AvatarChoiceDeleteConfirmUserTypes,
            }
            self.passwordEntry["focus"] = 1
