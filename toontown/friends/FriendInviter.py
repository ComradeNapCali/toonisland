...
class FriendInviter(DirectFrame):
    ...
    def __init__(self, avId, avName, avDisableName):
        ...
        self.fsm = ClassicFSM.ClassicFSM(
            ...
            [
                ...
                State.State(
                    "notAvailable", self.enterNotAvailable, self.exitNotAvailable
                ),
                State.State(
                    "notAcceptingFriends",
                ...
                State.State(
                    "endFriendship", self.enterEndFriendship, self.exitEndFriendship
                ),
                State.State(
                    "friendsNoMore", self.enterFriendsNoMore, self.exitFriendsNoMore
                ),
                ...
                State.State(
                    "otherTooMany", self.enterOtherTooMany, self.exitOtherTooMany
                ),
    ...
    def enterBegin(self):
        ...
        self.bToon.show()
        if (
            self.wantPlayerFriends
            and base.cr.productName != "DisneyOnline-UK"
            and base.cr.productName != "DisneyOnline-AP"
        ):
            self.bPlayer.show()
        else:
            self.__handleToon()
    ...
    def enterCheckAvailability(self):
        ...
        if self.playerFriend:
            ...
            self.accept(
                OTPGlobals.PlayerFriendAddEvent, self.__playerFriendAcceptResponse
            )
            self.bOk.show()
        else:
    ...
    def __friendConsidering(self, yesNoAlready, context):
        if yesNoAlready == 1:
            ...
            self.notify.warning(
                "Got unexpected response to friendConsidering: %s" % yesNoAlready
            )
    ...
    def __friendResponse(self, yesNoMaybe, context):
        if self.context != context:
            self.notify.warning(
                "Unexpected change of context from %s to %s." % (self.context, context)
            )
            self.context = context
        if yesNoMaybe == 1:
