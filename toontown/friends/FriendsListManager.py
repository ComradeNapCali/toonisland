...
class FriendsListManager:
    ...
    def enterFLM(self):
        ...
        self.accept(
            OTPGlobals.PlayerFriendInvitationEvent, self.__handlePlayerFriendInvitation
        )
        if base.cr.friendManager:
    ...
    def __handleClickedNametag(self, avatar, playerId=None):
        ...
        if avatar.isPet():
            ...
            if hasattr(self, "avatarPanel"):
                if self.avatarPanel:
                    if (
                        not hasattr(self.avatarPanel, "getAvId")
                        or self.avatarPanel.getAvId() == avatar.doId
                    ):
    ...
    def __handleClickedNametagPlayer(self, avatar, playerId, showType=1):
        ...
        if showType == 1:
            if hasattr(self, "avatarPanel"):
                if self.avatarPanel:
                    if (
                        not hasattr(self.avatarPanel, "getPlayerId")
