...
class GetAvatarInfoOperation(FSM):
    ...
    def enterGetAvatarInfo(self):
        self.mgr.air.dbInterface.queryObject(
            self.mgr.air.dbId, self.avId, self.__gotAvatarInfo
        )
    ...
    def enterFinished(self):
        if not self.isPet:
            self.mgr.avBasicInfoCache[self.avId] = {
                "expire": time.time()
                + config.GetInt("friend-detail-cache-expire", 3600),
                "avInfo": [
                    self.avId,
                    self.fields["setName"][0],
                    self.fields["setDNAString"][0],
                    self.fields["setPetId"][0],
                ],
            }

        self.callback(
            success=True, avId=self.senderId, fields=self.fields, isPet=self.isPet
        )
...
class GetFriendsListOperation(FSM):
    ...
    def enterGetFriendsList(self):
        self.mgr.air.dbInterface.queryObject(
            self.mgr.air.dbId, self.avId, self.__gotFriendsList
        )
    ...
    def __gotFriendsList(self, dclass, fields):
        if self.state != "GetFriendsList":
            self.demand(
                "Failure",
                "__gotFriendsList called when looking for friends list, avId %d"
                % self.avId,
            )
            return
    ...
    def enterGetFriendDetails(self):
        if len(self.friendsList) <= 0:
            self.callback(
                success=False, avId=self.avId, friendsDetails=None, onlineFriends=None
            )
            return
        ...
        for friendId, trueFriend in self.friendsList:
            ...
            newOperation = GetAvatarInfoOperation(
                self.mgr, self.avId, friendId, self.__gotAvatarInfo
            )
            newOperation.start()
    ...
    def __gotAvatarInfo(self, success, avId, fields, isPet):
        ...
        if not success:
            self.demand(
                "Failure",
                "__gotAvatarInfo received unsuccessful callback, avId=%d" % self.avId,
            )
            return
        ...
        if self.state != "GetFriendDetails":
            self.demand(
                "Failure",
                "__gotAvatarInfo while not looking for friends details, avId=%d"
                % self.avId,
            )
            return
        ...
        if avId != self.avId:
            self.demand(
                "Failure",
                "__gotAvatarInfo response for wrong requester. wrongId=%d, rightId=%d"
                % (self.avId, avId),
            )
            return
    ...
    def enterFinished(self):
        self.callback(
            success=True,
            avId=self.avId,
            friendsDetails=self.friendsDetails,
...
class UpdateAvatarFieldOperation(FSM):
    ...
    def __avatarOnlineResp(self, avId, activated):
        if self.state != "GetAvatarOnline":
            self.demand(
                "Failure",
                "Received __avatarOnlineResp while not in GetAvatarOnline state.",
            )
            return
...
class TIAFriendsManagerUD(DistributedObjectGlobalUD):
    notify = DirectNotifyGlobal.directNotify.newCategory("TIAFriendsManagerUD")
    ...
    def getAvatarDetails(self, avId):
        ...
        newOperation = GetAvatarInfoOperation(
            self, senderId, avId, self.__gotAvatarDetails
        )
        newOperation.start()
    ...
    def __gotAvatarDetails(self, success, avId, fields, isPet):
        ...
        if isPet:
            ...
            details = [
                ...
                [
                    "setExperience",
                    base64.b64encode(fields["setExperience"][0]).decode(),
                ],
                ...
                [
                    "setMailboxContents",
                    base64.b64encode(fields["setMailboxContents"][0]).decode(),
                ],
                [
                    "setAwardMailboxContents",
                    base64.b64encode(fields["setAwardMailboxContents"][0]).decode(),
                ],
                [
                    "setGiftSchedule",
                    base64.b64encode(fields["setGiftSchedule"][0]).decode(),
                ],
                [
                    "setDeliverySchedule",
                    base64.b64encode(fields["setDeliverySchedule"][0]).decode(),
                ],
                [
                    "setAwardSchedule",
                    base64.b64encode(fields["setAwardSchedule"][0]).decode(),
                ],
                [
                    "setHat",
                    fields["setHat"][0],
                ...
                [
                    "setBackpack",
                    fields["setBackpack"][0],
                    fields["setBackpack"][1],
                    fields["setBackpack"][2],
                ],
                [
                    "setShoes",
                    fields["setShoes"][0],
        ...
        self.sendUpdateToAvatarId(
            avId, "avatarDetailsResp", [fields["avId"], json.dumps(details)]
        )
    ...
    def __handleRemoveFriend(
        self, success, avId, fields, isPet, friendId=None, final=False
    ):
        self.deleteOperation(avId)
        ...
        if fields["avId"] not in [avId, friendId]:
            self.notify.warning(
                "__handleRemoveFriend received wrong Toon fields from DB, avId=%d"
                % avId
            )
            return
        ...
        newOperation = UpdateAvatarFieldOperation(
            self,
            avId,
            friendId if final else avId,
            functools.partial(
                self.__handleFriendRemoved, friendId=friendId, final=final
            ),
        )
        newOperation.start("setFriendsList", friendsList)
        self.operations[avId] = newOperation

    def __handleFriendRemoved(
        self, success, avId, online=False, friendId=None, final=False
    ):
        self.deleteOperation(avId)
        if not (success and friendId):
            return
        ...
        if not final:
            newOperation = GetAvatarInfoOperation(
                ...
                friendId,
                functools.partial(
                    self.__handleRemoveFriend, friendId=friendId, final=True
                ),
            )
            newOperation.start()
            self.operations[avId] = newOperation
            ...
            if online:
                dg = self.air.dclassesByName["DistributedToonUD"].aiFormatUpdate(
                    "friendsNotify", friendId, friendId, self.air.ourChannel, [avId, 1]
                )
                self.air.send(dg)

    def comingOnline(self, avId, friends):
        for friendId in friends:
            self.air.getActivated(
                friendId,
                functools.partial(self.__comingOnlineFriendOnline, otherId=avId),
            )
    ...
    def __comingOnlineFriendOnline(self, avId, activated, otherId=None):
        ...
        dg = PyDatagram()
        dg.addServerHeader(
            self.GetPuppetConnectionChannel(otherId),
            self.air.ourChannel,
            CLIENTAGENT_DECLARE_OBJECT,
        )
        dg.addUint32(avId)
    ...
    def goingOffline(self, avId):
        newOperation = GetAvatarInfoOperation(
            self, avId, avId, self.__handleGoingOffline
        )
        newOperation.start()
    ...
    def __handleGoingOffline(self, success, avId, fields, isPet):
        ...
        for friendId, trueFriend in fields["setFriendsList"][0]:
            self.air.getActivated(
                friendId,
                functools.partial(
                    self.__handleGoneOffline, otherId=avId, accId=fields["setDISLid"][0]
                ),
            )
    ...
    def __handleGoneOffline(self, avId, activated, otherId=None, accId=None):
        ...
        dg = PyDatagram()
        dg.addServerHeader(
            self.GetAccountConnectionChannel(accId),
            self.air.ourChannel,
            CLIENTAGENT_UNDECLARE_OBJECT,
        )
        dg.addUint32(avId)
    ...
    def clearList(self, avId):
        newOperation = GetAvatarInfoOperation(
            self, avId, avId, self.__handleClearListGotFriendsList
        )
        newOperation.start()
        self.operations[avId] = newOperation
    ...
    def __handleClearListGotFriendsList(self, success, avId, fields, isPet):
        ...
        newOperation = GetAvatarInfoOperation(
            ...
            friendIds[0][0],
            functools.partial(
                self.__handleClearListGotFriendData, friendIds=friendIds[1:]
            ),
        )
        newOperation.start()
        self.operations[avId] = newOperation

    def __handleClearListGotFriendData(
        self, success, avId, fields, isPet, friendIds=[]
    ):
        self.deleteOperation(avId)
        if not success:
            if friendIds:
        ...
        newOperation = UpdateAvatarFieldOperation(
            ...
            fields["avId"],
            functools.partial(
                self.__handleClearListUpdatedAvatarField,
                friendId=fields["avId"],
    ...
    def __handleClearListUpdatedAvatarField(
        self, success, avId, online=False, friendId=None, friendIds=[]
    ):
        self.deleteOperation(avId)
