...
class TwoDEnemy(DirectObject):
        ...
        if len(suitPosAttribs) == 3:
            ...
            if abs(finalZ - initZ) > 0.0:
                ...
                self.animTrack = Sequence(
                    ActorInterval(
                        self.suit, "landing", startFrame=8, endFrame=28, playRate=0.5
                    ),
                    ActorInterval(
                        self.suit, "landing", startFrame=8, endFrame=28, playRate=-0.5
                    ),
    ...
    def setupCollision(self):
        ...
        self.accept(
            self.game.uniqueName("enter" + self.suitName), self.handleEnemyCollision
        )
