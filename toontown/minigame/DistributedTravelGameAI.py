...
class DistributedTravelGameAI(DistributedMinigameAI):
    ...
    def giveBonusBeans(self, endingSwitch):
        ...
        for avId in list(self.avIdBonuses.keys()):
            ...
            if (
                self.avIdBonuses[avId][0] == endingSwitch
                and not self.stateDict[avId] == EXITED
            ):
                noOneGotBonus = False
    ...
    def checkForEndGame(self):
        ...
        if TravelGameGlobals.SpoofFour:
            numPlayers = 4
        delay = (
            TravelGameGlobals.DisplayVotesTimePerPlayer * (numPlayers + 1)
            + TravelGameGlobals.MoveTrolleyTime
            + TravelGameGlobals.FudgeTime
        )
        if didWeReachMiniGame:
            self.desiredNextGame = self.switchToMinigameDict[self.currentSwitch]
            taskMgr.doMethodLater(
                delay, self.moveTimeoutTaskGameOver, self.taskName("move-timeout")
            )
            self.giveBonusBeans(self.currentSwitch)
        else:
            taskMgr.doMethodLater(
                delay, self.moveTimeoutTask, self.taskName("move-timeout")
            )
        self.sendUpdate(
            "setServerChoices",
            [
    ...
    def setStartingVote(self, avId, startingVote):
        ...
        self.notify.debug(
            "setting current  vote of avId=%d to %d" % (avId, startingVote)
        )

    def handleExitedAvatar(self, avId):
        self.notify.warning(
            "DistrbutedTravelGameAI: handleExitedAvatar: avatar id exited: " + str(avId)
        )
