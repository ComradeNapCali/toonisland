...
class DivingFishSpawn(DirectObject):
    ...
    def createFish(self, fishcode):
        ...
        if fishcode is 0:
            fish = Actor.Actor(
                "phase_4/models/char/clownFish-zero.bam",
            ...
            fish = Actor.Actor(
                loadBase + "balloonFish-zero.bam",
                {"anim": loadBase + "balloonFish-swim.bam"},
            )
            fish.name = "balloon"
        elif fishcode is 4:
            fish = Actor.Actor(
                loadBase + "nurseShark-zero.bam",
                {"anim": loadBase + "nurseShark-swim.bam"},
            )
            fish.name = "nurse"
        elif fishcode is 5:
            fish = Actor.Actor(
                loadBase + "pianoTuna-zero.bam",
        ...
        fish.moveloop = Sequence(
            Wait(4),
            LerpScaleInterval(fish, startScale=1, scale=3, duration=1),
            Wait(1.5),
