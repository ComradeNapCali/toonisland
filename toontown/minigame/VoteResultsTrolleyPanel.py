...
class VoteResultsTrolleyPanel(DirectFrame):
    ...
    def setupResultLabel(self):
        ...
        if self.directionReason == TravelGameGlobals.ReasonVote:
            if self.directionToGo == 0:
                ...
                losingDirection = 0
            diffVotes = (
                self.directionTotals[self.directionToGo]
    ...
    def createOnePlayerSequence(self, index, duration):
        ...
        track.append(
            LerpFunc(totalTicker, duration=duration, name="countTotal %d" % index)
        )
        if index in self.avVotesLabel:
            ...
            label = self.avVotesLabel[index]
            track.append(
                Func(self.avVotesLabel[index].show, name="showName %d" % index)
            )
            if index in self.avArrows:
                track.append(
                    Func(self.avArrows[index].show, name="showArrow %d" % index)
                )
            ...
            track.append(
                LerpFunc(
                    avVotesTicker, duration=duration, name="countAvVotes %d" % index
                )
            )
        return track
