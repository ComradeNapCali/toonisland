...
class DistributedDivingGame(DistributedMinigame):
    ...
    def fishCollision(self, collEntry):
        ...
        if len(name) >= 7:
            ...
            if spawnId in self.spawners[spawnerId].fishArray:
                self.sendUpdate(
                    "handleFishCollision", [avId, spawnId, spawnerId, toonSD.status]
                )
        ...
        if fish.name == "clown":
            fish.moveLerp = Sequence(
                LerpPosInterval(
                    ...
                    + Point3(
                        50 * self.spawners[spawnerId].direction, 0, (offset - 4) / 2.0
                    ),
                    name=iName,
                ),
                func,
            ...
            fish.moveLerp = Sequence(
                LerpPosInterval(
                    ...
                    startPos=self.spawners[spawnerId].position,
                    pos=self.spawners[spawnerId].position
                    + Point3(50 * self.spawners[spawnerId].direction, 0, 0),
                    name=iName,
                ),
                func,
            )
    ...
    def onstage(self):
        ...
        self.mapModel.setPos(-0.185, 0.0, -1.125)
    ...
    def getTelemetryLimiter(self):
        return TLGatherAllAvs(
            "DivingGame", Functor(DivingGameRotationLimiter, 180, 180)
        )
    ...
    def setGameReady(self):
        ...
        self.difficultyPatterns = {
            ToontownGlobals.ToonIslandCentral: [1, 1.5, 65, 3],
            ToontownGlobals.RainbowRise: [1, 1.3, 65, 1],
            ToontownGlobals.WitheringWoods: [2, 1.2, 65, 1],
            ToontownGlobals.OliveOasis: [2, 1.0, 65, 1],
            ToontownGlobals.CirrusCircus: [3, 1.0, 65, 1],
            ToontownGlobals.MintyMines: [3, 1.0, 65, 1],
        }
        ...
        self.spawners.append(
            DivingFishSpawn.DivingFishSpawn(
                0, 1, Point3(-spawnX, spawnY, 25), self.collHandEvent
            )
        )
        self.spawners.append(
            DivingFishSpawn.DivingFishSpawn(
                1, -1, Point3(spawnX, spawnY, 16), self.collHandEvent
            )
        )
        self.spawners.append(
            DivingFishSpawn.DivingFishSpawn(
                2, 1, Point3(-spawnX, spawnY, 6), self.collHandEvent
            )
        )
        self.spawners.append(
            DivingFishSpawn.DivingFishSpawn(
                3, -1, Point3(spawnX, spawnY, -4), self.collHandEvent
            )
        )
        self.spawners.append(
            DivingFishSpawn.DivingFishSpawn(
                4, 1, Point3(-spawnX, spawnY, -15), self.collHandEvent
            )
        )
        self.spawners.append(
            DivingFishSpawn.DivingFishSpawn(
                5, -1, Point3(spawnX, spawnY, -23), self.collHandEvent
            )
        )
        for spawner in self.spawners:
            spawner.lastSpawn = 0
    ...
    def setGameStart(self, timestamp):
        ...
        self.treasurePanel.setPos(0.145, 0, -0.25)
        self.treasurePanel.setBin("fixed", 0)
        ...
        if toonSD.status == "treasure" and self.treasures and self.chestIcons:
            for i in range(self.NUMTREASURES):
                if self.treasures[i].grabbedId == avId:
                    ...
                    place = Parallel(
                        Func(
                            self.treasures[i].treasureNode.setPos,
    ...
    def performCrabCollision(self, avId, timestamp):
        ...
        if toonSD.status == "normal" or toonSD.status == "treasure":
            ...
            self.localLerp = Sequence(
                Func(toonSD.fsm.request, "freeze"),
                Wait(3.0),
    ...
    def performFishCollision(self, avId, spawnId, spawnerId, timestamp):
        ...
        if spawnId in self.spawners[spawnerId].fishArray:
            ...
            if fish.name == "clown" or fish.name == "piano":
                ...
                fish.moveLerp = Sequence(
                    LerpHprInterval(
                        fish, duration=0.4, startHpr=fish.getHpr(), hpr=endHpr
                    ),
                    LerpFunc(
                        fish.setX, duration=1.5, fromData=fish.getX(), toData=endX
                    ),
                    Func(self.fishRemove, str(spawnerId) + str(spawnId)),
                )
            elif fish.name == "pbj":
                fish.moveLerp = Sequence(
                    LerpFunc(fish.setX, duration=2, fromData=fish.getX(), toData=endX),
                    Func(self.fishRemove, str(spawnerId) + str(spawnId)),
                )
            elif fish.name == "balloon":
    ...
    def __crabTask(self, task):
        ...
        for crab in self.crabs:
            if not crab.moveLerp.isPlaying():
                ...
                crab.moveLerp.loop()
                self.sendUpdate(
                    "getCrabMoving", [crab.crabId, crab.getX(), crab.direction]
                )
                return Task.cont
    ...
    def setCrabMoving(self, crabId, timestamp, rand1, rand2, crabX, dir):
        ...
        crabSoundInterval = SoundInterval(
            self.crabSound, loop=0, duration=1.6, startTime=0.0
        )
        seq = Sequence(
            Wait(wait),
            LerpPosInterval(
