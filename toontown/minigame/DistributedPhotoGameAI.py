    ...
    def enterPlay(self):
        ...
        if not config.GetBool("endless-photo-game", 0):
            taskMgr.doMethodLater(
                self.data["TIME"], self.timerExpired, self.taskName("gameTimer")
            )
    ...
    def filmOut(self):
        ...
        if avId not in self.avIdList:
            self.air.writeServerEvent(
                "suspicious", avId, "PhotoGameAI.filmOut: unknown avatar"
            )
            return
        if (
            self.gameFSM.getCurrentState() is None
            or self.gameFSM.getCurrentState().getName() != "play"
        ):
            self.air.writeServerEvent(
                "suspicious", avId, "PhotoGameAI.filmOut: game not in play state"
            )
            return
    ...
    def newClientPhotoScore(self, subjectIndex, pose, score):
        ...
        if (
            ...
            self.air.writeServerEvent(
                "suspicious",
                avId,
                "PhotoGameAI.newClientPhotoScore: game not in play state %s"
                % gameState,
            )
            return
        ...
        if avId not in self.avIdList:
            self.air.writeServerEvent(
                "suspicious", avId, "PhotoGameAI.newClientPhotoScore: non-player avatar"
            )
            return
        playerIndex = self.avIdList.index(avId)
        ...
        if (
            assignmentIndex != None
            and self.assignmentData[assignmentIndex][2][playerIndex] < score
        ):
            ...
            if highScorer == None:
                ...
                self.assignmentData[assignmentIndex][2][highScorer]
                < self.assignmentData[assignmentIndex][2][playerIndex]
            ):
                self.assignmentData[assignmentIndex][3] = playerIndex
            self.sendUpdate("newAIPhotoScore", [avId, assignmentIndex, score])
        self.notify.debug(
            "newClientPhotoScore %s %s %s %s" % (avId, subjectIndex, pose, score)
        )
