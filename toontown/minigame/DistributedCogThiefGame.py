...
class DistributedCogThiefGame(DistributedMinigame):
    ...
    def moveCameraToTop(self):
        ...
        camera.setZ(
            camera.getZ() + base.config.GetFloat("cog-thief-z-camera-adjust", 0.0)
        )
    ...
    def initGameWalk(self):
        ...
        if self.useOrthoWalk:
            ...
            orthoDrive = OrthoDrive(
                self.ToonSpeed, customCollisionCallback=doCollisions, instantTurn=True
            )
    ...
    def localToonHitBySuit(self, suitNum):
        ...
        self.sendUpdate(
            "hitBySuit", [self.localAvId, timestamp, suitNum, pos[0], pos[1], pos[2]]
        )
    ...
    def showToonHitBySuit(self, avId, timestamp):
        ...
        trajectory = Trajectory.Trajectory(
            0, Point3(0, 0, 0), Point3(0, 0, 50), gravMult=0.55
        )
        ...
        soundTrack = Sequence(
            ...
            Wait(flyDur * (2.0 / 3.0)),
            SoundInterval(self.sndTable["falling"][i], duration=flyDur * (1.0 / 3.0)),
    ...
    def updateSuitGoal(
        self, timestamp, inResponseToClientStamp, suitNum, goalType, goalId, x, y, z
    ):
        if not self.hasLocalToon:
            return
        self.notify.debug(
    ...
    def handleEnterBarrel(self, colEntry):
        ...
        if "CogThiefSphere" in intoName:
            ...
            if (
                cog.barrel == CTGG.NoBarrelCarried
                and barrelIndex not in self.stolenBarrels
            ):
                timestamp = globalClockDelta.localToNetworkTime(
                    globalClock.getFrameTime(), bits=32
                )
    ...
    def makeCogCarryBarrel(
        self, timestamp, inResponseToClientStamp, cogIndex, barrelIndex, x, y, z
    ):
        if not self.hasLocalToon:
            return
        if self.gameIsEnding:
        ...
        cog.makeCogCarryBarrel(
            timestamp, inResponseToClientStamp, barrel, barrelIndex, cogPos
        )
    ...
    def controlKeyPressed(self):
        ...
        if self.gameIsEnding:
            return
        if (
            self.getCurrentGameTime() - self.lastTimeControlPressed
        ...
        toss = Track(
            (
                ...
                Sequence(
                    ...
                    Parallel(
                        ActorInterval(toon, "throw", startFrame=48, partName="torso"),
                        animPie,
                    ),
    ...
    def __updateRewardCountdown(self, task):
        ...
        if self.clockStopTime is not None:
            if self.clockStopTime < curTime:
                self.notify.debug(
                    "self.clockStopTime < curTime %s %s" % (self.clockStopTime, curTime)
                )
                self.__killRewardCountdown()
