...
class DistributedMinigameAI(DistributedObjectAI.DistributedObjectAI):
    ...
    def setExpectedAvatars(self, avIds):
        ...
        self.notify.debug(
            "BASE: setExpectedAvatars: expecting avatars: " + str(self.avIdList)
        )
    ...
    def setGameReady(self):
        self.notify.debug(
            "BASE: setGameReady: game ready with avatars: %s" % self.avIdList
        )
        self.normalExit = 1
    ...
    def setAvatarJoined(self):
        ...
        self.air.writeServerEvent(
            "minigame_joined", avId, "%s|%s" % (self.minigameId, self.trolleyZone)
        )
    ...
    def enterFrameworkWaitClientsReady(self):
        ...
        def handleTimeout(avIds, self=self):
            self.notify.debug(
                "BASE: timed out waiting for clients %s to report 'ready'" % avIds
            )
            self.setGameAbort()
    ...
    def setAvatarReady(self):
        ...
        self.notify.debug(
            "BASE: setAvatarReady: new avId states: " + str(self.stateDict)
        )
    ...
    def setAvatarExited(self):
        ...
        self.stateDict[avId] = EXITED
        self.notify.debug(
            "BASE: setAvatarExited: new avId states: " + str(self.stateDict)
        )
        self.__barrier.clear(avId)
    ...
    def enterFrameworkCleanup(self):
        self.notify.debug(
            "BASE: enterFrameworkCleanup: normalExit=%s" % self.normalExit
        )
        ...
        for avId in self.avIdList:
            ...
            if (
                ...
                or ToontownGlobals.JELLYBEAN_TROLLEY_HOLIDAY_MONTH
                in simbase.air.holidayManager.currentHolidays
            ):
    ...
    def handleMetagamePurchaseManager(self, scoreList):
        ...
        for avId in self.avIdList:
            if avId in votesToUse:
                ...
                self.notify.warning(
                    "votesToUse=%s does not have avId=%d" % (votesToUse, avId)
                )
                votesArray.append(0)
    ...
    def setStartingVote(self, avId, startingVote):
        ...
        self.notify.debug(
            "setting starting vote of avId=%d to %d" % (avId, startingVote)
        )
