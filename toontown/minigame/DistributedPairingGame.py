...
class DistributedPairingGame(DistributedMinigame):
    ...
    def load(self):
        ...
        for index in range(len(self.deck.cards)):
            ...
            self.notify.debug(
                "suit %s rank %s value %s" % (oneCard.suit, oneCard.rank, oneCard.value)
            )
        ...
        for i in range(4):
            self.signalSfx.append(
                base.loader.loadSfx("phase_4/audio/sfx/MG_pairing_jumping_signal.ogg")
            )
    ...
    def handleMatch(self, cardA, cardB, withBonus):
        ...
        for card in [cardA, cardB]:
            ...
            cardSeq = Sequence(
                LerpColorScaleInterval(
                    self.cards[card], duration=1, colorScale=Vec4(1.0, 1.0, 1.0, 0.0)
                ),
