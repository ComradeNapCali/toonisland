...
class TwoDGameToonSD(StateData.StateData):
    ...
    def __init__(self, avId, game):
        ...
        self.fsm = ClassicFSM.ClassicFSM(
            ...
            [
                ...
                State.State(
                    "fallDown", self.enterFallDown, self.exitFallDown, ["normal"]
                ),
        ...
        self.scoreText = None
        self.load()
        self.progressLineLength = (
            self.game.assetMgr.faceEndPos[0] - self.game.assetMgr.faceStartPos[0]
        )
    ...
    def load(self):
        ...
        self.squishSound = base.loader.loadSfx(
            "phase_3.5/audio/dial/AV_" + self.toon.style.getAnimal() + "_exclaim.ogg"
        )
    ...
    def enterFallBack(self):
        ...
        if self.isLocal:
            ...
            if self.toon.getH() == 90:
                endPoint = Point3(
                    self.toon.getX() + fallBackDist, self.toon.getY(), self.toon.getZ()
                )
            else:
                endPoint = Point3(
                    self.toon.getX() - fallBackDist, self.toon.getY(), self.toon.getZ()
            ...
            enemyHitTrajectory = LerpFunc(
                ...
                toData=endPoint.getX(),
                duration=0.75,
                name="enemyHitTrajectory",
            )
    ...
    def enterSquish(self):
        ...
        self.squishIval = Parallel(
            self.blinkRed, Sequence(Wait(2.5), Func(self.resume))
        )
    ...
    def enterVictory(self):
        ...
        outsideElevatorPos = self.game.sectionMgr.exitElevator.find(
            "**/loc_elevator_front"
        ).getPos(render)
        ...
        runToElevator = Parallel(
            LerpPosInterval(
                self.toon, self.game.timeToRunToElevator, outsideElevatorPos
            ),
            ActorInterval(
                self.toon, self.RunAnim, loop=1, duration=self.game.timeToRunToElevator
            ),
        )
        danceIval = Parallel(
            ...
            ActorInterval(
                ...
                self.Victory,
                loop=1,
                duration=ToonBlitzGlobals.GameDuration[self.game.getSafezoneId()],
            ),
        )
        waitToLand = 0.0
    ...
    def showScoreText(self, number, scale=1.25):
        if not number == 0:
            ...
            self.scoreTextSeq = Sequence(
                self.scoreText.posInterval(
                    0.5, Point3(0, 0, self.toon.height + 2), blendType="easeOut"
                ),
