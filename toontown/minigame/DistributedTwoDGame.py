...
class DistributedTwoDGame(DistributedMinigame):
    ...
    def enterShowScores(self):
        ...
        self.showScoreTrack = Parallel(
            ...
            Sequence(Wait(ToonBlitzGlobals.ShowScoresDuration), Func(self.gameOver)),
        )
    ...
    def __updateLocalToonTask(self, task):
        ...
        if self.gameFSM.getCurrentState().getName() == "play":
            if (
                not self.toonSDs[self.localAvId].fsm.getCurrentState().getName()
                == "victory"
            ):
    ...
    def setupHeadCollision(self):
        ...
        elif animal == "mouse" or animal == "duck" or animal == "deer":
        ...
        self.accept(
            "enterheadCollSphere-into-floor1", self.__handleHeadCollisionIntoFloor
        )
    ...
    def localToonFellDown(self):
        if self.toonSDs[self.localAvId].fsm.getCurrentState().getName() != "fallDown":
            ...
            self.updateScore(
                self.localAvId,
                ToonBlitzGlobals.ScoreLossPerFallDown[self.getSafezoneId()],
            )
    ...
    def localToonHitByEnemy(self):
        ...
        if not (currToonState == "fallBack" or currToonState == "squish"):
            ...
            self.updateScore(
                self.localAvId,
                ToonBlitzGlobals.ScoreLossPerEnemyCollision[self.getSafezoneId()],
            )
    ...
    def localToonSquished(self):
        ...
        if not (currToonState == "fallBack" or currToonState == "squish"):
            ...
            self.updateScore(
                self.localAvId,
                ToonBlitzGlobals.ScoreLossPerStomperSquish[self.getSafezoneId()],
            )
    ...
    def setTreasureGrabbed(self, avId, sectionIndex, treasureIndex):
        ...
        if self.gameFSM.getCurrentState().getName() == "play":
            self.notify.debug(
                "treasure %s-%s grabbed by %s" % (sectionIndex, treasureIndex, avId)
            )
    ...
    def __enemyShot(self, sectionIndex, enemyIndex):
        self.sectionMgr.sections[sectionIndex].enemyMgr.enemies[
            enemyIndex
        ].doShotTrack()
    ...
    def setEnemyShot(self, avId, sectionIndex, enemyIndex, enemyHealth):
        ...
        if self.gameFSM.getCurrentState().getName() == "play":
            ...
            if enemyHealth > 0:
                ...
                enemy = self.sectionMgr.sections[sectionIndex].enemyMgr.enemies[
                    enemyIndex
                ]
                treasureSpawnPoint = Point3(
                    enemy.suit.getX(),
                    enemy.suit.getY(),
                    enemy.suit.getZ() + enemy.suit.height / 2.0,
                )
    ...
    def getElevatorCloseTrack(self):
        ...
        return Sequence(
            Wait(self.timeToRunToElevator), Parallel(leftDoorClose, rightDoorClose)
        )
