...
def getRingPoints(
    segCount, centerX, centerY, radius, thickness=2.0, wideX=1.0, wideY=1.0
):
    returnShape = []
    for seg in range(0, int(segCount)):
        coordX = wideX * circleX(
            pi * 2.0 * float(float(seg) / float(segCount)),
            radius - thickness,
            centerX,
            centerY,
        )
...
class DistributedTargetGame(DistributedMinigame):
    ...
    UMBRELLA_TEXTURE_LIST = [
        "phase_4/maps/mg_slingshot_umbrella_blue.jpg",
    ...
    def __init__(self, cr):
        ...
        self.gameFSM = ClassicFSM.ClassicFSM(
            ...
            [
                State.State("off", self.enterOff, self.exitOff, ["prelaunch"]),
                State.State(
                    "prelaunch", self.enterPrelaunch, self.exitPrelaunch, ["launch"]
                ),
                State.State("launch", self.enterLaunch, self.exitLaunch, ["fly"]),
                State.State(
                    "fly", self.enterFly, self.exitFly, ["fall", "bouncing", "score"]
                ),
                State.State(
                    "fall", self.enterFall, self.exitFall, ["bouncing", "score"]
                ),
                State.State(
                    "bouncing", self.enterBouncing, self.exitBouncing, ["score"]
                ),
                State.State(
                    "score", self.enterScore, self.exitScore, ["cleanup", "prelaunch"]
                ),
    ...
    def load(self):
        ...
        self.fogOver = FogOverlay.FogOverlay(
            Point3(self.CLOUD_COLOR[0], self.CLOUD_COLOR[1], self.CLOUD_COLOR[2])
        )
                ...
                while subIndex:
                    ...
                    if first:
                        ...
                        thickness = (
                            abs(
                                self.targetSize[combinedIndex]
                                - self.targetSize[combinedIndex + 1]
                            )
                            - 0.0
                        )
                        geo = addRing(
    ...
    def onstage(self):
        ...
        localBand = RubberBand.RubberBand(
            toon, Point3(0, -1.75, 0), taskPriority=self.UPDATE_RUBBER_BAND_PRIORITY
        )
    ...
    def enterFall(self):
        ...
        camera.posHprInterval(
            2.5, newPos, newHpr, blendType="easeInOut", name=self.FLY2FALL_CAM_TASK
        ).start()
        open = self.umbrella.find("**/open_umbrella")
        open.show()
        closed = self.umbrella.find("**/closed_umbrella")
    ...
    def enterScore(self):
        ...
        if hitTarget:
            ...
            addRing(
                targetGN,
                TargetGameGlobals.colorBlack,
                100,
                self.targetSize[hitTarget[2]] + 0.5,
                0,
            )
            targetNodePathGeom.setColorScale(
        ...
        if self.hitInfo == "OnButt":
            self.localTrack.append(
                ActorInterval(
                    base.localAvatar, "slip-forward", startFrame=25, endFrame=55
                )
            )
            ...
            self.localTrack.append(
                ActorInterval(
                    base.localAvatar, "slip-forward", startFrame=50, endFrame=55
                )
            )
        ...
        camera.posHprInterval(
            5.0, newPos, newHpr, blendType="easeInOut", name=self.SCORE_CAM_TASK
        ).start()
        self.help.hide()
