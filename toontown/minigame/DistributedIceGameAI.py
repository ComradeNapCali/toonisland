...
class DistributedIceGameAI(DistributedMinigameAI.DistributedMinigameAI):
    ...
    def __init__(self, air, minigameId):
        try:
            ...
            self.gameFSM = ClassicFSM.ClassicFSM(
                ...
                [
                    State.State(
                        "off", self.enterOff, self.exitOff, ["waitClientsChoices"]
                    ),
                    State.State(
                        "waitClientsChoices",
    ...
    def enterProcessEndingPositions(self):
        ...
        if (
            self.curMatch == IceGameGlobals.NumMatches - 1
            and self.curRound == IceGameGlobals.NumRounds - 1
        ):
    ...
    def enterScoreMatch(self):
        ...
        for index in range(len(self.avIdList)):
            ...
            pointsToAdd = (
                int(points + 0.5) + IceGameGlobals.BonusPointsForPlace[bonusIndex]
            )
        ...
        scoreMovieDuration = (
            IceGameGlobals.FarthestLength * IceGameGlobals.ExpandFeetPerSec
        )
    ...
    def enterFinalResults(self):
        ...
        taskMgr.doMethodLater(
            IceGameGlobals.ShowScoresDuration,
    ...
    def allAvatarsChosen(self):
        for avId in list(self.avatarChoices.keys()):
            ...
            if (
                choice[0] == -1
                and not self.stateDict[avId] == DistributedMinigameAI.EXITED
            ):
                return False
    ...
    def endingPositions(self, positions):
        if (
            not self.gameFSM
            or not self.gameFSM.getCurrentState()
    ...
    def claimPenalty(self, penaltyNum):
        ...
        if penaltyNum < 0 or penaltyNum >= self.numPenalties:
            self.air.writeServerEvent(
                "warning", penaltyNum, "IceGameAI.claimPenalty penaltyNum out of range"
            )
            return
