...
class DistributedRaceGame(DistributedMinigame):
    ...
    def startTimer(self):
        ...
        self.timer.countdown(
            RaceGameGlobals.InputTimeout - elapsed, self.handleChoiceTimeout
        )
        self.timer.show()
    ...
    def showChanceCard(self, task):
        ...
        self.chanceCard.posQuatInterval(
            1.0, (19.62, 13.41, 13.14), quat, other=camera, name="cardLerp"
        ).start()
        return Task.done
                    ...
                    if avatar:
                        ...
                        if squares_walked > 4:
                            ...
                            self.runInPlace(
                                ...
                                oldPosition,
                                position,
                                self.getWalkDuration(squares_walked),
                            )
                        else:
                            if choice > 0:
    ...
    def walkInPlace(self, avatar, lane, place, time):
        ...
        walkSeq = Sequence(
            Func(avatar.setAnimState, "walk", 1),
    ...
    def runInPlace(self, avatar, lane, currentPlace, newPlace, time):
        ...
        runSeq = Sequence(
            Func(avatar.setAnimState, "run", 1),
            avatar.posQuatInterval(
                time / 3.0, pos1[:3], pos1Quat, other=self.raceBoard
            ),
            avatar.posQuatInterval(
                time / 3.0, pos2[:3], pos2Quat, other=self.raceBoard
            ),
            avatar.posQuatInterval(
                time / 3.0, pos3[:3], pos3Quat, other=self.raceBoard
            ),
            Func(stopRun),
        )
        runSeq.start()
