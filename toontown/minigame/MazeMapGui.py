...
from panda3d.core import Vec2, VBase4F, CardMaker, NodePath, Texture, PNMImage
...
class MazeMapGui(DirectFrame):
    ...
    def __init__(
        ...
        DirectFrame.__init__(
            self, relief=None, state=DGG.NORMAL, sortOrder=DGG.BACKGROUND_SORT_INDEX
        )
        self.hide()
    ...
    def _revealSmoothCircle(self, x, y, center):
        ...
        self._maskImage.setXelA(
            ...
            y,
            VBase4F(
                0.0, 0.0, 0.0, min(self._maskImage.getAlpha(x, y), goalAlpha * 2.0)
            ),
        )
    ...
    def gui2pos(self, x, y):
        return (
            x / self._maskResolution * 2.0 - 0.97,
            0,
    ...
    def updateToon(self, toon, tX, tY):
        ...
        if tY < 0 or tY >= len(self._revealedCells):
            ...
            if len(self._revealedCells) > 0:
                self.notify.warning(
                    "len(_revealedCells[0]): %s" % (len(self._revealedCells[0]),)
                )
            return
        if tX < 0 or tX >= len(self._revealedCells[tY]):
            ...
            if tY < len(self._revealedCells):
                self.notify.warning(
                    "len(_revealedCells[tY]): %s" % (len(self._revealedCells[tY]),)
                )
