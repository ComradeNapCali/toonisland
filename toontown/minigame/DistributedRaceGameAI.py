...
class DistributedRaceGameAI(DistributedMinigameAI):
    ...
    def setAvatarChoice(self, choice):
        ...
        self.notify.debug(
            "setAvatarChoice: avatar: " + str(avatarId) + " chose: " + str(choice)
        )
    ...
    def processChoice(self, avId, choice, freq=1):
        self.notify.debug(
            "processChoice: av = " + str(avId) + " choice = " + str(choice)
        )
        if freq == 1:
            if choice != 0:
    ...
    def checkForWinners(self):
        ...
        self.sendUpdate(
            "setServerChoices", [self.choiceArray, self.positionArray, self.rewardArray]
        )
        delay = 0.0
        ...
        if self.anyAvatarWon(self.avatarPositions):
            ...
            taskMgr.doMethodLater(
                delay, self.rewardTimeoutTaskGameOver, self.taskName("reward-timeout")
            )
        else:
            taskMgr.doMethodLater(
                delay, self.rewardTimeoutTask, self.taskName("reward-timeout")
            )
            ...
            if reward != -1:
                for avId in self.avIdList:
                    if self.avIdList.index(avId) == rewardIndex:
                        self.avatarChoices[avId] = RaceGameGlobals.ChanceRewards[
                            reward
                        ][0][0]
                    else:
                        self.avatarChoices[avId] = RaceGameGlobals.ChanceRewards[
