...
class Purchase(PurchaseBase):
    ...
    def __init__(
        ...
        self.fsm.addState(
            State.State("reward", self.enterReward, self.exitReward, ["purchase"])
        )
    ...
    def load(self):
        ...
        self.bg = loader.loadModel(
            "phase_%s/models/modules/toon_interior" % interiorPhase
        )
        ...
        for index in range(len(self.ids)):
            ...
            if (
                self.states[index] != PURCHASE_NO_CLIENT_STATE
    ...
    def handleDone(self, playAgain):
        ...
        if playAgain:
            self.doneStatus = {"loader": "minigame", "where": "minigame"}
        else:
            self.doneStatus = {"loader": "safeZoneLoader", "where": "playground"}
    ...
    def enterReward(self):
        ...
        if self.votesArray:
            ...
            self.voteMultiplier = (
                TravelGameGlobals.PercentOfVotesConverted[numToons] / 100.0
            )
        ...
        if self.metagameRound == TravelGameGlobals.FinalMetagameRoundIndex:
            ...
            taskMgr.doMethodLater(
                countVotesUpDelay, reqCountVotesUp, "countVotesUpTask"
            )
            countVotesUpTime = self.maxVotes * COUNT_UP_RATE + DELAY_AFTER_COUNT_UP
            countVotesDownDelay = countVotesUpDelay + countVotesUpTime
            taskMgr.doMethodLater(
                countVotesDownDelay, reqCountVotesDown, "countVotesDownTask"
            )
            celebrateDelay += (
                countVotesUpTime
                + self.maxVotes * COUNT_DOWN_RATE
    ...
    def _countDownTask(self, task):
        ...
        startT = task.getStartTime()
        if now >= startT + task.duration:
            for counter, total, toonId in zip(
                self.counters, self.totalCounters, self.ids
            ):
                if counter.count != 0:
    ...
    def countVotesDown(self):
        ...
        for count in range(0, self.maxVotes):
            for counter in self.counters:
                if count < counter.max:
                    index = self.counters.index(counter)
                    subtractTask = taskMgr.doMethodLater(
                        totalDelay, delaySubtract, "delaySubtract"
                    )
    ...
    def enterPurchase(self):
        ...
        if not self.tutorialMode:
            ...
            self.accept(
                "disableGagPanel",
                Functor(
                    self.toon.inventory.setActivateMode, "gagTutDisabled", gagTutMode=1
                ),
            )
    ...
    def doMetagamePlayAgain(self):
        ...
        if numToons > 1:
            if (
                ...
                and self.metagameRound < TravelGameGlobals.FinalMetagameRoundIndex
            ):
...
class PurchaseHeadFrame(DirectFrame):
    ...
    def __init__(self, av, purchaseModels):
        DirectFrame.__init__(
            self, relief=None, image=purchaseModels.find("**/Char_Pnl")
        )
    ...
    def setAvatarState(self, state):
        if state == PURCHASE_DISCONNECTED_STATE:
            self.statusLabel["text"] = (
                TTLocalizer.GagShopPlayerDisconnected % self.av.getName()
            )
            ...
            self.statusLabel["text"] = (
                TTLocalizer.GagShopPlayerExited % self.av.getName()
            )
            self.statusLabel["text_pos"] = (0.015, 0.072, 0)
            self.head.hide()
            self.tag1.hide()
            ...
            Purchase.notify.warning(
                "setAvatarState('no client state'); OK for gag purchase tutorial"
            )
        else:
