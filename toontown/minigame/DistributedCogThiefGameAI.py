...
class DistributedCogThiefGameAI(DistributedMinigameAI.DistributedMinigameAI):
    ...
    def enterPlay(self):
        ...
        if not config.GetBool("cog-thief-endless", 0):
            taskMgr.doMethodLater(
                CTGG.GameTime, self.timerExpired, self.taskName("gameTimer")
            )
    ...
    def makeCogCarryBarrel(self, timestamp, clientStamp, cogIndex, barrelIndex):
        if cogIndex in self.cogInfo and barrelIndex in self.barrelInfo:
            ...
            self.notify.warning(
                "makeCogCarryBarrel invalid cogIndex=%s barrelIndex=%s"
    ...
    def b_makeCogCarryBarrel(self, clientStamp, cogIndex, barrelIndex):
        ...
        self.notify.debug(
            "b_makeCogCarryBarrel timeStamp=%s clientStamp=%s cog=%s barrel=%s"
    ...
    def d_makeCogCarryBarrel(self, timestamp, clientStamp, cogIndex, barrelIndex):
        ...
        self.sendUpdate(
            "makeCogCarryBarrel",
            [timestamp, clientStamp, cogIndex, barrelIndex, pos[0], pos[1], pos[2]],
        )

    def makeCogDropBarrel(
        self, timestamp, clientStamp, cogIndex, barrelIndex, barrelPos
    ):
        if cogIndex in self.cogInfo and barrelIndex in self.barrelInfo:
            ...
            self.notify.warning(
                "makeCogDropBarrel invalid cogIndex=%s barrelIndex=%s"
                % (cogIndex, barrelIndex)
            )
    ...
    def b_makeCogDropBarrel(self, clientStamp, cogIndex, barrelIndex, barrelPos):
        if self.barrelInfo[barrelIndex]["carriedBy"] != cogIndex:
            self.notify.error(
                "self.barrelInfo[%s]['carriedBy'] != %s" % (barrelIndex, cogIndex)
            )
        timestamp = globalClockDelta.localToNetworkTime(
            globalClock.getFrameTime(), bits=32
        )
        self.makeCogDropBarrel(timestamp, clientStamp, cogIndex, barrelIndex, barrelPos)
        self.d_makeCogDropBarrel(
            timestamp, clientStamp, cogIndex, barrelIndex, barrelPos
        )

    def d_makeCogDropBarrel(
        self, timestamp, clientStamp, cogIndex, barrelIndex, barrelPos
    ):
        pos = barrelPos
        gameTime = self.getCurrentGameTime()
    ...
    def chaseToon(self, suitNum, avId):
        ...
        self.notify.debug(
            "chaseToon time=%s suitNum=%s, avId=%s" % (timestamp, suitNum, avId)
        )
        gameTime = self.getCurrentGameTime()
        self.sendUpdate(
            "updateSuitGoal",
    ...
    def chaseBarrel(self, suitNum, barrelIndex):
        ...
        self.notify.debug(
            "chaseBarrel time=%s suitNum=%s, barrelIndex=%s"
    ...
    def chooseReturnPos(self, cogIndex, cogPos):
        ...
        for retIndex in range(len(CTGG.CogReturnPositions)):
            ...
            if distance < shortestDistance:
                ...
                self.notify.debug(
                    "shortest distance=%s index=%s"
                    % (shortestDistance, shortestReturnIndex)
                )
    ...
    def cogAtReturnPos(self, clientstamp, cogIndex, barrelIndex):
        if cogIndex not in self.cogInfo or barrelIndex not in self.barrelInfo:
            ...
            self.air.writeServerEvent(
                "suspicious cogAtReturnPos avId=%s, cogIndex=%s, barrelIndex=%s"
                % (avId, cogIndex, barrelIndex)
            )
            return
