...
class DistributedIceGame(
    DistributedMinigame.DistributedMinigame, DistributedIceWorld.DistributedIceWorld
):
    ...
    def load(self):
        ...
        for tireIndex in range(4):
            ...
            self.tireSounds.append(
                {"tireHit": tireHit, "wallHit": wallHit, "obstacleHit": obstacleHit}
            )
    ...
    def enterInputChoice(self):
        ...
        self.notify.debug(
            "self.localForceArrow() heading = %s" % self.localForceArrow().getH()
        )
    ...
    def enterScoring(self):
        ...
        for index in range(len(sortedByDistance)):
            ...
            self.scoreMovie.append(
                Parallel(
                    LerpScaleInterval(
                        self.scoreCircle, time, Point3(scaleXY, scaleXY, 1.0)
                    ),
                    SoundInterval(
                        self.scoreCircleSound, duration=time, startTime=circleStartTime
                    ),
                )
            )
            ...
            self.scoreMovie.append(
                Parallel(
                    ...
                    Sequence(
                        LerpColorScaleInterval(
                            tireNp, duration / 6.0, VBase4(1, 0, 0, 1)
                        ),
                        LerpColorScaleInterval(
                            tireNp, duration / 6.0, VBase4(1, 1, 1, 1)
                        ),
                        LerpColorScaleInterval(
                            tireNp, duration / 6.0, VBase4(1, 0, 0, 1)
                        ),
    ...
    def enterFinalResults(self):
        ...
        self.showScoreTrack = Parallel(
            lerpTrack,
            Sequence(Wait(IceGameGlobals.ShowScoresDuration), Func(self.gameOver)),
        )
    ...
    def startTimer(self):
        ...
        self.timer.countdown(
            IceGameGlobals.InputTimeout - elapsed, self.handleChoiceTimeout
        )
        self.timer.show()
    ...
    def setupStartOfMatch(self):
        ...
        while len(self.treasures) < self.numTreasures:
            xPos = self.randomNumGen.randrange(
                IceGameGlobals.MinWall[0] + 5, IceGameGlobals.MaxWall[0] - 5
            )
            yPos = self.randomNumGen.randrange(
                IceGameGlobals.MinWall[1] + 5, IceGameGlobals.MaxWall[1] - 5
            )
            ...
            if goodSpot:
                for treasure in self.treasures:
                    if (
                        newTreasure.nodePath.getDistance(treasure.nodePath)
                        < treasureMargin
                    ):
                        goodSpot = False
                        break
        ...
        while len(self.penalties) < self.numPenalties:
            ...
            if goodSpot:
                for treasure in self.treasures:
                    if (
                        newPenalty.nodePath.getDistance(treasure.nodePath)
                        < treasureMargin
                    ):
                        goodSpot = False
                        break
            ...
            if goodSpot:
                for penalty in self.penalties:
                    if (
                        newPenalty.nodePath.getDistance(penalty.nodePath)
                        < treasureMargin
                    ):
                        goodSpot = False
                        break
