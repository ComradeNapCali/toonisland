...
class DistributedCannonGame(DistributedMinigame):
    ...
    def announceToonWillLandInWater(self, avId, landTime):
        if not self.hasLocalToon:
            return
        self.notify.debug(
            "announceToonWillLandInWater: " + str(avId) + ": time=" + str(landTime)
        )
    ...
    def __getCameraPositionBehindCannon(self):
        return Point3(
            self.cannonLocationDict[self.localAvId][0], CANNON_Y - 25.0, CANNON_Z + 7
        )
    ...
    def __fireCannonTask(self, task):
        ...
        (
            info["timeEnterTowerXY"],
            info["timeExitTowerXY"],
    ...
    def __flyTask(self, task):
        ...
        if (
            t >= task.info["timeEnterTowerXY"]
            and t <= task.info["timeExitTowerXY"]
            and pos[2] >= self.tower.getPos(render)[2] + TOWER_HEIGHT
        ):
            shadowPos.setZ(
        ...
        if task.info["avId"] == self.localAvId:
            ...
            task.info["maxCamPullback"] = max(
                ...
                CAMERA_PULLBACK_MIN
                + multiplier * (CAMERA_PULLBACK_MAX - CAMERA_PULLBACK_MIN),
            )
        ...
        if t == task.info["timeOfImpact"]:
            ...
            if task.info["hitWhat"] == self.HIT_WATER:
                ...
                s = Sequence(
                    Wait(0.5),
                    toon.posInterval(
                        duration=LAND_TIME - 0.5, pos=hitPos, blendType="easeIn"
                    ),
                )
    ...
    def __stopIntro(self):
        ...
        if self.__cameraLookAtInterval:
            self.__cameraLookAtInterval.finish()
            self.__cameraLookAtInterval = None
    ...
    def __spawnCameraLookAtLerp(self, targetPos, targetLookAt, duration):
        ...
        self.__cameraLookAtInterval = camera.posQuatInterval(
            duration,
            Point3(targetPos),
            targetQuat,
            blendType="easeInOut",
        ...
        self.__cameraLookAtInterval.start()

    def __taskLookInWater(self, task):
        task.data["cannonCenter"] = Point3(0, CANNON_Y, CANNON_Z)
        task.data["towerWaterCenter"] = Point3(
            self.towerPos + Point3(0, 0, TOWER_HEIGHT)
        )
        task.data["vecTowerToCannon"] = Point3(
            task.data["cannonCenter"] - task.data["towerWaterCenter"]
        )
    ...
    def __flyToBackOfCannon(self, task):
        ...
        self.__cameraLookAtInterval = Parallel(
            lerpNode.hprInterval(
                self.T_TOONHEAD2CANNONBACK, endRotation, blendType="easeInOut"
            ),
            camera.posInterval(
                self.T_TOONHEAD2CANNONBACK, endPos, blendType="easeInOut"
            ),
            name=self.INTRO_TASK_NAME_CAMERA_LERP,
        )
        self.__cameraLookAtInterval.start()
