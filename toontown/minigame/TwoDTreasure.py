...
class TwoDTreasure(DirectObject):
    ...
    def __init__(self, treasureMgr, index, pos, value, isEnemyGenerated, model):
        ...
        self.glowScalIval = Sequence(
            LerpScaleInterval(self.glowCard, 0.4, scale=4.1, startScale=4.9),
            LerpScaleInterval(self.glowCard, 0.4, scale=4.9, startScale=4.1),
        )
        self.glowScalIval.loop()
        self.modelScalIval = Sequence(
            LerpScaleInterval(self.model, 0.4, scale=1.0, startScale=1.03),
        ...
        if self.isEnemyGenerated:
            ...
            self.flash.setPos(
                self.model.getX(), self.model.getY() - 0.2, self.model.getZ()
            )
