...
def removeUnreleasedMinigames(startList, increaseChanceOfNewGames=0):
    ...
    for gameId in ToontownGlobals.MinigameReleaseDates:
        ...
        releaseTime = time.mktime(
            (dateTuple[0], dateTuple[1], dateTuple[2], 0, 0, 0, 0, 0, -1)
        )
        ...
        if currentTime < releaseTime:
            if gameId in randomList:
                doRemove = True
                if (
                    gameId == ToontownGlobals.CogThiefGameId
                    and simbase.air.config.GetBool("force-allow-thief-game", 0)
                ):
                    doRemove = False
                    if increaseChanceOfNewGames:
                        randomList += [gameId] * 4
                elif gameId == ToontownGlobals.IceGameId and simbase.air.config.GetBool(
                    "force-allow-ice-game", 0
                ):
                    doRemove = False
                    if increaseChanceOfNewGames:
                        randomList += [gameId] * 4
                elif (
                    gameId == ToontownGlobals.TwoDGameId
                    and simbase.air.config.GetBool("force-allow-2d-game", 0)
                ):
                    doRemove = False
                    if increaseChanceOfNewGames:
                        randomList += [gameId] * 4
                elif (
                    gameId == ToontownGlobals.PhotoGameId
                    and simbase.air.config.GetBool("force-allow-photo-game", 0)
                ):
                    doRemove = False
                    if increaseChanceOfNewGames:
                        randomList += [gameId] * 4
        ...
        if (
            ...
            and currentTime < releaseTimePlus1Week
            and gameId in randomList
            and increaseChanceOfNewGames
        ):
            randomList += [gameId] * 4
