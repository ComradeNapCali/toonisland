...
class TwoDBattleMgr(DirectObject):
    ...
    def __init__(self, game, toon):
        ...
        self.WATER_BULLET_END_POINT = Point3(
            0,
    ...
    def createShootCollision(self):
        ...
        self.waterBulletIval.append(
            LerpPosInterval(
                self.waterBullet,
                0.25,
                pos=Point3(self.WATER_BULLET_END_POINT),
                startPos=Point3(self.WATER_BULLET_START_POINT),
                name="waterBulletMoveFront",
            )
        )
        self.waterBulletIval.append(
            Func(self.waterBullet.setPos, self.WATER_BULLET_HIDE_POINT)
        )
    ...
    def getToonShootTrack(self):
        ...
        toonTrack = Sequence(
            ActorInterval(
                self.toon, "water-gun", startFrame=48, endFrame=58, partName="torso"
            ),
            ActorInterval(
                self.toon,
                "water-gun",
    ...
    def calcSprayEndPos(self):
        ...
        endPos = Point3(
            self.origin.getX() + self.SHOOT_DISTANCE * xDirection,
            self.origin.getY(),
            self.origin.getZ(),
        )
        self.target = endPos
    ...
    def getSprayTrack(self):
        ...
        track = Sequence(
            Wait(0.1),
            Func(self.calcSprayStartPos),
            Func(self.calcSprayEndPos),
