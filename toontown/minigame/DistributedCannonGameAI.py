...
class DistributedCannonGameAI(DistributedMinigameAI):
    ...
    def _checkCannonRange(self, zRot, angle, avId):
        outOfRange = 0
        if (
            zRot < CannonGameGlobals.CANNON_ROTATION_MIN
            or zRot > CannonGameGlobals.CANNON_ROTATION_MAX
        ):
            self.air.writeServerEvent(
                "suspicious", avId, "Cannon game z-rotation out of range: %s" % zRot
            )
            self.notify.warning(
            ...
            outOfRange = 1
        if (
            angle < CannonGameGlobals.CANNON_ANGLE_MIN
            or angle > CannonGameGlobals.CANNON_ANGLE_MAX
        ):
            self.air.writeServerEvent(
                "suspicious",
                avId,
