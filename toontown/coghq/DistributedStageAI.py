...
class DistributedStageAI(DistributedObjectAI.DistributedObjectAI):
    ...
    def __init__(
        self, air, stageId, zoneId, floorNum, avIds, layoutIndex, battleExpAggreg=None
    ):
    ...
    def generate(self):
        ...
        self.notify.info(
            "generate %s, id=%s, floor=%s" % (self.doId, self.stageId, self.floorNum)
        )
        ...
        if self.battleExpAggreg is None:
            self.battleExpAggreg = (
                BattleExperienceAggregatorAI.BattleExperienceAggregatorAI()
            )
