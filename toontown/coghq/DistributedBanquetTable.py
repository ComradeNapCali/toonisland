...
class DistributedBanquetTable(
    ...
    def makeToonGrabInterval(self, toon):
        ...
        self.toonPitcherPosition = Point3(
            self.handPos[0] - rightHandPos[0], self.handPos[1] - rightHandPos[1], 0
        )
            ...
            Parallel(
                ...
                Sequence(
                    ...
                    Parallel(
                        ...
                        LerpScaleInterval(
                            self.waterPitcherModel, 0.9, Point3(1, 1, destZScale)
                        ),
                    ),
                ),
            ),
    ...
    def d_sendPitcherPos(self):
        ...
        self.sendUpdate(
            "setPitcherPos", [self.changeSeq, self.waterPitcherNode.getH(), timestamp]
        )
    ...
    def getSprayTrack(
        ...
        track.append(
            LerpScaleInterval(sprayScale, dScaleDown, Point3(0.01, 0.01, 0.01))
        )
    ...
    def fireWater(self, origin, target):
        ...
        sprayTrack = self.getSprayTrack(
            color, origin, target, dScaleUp, dHold, dScaleDown, horizScale, vertScale
        )
        duration = self.squirtSfx.length()
        if sprayTrack.getDuration() < duration:
            duration = sprayTrack.getDuration()
        soundTrack = SoundInterval(
            self.squirtSfx, node=self.waterPitcherModel, duration=duration
        )
