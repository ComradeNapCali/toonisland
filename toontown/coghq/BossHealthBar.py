...
class BossHealthBar:
    ...
    def __init__(self):
        self.bossBarFrame = DirectFrame(
            pos=(0, 0, self.bossBarStartPosZ), scale=1.3, sortOrder=20
        )
        self.bossBar = DirectWaitBar(
            relief=DGG.SUNKEN,
            scale=(0.197, 0, 0.135),
            value=100,
            pos=(0.805, -0.120, 0.002),
            frameSize=(-2.0, 2.0, -0.2, 0.2),
            borderWidth=(0.02, 0.02),
            range=100,
            sortOrder=50,
            frameColor=(0.5, 0.5, 0.5, 0.6),
            barColor=(0.75, 0.75, 1.0, 0.7),
            text="0 / 0",
            text_scale=(0.3, 0.4),
            text_fg=(1, 1, 1, 1),
    ...
    def initialize(self, hp, maxhp):
        ...
        self.bossBar.show()
        Sequence(
            self.bossBarFrame.posInterval(
                1.0, Point3(0, 0, self.bossBarEndPosZ), blendType="easeOut"
            )
        ).start()
    ...
    def update(self, hp, maxHp):
        ...
        if self.maxHp != 0:
            if self.currHp != self.newHp:
                smoothUpdateTask = Task.loop(
                    Task(self.__smoothUpdate), Task.pause(0.01)
                )
    ...
    def deinitialize(self):
        Sequence(
            self.bossBarFrame.posInterval(
                1.0, Point3(0, 0, self.bossBarStartPosZ), blendType="easeIn"
            )
        ).start()
