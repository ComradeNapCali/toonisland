...
class SellbotCogHQLoader(CogHQLoader.CogHQLoader):
    ...
    def __init__(self, hood, parentFSMState, doneEvent):
        ...
        self.fsm.addState(
            State.State(
                "fatalInterior",
                self.enterFatalInterior,
                self.exitFatalInterior,
                ["quietZone", "factoryExterior"],
            )
        )
        for stateName in ["quietZone"]:
    ...
    def loadPlaceGeom(self, zoneId):
        ...
        if zoneId == ToontownGlobals.SellbotHQ:
            ...
            dgText = DirectGui.OnscreenText(
                text=TTLocalizer.WitheringWoods[-1],
            ...
            self.spotlightController.start(
                self.geom.find("**/SpotLights").getChildren()
            )
            ...
            fatalTypeText = DirectGui.OnscreenText(
                text=fatalTypeText,
                font=ToontownGlobals.getSuitFont(),
                pos=(0, -0.25),
                scale=0.075,
            ...
            fatalText = DirectGui.OnscreenText(
                text=fatalText,
                font=ToontownGlobals.getSuitFont(),
                pos=(0, -0.34),
                scale=0.1,
