...
class DistributedStomper(DistributedCrusherEntity.DistributedCrusherEntity):
        ...
        if self.motion is MotionLinear:
            ...
            def motionFunc(t, self=self):
                ...
                if t < stickTime:
                    ...
                    self.model.setFluidY(
                        (t - stickTime) * -self.range / (turnaround - stickTime)
                    )
                elif t > turnaround:
                    self.model.setFluidY(
                        -self.range + (t - turnaround) * self.range / (1 - turnaround)
                    )
            ...
            def motionFunc(t, self=self):
                ...
                if t < stickTime:
                    ...
                    self.model.setFluidY(
                        (t - stickTime)
                        * -self.range
                        / (turnaround - pauseAtTopTime - stickTime)
                    )
            ...
            motionIval = Sequence(
                ...
                Func(self.doCrush),
                LerpPosInterval(
                    self.model, tDown, Vec3(0, 0, 0), blendType="easeInOut", fluid=1
                ),
            )
            ...
            if mode == STOMPER_STOMP:
                motionIval = Sequence(
                    Func(self.doCrush),
                    LerpPosInterval(
                        self.model, 0.35, Vec3(0, 0, 0), blendType="easeInOut", fluid=1
                    ),
                )
    ...
    def startStomper(self, startTime, mode=STOMPER_START):
        ...
        if motionIval == None:
            return
        self.ival = Parallel(
            Sequence(motionIval, Func(self.__startSmokeTask), Func(self.sendStompToon)),
        ...
        if wantSound:
            ...
            self.ival.append(
                Sequence(
                    Wait(sndDur),
                    Func(base.playSfx, self.sound, node=self.model, volume=0.45),
                )
            )
    ...
    def __smokeTask(self, task):
        ...
        if self.smokeTrack:
            self.smokeTrack.finish()
            del self.smokeTrack
        self.smokeTrack = Sequence(
            Parallel(
                LerpScaleInterval(self.smoke, 0.2, Point3(4, 1, 4)),
