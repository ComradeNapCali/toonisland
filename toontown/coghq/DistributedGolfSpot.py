...
class DistributedGolfSpot(DistributedObject.DistributedObject, FSM.FSM):
    ...
    def makeToonReleaseInterval(self, toon):
        ...
        if self.gotHitByBoss:
            grabIval = Sequence(
                Func(self.detachClub), name="makeToonReleaseInterval-gotHitByBoss"
            )
            if not toon.isEmpty():
                toonIval = Sequence(
            ...
            if not toon.isEmpty():
                toonIval = Sequence(
                    Parallel(
                        ...
                        LerpPosInterval(toon, duration=1.0, pos=Point3(-10, 0, 0)),
                    ),
                    Func(toon.wrtReparentTo, render),
                )
                grabIval.append(toonIval)
    ...
    def startBallPlayback(self, power, angle, sequenceNum):
        ...
        flyWithSound = Parallel(
            fly, SoundInterval(self.hitBallSfx, node=self.root), name="flyWithSound"
        )
    ...
    def __flyBallHit(self, entry):
        ...
        if (
            flyBallCode == ToontownGlobals.PieCodeBossCog
            and self.avId == localAvatar.doId
            and self.lastHitSequenceNum != self.__flyBallSequenceNum
        ):
            self.lastHitSequenceNum = self.__flyBallSequenceNum
            self.boss.d_ballHitBoss(1)
        elif (
            flyBallCode == ToontownGlobals.PieCodeToon
            and self.avId == localAvatar.doId
            and self.lastHitSequenceNum != self.__flyBallSequenceNum
        ):
            self.lastHitSequenceNum = self.__flyBallSequenceNum
    ...
    def getFlyBallSplatInterval(self, x, y, z, flyBallCode, throwerId):
        ...
        if (
            flyBallCode == ToontownGlobals.PieCodeBossCog
            and localAvatar.doId == throwerId
        ):
            vol = 1.0
