...
class GardenPage(ShtikerPage.ShtikerPage):
    ...
    def createGardenSpecialsList(self):
        ...
        if len(specialsList) == 0:
            ...
            for entry in specialsList:
                if not firstEntry:
                    firstEntry = entry
                someItem = DirectScrolledListItem(
                    parent=self.gardenSpecialsList,
                    text="%s x %s"
                    % (GardenGlobals.Specials[entry[0]]["photoName"], entry[1]),
                    text_align=TextNode.ALeft,
                    text_fg=(0.0, 0.0, 0.0, 1),
                    text_bg=(1.0, 1.0, 1, 0),
                    text_scale=0.06,
                    relief=None,
    ...
    def updatePage(self):
        ...
        if hasattr(self, "shovelBar"):
            ...
            maxWateringCanSkill = GardenGlobals.WateringCanAttributes[wateringCan][
                "skillPts"
            ]
            ...
            self.shovelBar["value"] = (
                float(curShovelSkill) / float(maxShovelSkill) * self.barLength
                + self.FUDGE_FACTOR
            )
            ...
            self.wateringCanBar["value"] = (
                float(curWateringCanSkill) / float(maxWateringCanSkill) * self.barLength
                + self.FUDGE_FACTOR
            )
        else:
            print("no shovel bar")
