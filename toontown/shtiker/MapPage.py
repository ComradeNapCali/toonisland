...
class MapPage(ShtikerPage.ShtikerPage):
    ...
    def load(self):
        ...
        for hood in ToontownGlobals.Hoods:
            if hood not in [ToontownGlobals.GolfZone, ToontownGlobals.CirrusCircus]:
                self.allZones.append(hood)

        self.cloudScaleList = (
            ((0.55, 0, 0.4), (0.35, 0, 0.25)),
            (),
        ...
        for hood in self.allZones:
            ...
            hoodClouds = []
            for cloudScale, cloudPos in zip(
                self.cloudScaleList[hoodIndex], self.cloudPosList[hoodIndex]
            ):
                cloud = DirectFrame(
                    parent=self.map,
                    relief=None,
    ...
    def enter(self):
        ...
        if (
            base.cr.playGame.getPlaceId() == ToontownGlobals.MyEstate
        ...
        if base.cr.playGame.getPlaceId() == ToontownGlobals.MyEstate:
            ...
            hoodName = ToontownGlobals.hoodNameMap.get(
                ZoneUtil.getCanonicalHoodId(zone), ("",)
            )[-1]
            streetName = ToontownGlobals.StreetNames.get(
                ZoneUtil.getCanonicalBranchZone(zone), ("",)
            )[-1]
            if hoodName:
    ...
    def backToSafeZone(self):
        self.doneStatus = {"mode": "teleport", "hood": base.localAvatar.lastHood}
    ...
    def __buttonCallback(self, hood):
        if (
            hood in base.localAvatar.getTeleportAccess()
