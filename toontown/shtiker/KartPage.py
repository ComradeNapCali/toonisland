...
class KartPage(ShtikerPage):
    ...
    def load(self):
        ...
        self.title = DirectLabel(
            parent=self, relief=None, text="", text_scale=0.1, pos=(0, 0, 0.65)
        )
        normalColor = (1, 1, 1, 1)
...
class KartCustomizeUI(DirectFrame):
    ...
    def __init__(self, avatar, parent=aspect2d):
        ...
        DirectFrame.__init__(
            self, parent=parent, relief=None, pos=(0.0, 0.0, 0.0), scale=(1.0, 1.0, 1.0)
        )
        return
...
class RacingTrophiesUI(DirectFrame):
    ...
    def grow(self, index, pos):
        ...
        self.trophyTextDisplay["text"] = TTLocalizer.KartPageTrophyDetail % (
            index + 1,
            TTLocalizer.KartTrophyDescriptions[index],
        )
...
class ItemSelector(DirectFrame):
    ...
    class ItemViewer(DirectFrame):
        ...
        def __init__(self, avatar, parent=aspect2d):
            ...
            self.texCount = 1
            DirectFrame.__init__(
                self, parent=parent, relief=None, pos=(0, 0, 0), scale=(1.0, 1.0, 1.0)
            )
            return
            ...
            if category == InvalidEntry:
                ...
                if category in colorTypeList:
                    if (
                        ...
                        or self.currItem not in accessDict.get(KartDNA.bodyColor)
                    ):
                        self.__updateDeleteButton(DGG.DISABLED)
                    else:
                        self.__updateDeleteButton(
                ...
                if len(self.itemList) == 1:
                    if self.currAccessoryType == KartDNA.rimsType:
                        self.disable()
                        self.setViewerText(
                            TTLocalizer.KartShtikerDefault
                            % getattr(
                                TTLocalizer,
        ...
        def __updateViewerUI(self):
            ...
            if self.currItem != InvalidEntry:
                self.__handleShowItem()
                if (
                    self.currItem not in self.avatar.accessories
                    and self.currAccessoryType in accList
                ):
                    self.__updateDeleteButton(DGG.DISABLED)
                else:
        ...
        def __handleItemDelete(self):
            ...
            self.notify.debug(
                "__handleItemDelete: Delete request on accessory %s" % self.currItem
            )
            self.confirmDlg.hide()
            messenger.send("wakeup")
    ...
    def load(self, uiRootNode):
        ...
        self.itemViewers["main"].setPos(
            self.getParent(), uiRootNode.find("**/uiAccessoryView").getPos()
        )
...
class KartViewer(DirectFrame):
    ...
    def load(
        ...
        self.uiBgFrame = DirectFrame(
            parent=self, relief=None, geom=uiRootNode.find("**/" + bgFrame), scale=1.0
        )
