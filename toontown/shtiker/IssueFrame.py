...
class IssueFrame(DirectFrame):
    ...
    def load(self):
        ...
        self.guiNav = loader.loadModel(
            "phase_3.5/models/gui/tt_m_gui_ign_directNewsGuiNav"
        )
        numPagesLoaded = 0
    ...
    def createPage(self, section, subsection, fullFilename):
        ...
        if "error_" in str(fullFilename):
            pageFrame["text"] = (
                TTLocalizer.NewsPageErrorDownloadingFileCanStillRead
                % fullFilename[len("error_") :]
            )
        else:
            ...
            if quad:
                ...
                pageFrame["text"] = (
                    TTLocalizer.NewsPageErrorDownloadingFileCanStillRead % fullFilename
                )
    ...
    def loadNavButtons(self, pageFrame):
        ...
        frameSizeAdj1 = 0.1
        frameSize1 = (-0.04 + frameSizeAdj1, 0.04 + frameSizeAdj1, -0.04, 0.04)
        frameSizeAdj2 = 0.21
        frameSize2 = (-0.04 + frameSizeAdj2, 0.04 + frameSizeAdj2, -0.04, 0.04)
        frameSizes = (
            frameSize1,
            frameSize1,
            frameSize1,
    ...
    def gotoPage(self, section, subsection):
        ...
        base.cr.centralLogger.writeClientEvent(
            "news gotoPage %s %s %s" % (self.dateStr, section, subsection)
        )
    ...
    def loadMainPage(self):
        self.mainFrame = DirectFrame(
            parent=self, frameSize=self.FrameDimensions, frameColor=(1, 0, 0, 1)
        )
