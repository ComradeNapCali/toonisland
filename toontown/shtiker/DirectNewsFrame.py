...
class DirectNewsFrame(DirectObject.DirectObject):
    ...
    def parseNewsContent(self):
            ...
            if allHomeFiles:
                for myIssueIndex, oneHomeFile in enumerate(allHomeFiles):
                    ...
                    if majorVer == 1:
                        ...
                        self.notify.warning(
                            "Dont know how to handle version %s, asuming v2" % majorVer
                        )
                        oneIssue = IssueFrameV2.IssueFrameV2(
                            self.backFrame,
                            newsDir,
        ...
        if hasattr(base.cr, "inGameNewsMgr") and base.cr.inGameNewsMgr:
            ...
            self.notify.debug(
                "setting created time to latest issue %s" % self.createdTime
            )
        else:
            self.createdTime = base.cr.toontownTimeManager.getCurServerDateTime()
            self.notify.debug(
                "setting created time cur server time %s" % self.createdTime
            )
        return result
    ...
    def findNewsDir(self):
        ...
        if not found:
            ...
            self.setErrorMessage(
                TTLocalizer.NewsPageErrorDownloadingFile % self.NewsIndexFilename
            )
            return None
    ...
    def downloadNextFile(self, task):
        while (
            self.nextNewsFile < len(self.newsFiles)
            and "aaver" in self.newsFiles[self.nextNewsFile]
        ):
            self.nextNewsFile += 1
        ...
        if self.filename in self.newsCache:
            ...
            if (
                date
                and localFilename.exists()
                and (size == 0 or localFilename.getFileSize() == size)
            ):
                doc.setDate(date)
    ...
    def saveNewsCache(self):
        ...
        try:
            ...
            self.notify.warning(
                "error opening news cache file %s: %s" % (cacheIndexFilename, str(e))
            )
            return
