...
class EventsPage(ShtikerPage.ShtikerPage):
    ...
    def loadHostingTab(self):
        ...
        (
            self.hostingDecorationList,
            self.hostingDecorationLabel,
        ) = self.createListAndLabel(
        ...
        self.accept(
            self.confirmPublicPrivateChangeEvent, self.confirmPublicPrivateChange
        )
    ...
    def loadInvitationsTab(self):
        ...
        (
            self.invitationActivityList,
            self.invitationActivityLabel,
        ) = self.createListAndLabel(
            self.invitationDisplay,
            self.invitationGui,
            "activities",
    ...
    def loadNewsTab(self):
        ...
        self.newsFrame = DirectLabel(
            relief=None, parent=self.newsDisplay, pos=(0, 0, -0.1)
        )
    ...
    def getActivityItem(self, activityBase, count=1):
        activityName = TTLocalizer.PartyActivityNameDict[activityBase.activityId][
            "generic"
        ]
        if count == 1:
    ...
    def getDecorationItem(self, decorBase, count=1):
        decorationName = TTLocalizer.PartyDecorationNameDict[decorBase.decorId][
            "editor"
        ]
        if count == 1:
        ...
        for partyInfo in base.localAvatar.partiesInvitedTo:
            ...
            if inviteInfo is None:
                EventsPage.notify.error(
                    "No invitation info for party id %d" % partyInfo.partyId
                )
                return
    ...
    def fillInviteActivityList(self, activityIds):
        ...
        for activityId in countDict:
            if countDict[activityId] == 1:
                ...
                textOfActivity = (
                    TTLocalizer.PartyActivityNameDict[activityId]["generic"]
                    + " x "
                    + str(countDict[activityId])
                )
            geom = getPartyActivityIcon(
                self.activityIconsModel, PartyGlobals.ActivityIds.getString(activityId)
            )
            item = DirectLabel(
                relief=None,
                text=textOfActivity,
    ...
    def loadHostedPartyInfo(self):
        ...
        self.cancelPartyResultGui.hide()
        if (
            base.localAvatar.hostedParties is not None
            and len(base.localAvatar.hostedParties) > 0
        ):
            for partyInfo in base.localAvatar.hostedParties:
                if (
                    ...
                    self.hostingDateLabel[
                        "text"
                    ] = TTLocalizer.EventsPageHostTabDateTimeLabel % (
                        PartyUtils.formatDate(
                            partyInfo.startTime.year,
                            partyInfo.startTime.month,
                            partyInfo.startTime.day,
                        ),
                        PartyUtils.formatTime(
                            partyInfo.startTime.hour, partyInfo.startTime.minute
                        ),
                    )
                    ...
                    if partyInfo.status == PartyGlobals.PartyStatus.CanStart:
                        ...
                        if isinstance(place, Party):
                            if hasattr(base, "distributedParty"):
                                if (
                                    base.distributedParty.partyInfo.hostId
                                    == base.localAvatar.doId
                                ):
                                    self.partyGoButton[
                                        "state"
                                    ] = DirectGuiGlobals.DISABLED
                                else:
                                    self.partyGoButton[
                                        "state"
                                    ] = DirectGuiGlobals.NORMAL
                            else:
                                self.partyGoButton["state"] = DirectGuiGlobals.NORMAL
                                self.notify.warning(
        ...
        if self.mode == EventsPage_Host:
            ...
            if not self.calendarGuiMonth:
                ...
                self.calendarGuiMonth = CalendarGuiMonth(
                    self.calendarDisplay, curServerDate, onlyFutureMonthsClickable=True
                )
        ...
        if errorCode == PartyGlobals.ChangePartyFieldErrorCode.AllOk:
            ...
            if errorCode == PartyGlobals.ChangePartyFieldErrorCode.AlreadyStarted:
                self.confirmPublicPrivateGui[
                    "text"
                ] = TTLocalizer.EventsPagePublicPrivateAlreadyStarted
            else:
                self.confirmPublicPrivateGui[
                    "text"
                ] = TTLocalizer.EventsPagePublicPrivateNoGo
        self.__setPublicPrivateButton()

    def __doCancelParty(self):
        if self.hostedPartyInfo:
            if (
                self.hostedPartyInfo.status == PartyGlobals.PartyStatus.Pending
                or self.hostedPartyInfo.status == PartyGlobals.PartyStatus.CanStart
                or self.hostedPartyInfo.status == PartyGlobals.PartyStatus.NeverStarted
            ):
                self.hostingCancelButton["state"] = DirectGuiGlobals.DISABLED
    ...
    def changePartyStatusResponseReceived(
        self, partyId, newPartyStatus, errorCode, beansRefunded
    ):
        EventsPage.notify.debug(
        ...
        if errorCode == PartyGlobals.ChangePartyFieldErrorCode.AllOk:
            if newPartyStatus == PartyGlobals.PartyStatus.Cancelled:
                self.loadHostedPartyInfo()
                self.cancelPartyResultGui["text"] = (
                    TTLocalizer.EventsPageCancelPartyResultOk % beansRefunded
                )
                self.cancelPartyResultGui.show()
        elif (
            errorCode == PartyGlobals.ChangePartyFieldErrorCode.AlreadyRefunded
            and newPartyStatus == PartyGlobals.PartyStatus.NeverStarted
        ):
            self.loadHostedPartyInfo()
            self.cancelPartyResultGui[
                "text"
            ] = TTLocalizer.EventsPageCancelPartyAlreadyRefunded
            self.cancelPartyResultGui.show()
        else:
            self.cancelPartyResultGui[
                "text"
            ] = TTLocalizer.EventsPageCancelPartyResultError
            self.cancelPartyResultGui.show()
            self.hostingCancelButton["state"] = DirectGuiGlobals.NORMAL
    ...
    def createArticleTextList(self):
        ...
        self.notify.debug(
            "bottomLeft=%s topRight=%s" % (bottomLeft.getPos(), topRight.getPos())
        )
        buttonOffSet = 0.045
        selectedIndex = 0
        self.articleListXorigin = bottomLeft.getPos().getX()
        self.articleListFrameSizeX = (
            topRight.getPos().getX() - bottomLeft.getPos().getX()
        )
        self.articleListZorigin = bottomLeft.getPos().getZ()
        self.articleListFrameSizeZ = (
            topRight.getPos().getZ() - bottomLeft.getPos().getZ()
        )
        ...
        def makeButton(itemName, itemNum, *extraArgs):
            ...
            return DirectLabel(
                text=itemName, relief=None, text_align=TextNode.ALeft, scale=0.06
            )

        itemHeight = 0.062
        topLeftStart = topLeft.getPos()
        self.notify.debug(
            "topLeft=%s topLeftStart=%s" % (topLeft.getPos(), topLeftStart)
        )
        ...
        self.articleTextList = DirectScrolledList(
            ...
            text_scale=0.05,
            frameSize=(
                self.articleListXorigin,
                self.articleListXorigin + self.articleListFrameSizeX,
                self.articleListZorigin,
                self.articleListZorigin + self.articleListFrameSizeZ,
            ),
            frameColor=(0.82, 0.8, 0.75, 1),
    ...
    def articleIndexChanged(self):
        ...
        if (
            curArticleIndex in self.articleImages
            and curArticleIndex in self.articleText
        ):
            self.displayArticle(
                self.articleImages[curArticleIndex], self.articleText[curArticleIndex]
            )
    ...
    def getRssFeed(self):
        ...
        def addFeedText(
            unicodeText, textSize=0.03, color=(0, 0, 0, 1), feedText=feedText
        ):
            feedText.append(
                DirectLabel(
                    relief=None,
    ...
    def downloadArticles(self):
        ...
        if not self.NonblockingDownload:
            if not taskMgr.hasTaskNamed(self.DownloadArticlesTaskName):
                taskMgr.doMethodLater(
                    0.5, self.downloadArticlesTask, self.DownloadArticlesTaskName
                )
    ...
    def downloadArticlesTask(self, task):
        ...
        for index in range(len(urls) / 2):
            ...
            try:
                ...
                self.notify.warning(
                    "image url %d could not open %s" % (index, imageUrl)
                )
    ...
    def createNewsList(self):
        ...
        self.newsList = DirectScrolledList(
            ...
            incButton_pos=(
                ...
                0,
                self.newsListZorigin - buttonOffSet,
            ),
            ...
            decButton_pos=(
                self.newsButtonXstart,
                0,
                self.newsListZorigin + self.newsListFrameSizeZ + buttonOffSet,
            ),
            ...
            itemFrame_frameSize=(
                self.newsListXorigin,
                self.newsListXorigin + self.newsListFrameSizeX,
                self.newsListZorigin,
                self.newsListZorigin + self.newsListFrameSizeZ,
            ),
            itemFrame_frameColor=(0.82, 0.8, 0.75, 1),
        ...
        if url == self.getNewsUrl():
            ...
            if self.curDownloadIsJpg:
                ...
                try:
                    ...
                    self.notify.warning(
                        "image url %d could not read %s" % (self.curArticleIndex, url)
                    )
    ...
    def startCheckingAsyncRequest(self, url):
        ...
        task = taskMgr.doMethodLater(
            0.5, self.pollDownloadTask, self.DownloadArticlesTaskName
        )
        task.url = url
    ...
    def pollDownloadTask(self, task):
        ...
        if result == 0:
            ...
            if self.nonBlock.getStatusString() == "OK":
                ...
                self.notify.warning(
                    "%s for %s" % (self.nonBlock.getStatusString(), task.url)
                )
