...
class QuestMap(DirectFrame):
    def __init__(self, av, **kw):
        ...
        for currHoodInfo in SuitPlannerBase.SuitPlannerBase.SuitHoodInfo:
            ...
            self.suitPercentage[
                currHoodInfo[SuitPlannerBase.SuitPlannerBase.SUIT_HOOD_INFO_ZONE]
            ] = tracks
    ...
    def update(self, task):
        ...
        for buildingMarker in self.buildingMarkers:
            buildingMarker.setScale(
                (math.sin(task.time * 16.0 + i * math.pi / 3.0) + 1) * 0.005 + 0.04
            )
            i = i + 1
        ...
        if self.av:
            try:
                ...
                mapsGeom = loader.loadModel(
                    "phase_4/models/questmap/%s_maps" % ToontownGlobals.dnaMap[hoodId]
                )
    ...
    def initMarker(self, task):
        if self.av:
            if (
                not hasattr(base.cr.playGame.getPlace(), "isInterior")
    ...
    def handleMarker(self):
        if (
            hasattr(base.cr.playGame.getPlace(), "isInterior")
            and base.cr.playGame.getPlace().isInterior
        ):
