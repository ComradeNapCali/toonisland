...
class InteractiveAnimatedProp(GenericAnimatedProp.GenericAnimatedProp, FSM.FSM):
    ...
    def createBattleCheerInterval(self):
        ...
        for i in range(self.numFightAnims):
            ...
            if self.hasOverrideIval(origAnimName):
                ...
                result.append(
                    Parallel(animAndSoundIval, self.getSpecialIval(origAnimName))
                )
            else:
        ...
        battleCheerTextIval = Sequence(
            ...
            self.hpText.posInterval(
                duration=4.0, pos=Vec3(0, 0, 7), startPos=(0, 0, 3)
            ),
    ...
    def exit(self):
        self.okToStartNextAnim = False
        self.notify.debug(
            "%s %d okToStartNextAnim=%s" % (self, self.visId, self.okToStartNextAnim)
        )
    ...
    def startNextIdleAnim(self):
        self.notify.debug(
            "startNextAnim self.okToStartNextAnim=%s" % self.okToStartNextAnim
        )
        if not hasattr(self, "node") or not self.node:
            self.notify.warning(
        ...
        if self.okToStartNextAnim:
            ...
            self.notify.debug(
                "starting curIval of length %s" % self.curIval.getDuration()
            )
            self.curIval.start()
        else:
            self.curIval = Wait(10)
            self.notify.debug(
                "false self.okToStartNextAnim=%s" % self.okToStartNextAnim
            )
        return

    def createIdleAnimAndSoundInterval(self, whichIdleAnim, startingTime=0):
        animIval = self.node.actorInterval(
            "idle%d" % whichIdleAnim, startTime=startingTime
        )
    ...
    def createIdleAnimSequence(self, whichIdleAnim):
        ...
        result = Sequence(
            idleAnimAndSound, Wait(self.IdlePauseTime), Func(self.startNextIdleAnim)
        )
        if (
            isinstance(self.ZoneToIdles[self.hoodId][whichIdleAnim], tuple)
            and len(self.ZoneToIdles[self.hoodId][whichIdleAnim]) > 2
        ):
    ...
    def gotoIdle(self):
        ...
        if base.cr.newsManager.isHolidayRunning(self.holidayId):
            ...
            self.notify.debug(
                "not going to idle because holiday %d is not running" % self.holidayId
            )
    ...
    def gotoSad(self, buildingDoId):
        ...
        if base.cr.newsManager.isHolidayRunning(self.holidayId):
            ...
            self.notify.debug(
                "not going to sad because holiday %d is not running" % self.holidayId
            )
    ...
    def getSettleName(self, whichIdleAnim):
        ...
        if (
            isinstance(self.ZoneToIdles[self.hoodId][whichIdleAnim], tuple)
            and len(self.ZoneToIdles[self.hoodId][whichIdleAnim]) > 3
        ):
