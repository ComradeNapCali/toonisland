...
class CSHoodDataAI(HoodDataAI.HoodDataAI):
    ...
    def startup(self):
        ...
        self.testElev0 = (
            DistributedFactoryElevatorExtAI.DistributedFactoryElevatorExtAI(
                self.air,
                self.air.factoryMgr,
        ...
        self.fatalElev = (
            DistributedFactoryElevatorExtAI.DistributedFactoryElevatorExtAI(
                self.air,
                self.air.factoryMgr,
                ToontownGlobals.SellbotFatalInt,
                2,
                antiShuffle=0,
                minLaff=mins[2],
            )
        )
        self.fatalElev.generateWithRequired(ToontownGlobals.SellbotFactoryExt)
        self.addDistObj(self.testElev1)
        self.addDistObj(self.fatalElev)
        ...
        self.lobbyElevator = DistributedVPElevatorAI.DistributedVPElevatorAI(
            self.air, self.lobbyMgr, ToontownGlobals.SellbotLobby, antiShuffle=1
        )
        ...
        factoryIdList = [self.testElev0.doId, self.testElev1.doId, self.fatalElev.doId]
        if simbase.config.GetBool("want-boarding-groups", 1):
            self.factoryBoardingParty = (
        ...
        intDoor0 = DistributedCogHQDoorAI.DistributedCogHQDoorAI(
            self.air, 0, DoorTypes.INT_COGHQ, ToontownGlobals.SellbotHQ, doorIndex=0
        )
