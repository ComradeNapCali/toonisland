...
class ToonHood(Hood.Hood):
        ...
        self.fsm = ClassicFSM.ClassicFSM(
            ...
            [
                ...
                State.State(
                    ...
                    self.exitTownLoader,
                    ["quietZone", "safeZoneLoader", "suitInterior", "cogdoInterior"],
                ),
                State.State(
                    "safeZoneLoader",
                ...
                State.State(
                    "minigame", self.enterMinigame, self.exitMinigame, ["purchase"]
                ),
                State.State(
                    "quietZone",
                    self.enterQuietZone,
    ...
    def loadLoader(self, requestStatus):
        ...
        if loaderName == "safeZoneLoader":
            ...
            self.loader = self.townLoaderClass(
                self, self.fsm.getStateNamed("townLoader"), self.loaderDoneEvent
            )
            self.loader.load(requestStatus["zoneId"])

    def enterTownLoader(self, requestStatus):
        teleportDebug(
            requestStatus, "ToonHood.enterTownLoader, status=%s" % (requestStatus,)
        )
