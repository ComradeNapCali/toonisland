...
class HydrantInteractiveProp(InteractiveAnimatedProp.InteractiveAnimatedProp):
    ...
    def hasOverrideIval(self, origAnimName):
        result = False
        if (
            "fightBoost" in origAnimName or "fightCheer" in origAnimName
        ) and not self.hoodId == ToontownGlobals.CirrusCircus:
            result = True
        return result
