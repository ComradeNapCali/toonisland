...
class ToonBase(OTPBase.OTPBase):
    ...
    def __init__(self):
        ...
        limeText = TextProperties()
        limeText.setTextColor(0.2, 1.0, 0, 1)
        limeText.setTextScale(1.2)
        tpMgr.setProperties("limeText", limeText)
        del tpMgr
    ...
    def setSprinting(self):
        if hasattr(base, "localAvatar"):
            ...
            base.localAvatar.controlManager.setSpeeds(
                OTPGlobals.ToonForwardSprintSpeed,
                OTPGlobals.ToonJumpForce,
                OTPGlobals.ToonReverseSprintSpeed,
    ...
    def takeScreenShot(self):
        ...
        self.screenshot(
            namePrefix=namePrefix, imageComment=ctext + " " + self.screenshotStr
        )
        self.lastScreenShotTime = globalClock.getRealTime()
        if coordOnScreen:
    ...
    def initNametagGlobals(self):
        ...
        thought3d = ChatBalloon(
            loader.loadModel("phase_3/models/props/chatbox_thought_cutout").node()
        )
        speech2d = ChatBalloon(
            loader.loadModel("phase_3/models/props/chatbox_noarrow").node()
        )
    ...
    def getShardPopLimits(self):
        if self.cr.productName == "JP":
            return (
                config.GetInt("shard-low-pop", ToontownGlobals.LOW_POP_JP),
            ...
            return (
                config.GetInt("shard-low-pop", ToontownGlobals.LOW_POP_INTL),
    ...
    def loadFromSettings(self):
        if not config.GetInt("ignore-user-options", 0):
            ...
            loadPrcFileData(
                "toonBase Settings Sound Active", "audio-sfx-active %s" % sfx
            )
            loadPrcFileData(
                "toonBase Settings Music Volume",
