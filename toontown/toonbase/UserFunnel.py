...
class UserFunnel:
    ...
    def whatOSver(self):
        ...
        if sys.platform == "darwin":
            ...
            try:
                ...
                self.osRevver = parseLine[
                    versionStringStart + 5 : versionStringStart + 7
                ].strip(" ")
                self.osBuild = parseLine[
                    int(parseLine.find("(")) + 1 : parseLine.find(")")
                ]
            except:
    ...
    def getFunnelURL(self):
        ...
        if patcherVer() == []:
            ...
            while self.patcherURL:
                ...
                if (
                    self.confLine.find("FUNNEL_LOG=") != -1
    ...
    def buildURL(self):
        ...
        if self.CurrentHost == 1:
            self.URLtoSend = (
                "http://"
                + self.hostDict[self.CurrentHost]
                + "hb="
                + str(self.hitboxAcct)
                + "&n="
                + str(self.milestone)
                + "&ln="
                + self.language
                + "&gp=STARTGAME&fnl=TOONTOWN_FUNNEL&vcon=/"
                + self.cgRoot
                + "/"
                + self.cgLocation
                + "/"
...
def getreg(regVar):
    if sys.platform != "win32":
        print(
            "System is not MS-Windows. I haven't been setup yet to work with systems other than MS-Win using MS-Internet Explorer Cookies"
        )
        return ""
        ...
        if sys.platform == "darwin":
            ...
            if macconfdata:
                if macconfdata[0].find("MAC Address") != -1:
                    pa = macconfdata[0][
                        macconfdata[0].find(":") + 2 : macconfdata[0].find(":") + 22
                    ].strip("\n")
...
def reportMemoryLeaks():
    ...
    if basePort != 80:
        finalURL = (
            ...
            + ":"
            + str(basePort)
            + "/logging/memory_leak.php?leakcount="
            + str(printUnreachableNum())
        )
    else:
        finalURL = (
            "http://"
