...
def syncLoginFSM(task=None):
    ...
    if (
        introduction.getCurrentOrNextState() != "Label"
        and introduction.label.getText() != TTLocalizer.LoaderLabel
    ):
        ...
        if style == OTPDialog.CancelOnly:
            introduction.request(
                "ExitDialog", message, base.cr.loginFSM.request, ["shutdown"]
            )
        else:
            introduction.request(
                "YesNoDialog",
                message,
                base.cr.loginFSM.request,
                ["connect", [base.cr.serverList]],
                base.cr.loginFSM.request,
                ["shutdown"],
            )
    elif stateName == "noConnection":
        if (
            base.cr.bootedIndex is not None
            and base.cr.bootedIndex in OTPLocalizer.CRBootedReasons
        ):
...
presentsTrack = Sequence(
    Func(introduction.request, "Presents"), Wait(7), Func(syncLoginFSM)
)
disclaimerTrack = Sequence(
    Func(introduction.request, "Disclaimer"), Wait(7), Func(presentsTrack.start)
)
