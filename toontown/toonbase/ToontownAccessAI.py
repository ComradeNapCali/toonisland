...
def canAccess(avatarId, zoneId, function=""):
    ...
    if (
        avatar
        and avatar.getGameAccess() != OTPGlobals.AccessFull
        and not openToAll(zoneId, avatar)
    ):
        ...
        if simbase.config.GetBool("want-ban-ispaid", True):
            commentStr = (
                "User with rights: %s tried to gain access zone %s from function %s, an area they were not allowed to using TTInjector Hack"
                % (avatar.getGameAccess(), zoneId, function)
            )
...
def openToAll(zoneId, avatar):
    ...
    if canonicalZoneId in allowedZones or avatar.isInEstate():
        ...
        canonicalZoneId >= ToontownGlobals.DynamicZonesBegin
        and not avatar.getTutorialAck()
    ):
