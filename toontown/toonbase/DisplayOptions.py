...
class DisplayOptions:
    ...
    def loadFromSettings(self):
        ...
        if antialias:
            loadPrcFileData(
                "toonBase Settings Framebuffer MSAA", "framebuffer-multisample 1"
            )
            loadPrcFileData(
                "toonBase Settings MSAA Level", "multisamples %i" % antialias
            )
        else:
            base.settings.updateSetting("game", "antialiasing", antialias)
            loadPrcFileData(
                "toonBase Settings Framebuffer MSAA", "framebuffer-multisample 0"
            )
        ...
        if not fullscreen and not self.isWindowedPossible():
            ...
            fullscreen = True
        loadPrcFileData(
            "toonBase Settings Window Res", "win-size %s %s" % (res[0], res[1])
        )
        self.notify.debug("settings resolution = %s" % str(res))
        loadPrcFileData(
            "toonBase Settings Window FullScreen", "fullscreen %s" % fullscreen
        )
        self.notify.debug("settings full screen mode=%s" % str(fullscreen))
        loadPrcFileData(
            "toonBase Settings Music Active", "audio-music-active %s" % music
        )
        loadPrcFileData("toonBase Settings Sound Active", "audio-sfx-active %s" % sfx)
        loadPrcFileData(
            "toonBase Settings Music Volume", "audio-master-music-volume %s" % musicVol
        )
        loadPrcFileData(
            "toonBase Settings Sfx Volume", "audio-master-sfx-volume %s" % sfxVol
        )
        loadPrcFileData(
            "toonBase Settings Toon Chat Sounds", "toon-chat-sounds %s" % toonChatSounds
        )
    ...
    def set(self, pipe, width, height, fullscreen, embedded):
        ...
        if (
            ...
            and bool(self.current_properties.getParentWindow()) == bool(embedded)
            and self.current_properties.getFullscreen() == fullscreen
            and self.current_properties.getXSize() == width
            and self.current_properties.getYSize() == height
        ):
            ...
            if self.resetWindowProperties(pipe, properties):
                ...
                self.notify.debug(
                    "  EMBEDDED:   %s" % bool(properties.getParentWindow())
                )
                self.notify.debug("  FULLSCREEN: %s" % bool(properties.getFullscreen()))
                self.notify.debug("  X SIZE:     %s" % properties.getXSize())
                self.notify.debug("  Y SIZE:     %s" % properties.getYSize())
                if (
                    bool(properties.getParentWindow()) == bool(embedded)
                    and properties.getFullscreen() == fullscreen
                    and properties.getXSize() == width
                    and properties.getYSize() == height
                ):
    ...
    def resetWindowProperties(self, pipe, properties):
        ...
        if (
            gsg == None
            or currentProperties.getFullscreen() != newProperties.getFullscreen()
            ...
            if "libotp" in sys.modules:
                from dependencies.libotp import NametagGlobals
