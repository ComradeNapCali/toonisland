    ...
    def setup(self):
        ...
        for holeLIndex in range(self.golfCourse.numHoles):
            holeLabel = DirectLabel(
                parent=self.scoreboard,
                relief=None,
                pos=(self.lineVStart + 0.055 + horzOffset * holeLIndex, 0, holeTop),
                text_align=TextNode.ACenter,
                text="%s" % (holeLIndex + 1),
                text_scale=0.05,
            )
        ...
        for parHoleIndex in range(self.golfCourse.numHoles):
            parLabel = DirectLabel(
                ...
                relief=None,
                pos=(
                    self.lineVStart + 0.055 + horzOffset * parHoleIndex,
                    0,
                    holeTop - 0.1,
                ),
                text_align=TextNode.ACenter,
        ...
        parLabel = DirectLabel(
            parent=self.scoreboard,
            relief=None,
            pos=(self.lineVStart + 0.1 + horzOffset * 9.5, 0, holeTop - 0.1),
            text_align=TextNode.ACenter,
            text="%s" % totPar,
            text_scale=0.05,
            text_wordwrap=10,
        ...
        for playaIndex in range(self.numPlayas):
            ...
            if type(textN) == OnscreenText:
                try:
                    if textN.textNode.getWordwrappedWtext() != name:
                        ...
                        vert = (
                            self.playaTop - self.vertOffset * playaIndex - self.vCenter
                        )
                except:
                    vert = self.playaTop - self.vertOffset * playaIndex
            ...
            self.notify.debug(
                "self.text height = %f" % self.playaTags[playaIndex].getHeight()
            )
            holeIndex = 0
            for holeIndex in range(self.golfCourse.numHoles):
                holeLabel = DirectLabel(
                    ...
                    relief=None,
                    pos=(
                        self.lineVStart + 0.055 + horzOffset * holeIndex,
                        0,
                        self.playaTop - self.vertOffset * playaIndex - self.vCenter,
                    ),
                    text_align=TextNode.ACenter,
            ...
            holeLabel = DirectLabel(
                ...
                relief=None,
                pos=(
                    self.lineVStart + 0.1 + horzOffset * 9.5,
                    0,
                    self.playaTop - self.vertOffset * playaIndex - self.vCenter,
                ),
                text_align=TextNode.ACenter,
        ...
        self.lines.drawTo(
            self.lineVStart + 11 * self.lineVertOffset, 0, self.lineHStart + 0.19
        )
        self.lines.moveTo(self.lineVStart - 0.45, 0, self.lineHStart + 0.09)
        self.lines.drawTo(
            self.lineVStart + 11 * self.lineVertOffset, 0, self.lineHStart + 0.09
        )
        self.lines.moveTo(self.lineVStart - 0.45, 0, self.lineHStart)
        self.lines.drawTo(
            self.lineVStart + 11 * self.lineVertOffset, 0, self.lineHStart
        )
        self.lines.moveTo(self.lineVStart - 0.45, 0, self.lineHStart + 0.19)
        ...
        for x in range(4):
            self.lines.moveTo(
                self.lineVStart - 0.45,
                0,
                self.lineHStart - (x + 1) * self.lineHorOffset,
            )
            self.lines.drawTo(
                self.lineVStart + 11 * self.lineVertOffset + 0.005,
                0,
                self.lineHStart - (x + 1) * self.lineHorOffset,
            )
        ...
        for y in range(10):
            self.lines.moveTo(
                self.lineVStart + y * self.lineVertOffset, 0, self.lineHStart + 0.19
            )
            self.lines.drawTo(
                self.lineVStart + y * self.lineVertOffset, 0, self.lineHStart - 4 * 0.13
            )
        ...
        self.lines.drawTo(
            self.lineVStart + 11 * self.lineVertOffset, 0, self.lineHStart - 4 * 0.13
        )
        ...
        for playaIndex in range(self.numPlayas):
            if self.golfCourse.isGameDone():
                ...
                self.highlightCur.setPos(
                    -0.003, 0, 0.038 - playaIndex * (self.lineVertOffset + 0.005)
                )
                self.highlightCur.show()
            else:
                self.playaTags[playaIndex].setColor(0, 0, 0, 1)
        ...
        for avId in self.avIdList:
            ...
            for y in range(len(self.golfCourse.exitedAvIdList)):
                if self.golfCourse.exitedAvIdList[y] == avId:
                    ...
                    holeIndex = 0
                    for holeIndex in range(self.golfCourse.numHoles):
                        self.getScoreLabel(self.avIdList[x], holeIndex).setColor(
                            0.7, 0.7, 0.7, 1
                        )
            ...
            if playerExited == False:
                for holeIndex in range(self.golfCourse.numHoles):
                    if holeIndex <= self.golfCourse.curHoleIndex:
                        self.getScoreLabel(avId, holeIndex)["text"] = (
                            "%s" % scoreDict[avId][holeIndex]
                        )
            ...
            if self.golfCourse.isGameDone():
                self.getScoreLabel(avId, self.golfCourse.numHoles - 1).setColor(
                    0, 0, 0, 1
                )
