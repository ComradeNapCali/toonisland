...
class NameShop(StateData.StateData):
        ...
        if self.listsLoaded:
            ...
            if self.firstActive:
                ...
                newfirst = self.firstnameScrollList["items"][
                    self.firstnameScrollList.index + 2
                ]["text"]
                if newfirst == "von":
                    nt = "Von"
                else:
            ...
            if self.lastActive:
                ...
                lp = self.lastprefixScrollList["items"][
                    self.lastprefixScrollList.index + 2
                ]["text"]
                ls = self.lastsuffixScrollList["items"][
                    self.lastsuffixScrollList.index + 2
                ]["text"]
    ...
    def makeScrollList(
        self, gui, ipos, mcolor, nitems, nitemMakeFunction, nitemMakeExtraArgs
    ):
    ...
    def checkNamePattern(self):
        ...
        base.cr.gameServicesManager.sendSetNamePattern(
            self.avId,
            self.nameIndices[0],
            self.nameFlags[0],
            self.nameIndices[1],
            ...
            self.nameIndices[3],
            self.nameFlags[3],
            self.handleSetNamePatternResponse,
        )
        self.waitForServer()
    ...
    def handleSetNamePatternResponse(self, avId, status):
        ...
        if status == 1:
            ...
            newPotAv = PotentialAvatar.PotentialAvatar(
                avId, self.names, avDNA, self.index, 0
            )
    ...
    def _submitTypeANameAsPickAName(self):
        ...
        if pnp.hasNamePattern():
            ...
            for i in range(len(pattern)):
                if pattern[i] != -1:
                    names.append(
                        pnp.getNamePartString(self.toon.style.gender, i, pattern[i])
                    )
                else:
                    names.append("")
    ...
    def checkNameTyped(self, justCheck=False):
        ...
        if justCheck:
            ...
            avId = self.avId
        base.cr.gameServicesManager.sendSetNameTyped(
            avId, self.nameEntry.get(), self.handleNameTypedResponse
        )
        self.waitForServer()
    ...
    def handleNameTypedResponse(self, avId, status):
        ...
        if avId == 0:
            ...
            if status == 1:
                ...
                newPotAv = PotentialAvatar.PotentialAvatar(
                    avId, self.names, avDNA, self.index, 1
                )
                if not self.newwarp:
                    self.avList.append(newPotAv)
    ...
    def printTypeANameInfo(self, str):
        ...
        self.notify.debug(
            "ParentPos = %.2f %.2f %.2f" % (parentPos[0], parentPos[1], parentPos[2])
        )
