...
class DistributedGoon(
    DistributedCrushableEntity.DistributedCrushableEntity, Goon.Goon, FSM.FSM
):
    ...
    def announceGenerate(self):
        ...
        if self.level:
            ...
            taskMgr.doMethodLater(
                0.1, self.makeCollidable, self.taskName("makeCollidable")
            )
    ...
    def getRecoveryTrack(self):
        return Parallel(
            Sequence(ActorInterval(self, "recovery"), Func(self.pose, "recovery", 96)),
    ...
    def setMovie(self, mode, avId, pauseTime, timestamp):
        ...
        self.notify.debug(
            "%s: setMovie(%s,%s,%s,%s)" % (self.doId, mode, avId, pauseTime, ts)
        )
        if mode == GOON_MOVIE_BATTLE:
            ...
            if self.state != "Stunned":
                ...
                if toon:
                    ...
                    if toonDistance > self.attackRadius:
                        self.notify.warning(
                            "Stunned a goon, but outside of attack radius"
                        )
                        return
                    else:
