...
class SuitPlannerBase:
    ...
    def initDNAInfo(self):
        ...
        if numGraphs != 1:
            self.notify.info(
                "zone %s has %s disconnected suit paths." % (self.zoneId, numGraphs)
            )
        self.battlePosDict = {}
                        ...
                        if battleCellId == -1:
                            ...
                            if zoneId in self.cellToGagBonusDict:
                                self.notify.error(
                                    ...
                                    % (
                                        zoneId,
                                        self.cellToGagBonusDict[zoneId],
                                        childDnaGroup,
                                    )
                                )
                            else:
                                ...
                                if (
                                    propType
                                    in ToontownBattleGlobals.PropTypeToTrackBonus
                                ):
                                    trackBonus = (
                                        ToontownBattleGlobals.PropTypeToTrackBonus[
                                            propType
                                        ]
                                    )
        ...
        for i in range(numPoints):
            ...
            if point.getPointType() == DNASuitPoint.FRONTDOORPOINT:
                ...
                point.getPointType() == DNASuitPoint.COGHQINPOINT
                or point.getPointType() == DNASuitPoint.COGHQOUTPOINT
            ):
    ...
    def performPathTest(self):
        ...
        for i in range(numPathPoints - 1):
            ...
            travelTime = self.dnaStore.getSuitEdgeTravelTime(
                path.getPointIndex(i), path.getPointIndex(i + 1), self.suitWalkSpeed
            )
            self.notify.debug(
                "edge from point "
                + repr(i)
