...
class DistributedLawbotBossAI(DistributedBossCogAI.DistributedBossCogAI, FSM.FSM):
    ...
    def hitBoss(self, bossDamage):
        ...
        if not self.validate(
            avId, avId in self.involvedToons, "hitBoss from unknown avatar"
        ):
            return
    ...
    def hitBossInsides(self):
        avId = self.air.getAvatarIdFromSender()
        if not self.validate(
            avId, avId in self.involvedToons, "hitBossInsides from unknown avatar"
        ):
            return
        currState = self.getCurrentOrNextState()
        if currState != "BattleThree":
    ...
    def touchCage(self):
        ...
        if currState != "BattleThree" and currState != "NearVictory":
            return
        if not self.validate(
            avId, avId in self.involvedToons, "touchCage from unknown avatar"
        ):
            return
    ...
    def enterRollToBattleTwo(self):
        ...
        self.barrier = self.beginBarrier(
            "RollToBattleTwo", self.involvedToons, 50, self.__doneRollToBattleTwo
        )
    ...
    def enterPrepareBattleTwo(self):
        self.__makeCannons()
        self.barrier = self.beginBarrier(
            "PrepareBattleTwo", self.involvedToons, 45, self.__donePrepareBattleTwo
        )
        ...
        if self.cannons == None:
            ...
            for index in range(numToons):
                ...
                cannon = DistributedLawbotCannonAI.DistributedLawbotCannonAI(
                    self.air, self, index, newPos[0], newPos[1], newPos[2], -90, 0, 0
                )
    ...
    def __makeChairs(self):
        if self.chairs == None:
            ...
            for index in range(12):
                chair = DistributedLawbotChairAI.DistributedLawbotChairAI(
                    self.air, self, index
                )
    ...
    def decrementCannonBallsLeft(self, avId):
        if avId in self.cannonBallsLeft:
            ...
            if self.cannonBallsLeft[avId] < 0:
                self.notify.warning(
                    "decrementCannonBallsLeft <0 cannonballs for %d" % avId
                )
    ...
    def exitBattleTwo(self):
        ...
        self.air.writeServerEvent(
            "jurorsSeated",
            self.doId,
            "%s|%s|%s" % (self.dept, self.involvedToons, self.numToonJurorsSeated),
        )
        self.__deleteCannons()
        self.__stopChairs()
    ...
    def enterRollToBattleThree(self):
        self.divideToons()
        self.barrier = self.beginBarrier(
            "RollToBattleThree", self.involvedToons, 20, self.__doneRollToBattleThree
        )
    ...
    def enterPrepareBattleThree(self):
        ...
        self.barrier = self.beginBarrier(
            "PrepareBattleThree", self.involvedToons, 45, self.__donePrepareBattleThree
        )
    ...
    def enterBattleThree(self):
        ...
        self.notify.debug(
            "diffLevel=%d ammoCount=%d gavels=%d lawyers = %d, toonup=%d"
            % (
                self.battleDifficulty,
                self.ammoCount,
                self.numGavels,
        ...
        self.setPieType()
        jurorsOver = (
            self.numToonJurorsSeated - ToontownGlobals.LawbotBossJurorsForBalancedScale
        )
    ...
    def getToonDifficulty(self):
        ...
        self.notify.debug(
            "toons average level = %f, highest level = %d"
            % (averageLevel, highestCogSuitLevel)
        )
    ...
    def __makeBattleThreeObjects(self):
        if self.gavels == None:
            ...
            for index in range(self.numGavels):
                gavel = DistributedLawbotBossGavelAI.DistributedLawbotBossGavelAI(
                    self.air, self, index
                )
    ...
    def doBattleThreeInfo(self):
        ...
        self.battleThreeTimeInMin = (
            globalClock.getFrameTime() - self.battleThreeTimeStarted
        )
    ...
    def enterVictory(self):
        ...
        self.barrier = self.beginBarrier(
            "Victory", self.involvedToons, 30, self.__doneVictory
        )
        return
    ...
    def giveCogSummonReward(self, toon, prefDeptIndex, prefSummonType):
        ...
        hasSummon = toon.hasParticularCogSummons(
            prefDeptIndex, cogLevel, prefSummonType
        )
        if hasSummon:
            ...
            if not toon.hasParticularCogSummons(prefDeptIndex, cogLevel, "single"):
                ...
                for curDeptIndex in range(len(SuitDNA.suitDepts)):
                    if not toon.hasParticularCogSummons(
                        curDeptIndex, cogLevel, prefSummonType
                    ):
                        deptIndex = curDeptIndex
                        foundOne = True
                        break
                    elif not toon.hasParticularCogSummons(
                        curDeptIndex, cogLevel, "single"
                    ):
                        deptIndex = curDeptIndex
                        summonType = "single"
                        foundOne = True
                        ...
                        curDeptIndex, cogLevel, "building"
                    ):
                        deptIndex = curDeptIndex
                        summonType = "building"
                        foundOne = True
                        ...
                        curDeptIndex, cogLevel, "invasion"
                    ):
                        summonType = "invasion"
                        deptIndex = curDeptIndex
                        foundOne = True
                ...
                if not foundOne:
                    for i in range(5):
                        ...
                        if not toon.hasParticularCogSummons(
                            randomDeptIndex, randomCogLevel, randomSummonType
                        ):
                            foundOne = True
                    ...
                    for curCogLevel in possibleCogLevel:
                        ...
                        for curDeptIndex in possibleDeptIndex:
                            if foundOne:
                                break
                            if not toon.hasParticularCogSummons(
                                curDeptIndex, curCogLevel, curType
                            ):
                                foundOne = True
    ...
    def enterDefeat(self):
        self.resetBattles()
        self.barrier = self.beginBarrier(
            "Defeat", self.involvedToons, 10, self.__doneDefeat
        )
    ...
    def hitChair(self, chairIndex, npcToonIndex):
        avId = self.air.getAvatarIdFromSender()
        if not self.validate(
            avId, avId in self.involvedToons, "hitChair from unknown avatar"
        ):
            return
        if not self.chairs:
            return
    ...
    def calculateWeightPerToon(self):
        for toonId in self.involvedToons:
            ...
            if not cannonIndex == None:
                ...
                if diffSettings[4]:
                    bonusWeight = (
                        self.numJurorsSeatedByCannon(cannonIndex) - diffSettings[5]
                    )
                    if bonusWeight < 0:
                        bonusWeight = 0
    ...
    def calcAndSetBattleDifficulty(self):
        ...
        battleDifficulty = int(
            self.toonLevels / self.maxToonLevels * numDifficultyLevels
        )
