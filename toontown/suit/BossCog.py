...
class BossCog(Avatar.Avatar):
    ...
    def __setupDoor(self, jointName, name, callback, openedHpr, closedHpr, cPoly):
        ...
        fsm = self.DoorFSM(
            name, animate, callback, openedHpr, closedHpr, self.uniqueName
        )
        return fsm

    def doAnimate(
        self, anim=None, now=0, queueNeutral=1, raised=None, forward=None, happy=None
    ):
        if now:
            self.stopAnimate()
    ...
    def __getNextAnim(self):
        if self.queuedAnimIvals:
            ...
            ival, changed = self.__getAnimIval(
                None, self.raised, self.forward, self.happy
            )
            raised = self.raised
            forward = self.forward
            happy = self.happy
    ...
    def __doGetAnimIval(self, anim, raised, forward, happy):
        ...
        if not raised and self.raised:
            ...
            if forward:
                ...
                ival = Sequence(
                    ival, Func(self.reverseBody), downIval, Func(self.forwardBody)
                )
    ...
    def getAnim(self, anim):
        ...
        if anim == None:
            ...
            if anim == "ltSwing":
                ival = Sequence(
                    Track(
                        (0, self.getAngryActorInterval("Fb_downLtSwing")),
                ...
                ival = Sequence(
                    Track(
                        (0, self.getAngryActorInterval("Fb_downRtSwing")),
                        (0.9, SoundInterval(self.swingSfx, node=self)),
                ...
                ival = Sequence(
                    ...
                    ParallelEndTogether(
                        ...
                        self.pelvis.hprInterval(
                            0.5, self.pelvisForwardHpr, blendType="easeInOut"
                        ),
                    ),
                )
            ival = Sequence(
            ...
            ival = Parallel(
                ...
                Sequence(
                    SoundInterval(self.reelSfx, node=self), SoundInterval(self.deathSfx)
                ),
            )
