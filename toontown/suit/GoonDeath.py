...
def createExplosionTrack(parent, deathNode, scale):
    ...
    return Sequence(
        Func(deathNode.reparentTo, parent), Wait(0.6), Func(deathNode.detachNode)
    )
...
def createGoonExplosion(parent, explosionPoint, scale):
    ...
    return Parallel(
        ...
        ParticleInterval(
            smallGearExplosion, deathNode, worldRelative=0, duration=4.3, cleanup=True
        ),
        ParticleInterval(
            bigGearExplosion, deathNode, worldRelative=0, duration=1.0, cleanup=True
        ),
        name="gears2MTrack",
    )
