...
class DistributedSellbotBoss(DistributedBossCog.DistributedBossCog, FSM.FSM):
    ...
    def __walkToonToPromotion(self, toonId, delay, mopath, track, delayDeletes):
        ...
        if toon:
            ...
            ival = Sequence(
                ...
                ParallelEndTogether(
                    walkMopath, toon.posInterval(2, destPos, blendType="noBlend")
                ),
            ...
            track.append(ival)
            delayDeletes.append(
                DelayDelete.DelayDelete(toon, "SellbotBoss.__walkToonToPromotion")
            )
        ...
        dialogTrack = Track(
            ...
            (
                ...
                Sequence(
                    Func(self.clearChat),
                    self.loseCogSuits(
                        self.toonsA, self.battleANode, (0, 18, 5, -180, 0, 0)
                    ),
                    self.loseCogSuits(
                        self.toonsB, self.battleBNode, (0, 18, 5, -180, 0, 0)
                    ),
                ),
            ),
            (
                ...
                Sequence(
                    ...
                    Parallel(
                        ...
                        Sequence(
                            Wait(2), Func(self.setChatAbsolute, attackToons, CFSpeech)
                        ),
                    ),
                ),
            ),
    ...
    def __makeRollToBattleTwoMovie(self):
        startPos = Point3(
            ToontownGlobals.SellbotBossBattleOnePosHpr[0],
        ...
        battlePos = Point3(
            ToontownGlobals.SellbotBossBattleTwoPosHpr[0],
            ToontownGlobals.SellbotBossBattleTwoPosHpr[1],
            ToontownGlobals.SellbotBossBattleTwoPosHpr[2],
        )
        battleHpr = VBase3(
            ToontownGlobals.SellbotBossBattleTwoPosHpr[3],
    ...
    def cagedToonMovieFunction(self, instruct, cageIndex):
        ...
        if not (
            hasattr(self, "cagedToon")
            and hasattr(self.cagedToon, "nametag")
            and hasattr(self.cagedToon, "nametag3d")
        ):
            return
        if instruct == 1:
    ...
    def makeEndOfBattleMovie(self, hasLocalToon):
        ...
        seq += [
            Wait(0.5),
            Parallel(
                self.cage.posInterval(
                    1, self.cagePos[self.cageIndex + 1], blendType="easeInOut"
                ),
    ...
    def __talkAboutPromotion(self, speech):
        if not self.localToonPromoted:
            ...
            if newCogSuitLevel == ToontownGlobals.MaxCogSuitLevel:
                speech += TTLocalizer.CagedToonLastPromotion % (
    ...
    def loadEnvironment(self):
        ...
        self.rope.setup(
            2, ((self.cage, (0.15, 0.13, 16)), (self.geom, (0.23, 78, 120)))
        )
    ...
    def enterRampExtend(self, animate):
        ...
        ival = Parallel(
            ...
            animate.posInterval(
                adjustTime, Point3(0, 0, 0), blendType="easeInOut", name=intervalName
            ),
        )
        ival.start()
    ...
    def enterRampRetract(self, animate):
        ...
        ival = Parallel(
            SoundInterval(self.rampSlideSfx, node=animate),
            animate.posInterval(
                adjustTime, Point3(18, 0, 0), blendType="easeInOut", name=intervalName
            ),
        )
        ival.start()
        self.storeInterval(ival, intervalName)
    ...
    def enterReward(self):
        ...
        for toonId in self.involvedToons:
            ...
            if toon:
                delayDeletes.append(
                    DelayDelete.DelayDelete(toon, "SellbotBoss.enterReward")
                )
    ...
    def __showEasyBarrels(self):
        ...
        seq = Sequence(
            LerpPosInterval(
                self.barrelsRootNode,
                0.5,
                Vec3(*SellbotBossGlobals.BarrelsFinalPos),
                blendType="easeInOut",
            ),
            name=intervalName,
        )
