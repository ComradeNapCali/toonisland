...
class DistributedCashbotBoss(DistributedBossCog.DistributedBossCog, FSM.FSM):
    ...
    def __makeGoonMovieForIntro(self):
        ...
        return Sequence(
            Func(self.__showFakeGoons, "Walk"), goonTrack, Func(self.__hideFakeGoons)
        )
    ...
    def makeIntroductionMovie(self, delayDeletes):
        for toonId in self.involvedToons:
            ...
            if toon:
                delayDeletes.append(
                    DelayDelete.DelayDelete(toon, "CashbotBoss.makeIntroductionMovie")
                )
        ...
        track = Sequence(
            ...
            camera.posHprInterval(
                1.5, Point3(93.3, -230, 0.7), VBase3(-92.9, 39.7, 8.3)
            ),
            Func(self.setChatAbsolute, TTL.CashbotBossDiscoverToons2, CFSpeech),
            Wait(4),
            Func(self.clearChat),
            self.loseCogSuits(
                self.toonsA + self.toonsB, render, (113, -228, 10, 90, 0, 0)
            ),
            Wait(1),
            Func(rToon.setHpr, 0, 0, 0),
    ...
    def __makeGoonMovieForBattleThree(self):
        ...
        for i in range(1, self.numFakeGoons):
            ...
            goonLoop.append(
                Sequence(
                    goon.posHprInterval(8, goonPosHprs[i][0], goonPosHprs[i][1]),
    ...
    def makePrepareBattleThreeMovie(self, delayDeletes, crane, safe):
        ...
        track = Sequence(
            ...
            Func(self.clearChat),
            camera.posHprInterval(
                1.5, Point3(105, -326, 5), Point3(136.3, 0, 0), blendType="easeInOut"
            ),
    ...
    def makeBossFleeMovie(self):
        ...
        bossTrack = Track(
            ...
            (
                9.5,
                Sequence(self.posInterval(0.4, Point3(0, -250, 0)), Func(self.stash)),
            ),
        )
        return bossTrack
    ...
    def grabObject(self, obj):
        ...
        obj.lerpInterval = Parallel(
            obj.posInterval(
                ToontownGlobals.CashbotBossToMagnetTime, Point3(-1, 0, 0.2)
            ),
            obj.quatInterval(
                ToontownGlobals.CashbotBossToMagnetTime, VBase3(0, -90, 90)
            ),
            Sequence(
                Wait(ToontownGlobals.CashbotBossToMagnetTime), ShowInterval(self.eyes)
            ),
    ...
    def __talkAboutPromotion(self, speech):
        if self.prevCogSuitLevel < ToontownGlobals.MaxCogSuitLevel:
            ...
            speech += TTLocalizer.ResistanceToonMaxed % (
                ToontownGlobals.MaxCogSuitLevel + 1
            )
        return speech
    ...
    def enterPrepareBattleThree(self):
        ...
        seq = Sequence(
            self.makePrepareBattleThreeMovie(
                delayDeletes, self.movieCrane, self.movieSafe
            ),
    ...
    def exitPrepareBattleThree(self):
        ...
        ElevatorUtils.closeDoors(
            self.leftDoor, self.rightDoor, ElevatorConstants.ELEVATOR_CFO
        )
    ...
    def enterVictory(self):
        ...
        seq = Sequence(
            self.makeBossFleeMovie(), Func(self.__continueVictory), name=intervalName
        )
        seq.start()
        self.storeInterval(seq, intervalName)
    ...
    def __epilogueChatNext(self, pageNumber, elapsed):
        if pageNumber == 1:
            ...
            track = Sequence(
                ActorInterval(
                    toon, "victory", playRate=playRate, startFrame=0, endFrame=9
                ),
                ActorInterval(
                    toon, "victory", playRate=playRate, startFrame=9, endFrame=0
                ),
