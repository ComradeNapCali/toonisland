...
class DistributedLawbotBoss(DistributedBossCog.DistributedBossCog, FSM.FSM):
    ...
    def setBossDamage(self, bossDamage, recoverRate, timestamp):
        ...
        if self.bossDamageMovie:
            if self.bossDamage >= self.bossMaxDamage:
                ...
                self.bossDamageMovie.resumeUntil(
                    self.bossDamage * self.bossDamageToMovie
                )
                if self.recoverRate:
    ...
    def getBossDamage(self):
        ...
        self.bossHealthBar.initialize(
            self.bossMaxDamage - self.bossDamage, self.bossMaxDamage
        )
    ...
    def __walkToonToPromotion(self, toonId, delay, mopath, track, delayDeletes):
        ...
        if toon:
            ...
            ival = Sequence(
                ...
                ParallelEndTogether(
                    MopathInterval(mopath, toon),
                    toon.posInterval(2, destPos, blendType="noBlend"),
                ),
                Func(toon.suit.loop, "neutral"),
            )
            track.append(ival)
            delayDeletes.append(
                DelayDelete.DelayDelete(toon, "LawbotBoss.__walkToonToPromotion")
            )
    ...
    def __makeRollToBattleTwoMovie(self):
        startPos = Point3(
            ToontownGlobals.LawbotBossBattleOnePosHpr[0],
        ...
        battlePos = Point3(
            ToontownGlobals.LawbotBossBattleTwoPosHpr[0],
            ToontownGlobals.LawbotBossBattleTwoPosHpr[1],
            ToontownGlobals.LawbotBossBattleTwoPosHpr[2],
        )
        battleHpr = VBase3(
            ToontownGlobals.LawbotBossBattleTwoPosHpr[3],
        ...
        myInterval = camera.posHprInterval(
            8,
            Point3(-22, -100, 35),
            Point3(-10, -13, 0),
    ...
    def __makeRollToBattleThreeMovie(self):
        ...
        battlePos = Point3(
            ToontownGlobals.LawbotBossBattleThreePosHpr[0],
            ToontownGlobals.LawbotBossBattleThreePosHpr[1],
            ToontownGlobals.LawbotBossBattleThreePosHpr[2],
        )
        battleHpr = VBase3(
            ToontownGlobals.LawbotBossBattleThreePosHpr[3],
    ...
    def loadJuryBox(self):
        ...
        newReflectedPos = reflectedJuryBoxPos - Point3(
            *ToontownGlobals.LawbotBossJuryBoxRelativeEndPos
        )
        if not self.debugPositions:
        ...
        self.reflectedJuryBox.setZ(
            self.reflectedJuryBox.getZ()
    ...
    def loadWitnessStand(self):
        ...
        if not self.reflectedWitnessStand.isEmpty():
            pass
        colNode = self.realWitnessStand.find(
            "**/witnessStandCollisions/Witnessstand_Collision"
        )
    ...
    def __debugScale(self):
        ...
        origin = Point3(0, 0, 0)
        prosecutionPanRelPos = self.scaleNodePath.getRelativePoint(
            self.prosecutionPanNodePath, origin
        )
        ...
        prosecutionLocatorRelPos = self.scaleNodePath.getRelativePoint(
            self.prosecutionLocator, origin
        )
    ...
    def loadScaleNew(self):
        ...
        self.defenseColNodePath.setTag(
            "pieCode", str(ToontownGlobals.PieCodeDefensePan)
        )
        self.prosecutionColNodePath = self.scaleNodePath.find("**/ProsecutionCol")
        self.prosecutionColNodePath.setTag(
            "pieCode", str(ToontownGlobals.PieCodeProsecutionPan)
        )
        ...
        self.notify.debug(
            "defensePanNodePath.getPos()=%s" % self.defensePanNodePath.getPos()
        )
    ...
    def loadScaleOld(self):
        ...
        prosecutionPanGeom = self.createBlock(
            0.5, 0.5, 0, -0.5, -0.5, -2, 1.0, 0, 0, 1.0
        )
        ...
        self.prosecutionColNodePath = self.prosecutionPanNodePath.attachNewNode(
            prosecutionCollNode
        )
        self.prosecutionColNodePath.setTag(
            "pieCode", str(ToontownGlobals.PieCodeProsecutionPan)
        ...
        for toonId in self.involvedToons:
            ...
            if toon:
                if index in self.cannons:
                    ...
                    self.notify.warning(
                        "No cannon %d but we have a toon =%d" % (index, toonId)
                    )
    ...
    def __onToBattleTwo(self, elapsedTime=0):
        ...
        taskMgr.doMethodLater(
            1, self.__showWaitingMessage, self.uniqueName("WaitingMessage")
        )
    ...
    def startJuryBoxMoving(self):
        ...
        endingAbsPos = Point3(
            curPos[0] + ToontownGlobals.LawbotBossJuryBoxRelativeEndPos[0],
        ...
        reflectedEndingAbsPos = Point3(
            curReflectedPos[0] + ToontownGlobals.LawbotBossJuryBoxRelativeEndPos[0],
        ...
        self.juryBoxIval = Parallel(
            self.juryBox.posInterval(
                ToontownGlobals.LawbotBossJuryBoxMoveTime, endingAbsPos
            ),
            self.reflectedJuryBox.posInterval(
                ToontownGlobals.LawbotBossJuryBoxMoveTime, reflectedEndingAbsPos
            ),
            SoundInterval(
                self.juryMovesSfx,
                node=self.chairs[2].nodePath,
                duration=ToontownGlobals.LawbotBossJuryBoxMoveTime,
                loop=1,
                volume=1.0,
            ),
        )
    ...
    def enterRollToBattleThree(self):
        ...
        seq = Sequence(
            self.__makeRollToBattleThreeMovie(),
    ...
    def enterNearVictory(self):
        ...
        self.setDizzy(1)
        base.playMusic(
            self.battleThreeMusic, looping=1, volume=0.9, time=self.battleThreeMusicTime
        )
    ...
    def enterVictory(self):
        ...
        seq = Sequence(
            self.makeVictoryMovie(), Func(self.__continueVictory), name=intervalName
        )
        seq.start()
        self.storeInterval(seq, intervalName)
        base.playMusic(
    ...
    def enterDefeat(self):
        ...
        seq = Sequence(
            self.makeDefeatMovie(), Func(self.__continueDefeat), name=intervalName
        )
        seq.start()
        self.storeInterval(seq, intervalName)
        base.playMusic(
    ...
    def enterReward(self):
        ...
        for toonId in self.involvedToons:
            ...
            if toon:
                delayDeletes.append(
                    DelayDelete.DelayDelete(toon, "LawbotBoss.enterReward")
                )
    ...
    def __touchedWitnessStand(self, entry):
        ...
        if not self.everThrownPie:
            taskMgr.doMethodLater(
                30, self.__howToThrowPies, self.uniqueName("PieAdvice")
            )
    ...
    def __localPieSplat(self, pieCode, entry):
        ...
        if avatarDoId == "":
            self.notify.warning(
                "Toon %s has no avatarDoId tag." % repr(entry.getIntoNodePath())
            )
            return
        doId = int(avatarDoId)
            ...
            gearTrack.append(
                Sequence(
                    ...
                    Parallel(
                        ...
                        Sequence(
                            SoundInterval(self.strafeSfx[i], volume=0.2, node=self),
                            duration=0,
                        ),
                    ),
    ...
    def replaceCollisionPolysWithPlanes(self, model):
        ...
        threshold = 0.1
        planes.sort(
            key=functools.cmp_to_key((lambda p1, p2: p1.compareTo(p2, threshold)))
        )
        lastPlane = None
        for plane in planes:
    ...
    def makeIntroductionMovie(self, delayDeletes):
        ...
        for toonId in self.involvedToons:
            ...
            if toon:
                delayDeletes.append(
                    DelayDelete.DelayDelete(toon, "LawbotBoss.makeIntroductionMovie")
                )
        ...
        dialogTrack = Track(
            ...
            (
                5.6,
                Func(self.setChatAbsolute, TTLocalizer.LawbotBossTempIntro1, CFSpeech),
            ),
            (
                12,
                Func(self.setChatAbsolute, TTLocalizer.LawbotBossTempIntro2, CFSpeech),
            ),
            (
                18,
                Func(self.setChatAbsolute, TTLocalizer.LawbotBossTempIntro3, CFSpeech),
            ),
            (
                22,
                Func(self.setChatAbsolute, TTLocalizer.LawbotBossTempIntro4, CFSpeech),
            ),
            (
                24,
                Sequence(
                    Func(self.clearChat),
                    self.loseCogSuits(
                        self.toonsA + self.toonsB, render, (-2.798, -70, 10, 180, 0, 0)
                    ),
                ),
            ),
            (
    ...
    def walkToonsToBattlePosition(self, toonIds, battleNode):
        ...
        for i in range(len(toonIds)):
            ...
            if toon:
                ...
                self.notify.debug(
                    "batlleNode.getTransform = %s  render.getTransform=%s"
                    % (battleNode.getTransform(), render.getTransform())
                )
                self.notify.debug(
                    "render.getScale()=%s  battleNode.getScale()=%s"
                    % (render.getScale(), battleNode.getScale())
                )
                myCurPos = self.getPos()
                self.notify.debug("myCurPos = %s" % self.getPos())
                self.notify.debug("battleNode.parent() = %s" % battleNode.getParent())
                self.notify.debug(
                    "battleNode.parent().getPos() = %s"
                    % battleNode.getParent().getPos()
                )
                ...
                self.notify.debug(
                    "walktToonsToBattlePosition: render.getRelativePoint result = %s"
                    % pos
                )
    ...
    def toonsToBattlePosition(self, toonIds, battleNode):
        self.notify.debug(
            "DistrutedLawbotBoss.toonsToBattlePosition----------------------------------------"
        )
        self.notify.debug("toonIds=%s battleNode=%s" % (toonIds, battleNode))
        if len(toonIds) < 5:
            points = BattleBase.BattleBase.toonPoints[len(toonIds) - 1]
    ...
    def touchedGavel(self, gavel, entry):
        ...
        if attackCodeStr == "":
            self.notify.warning(
                "Node %s has no attackCode tag." % repr(entry.getIntoNodePath())
            )
            return
    ...
    def makeVictoryMovie(self):
        ...
        rollThroughDoor = self.rollBossToPoint(
            fromPos=myFromPos, fromHpr=None, toPos=myToPos, toHpr=None, reverse=0
        )
        rollTrack = Sequence(
        ...
        bossTrack = Track(
            ...
            (
                1.0,
                Func(
                    self.setChatAbsolute, TTLocalizer.LawbotBossDefenseWins1, CFSpeech
                ),
            ),
            (
                5.5,
                Func(
                    self.setChatAbsolute, TTLocalizer.LawbotBossDefenseWins2, CFSpeech
                ),
            ),
            ...
            (
                ...
                Parallel(
                    ...
                    Func(
                        ...
                        TTLocalizer.LawbotBossDefenseWins3,
                        CFSpeech,
                    ),
    ...
    def makeDefeatMovie(self):
        bossTrack = Track(
            ...
            (
                ...
                Func(
                    self.setChatAbsolute,
                    TTLocalizer.LawbotBossProsecutionWins,
                    CFSpeech,
                ),
            ),
        )
    ...
    def __talkAboutPromotion(self, speech):
        if self.prevCogSuitLevel < ToontownGlobals.MaxCogSuitLevel:
            ...
            if newCogSuitLevel == ToontownGlobals.MaxCogSuitLevel:
                speech += TTLocalizer.WitnessToonLastPromotion % (
            ...
            speech += TTLocalizer.WitnessToonMaxed % (
                ToontownGlobals.MaxCogSuitLevel + 1
            )
        return speech
    ...
    def __makePrepareBattleTwoMovie(self):
        chatString = (
            TTLocalizer.WitnessToonPrepareBattleTwo
    ...
    def __doWitnessPrepareBattleThreeChat(self):
        self.notify.debug(
            "__doWitnessPrepareBattleThreeChat: original self.numToonJurorsSeated = %d"
            % self.numToonJurorsSeated
        )
        self.countToonJurors()
        self.notify.debug(
            "after calling self.countToonJurors, numToonJurorsSeated=%d"
            % self.numToonJurorsSeated
        )
        ...
        if diffSettings[4]:
            (
                newWeight,
                self.bonusWeight,
                self.numJurorsLocalToonSeated,
            ) = self.calculateWeightOfToon(base.localAvatar.doId)
            if self.bonusWeight > 0:
                if self.bonusWeight == 1:
                    juryWeightBonus = (
                        TTLocalizer.WitnessToonJuryWeightBonusSingular.get(
                            self.battleDifficulty
                        )
                    )
                else:
                    juryWeightBonus = TTLocalizer.WitnessToonJuryWeightBonusPlural.get(
                        self.battleDifficulty
                    )
                if juryWeightBonus:
    ...
    def countToonJurors(self):
        ...
        for key in list(self.chairs.keys()):
            ...
            if (
                chair.state == "ToonJuror"
                or chair.state == None
                and chair.newState == "ToonJuror"
            ):
    ...
    def flashPanBlue(self):
        ...
        seq = Sequence(
            self.defensePanNodePath.colorScaleInterval(
                0.1, colorScale=VBase4(0, 0, 1, 1)
            ),
            self.defensePanNodePath.colorScaleInterval(
                0.3, colorScale=VBase4(1, 1, 1, 1)
            ),
            name=intervalName,
        )
    ...
    def setTaunt(self, tauntIndex, extraInfo):
        ...
        if not hasattr(self, "state"):
            ...
            self.notify.warning(
                "returning from setTaunt, not in battle three state, state=%s",
                self.state,
            )
            gotError = True
        ...
        if tauntIndex == 0:
            if extraInfo < len(self.involvedToons):
                ...
                if toon:
                    chatString = (
                        TTLocalizer.LawbotBossTaunts[tauntIndex] % toon.getName()
                    )
        else:
    ...
    def enteredBonusState(self):
        ...
        self.bonusTimer.countdown(
            ToontownGlobals.LawbotBossBonusDuration, self.hideBonusTimer
        )
