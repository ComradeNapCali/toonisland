...
from toontown.quest.Quests import QuestDict
...
class DistributedSuitPlannerAI(
    DistributedObjectAI.DistributedObjectAI, SuitPlannerBase.SuitPlannerBase
):
    ...
    def __init__(self, air, zoneId):
        ...
        self.targetNumSuitBuildings = self.SuitHoodInfo[self.hoodInfoIdx][
        ...
        if self.notify.getDebug():
            self.notify.debug(
                "Creating a building manager AI in zone" + str(self.zoneId)
            )
        ...
        if suitHood >= 0:
            if (
                self.SuitHoodInfo[self.hoodInfoIdx][self.SUIT_HOOD_INFO_ZONE]
                != suitHood
            ):
                self.currDesired = 0
    ...
    def initBuildingsAndPoints(self):
        ...
        for p in self.frontdoorPointList:
            ...
            if p < 0:
                self.notify.debug(
                    "No landmark building for (%s) in zone %d" % (repr(p), self.zoneId)
                )
    ...
    def createNewSuit(
        ...
        if blockNumber != None:
            ...
            if newSuit.attemptingTakeover:
                ...
                if cogdoTakeover and suitTrack == None:
                    # Choose random cog department. Ok what the hell is this lore.
        ...
        if suitLevel == None and buildingHeight != None:
            if not cogdoTakeover:
                ...
                suitLevel = (
                    self.SuitHoodInfo[self.hoodInfoIdx][self.SUIT_HOOD_INFO_LVL][-1] + 1
                )
    ...
    def countNumNeededBuildings(self):
        if not self.buildingMgr:
            return 0
        numSuitBuildings = len(self.buildingMgr.getSuitBlocks()) - len(
            self.buildingMgr.getCogdoBlocks()
        )
    ...
    def chooseDestination(
        ...
        if toonBlockTakeover != None:
            ...
            for blockNumber in self.buildingMgr.getToonBlocks():
                ...
                # Check to see if cogdo (field office) takeover building is part of taskline,
                # if so, skip it. Any taskline building can be takeover over via regular cog building however.
                interInTask = any([extZoneId == i for i in list(QuestDict.keys())])
                if (not interInTask) and cogdoTakeover:
                    continue
                else:
    ...
    def pathCollision(self, path, elapsedTime):
        ...
        while (
            point.getPointType() == DNASuitPoint.FRONTDOORPOINT
    ...
    def __waitForNextUpkeep(self):
        ...
        taskMgr.doMethodLater(
            t, self.upkeepSuitPopulation, self.taskName("sptUpkeepPopulation")
        )

    def __waitForNextAdjust(self):
        t = random.random() * 10.0 + self.POP_ADJUST_DELAY
        taskMgr.doMethodLater(
            t, self.adjustSuitPopulation, self.taskName("sptAdjustPopulation")
        )

    def upkeepSuitPopulation(self, task):
        targetFlyInNum = self.calcDesiredNumFlyInSuits()
        targetFlyInNum = min(
            targetFlyInNum, self.TOTAL_MAX_SUITS - self.numBuildingSuits
        )
        ...
        targetBuildingNum = min(
            targetBuildingNum, self.TOTAL_MAX_SUITS - self.numFlyInSuits
        )
            ...
            if timeout != None:
                ...
                for b in suitBuildings:
                    ...
                    if hasattr(building, "elevator"):
                        if (
                            building.elevator.fsm.getCurrentState().getName()
                            == "waitEmpty"
                        ):
    ...
    def suitTakeOver(self, blockNumber, suitTrack, difficulty, buildingHeight):
        ...
        self.notify.warning(
            "Taking over with building: {}, {}!".format(blockNumber, building)
        )
    ...
    def cogdoTakeOver(self, blockNumber, suitTrack, difficulty, buildingHeight):
        ...
        self.notify.warning(
            "Taking over with cogdo: {}, {}!".format(blockNumber, building)
        )
    ...
    def assignInitialSuitBuildings(self):
        ...
        for sp in list(self.air.suitPlanners.values()):
            ...
            if sp.buildingMgr:
                ...
                actualSuitBuildings += (
                    len(sp.buildingMgr.getSuitBlocks()) - numCogdoBlocks
                )
        ...
        if actualSuitBuildings > 0:
            ...
            for sp in list(self.air.suitPlanners.values()):
                if sp.buildingMgr:
                    numBuildings = len(sp.buildingMgr.getSuitBlocks()) - len(
                        sp.buildingMgr.getCogdoBlocks()
                    )
                else:
                    numBuildings = 0
        ...
        if simbase.air.wantCogdominiums:
            if actualCogdos > 0:
                ...
                if numReassigned > 0:
                    self.notify.debug(
                        "Assigned %d cogdos where cogdos already existed."
                        % numReassigned
                    )
    ...
    def assignSuitBuildings(self, numToAssign):
        ...
        while numToAssign > 0:
            ...
            if not smallestTracks:
                self.notify.info(
                    "No more room for buildings, with %s still to assign." % numToAssign
                )
                return
            ...
            while repeat and buildingTrack != None and buildingHeight != None:
                ...
                if zoneId in self.air.suitPlanners:
                    ...
                    numTotalBuildings = 0
                if (
                    numCogdos + numBldgs >= currHoodInfo[self.SUIT_HOOD_INFO_BMAX]
                    or numCogdos + numBldgs >= numTotalBuildings
                ):
        ...
        while numToAssign > 0:
            ...
            while repeat:
                ...
                if overallStrapped or bldgStrapped:
                    self.notify.info(
                        "Zone %d can't remove any more buildings." % zoneId
                    )
                    hoodInfo.remove(currHoodInfo)
    ...
    def chooseStreetNoPreference(self, hoodInfo, totalWeight):
        ...
        self.notify.warning(
            "Weighted random choice failed!  Total is %s, chose %s" % (t, c)
        )
    ...
    def chooseStreetWithPreference(self, hoodInfo, buildingTrackIndex, buildingHeight):
        ...
        for currHoodInfo in hoodInfo:
            ...
            thisValue = (
                weight
                * currHoodInfo[self.SUIT_HOOD_INFO_TRACK][buildingTrackIndex]
    ...
    def requestBattle(self, zoneId, suit, toonId):
        self.notify.debug(
            "requestBattle() - zone: %d suit: %d toon: %d" % (zoneId, suit.doId, toonId)
        )
        ...
        if (
            simbase.config.GetBool("props-buff-battles", True)
            and canonicalZoneId in self.cellToGagBonusDict
        ):
        ...
        for currOther in self.zoneInfo[zoneId]:
            self.notify.debug(
                "Found suit %d in this new battle zone %d"
    ...
    def __battleFinished(self, zoneId):
        self.notify.debug(
            "DistSuitPlannerAI:  battle in zone " + str(zoneId) + " finished"
        )
        currBattleIdx = 0
    ...
    def checkForBattle(self, zoneId, suit):
        if self.battleMgr.cellHasBattle(zoneId):
            if self.__suitCanJoinBattle(zoneId) and self.battleMgr.requestBattleAddSuit(
                zoneId, suit
            ):
                pass
            else:
    ...
    def postBattleResumeCheck(self, suit):
        self.notify.debug(
            "DistSuitPlannerAI:postBattleResumeCheck:  suit "
            + str(suit.getDoId())
            + " is leaving battle"
        )
        battleIndex = 0
        for currBattle in self.battleList:
            if suit.zoneId == currBattle[0]:
                ...
                for currPath in currBattle[1]:
                    for currPathPtSuit in range(
                        suit.currWpt, suit.myPath.getNumPoints()
                    ):
                        ptIdx = suit.myPath.getPointIndex(currPathPtSuit)
                        if self.notify.getDebug():
                            self.notify.debug(
                                "    comparing" + str(ptIdx) + "with" + str(currPath)
                            )
                        if currPath == ptIdx:
                            if self.notify.getDebug():
                                self.notify.debug(
                                    "    match found, telling" + "suit to fly"
                                )
                            return 0
    ...
    def suitListQuery(self):
        ...
        self.sendUpdateToAvatarId(
            self.air.getAvatarIdFromSender(), "suitListResponse", [suitIndexList]
        )
    ...
    def buildingListQuery(self):
        ...
        self.sendUpdateToAvatarId(
            self.air.getAvatarIdFromSender(), "buildingListResponse", [buildingList]
        )
    ...
    def pickLevelTypeAndTrack(self, level=None, type=None, track=None):
        ...
        if track == None:
            track = SuitDNA.suitDepts[
                SuitBattleGlobals.pickFromFreqList(
